...
BEGIN_TEST(testSQLGetEnvAttr1)
{
      ...
      retcode = ::SQLGetEnvAttr(SQL_NULL_HANDLE, 1, NULL, 0, NULL);
      ASSERT(retcode == SQL_INVALID_HANDLE,
          "Expected SQL_INVALID_HANDLE from SQLGetEnvAttr");

      retcode = ::SQLGetEnvAttr(henv, 1, NULL, 0, NULL);
      ASSERT(retcode == SQL_ERROR, "Expected SQL_ERROR from SQLGetEnvAttr");

      retcode = ::SQLGetEnvAttr(henv, SQL_ATTR_CONNECTION_POOLING, NULL, 0,
          NULL);
      ASSERT(retcode == SQL_ERROR, "Expected SQL_ERROR from SQLGetEnvAttr");

      retcode = ::SQLGetEnvAttr(henv, SQL_ATTR_CP_MATCH, NULL, 0, NULL);
      ASSERT(retcode == SQL_ERROR, "Expected SQL_ERROR from SQLGetEnvAttr");

      retcode = ::SQLGetEnvAttr(henv, SQL_ATTR_ODBC_VERSION, NULL, 0, NULL);
      ASSERT(retcode == SQL_SUCCESS,
          "Expected SQL_SUCCESS from SQLGetEnvAttr");

      retcode = ::SQLGetEnvAttr(henv, SQL_ATTR_OUTPUT_NTS, NULL, 0, NULL);
      ASSERT(retcode == SQL_SUCCESS,
          "Expected SQL_SUCCESS from SQLGetEnvAttr");

      SQLINTEGER attribute;
      retcode = ::SQLGetEnvAttr(henv, SQL_ATTR_ODBC_VERSION, &attribute, 0,
          NULL);
      ASSERT(retcode == SQL_SUCCESS,
          "Expected SQL_SUCCESS from SQLGetEnvAttr");
      ASSERT(attribute == SQL_OV_ODBC3,
          "ODBC version supported should be 3.0");

      retcode = ::SQLGetEnvAttr(henv, SQL_ATTR_OUTPUT_NTS, &attribute, 0,
          NULL);
      ASSERT(retcode == SQL_SUCCESS,
          "Expected SQL_SUCCESS from SQLGetEnvAttr");
      ASSERT(attribute == SQL_TRUE, "SQL_ATTR_OUTPUT_NTS should be SQL_TRUE");

      retcode = ::SQLFreeHandle(SQL_HANDLE_ENV, henv);
      ASSERT(retcode != SQL_ERROR, "SQLFreeHandle returned SQL_ERROR");

    }END_TEST(testSQLGetEnvAttr1)

BEGIN_TEST(testSQLGetEnvAttr2)
{
  /* ------------------------------------------------------------------------- */
      ...
      DIAGRECCHECK(SQL_HANDLE_ENV, henv, 1, SQL_ERROR, retcode,
          "SQLGetEnvAttr (SQL_ATTR_CONNECTION_POOLING) Not Supported");
