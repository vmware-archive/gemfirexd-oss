...
//fixed in a new release we'll go back to original code by undefining SOLARIS_USE_BB
...
: virtual public NamingContext
{
  ...
  {
    m_context.close();
    char * envTestName = ACE_OS::getenv("TESTNAME");
    if(envTestName != NULL)
    	ACE_OS::unlink(envTestName);
  }
  ...
  virtual int rebind( const char* key, const char* value ) {
    int res = -1;
    int attempts = 10;
    while ( (res = m_context.rebind( key, value, (char*)"" )) == -1 && attempts--) {
      millisleep(10);
    }
  ...
  void open( bool local = true )
  {
    ...
    char* envTestName = ACE_OS::getenv("TESTNAME");
    if(envTestName != NULL){
      name_options->database(envTestName);
    }
  ...
  {
    char buf[1024] = {0};
    ACE_OS::sprintf( buf, "dunit.context.%s%d", ACE::basename( g_programName ), g_masterPid );
  ...
  {
    char buf[1024] = {0};
    ACE_OS::sprintf( buf, "dunit.mutex.%s%d", ACE::basename( g_programName ), g_masterPid );
    std::string b_str( buf );
    return b_str;
  }
  ...
  virtual void dump()
  {
    ...
    if ( this->m_context.list_name_entries( set, "" ) != 0 ) {
      ...
      for( ACE_Name_Binding* entry = 0; set_iterator.next( entry ) != 0; set_iterator.advance() ) {
        ACE_Name_Binding binding( *entry );
        char buf[1000] = {0};
        ACE_OS::sprintf( buf, "%s => %s", binding.name_.char_rep(), binding.value_.char_rep() );
        LOGMASTER( buf );
      }
    }
#endif
...
class Dunit
{
  ...
  void setNextSlave( SlaveId& sId ) 
  {
    m_globals.rebind( "SlaveId", sId.getId() );
  }
  
  /** get the next slave id */
  int getNextSlave( ) 
  {
    return m_globals.getIntValue( "SlaveId" );
  }
  ...
  void setMustQuit( )
  {
    m_globals.rebind( "TerminateAllSlaves", 1 );
  }

  /** signal to test driver that an error occurred. */
  void setFailed( )
  {
    m_globals.rebind( "Failure", 1 );
  }

  bool getFailed( )
  {
    return m_globals.getIntValue( "Failure" ) ? true : false;
  }
...
void Task::setTimeout( int seconds ) { 
	if ( seconds > 0 ) 
		DUNIT->setSlaveTimeout( m_id, seconds ); 
	else
		DUNIT->setSlaveTimeout( m_id, TASK_TIMEOUT ); 
}

class TestProcess
: virtual public dunit::Manager
{
  private:
...
class TestDriver
{
  ...
  TestDriver( ) 
  {
    ...
    for( uint32_t i = 1; i < 5; i++ ) {
      ...
      if ( profilerCmd != NULL && profilerCmd[0] != '$' && profilerCmd[0] != '\0' ) {
        // replace %d's in profilerCmd with PID and slave ID
        char cmdbuf[2048] = {0};
        ACE_OS::sprintf(cmdbuf, profilerCmd, ACE_OS::gettimeofday().msec(), g_masterPid, i);
        ACE_OS::sprintf( cmdline, "%s %s -s%d -m%d", cmdbuf, g_programName, i, g_masterPid );
      } else {
    ...
    for( uint32_t j = 1; j < 5; j++ ) {
      m_slaves[j-1]->doWork();
      ACE_OS::sleep(2); // do not increase this to avoid precheckin runs taking much longer.
    }
  }
  ...
  int begin( ) 
  {
    ...
    while( (nextSlave = TaskQueues::getSlaveId()) != 0 ) {
      DUNIT->setSlaveState( nextSlave, SLAVE_STATE_SCHEDULED );
...
//      sleepTime.msec( 10 );
      ...
      readyCount = 0;
      for( uint32_t i = 1; i < 5; i++ ) {
        int state = DUNIT->getSlaveState( i );
  ...
  void checkSlaveDeath()
  {
    for( uint32_t i = 0; i < 4; i++ ) {
      if (! m_slaves[i]->running()) {
        char msg[1000] = {0};
        sprintf( msg, "Error: Slave %s terminated prematurely.", m_slaves[i]->getSlaveId().getIdName() );
        LOG( msg );
        DUNIT->setFailed();
        DUNIT->setMustQuit();
        FAIL( msg );
...
// log a message and print the slave id as well.. used by fw_helper with no slave id.
void log( std::string s, int lineno, const char* filename, int id )
{
   ...
   char buf[128] = {0};
   dunit::getTimeStr(buf);
   fprintf( stdout, "[TEST %s:pid(%d)] %s at line: %d\n", timestamp, ACE_OS::getpid(), s.c_str(), lineno );
   fflush( stdout );
}
...
void log( std::string s)
{
   ...
   char buf[128] = {0};
   dunit::getTimeStr(buf);
   fprintf( stdout, "[TEST %s:pid(%d)] %s\n", timestamp, ACE_OS::getpid(), s.c_str());
   fflush( stdout );
}
...
int dmain( int argc, ACE_TCHAR* argv[] )
{
 ...
 // tb->arm(); // leak this on purpose.
  ...
  //  }
    ...
    // currently this is used for giving a unique per run id to shared resources.
    if ( g_masterPid == 0 ) {
...
TimeStamp::TimeStamp( int64_t msec )
: m_msec( msec )
{
}
...
void TimeStamp::msec( int64_t t )
{
  m_msec = t;
}

Record::Record( std::string testName, const long ops, const TimeStamp& start, const TimeStamp& stop )
: m_testName( testName ),
  m_operations( ops ),
  m_startTime( start ),
  m_stopTime( stop )
...
//{
//  output.writeASCII( m_testName.c_str(), (uint32_t)m_testName.length() );
...
int Record::elapsed( )
{
  return (int) ( m_stopTime.msec() - m_startTime.msec() );
}

int Record::perSec( )
{
  return (int) ((((double) 1000 * m_operations) / (double) elapsed()) + 0.5); 
}
...
std::string Record::asString( )
{
  ...
  tmp += buf;
  sprintf( buf, "%d ops, ", (int) m_operations );
  tmp += buf;
...
PerfSuite::PerfSuite( const char* suiteName )
: m_suiteName( suiteName )
{
}

void PerfSuite::addRecord( std::string testName, const long ops, const TimeStamp& start, const TimeStamp& stop )
{
  Record tmp( testName, ops, start, stop );
  m_records[ testName ] = tmp;
  int64_t elapsed = stop.msec() - start.msec();
  int64_t opspersec = ( 1000 * ops ) / elapsed;
...
void PerfSuite::save( )
{
  ...
  for( RecordMap::iterator iter = m_records.begin(); iter != m_records.end(); iter++ ) {
...
#endif
  int res =
    m_threadDef.activate( thrAttrs, m_thrCount );
