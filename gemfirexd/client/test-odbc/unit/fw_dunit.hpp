...
#define ASSERT(x,y) if (!(x)) throw dunit::TestException(y,__LINE__,__FILE__)
#define XASSERT(x) if (!(x)) throw dunit::TestException(#x,__LINE__,__FILE__)
...
#define DUNIT_TASK(x,y) class DCLASSNAME(y) : virtual public dunit::Task { public: DCLASSNAME(y)( ) { init( x ); } public: virtual void doTask( ) { const char* fwtest_Name = DTASKDESC(y,__LINE__); try {
...
#define END_TASK(y) } catch( gemfire::Exception & ex ) { ex.printStackTrace(); FAIL( ex.getMessage() ); } catch( std::exception & ex ) { FAIL( ex.what() ); } catch( dunit::TestException& ) { throw; } catch( ... ) { FAIL( "Unknown exception type caught." ); } } } SYMJOIN(a_,__LINE__);
#define ENDTASK     } catch( gemfire::Exception & ex ) { ex.printStackTrace(); FAIL( ex.getMessage() ); } catch( std::exception & ex ) { FAIL( ex.what() ); } catch( dunit::TestException& ) { throw; } catch( ... ) { FAIL( "Unknown exception type caught." ); } } } SYMJOIN(a_,__LINE__);


#define DUNIT_TASK_DEFINITION(x,y) class DCLASSDEF(y) : virtual public dunit::Task { public: DCLASSDEF(y)( ) { init( x ); } public: virtual void doTask( ) { const char* fwtest_Name = DTASKDESC(y,__LINE__); try {
#define END_TASK_DEFINITION  } catch( gemfire::Exception & ex ) { ex.printStackTrace(); FAIL( ex.getMessage() ); } catch( std::exception & ex ) { FAIL( ex.what() ); } catch( dunit::TestException& ) { throw; } catch( ... ) { FAIL( "Unknown exception type caught." ); } } };
#define CALL_TASK(y) DCLASSDEF(y) * DVARNAME(y) = new DCLASSDEF(y) ( );


#define DUNIT_MAIN class DCLASSNAME(Main) { public: DCLASSNAME(Main)( ) { try {
#define END_MAIN  } catch( gemfire::Exception & ex ) { ex.printStackTrace(); FAIL( ex.getMessage() ); } catch( std::exception & ex ) { FAIL( ex.what() ); } catch( dunit::TestException& ) { throw; } catch( ... ) { FAIL( "Unknown exception type caught." ); } } } SYMJOIN(a_,__LINE__);
