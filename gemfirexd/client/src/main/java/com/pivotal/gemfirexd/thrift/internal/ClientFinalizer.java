...
public final class ClientFinalizer extends FinalizeObject implements
  ...
  protected void clearThis() {
    this.id = gfxdConstants.INVALID_ID;
    this.service = null;
    this.source = null;
  ...
  private void addBulkCloseArgs(ClientFinalizer finalizer,
    ...
    if (type == gfxdConstants.BULK_CLOSE_CONNECTION) {
      if (service == null
    ...
    if (id == gfxdConstants.INVALID_ID || service == null || source == null
        || !service.isOpen) {
      return;
    }
  ...
  public BatchFinalize merge(BatchFinalize o) {
    if (o instanceof ClientFinalizer) {
      ...
      // GFXDServiceImpl, which means a connection/ClientService may send
