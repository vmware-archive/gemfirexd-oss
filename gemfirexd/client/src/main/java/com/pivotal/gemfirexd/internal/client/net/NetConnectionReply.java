/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.client.net.NetConnectionReply
...
import com.pivotal.gemfirexd.internal.client.am.UnitOfWorkListener;
...
// GemStone changes BEGIN
                int txdata = readByte();
                if (txdata == ClientSharedData.CLIENT_TXID_WRITTEN) {
                  long memberId = readLong();
                  long uniqueId = readLong();
                  netAgent_.netConnection_.setTXID(memberId, uniqueId);
                }
...
// GemStone changes BEGIN
        // throw back original server-side exception instead of "08006"
        String sqlState = netSqlca.getSqlState();
        if (sqlState != null && sqlState.length() == 5) {
          agent_.accumulateChainBreakingReadExceptionAndThrow(
              new DisconnectException(agent_, netSqlca));
        }
        else
// GemStone changes END
        agent_.accumulateChainBreakingReadExceptionAndThrow(
...
// GemStone changes BEGIN
                case CodePoint.PBSD_SVRVER:
                      short serverOrdinal = readShort();
                      Version v; 
                      try {
                        v = Version.fromOrdinalCheck(serverOrdinal, false);
                      } catch (UnsupportedGFXDVersionException ugve) {
                        v = null;
                      }
                      netAgent_.netConnection_.serverVersion = v;
                  break;
                case CodePoint.PBSD_TXID:
                  int hasTxId = readUnsignedByte();
                  if (hasTxId == 1) {
                    completePiggyBackTxId(readLong(), readLong());
                  }
                  break;
// GemStone changes END
                default:
...
// GemStone changes BEGIN
            case CodePoint.PBSD_TXID:
              ...
              break;
            case CodePoint.PBSD_SVRVER:
              short ordinal = readShort();
              Version version;
              try {
                version = Version.fromOrdinalCheck(ordinal, false);
              } catch (UnsupportedGFXDVersionException ugve) {
                version = null;
              }
              netAgent_.netConnection_.setServerVersion(version);
              break;
            case CodePoint.PBSD_REGIONS:
              int numRegions = readShort();
              if (numRegions > 0) {
                int prid = readShort();
                int bid = readShort();
                this.netAgent_.netConnection_.updateRegionInfoForCommit(prid, bid);
              }
              break;
// GemStone changes END              
            default:
...
// GemStone changes BEGIN
    private void completePiggyBackTxId(long memberIdOfTxid, long uniqIdOfTxId) {
      // System.out.print("KN: memberId = " + memberIdOfTxid + ", uniqId = " + uniqIdOfTxId);
      netAgent_.netConnection_.setTXID(memberIdOfTxid, uniqIdOfTxId);
    }
// GemStone changes END
}
