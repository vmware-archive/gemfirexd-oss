/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.client.am.LogWriter
...
public class LogWriter {
    ...
    final protected boolean traceSuspended() {
        return com.pivotal.gemfirexd.internal.client.am.Configuration.traceSuspended__;
    }
...
// GemStone changes BEGIN
    ...
    public final Level getLogLevel() {
      if (traceLevel_ == ClientDataSource.TRACE_ALL) {
        return Level.ALL;
      }
      else if (loggingEnabled(ClientDataSource.TRACE_DIAGNOSTICS)) {
        return Level.FINEST;
      }
      else if (loggingEnabled(ClientDataSource.TRACE_RESULT_SET_CALLS)) {
        return Level.FINER;
      }
      else if (loggingEnabled(ClientDataSource.TRACE_STATEMENT_CALLS
          | ClientDataSource.TRACE_PROTOCOL_FLOWS)) {
        return Level.FINE;
      }
      else {
        return Level.CONFIG;
    ...
    void close() {
        if (printWriterNeedsToBeClosed_) {
            // also remove from SanityManager if this PrintWriter is the same
            // as the one inside it
            com.pivotal.gemfirexd.internal.shared.common.sanity.SanityManager
                .CLEAR_DEBUG_STREAM(printWriter_);
    /* (original code)
    void close() {
        if (printWriterNeedsToBeClosed_) {
    */
// GemStone changes END
    ...
    public void dncprintln(String s) {
        synchronized (printWriter_) {
            printWriter_.println("[gemfirexd] " + s);
            printWriter_.flush();
        }
    }
    ...
    private void dncprint(String s) {
        synchronized (printWriter_) {
            printWriter_.print("[gemfirexd] " + s);
            printWriter_.flush();
        }
    }
    ...
    private void dncprintln(String header, String s) {
        synchronized (printWriter_) {
            printWriter_.println("[gemfirexd]" + header + " " + s);
            printWriter_.flush();
        }
    }
    ...
    private void dncprint(String header, String s) {
        synchronized (printWriter_) {
            printWriter_.print("[gemfirexd]" + header + " " + s);
            printWriter_.flush();
        }
    }
    ...
    private String getClassNameOfInstanceIfTraced(Object instance) {
        ...
        // we don't use instanceof javax.transaction.XAResource to avoid dependency on j2ee.jar
                ...
                instance.getClass().getName().startsWith("com.pivotal.gemfirexd.internal.client.net.NetXAResource")) {
            return "NetXAResource";
        } else if (loggingEnabled(ClientDataSource.TRACE_ALL) &&
                instance.getClass().getName().equals("com.pivotal.gemfirexd.internal.client.ClientPooledConnection")) {
            return "ClientPooledConnection";
        } else if (loggingEnabled(ClientDataSource.TRACE_ALL) &&
                instance.getClass().getName().equals("com.pivotal.gemfirexd.internal.jdbc.ClientConnectionPoolDataSource")) {
            return "ClientConnectionPoolDataSource";
        } else if (loggingEnabled(ClientDataSource.TRACE_ALL) &&
                instance.getClass().getName().equals("com.pivotal.gemfirexd.internal.client.ClientXAConnection")) {
            return "ClientXAConnection";
        } else if (loggingEnabled(ClientDataSource.TRACE_ALL) &&
                instance.getClass().getName().equals("com.pivotal.gemfirexd.internal.jdbc.ClientDataSource")) {
            return "ClientDataSource";
        } else if (loggingEnabled(ClientDataSource.TRACE_ALL) &&
                instance.getClass().getName().equals("com.pivotal.gemfirexd.internal.jdbc.ClientXADataSource")) {
    ...
    public void traceDiagnosable(SqlException e) {
        ...
        synchronized (printWriter_) {
            ...
            ExceptionFormatter.printTrace(e, printWriter_, "[gemfirexd]", true); // true means return tokens only
    ...
    public void traceDiagnosable(javax.transaction.xa.XAException e) {
        ...
        synchronized (printWriter_) {
            dncprintln("BEGIN TRACE_DIAGNOSTICS");
            ExceptionFormatter.printTrace(e, printWriter_, "[gemfirexd]");
            dncprintln("END TRACE_DIAGNOSTICS");
        }
    }
    // ------------------------ meta data tracing --------------------------------
    ...
    public void traceConnectsResetExit(com.pivotal.gemfirexd.internal.client.am.Connection c) {
        if (traceSuspended()) {
            return;
        }
        synchronized (printWriter_) {
    ...
    public void writeDriverConfiguration() {
        com.pivotal.gemfirexd.internal.client.am.Version.writeDriverConfiguration(printWriter_);
    }
    					...
    					return new java.io.PrintWriter(
    							new java.io.BufferedOutputStream(
    									new java.io.FileOutputStream(
    											fileCanonicalPath, fileAppend), 32768), true);
            			}
            		});
    ...
    throws SqlException {
        ...
        try {
            ...
            for (Enumeration e = ref.getAll(); e.hasMoreElements();) {
                ...
                if (ClientAttribute.PASSWORD.equals(propertyKey)) {
                    value = "********";
                }
