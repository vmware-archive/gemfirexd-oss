/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.client.net.NetCursor
...
import com.pivotal.gemfirexd.internal.client.am.SqlWarning;
...
public class NetCursor extends com.pivotal.gemfirexd.internal.client.am.Cursor {
    ...
    private int readFdocaInt() throws com.pivotal.gemfirexd.internal.client.am.DisconnectException, SqlException {
    ...
    private long readFdocaLong() throws 
            com.pivotal.gemfirexd.internal.client.am.DisconnectException, SqlException {
    ...
    private int readFdocaOneByte() throws com.pivotal.gemfirexd.internal.client.am.DisconnectException, SqlException {
    ...
    // Reads 1-byte from the dataBuffer from the current position.
    // If position is already at the end of the buffer, send CNTQRY to get more data.
    private int readFdocaOneByte(int index) throws com.pivotal.gemfirexd.internal.client.am.DisconnectException, SqlException {
        // For singleton select, the complete row always comes back, even if multiple query blocks are required,
        // so there is no need to drive a flowFetch (continue query) request for singleton select.
        if (position_ == lastValidBytePosition_) {
            // Check for ENDQRYRM, throw SqlException if already received one.
    ...
    private byte[] readFdocaBytes(int length) throws com.pivotal.gemfirexd.internal.client.am.DisconnectException, SqlException {
    ...
    private int readFdocaTwoByteLength() throws com.pivotal.gemfirexd.internal.client.am.DisconnectException, SqlException {
        // For singleton select, the complete row always comes back, even if multiple query blocks are required,
        // so there is no need to drive a flowFetch (continue query) request for singleton select.
    ...
    private int readFdocaTwoByteLength(int index) throws com.pivotal.gemfirexd.internal.client.am.DisconnectException, SqlException {
        // For singleton select, the complete row always comes back, even if multiple query blocks are required,
        // so there is no need to drive a flowFetch (continue query) request for singleton select.
        if ((position_ + 2) > lastValidBytePosition_) {
            // Check for ENDQRYRM, throw SqlException if already received one.
    ...
    private int skipFdocaBytes(int length) throws com.pivotal.gemfirexd.internal.client.am.DisconnectException, SqlException {
        // For singleton select, the complete row always comes back, even if multiple query blocks are required,
        // so there is no need to drive a flowFetch (continue query) request for singleton select.
        if ((position_ + length) > lastValidBytePosition_) {
            // Check for ENDQRYRM, throw SqlException if already received one.
    ...
    private int skipFdocaBytes(int length, int index) throws com.pivotal.gemfirexd.internal.client.am.DisconnectException, SqlException {
        // For singleton select, the complete row always comes back, even if multiple query blocks are required,
        // so there is no need to drive a flowFetch (continue query) request for singleton select.
        if ((position_ + length) > lastValidBytePosition_) {
            // Check for ENDQRYRM, throw SqlException if already received one.
...
// GemStone changes END
    ...
    NetSqlca[] parseSQLCARD(Typdef typdef) throws com.pivotal.gemfirexd.internal.client.am.DisconnectException, SqlException {
    ...
    private NetSqlca[] parseSQLCAGRP(Typdef typdef) throws com.pivotal.gemfirexd.internal.client.am.DisconnectException, SqlException {
    ...
    private String readFdocaString(int length, String encoding) throws DisconnectException, SqlException {
        ...
        try {
            ...
            netAgent_.accumulateChainBreakingReadExceptionAndThrow(
                new com.pivotal.gemfirexd.internal.client.am.DisconnectException(
                    netAgent_, 
...
// GemStone changes BEGIN
    protected final String getColumnLabel(int column) throws SqlException {
      ...
      if (this.netResultSet_ != null
          && (metaData = this.netResultSet_.resultSetMetaData_) != null) {
        return metaData.getColumnLabelX(column);
      }
      return null;
    }
// GemStone changes END
    /**
     * @see com.pivotal.gemfirexd.internal.client.am.Cursor#getBlobColumn_
     */
    ...
    /**
     * @see com.pivotal.gemfirexd.internal.client.am.Cursor#getClobColumn_
     */
    ...
    private void checkAndThrowReceivedEndqryrm() throws SqlException {
        ...
        if (!netResultSet_.openOnServer_) {
            ...
            int sqlcode = com.pivotal.gemfirexd.internal.client.am.Utils.getSqlcodeFromSqlca(netResultSet_.queryTerminatingSqlca_);
            if (sqlcode < 0) {
