/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.client.net.NetConnectionRequest
...
public class NetConnectionRequest extends Request implements ConnectionRequestInterface {
    ...
    void buildRDBCMM() throws SqlException {
         ...
         */
        int codepoint = CodePoint.RDBCMM;
        int sz = 0;
        if (netAgent_.netConnection_.bucketRegionIds != null) {
          sz = netAgent_.netConnection_.bucketRegionIds.size();
          codepoint = CodePoint.RDBCMMMOD;
        }
        writeLengthCodePoint(0x04, codepoint);
        if (SanityManager.TraceClientHA | SanityManager.TraceSingleHop) {
          String regions = buildRegionString();
          SanityManager.DEBUG_PRINT(SanityManager.TRACE_CLIENT_HA,
              "NetConnectionRequest::buildRDBCMM codepoint being written="
                  + codepoint + ", regions = " + regions);
        }
        if (codepoint == CodePoint.RDBCMMMOD) {
          ...
          write2Bytes(sz);
          if (sz > 0) {
            // write prid then length of bucket ids and then bucket ids
            for (Entry<Integer, HashSet<Integer>> ent : netAgent_.netConnection_.bucketRegionIds
                .entrySet()) {
              int prid = ent.getKey();
              write2Bytes(prid);
              HashSet<Integer> bucketids = ent.getValue();
              int numBuckets = bucketids != null ? bucketids.size() : 0;
              if (bucketids == null || numBuckets == 0) {
                throw new RuntimeException("numBuckets cannot be null");
              }
              write2Bytes(numBuckets);
              if (bucketids != null) {
...
// GemStone changes BEGIN
  private String buildRegionString() {
...
// GemStone changes END
 ...
 // GemStone changes BEGIN
    private void buildNCJBATCHSIZE() throws SqlException {
      final NetConnection conn = netAgent_.netConnection_;
      writeScalar4Bytes(CodePoint.NCJBATCHSIZE, conn.ncjBatchSize_);
      if (SanityManager.TraceClientStatementHA) {
        SanityManager.DEBUG_PRINT(SanityManager.TRACE_CLIENT_HA,
            "setting ncj batch size =" + conn.ncjBatchSize_
                + " for the connection");
      }
    }
  
    private void buildNCJCACHESIZE() throws SqlException {
      final NetConnection conn = netAgent_.netConnection_;
      writeScalar4Bytes(CodePoint.NCJCACHESIZE, conn.ncjCacheSize_);
      if (SanityManager.TraceClientStatementHA) {
        SanityManager.DEBUG_PRINT(SanityManager.TRACE_CLIENT_HA,
            "setting ncj cache size =" + conn.ncjCacheSize_
                + " for the connection");
      }
    }
 // GemStone changes END
...
// GemStone changes BEGIN
          ...
          byte lowerNibble = 0x0;
          byte higherNibble = CodePoint.FALSE;
          final NetConnection conn = netAgent_.netConnection_;
          if (conn.statementFailover_) {
            if (SanityManager.TraceClientStatementHA) {
              SanityManager.DEBUG_PRINT(SanityManager.TRACE_CLIENT_HA,
                  "setting failover flag for the connection");
            }
            // clear the higher bit position for FAILOVER
            higherNibble &= ~CodePoint.FAILOVER;
          }
          if (conn.disableStreaming_) {
            if (SanityManager.TraceClientStatementHA) {
              SanityManager.DEBUG_PRINT(SanityManager.TRACE_CLIENT_HA,
                  "disabling server-side streaming for the connection");
            }
            lowerNibble |= CodePoint.DISABLE_STREAMING;
          }
          if (conn.skipListeners_) {
            if (SanityManager.TraceClientStatementHA) {
              SanityManager.DEBUG_PRINT(SanityManager.TRACE_CLIENT_HA,
                  "enabling skip-listener property for the connection");
            }
            lowerNibble |= CodePoint.SKIP_LISTENERS;
          }    
          if (conn.skipConstraints_) {
            if (SanityManager.TraceClientStatementHA) {
              SanityManager.DEBUG_PRINT(SanityManager.TRACE_CLIENT_HA,
                  "setting skip-constraints=true for the connection");
            }
            lowerNibble |= CodePoint.SKIP_CONSTRAINTS;
          }
          if (conn.syncCommits_) {
            if (SanityManager.TraceClientStatementHA) {
              SanityManager.DEBUG_PRINT(SanityManager.TRACE_CLIENT_HA,
                  "enabling sync commits for the connection");
            }
            // clear the higher bit position for SYNC_COMMITS
            higherNibble &= ~CodePoint.SYNC_COMMITS;
          }
          if (conn.disableTXBatching_) {
            if (SanityManager.TraceClientStatementHA) {
              SanityManager.DEBUG_PRINT(SanityManager.TRACE_CLIENT_HA,
                  "disabling txn batching for the connection");
            }
            // clear the higher bit position for DISABLE_TX_BATCHING
            higherNibble &= ~CodePoint.DISABLE_TX_BATCHING;
          }
          if (conn.queryHDFS_) {
            higherNibble &= CodePoint.QUERY_HDFS;
          }      
          if (lowerNibble != 0 || higherNibble != CodePoint.FALSE) {
            // the 0x1 is the remaining part from CodePoint.TRUE
            writeScalar1Byte(CodePoint.RDBALWUPD,
                lowerNibble | higherNibble | 0x1);
          }
          if (conn.statementFailover_
              && Version.GFXD_20.compareTo(conn.serverVersion) <= 0) {
            TxID txid = conn.getTxID();
            if (txid != null) {
              int il = 0;
              try {
                il = conn.getTransactionIsolation();
                conn.getTransactionIsolation();
              } catch (SQLException se) {
                throw new SqlException(se);
              }
              writeByte(ClientSharedData.CLIENT_FAILOVER_CONTEXT_WRITTEN);
              writeInt(il);
              writeInt(conn.getSavePointToRollbackTo());
              writeLong(txid.getMemberId());
              writeLong(txid.getUniqId());
              if (SanityManager.TraceClientHA | SanityManager.TraceSingleHop) {
                SanityManager.DEBUG_PRINT(
                    SanityManager.TRACE_CLIENT_HA,
                    "at failover sending savepoint = "
                        + conn.getSavePointToRollbackTo() + ", txId = " + txid);
              }
            }
            else {
              writeByte(ClientSharedData.CLIENT_FAILOVER_CONTEXT_NOT_WRITTEN);
            }
          }
          else if (conn.statementFailover_) {
            writeByte(ClientSharedData.CLIENT_FAILOVER_CONTEXT_NOT_WRITTEN);
          }
        }
// GemStone changes END
