...
public class ClientStatement extends ClientFetchColumnValue implements
    ...
    if (txFlags != null) {
      ...
      try {
        ...
        for (Map.Entry<TransactionAttribute, Boolean> e : txFlags.entrySet()) {
          ...
          boolean isSet = e.getValue().booleanValue();
  ...
  protected void setCurrentRowSet(RowSet rs) {
    if (rs != null) {
      ...
      setCurrentSource(gfxdConstants.BULK_CLOSE_STATEMENT, rs.statementId, rs);
    }
    else {
      this.currentRowSet = null;
      setCurrentSource(gfxdConstants.BULK_CLOSE_STATEMENT,
          gfxdConstants.INVALID_ID, null);
    }
  }
  ...
  public int getFetchSize() {
    return this.attrs.isSetBatchSize() ? this.attrs.batchSize
        : gfxdConstants.DEFAULT_RESULTSET_BATCHSIZE;
  }
  ...
  public boolean getMoreResults(int current) throws SQLException {
    ...
    if (rs != null && (rs.flags & gfxdConstants.ROWSET_HAS_MORE_ROWSETS) != 0) {
      byte rsFlag = gfxdConstants.NEXTRS_CLOSE_ALL_RESULTS;
      switch (current) {
        case CLOSE_ALL_RESULTS:
          break;
        case CLOSE_CURRENT_RESULT:
          rsFlag = gfxdConstants.NEXTRS_CLOSE_CURRENT_RESULT;
          break;
        case KEEP_CURRENT_RESULT:
          rsFlag = gfxdConstants.NEXTRS_KEEP_CURRENT_RESULT;
          break;
      }
      // go to next results
      try {
  ...
  private final ArrayList<Integer> getIntegerList(int[] array) {
