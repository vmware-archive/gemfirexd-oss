...
// GemStone changes END
    ...
    public static short getSecurityMechanism(Properties properties) {
        ...
        String securityMechanismString =
            properties.getProperty(ClientAttribute.CLIENT_SECURITY_MECHANISM);
        ...
        else
        {
            ...
            String passwordString = properties.getProperty(ClientAttribute.PASSWORD);
...
// GemStone changes BEGIN
    public static void setSecurityMechanism(Properties properties, int secmec) {
      if (secmec != SECMEC_HAS_NOT_EXPLICITLY_SET) {
        properties.setProperty(ClientAttribute.CLIENT_SECURITY_MECHANISM,
            Integer.toString(secmec));
      }
    }
...
// GemStone changes END
    ...
    public static boolean getRetrieveMessageText(Properties properties) {
        String retrieveMessageTextString = properties.getProperty(ClientAttribute.CLIENT_RETRIEVE_MESSAGE_TEXT);
    ...
    public static String getTraceFile(Properties properties) {
        return properties.getProperty(ClientAttribute.CLIENT_TRACE_FILE);
    }
    ...
    public static String getTraceDirectory(Properties properties) {
    	...
    	traceDirectoryString  = readSystemProperty(Attribute.CLIENT_JVM_PROPERTY_PREFIX+ClientAttribute.CLIENT_TRACE_DIRECTORY);

		if (traceDirectoryString == null) 
			return properties.getProperty(ClientAttribute.CLIENT_TRACE_DIRECTORY);
		else
     ...
     */
// GemStone changes BEGIN
    public static String readSystemProperty(final String key) {
    /* (original code)
...
// GemStone changes END
    ...
    public static boolean getTraceFileAppend(Properties properties) {
        String traceFileAppendString = properties.getProperty(ClientAttribute.CLIENT_TRACE_APPEND);
    ...
    public static boolean getIsODBCDriver(Properties properties) {
      return parseBoolean(properties.getProperty(ClientAttribute.USING_ODBC_DRIVER), false);
  }
    // GemStone changes END
    ...
    public Reference getReference() throws NamingException {
        ...
        // name of the object factory, com.pivotal.gemfirexd.internal.client.ClientBaseDataSourceFactory,
...
// GemStone changes END
  ...
  // increasing visibility
    ...
    // GemStone changes END
        ...
        checkBoolean(augmentedProperties, ClientAttribute.CLIENT_RETRIEVE_MESSAGE_TEXT);
...
// display in the error message.
    ...
    public static int getTraceLevel(Properties properties) {
    	...
    	traceLevelString  = readSystemProperty(Attribute.CLIENT_JVM_PROPERTY_PREFIX+ClientAttribute.CLIENT_TRACE_LEVEL);
		if (traceLevelString == null) 
			traceLevelString = properties.getProperty(ClientAttribute.CLIENT_TRACE_LEVEL);
        ...
        throws SqlException
    {
        ...
        // GemStone changes BEGIN
        /*(original code) if (prop.containsKey(Attribute.USERNAME_ATTR)) {*/
        if (prop.containsKey(ClientAttribute.USERNAME)
            || prop.containsKey(ClientAttribute.USERNAME_ALT)) {
          // GemStone changes END
            setUser(getUser(prop));
        }
        if (prop.containsKey(ClientAttribute.CLIENT_SECURITY_MECHANISM)) {
            setSecurityMechanism(getSecurityMechanism(prop));
        }
        if (prop.containsKey(ClientAttribute.CLIENT_TRACE_FILE)) {
            setTraceFile(getTraceFile(prop));
        }
        if (prop.containsKey(ClientAttribute.CLIENT_TRACE_DIRECTORY)) {
            setTraceDirectory(getTraceDirectory(prop));
        }
        if (prop.containsKey(ClientAttribute.CLIENT_TRACE_APPEND)) {
            setTraceFileAppend(getTraceFileAppend(prop));
        }
        if (prop.containsKey(ClientAttribute.CLIENT_RETRIEVE_MESSAGE_TEXT)) {
            setRetrieveMessageText(getRetrieveMessageText(prop));
        }
        if (prop.containsKey(ClientAttribute.SSL)) {
