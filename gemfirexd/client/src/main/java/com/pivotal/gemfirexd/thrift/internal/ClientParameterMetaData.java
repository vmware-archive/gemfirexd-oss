...
public class ClientParameterMetaData implements ParameterMetaData {
  ...
  final void checkForValidColumn(int column) throws SQLException {
    ...
    if (column < 1 || column > numColumns) {
      ThriftExceptionUtil.newSQLException(
          SQLState.LANG_INVALID_PARAM_POSITION, null, Integer.valueOf(column),
  ...
  public int getParameterMode(int param) throws SQLException {
    ColumnDescriptor desc = getDescriptor(param);
    short flags = desc.getDescFlags();
    if ((flags & gfxdConstants.PARAMETER_MODE_IN) != 0) {
      return parameterModeIn;
    }
    else if ((flags & gfxdConstants.PARAMETER_MODE_OUT) != 0) {
      return parameterModeOut;
    }
    else if ((flags & gfxdConstants.PARAMETER_MODE_INOUT) != 0) {
