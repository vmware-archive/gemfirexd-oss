...
{
    /// <summary>
    /// Verifies creating the GFXDDataAdapter from the GFXDCommand object
    /// </summary>
    class CreateDataAdapterFromCommand : GFXDTest
    {
        public CreateDataAdapterFromCommand(ManualResetEvent resetEvent)
            : base(resetEvent)
        {   
        }
        ...
        public override void Run(object context)
        {
            ...
            try
            {
                adapter = Command.CreateDataAdapter();
                
                if (adapter.SelectCommand != Command)
                {
                    Fail("SelectCommand does not reference command.");
                }
                if (adapter.SelectCommand.CommandText != Command.CommandText)
                {
                    Fail("SelectCommand.CommandText does not reference command's CommandText");
                }
