...
{
    /// <summary>
    /// Performs concurrent read-committed update transactions on the same data record. Expects 
    /// a GFXDException for udpate conflict in one of the transactions when the other active 
    /// transaction has not been committed.
    /// </summary>
    class ConcurrentTransactionsUpdateSameRowReadComit : GFXDTest
    {
        private bool updateStarted;

        public ConcurrentTransactionsUpdateSameRowReadComit(ManualResetEvent resetEvent)
            : base(resetEvent)
        {
        }
