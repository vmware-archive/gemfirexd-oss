...
import com.pivotal.gemfirexd.internal.iapi.services.classfile.*;
...
/**
	...
	file that starts with 'gemfirexd.module.' is taken as a class name.
...
// GemStone changes BEGIN
	protected String[] excludePrefixes = null;
	protected boolean firstLevelOnly = false;
...
// GemStone changes BEGIN
		String excludePrefix = System.getProperty("excludePrefix");
		if (excludePrefix != null && excludePrefix.length() > 0) {
		  this.excludePrefixes = excludePrefix.split(",");
		}
		firstLevelOnly = Boolean.getBoolean("firstLevelOnly");
// GemStone changes END
	}
...
// GemStone changes BEGIN
    // don't include com.gemstone classes unless
    // it includes "gemfirexd"
      if (className.startsWith("com.gemstone")
          && className.indexOf("gemfirexd.") < 0) {
        pwOut.println("Skipping GEMFIRE " + className);
        return;
      }

      // don't include com.sun classes
      if (className.startsWith("com.sun")) {
        pwOut.println("Skipping com.sun class : " + className);
        return;
      }

      if (this.excludePrefixes != null && this.excludePrefixes.length > 0) {
        for (String excludePrefix : this.excludePrefixes) {
          if (className.startsWith(excludePrefix)) {
            pwOut.println("Skipping for " + excludePrefix
                + " class: " + className);
            return;
          }
        }
...
// GemStone changes END
		...
		{
		    // Allow opensource com.pivotal.gemfirexd.internal classes
		...
		if (db2jtools || db2jdrda) {
			...
			if (
				   className.startsWith("com.pivotal.gemfirexd.internal.authentication.")
				|| className.startsWith("com.pivotal.gemfirexd.internal.catalog.")
				|| className.startsWith("com.pivotal.gemfirexd.internal.iapi.db.")
				|| className.startsWith("com.pivotal.gemfirexd.internal.diag.")
		...
		if (db2jdrda) {
			...
			if (
				   className.startsWith("com.pivotal.gemfirexd.internal.impl.sql")
				|| className.startsWith("com.pivotal.gemfirexd.internal.impl.jdbc")
				|| className.startsWith("com.pivotal.gemfirexd.internal.impl.services")
				|| className.startsWith("com.pivotal.gemfirexd.internal.iapi.")
				|| className.startsWith("com.pivotal.gemfirexd.internal.security.")
				)
			{
				return;
			}
		...
		*/
		...
		/*
		if (className.equals("com.pivotal.gemfirexd.internal.iapi.reference.ClassName") ||
			className.equals("com.pivotal.gemfirexd.internal.iapi.services.io.RegisteredFormatIds")) {
		...
		*/
        ...
        try
        {
		    ...
		    for (Enumeration e = ch/*.getClassInfo()*/.referencedClasses(); e.hasMoreElements(); ) {
				...
				if (!com.pivotal.gemfirexd.internal.iapi.services.sanity.SanityManager.DEBUG) {
...
// GemStone changes END
	...
	protected boolean isCloudscapeCode(String name) {
		return name.startsWith("com.ibm.db2j.") ||
		       ...
		       name.startsWith("db2j.") ||
                       name.startsWith("com.pivotal.gemfirexd");
	}
	...
	scoreboard.put(key, Integer.valueOf(indentLevel));
