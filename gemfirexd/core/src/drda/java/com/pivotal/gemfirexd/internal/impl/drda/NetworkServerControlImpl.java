/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.drda.NetworkServerControlImpl
...
// not using CheapDateFormatter anymore for consistent formatting
//import com.pivotal.gemfirexd.internal.iapi.util.CheapDateFormatter;
// use interface DerbySQLException
...
public final class NetworkServerControlImpl {
	...
	protected PrintWriter cloudscapeLogWriter;              // gemfirexd.log
    ...
    // variable to store value set to gemfirexd.drda.securityMechanism
...
// GemStone changes BEGIN
	// connection listener, if any for this server
	private ConnectionListener connListener;
	// number of millis to wait before stopping the DRDAConnThread for
	// lack of activity (see #47633)
	protected static final int DEFAULT_RUNQUEUE_MAXWAIT = 30000; // 30secs
	private int runQueueMaxWait = DEFAULT_RUNQUEUE_MAXWAIT;
// GemStone changes END
    /**
    ...
    private void init() throws Exception
    {
		...
		// if on GFXD 1.3.0.2 or greater, check for hashing scheme
		// being used; if its for older GFXD then masquerade as an
		// older server to the client so it can use appropriate hashing
		if (ResolverUtils.isUsingGFXD1302Hashing()) {
		  prdId += drdaMaintStr;
		}
		else if ("10".equals(majorStr) && "4".equals(minorStr)
		    && "1".equals(drdaMaintStr)) {
		  prdId += "0";
		}
		else {
		  prdId += drdaMaintStr;
	 ...
	 * and possibly gemfirexd.log.  To turn off logging the errors to the console,
...
// GemStone changes END
		...
		throws Exception
	{
		...
		try{
			serverSocket = 
				AccessController.doPrivileged(new PrivilegedExceptionAction<ServerSocket>() {
		...
		case SSL_OFF:
			consolePropertyMessage("DRDA_Ready.I", new String []
                            // GemStone changes BEGIN
                                // move to original code once we fix the product name issue in
                                // gemfirexd/build.xml (pivotal.name.full property).
                                {Integer.toString(portNumber), SharedUtils.GFXD_PRODUCT_NAME, versionString,
                                /* (original code)
...
// GemStone changes BEGIN
 				// ignore GFE CancelExceptions
 				try {
 				  session.close();
 				} catch (com.gemstone.gemfire.CancelException ex) {
 				  // ignore since this is can happen during shutdown
 				}
 				/* (original derby code) session.close(); */
// GemStone changes END
 			}
 		}
...
// GemStone changes BEGIN
					cloudscapeDriver.connect(com.pivotal.gemfirexd.Attribute.PROTOCOL
					    + ";shutdown=true", p);
...
// GemStone changes END
    ...
    public void checkShutdownPrivileges() throws SQLException {    
        ...
        if (auth != null) {
            ...
            String failure;
            if ((failure = auth.authenticate((String)null, finfo)) != null) {
                // not a valid user
...
// GemStone changes BEGIN
			// changed to use Integer.toString()
                        args[1] = Integer.toString(connNum, 10);
                        /* (original code)
...
// GemStone changes END
	 ...
	 * written to gemfirexd.log each time a connection is made.
	 *
...
// GemStone changes END
	...
	{
// GemStone changes BEGIN
	  final Session session = (Session)this.sessionTable.remove(Integer
	      .valueOf(sessionid));
	  if (this.connListener != null) {
	    this.connListener.connectionClosed(session.clientSocket,
	        sessionid);
	  }
	  /* (original code)
...
// GemStone changes BEGIN
									    // changed to use Integer.toString()
									    new String [] {Integer.toString(sessionArg, 10)}));
									    /* (original code)
...
// GemStone changes BEGIN
							...
							freeThreads++;
							// cleanup this thread if it is idle for
							// long so don't wait indefinitely (#47633)
							runQueue.wait(runQueueMaxWait);
							/* (original code)
							freeThreads++;
							runQueue.wait();
							*/
...
// GemStone changes BEGIN
				se instanceof DerbySQLException)
			  /* (original code)
...
// GemStone changes BEGIN
					currentSession.langUtil.getLocale(), ((DerbySQLException)se).getMessageId(),
					((DerbySQLException)se).getArguments()));
				/* (original code)
...
// GemStone changes END
	...
	private String getCLSSysInfo() throws IOException
	{
		...
		com.pivotal.gemfirexd.internal.impl.tools.sysinfo.Main.getMainInfo(aw, false);
		return bos.toString();
	}
...
// GemStone changes BEGIN
				        // changed to use Integer.toString()
				        {hostArg, Integer.toString(portNumber, 10)});
				        /* (original code)
...
// GemStone changes BEGIN
					        // changed to use Integer.toString()
					        {Integer.toString(max, 10), "maxthreads"});
				                /* (original code)
...
// GemStone changes BEGIN
					        // changed to use Integer.toString()
					        {Integer.toString(timeslice, 10), "timeslice"});
				                /* (original code)
		...
		try {
			...
			if (e1 instanceof UnknownHostException) {
					...
					consolePropertyMessage("DRDA_NoIO.S",
										   new String [] {hostArg, 
// GemStone changes BEGIN
					    Integer.toString(portNumber, 10),
					    /* (original code)
...
// GemStone changes END
	...
	private void buildLocalAddressList(InetAddress bindAddr) 
	{
			...
			try {
// GemStone changes BEGIN
				localAddresses.add(ClientSharedUtils
				    .getLocalHost());
				/* (original code)
...
// GemStone changes END
	...
	private void  getPropertyInfo() throws Exception
	{
		...
		String propval = PropertyUtil.getSystemProperty(
			com.pivotal.gemfirexd.Property.DRDA_PROP_LOGCONNECTIONS);
		if (propval != null && StringUtil.SQLEqualsIgnoreCase(propval,"true"))  
			setLogConnections(true);
		propval = PropertyUtil.getSystemProperty(com.pivotal.gemfirexd.Property.DRDA_PROP_TRACEALL);
		if (propval != null && StringUtil.SQLEqualsIgnoreCase(propval, 
															  "true"))  
			setTraceAll(true);

		//If the gemfirexd.system.home property has been set, it is the default. 
		//Otherwise, the default is the current directory. 
		//If gemfirexd.system.home is not set, directory will be null and trace files will get
		//created in current directory.
		propval = PropertyUtil.getSystemProperty(com.pivotal.gemfirexd.Property.DRDA_PROP_TRACEDIRECTORY,directory);
		if(propval != null){
			if(propval.equals(""))
				propval = directory;
		...
		propval = PropertyUtil.getSystemProperty( 
			com.pivotal.gemfirexd.Property.DRDA_PROP_MINTHREADS);
		if (propval != null){
			if(propval.equals(""))
				propval = "0";
			setMinThreads(getIntPropVal(com.pivotal.gemfirexd.Property.DRDA_PROP_MINTHREADS, propval));
		}
		...
		if (propval != null){
			if(propval.equals(""))
				propval = "0";
			setMaxThreads(getIntPropVal(com.pivotal.gemfirexd.Property.DRDA_PROP_MAXTHREADS, propval));
		}
		...
		if (propval != null){
			if(propval.equals(""))
				propval = "0";
			setTimeSlice(getIntPropVal(com.pivotal.gemfirexd.Property.DRDA_PROP_TIMESLICE, propval));
		}
// GemStone changes BEGIN

		propval = PropertyUtil.getSystemProperty(
		    com.pivotal.gemfirexd.Property.DRDA_PROP_THREAD_MAXIDLETIME);
		if (propval != null) {
		  if (propval.length() == 0) {
		    runQueueMaxWait = DEFAULT_RUNQUEUE_MAXWAIT;
		  }
		  else {
		    runQueueMaxWait = getIntPropVal(com.pivotal.gemfirexd.Property
		        .DRDA_PROP_THREAD_MAXIDLETIME, propval);
		  }
		}
// GemStone changes END
		propval = PropertyUtil.getSystemProperty( 
			com.pivotal.gemfirexd.Property.DRDA_PROP_PORTNUMBER);
		if (propval != null){
			if(propval.equals(""))
				propval = String.valueOf(NetworkServerControl.DEFAULT_PORTNUMBER);
			portNumber = getIntPropVal(com.pivotal.gemfirexd.Property.DRDA_PROP_PORTNUMBER, propval);
		}

		propval = PropertyUtil.getSystemProperty(
			com.pivotal.gemfirexd.Property.DRDA_PROP_SSL_MODE);
		setSSLMode(getSSLModeValue(propval));
												 
		propval = PropertyUtil.getSystemProperty( 
			com.pivotal.gemfirexd.Property.DRDA_PROP_KEEPALIVE);
		if (propval != null && 
			StringUtil.SQLEqualsIgnoreCase(propval,"false"))
			keepAlive = false;
		
		propval = PropertyUtil.getSystemProperty( 
			com.pivotal.gemfirexd.Property.DRDA_PROP_HOSTNAME);
		if (propval != null){
			if(propval.equals(""))
        ...
        throws Exception
	{
		if (s != null){
			if (StringUtil.SQLEqualsIgnoreCase(s,"off")) {
				...
				// Unknown value
                consolePropertyMessage("DRDA_InvalidValue.U", 
                                       new String [] {s, com.pivotal.gemfirexd.Property.DRDA_PROP_SSL_MODE});
...
// GemStone changes END
	...
	/**
     * Set the security mechanism for gemfirexd.drda.securityMechanism
     ...
     * @see com.pivotal.gemfirexd.Property#DRDA_PROP_SECURITYMECHANISM 
	 */
        ...
        throws Exception
    {
       ...
       // gemfirexd.drda.securityMechanism to be set to EUSRIDPWD security
       // mechanism
       if ((allowOnlySecurityMechanism == INVALID_OR_NOTSET_SECURITYMECHANISM) ||
              (allowOnlySecurityMechanism == CodePoint.SECMEC_EUSRIDPWD &&
              !SUPPORTS_EUSRIDPWD))
           consolePropertyMessage("DRDA_InvalidValue.U", new String [] 
                       {s, com.pivotal.gemfirexd.Property.DRDA_PROP_SECURITYMECHANISM});
    }
     ...
     * the gemfirexd.drda.securityMechanism was set to, if it is not set, then
...
// GemStone changes BEGIN
			...
			final Session session = (Session) sessionTable.get(
			    Integer.valueOf(sessionArg));
			/* (original code)
...
// GemStone changes BEGIN
			        // changed to use Integer.toString()
			        {Integer.toString(value, 10), "timeslice"});
			/* (original code)
...
// GemStone changes BEGIN
			        // changed to use Integer.toString()
			        {Integer.toString(value, 10), "maxthreads"});
			/* (original code)
...
// GemStone changes BEGIN
		// changed to use Integer.toString() and Boolean.toString()
		retval.put(com.pivotal.gemfirexd.Property.DRDA_PROP_PORTNUMBER,
		    Integer.toString(portNumber, 10));
		retval.put(com.pivotal.gemfirexd.Property.DRDA_PROP_HOSTNAME, hostArg);
		retval.put(com.pivotal.gemfirexd.Property.DRDA_PROP_KEEPALIVE,
		    Boolean.toString(keepAlive));

		String tracedir = getTraceDirectory();
		if (tracedir != null)
		  retval.put(com.pivotal.gemfirexd.Property.DRDA_PROP_TRACEDIRECTORY, tracedir);
		retval.put(com.pivotal.gemfirexd.Property.DRDA_PROP_TRACEALL, Boolean.toString(
		    getTraceAll()));
		retval.put(com.pivotal.gemfirexd.Property.DRDA_PROP_MINTHREADS, Integer.toString(
		    getMinThreads(), 10));
		retval.put(com.pivotal.gemfirexd.Property.DRDA_PROP_MAXTHREADS, Integer.toString(
		    getMaxThreads(), 10));
		retval.put(com.pivotal.gemfirexd.Property.DRDA_PROP_TIMESLICE, Integer.toString(
		    getTimeSlice(), 10));

		retval.put(com.pivotal.gemfirexd.Property.DRDA_PROP_TIMESLICE, Integer.toString(
		    getTimeSlice(), 10));
		retval.put(com.pivotal.gemfirexd.Property.DRDA_PROP_LOGCONNECTIONS, Boolean.toString(
		    getLogConnections()));
		/* (original code)
...
// GemStone changes END
		...
		// DERBY-2108 SSL
		retval.put(com.pivotal.gemfirexd.Property.DRDA_PROP_SSL_MODE, getSSLModeString(getSSLMode()));
        ...
        if ( getSecurityMechanism() != INVALID_OR_NOTSET_SECURITYMECHANISM )
            retval.put( com.pivotal.gemfirexd.Property.DRDA_PROP_SECURITYMECHANISM, getStringValueForSecMec(getSecurityMechanism()));
			...
			synchronized(sessionTable) {
				...
				{	
					...
					if (session.isTraceOn())
						retval.put(com.pivotal.gemfirexd.Property.DRDA_PROP_TRACE+"."+session.getConnNum(), "true");
				}
			}
		}
		return retval;
...
// GemStone changes BEGIN
		this.sessionTable.put(Integer.valueOf(connectionNumber),
		    session);
		if (this.connListener != null) {
		  this.connListener.connectionOpened(clientSocket,
		      connectionNumber);
		}
		/* (original code)
...
// GemStone changes BEGIN
				  // in case of OOM in starting a thread due to ulimit
				  // exceeded or otherwise, cleanup the thread (#47633)
				  try {
// GemStone changes END
...
// GemStone changes END
	 ...
	 * network server messages and the boot message get written to gemfirexd.log.   
	 * 
	 * @return current timestamp formatted in GMT
	 */
	private String getFormattedTimestamp(){
// GemStone changes BEGIN
	  // we now return the time in current timezone to be consistent
  ...
  /**
   * DRDAConnection aggregated statistics.
   * 
   */
  public void collectStatisticsSample() {
    ...
    long totNumTimesWaited = 0, totalDRDAThreadIdleTime = 0, totalNumCommandsProcessed = 0, totalProcessTime = 0;
    ...
    synchronized (threadsSync) {
      ...
      for (int i = 0; i < totalThreads; i++) {
        ...
        if (currWaitingTime > 0) {
          ...
          totalDRDAThreadIdleTime += (currWaitingTime * 1000000);
        }
  ...
  public void getSessionInfo(SessionsVTI.SessionInfo info) throws StandardException {
    synchronized (threadsSync) {
      for (int i = 0; i < threadList.size(); i++) {
        final DRDAConnThread connThread = ((DRDAConnThread)threadList.get(i));
        final Session session = connThread.getSession();
        if (session == null) {
          // this thread is idle.
          continue;
        }
        
        final SessionsVTI.SessionInfo.ClientSession cs = new SessionsVTI.SessionInfo.ClientSession();
    ...
    synchronized (runQueue) {
      for (int i = 0; i < runQueue.size(); i++) {
        Session session = (Session) runQueue.get(i);
        ...
        if (st != null) {
          ...
          long accessFrequency = st.getAccessFrequency();
