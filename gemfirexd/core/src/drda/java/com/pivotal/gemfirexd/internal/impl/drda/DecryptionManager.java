/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.drda.DecryptionManager
...
class DecryptionManager
{
  ...
  DecryptionManager () throws SQLException
  {
    ...
    */
      ...
      if (GemFireXDUtils.TraceAuthentication) {
        SanityManager.DEBUG_PRINT("info:", "JCE provider list is " + Arrays.toString(list));
      }
      provider = list[0];
      providerName = provider.getName();
      paramSpec_ = new DHParameterSpec (modulus__, base__, exponential_length__);
      keyPairGenerator_ = KeyPairGenerator.getInstance ("DH", providerName);
			     ...
			     byte[] sourcePublicKey) throws SQLException
  {
    ...
    try {
      ...
      // GemStone changes BEGIN
      /*(original code) KeyFactory keyFac = KeyFactory.getInstance ("DH", "IBMJCE");*/
      ...
      // GemStone changes BEGIN
      /*(original code) Cipher cipher= Cipher.getInstance ("DES/CBC/PKCS5Padding", "IBMJCE");*/
