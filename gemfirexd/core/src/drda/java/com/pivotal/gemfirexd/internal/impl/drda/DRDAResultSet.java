/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.drda.DRDAResultSet
...
// GemStone changes BEGIN
		...
		rsExtPositions.add (Integer.valueOf(jdbcIndex -1 ));
		/* (original code)
...
// GemStone changes END
		...
		throws SQLException
	{
		if (!gotPrctyp && qryprctyp == CodePoint.LMTBLKPRC)
		{
			gotPrctyp = true;
// GemStone changes BEGIN
			if (rs == null || (rs instanceof EmbedResultSet &&
			    ((EmbedResultSet)rs).isForUpdate()) ||
			/* (original code)
