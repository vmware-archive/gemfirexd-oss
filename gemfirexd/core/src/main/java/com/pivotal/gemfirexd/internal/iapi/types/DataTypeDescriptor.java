/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.types.DataTypeDescriptor
...
// GemStone changes BEGIN
  public static DataTypeDescriptor getSQLDataTypeDescriptor(
      String javaTypeName, Object typeParam, boolean isNullable)
      throws StandardException {
  ...
  public int getDVDTypeFormatId() {
    switch (this.typeId.getTypeFormatId()) {
      ...
      default:
        SanityManager.THROWASSERT("unexpected formatId in getDVDTypeFormatId() "
                + this.typeId.getTypeFormatId());
        return -1;
    }
  }
...
// GemStone changes END
		...
		if (typeId.isRefTypeId() || compareWithTypeID.isRefTypeId())
			return false;
                ...
                // Especially to handle DVDSet for NCJ Usecase
                if (typeId.isUserDefinedTypeId()) {
                  Object classParam = ((UserDefinedTypeIdImpl)typeId.getBaseTypeId())
                      .getClassParam();
                  if (classParam != null && classParam instanceof DataTypeDescriptor) {
  ...
  // GemStone changes BEGIN
    public final void assertTypeEquivalent(final DataValueDescriptor dvd, final int colIdx) {
      typeId.assertTypeEquivalent(dvd, colIdx);
    }
  // GemStone changes END
}
