/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.catalog.SYSCONGLOMERATESRowFactory
...
// GemStone changes BEGIN
			...
			conglomNumber = Long.valueOf(conglomerate.getConglomerateNumber());
			/* (original code)
...
// GemStone changes BEGIN
			// changed to use valueOf()
			supportsIndex = Boolean.valueOf(conglomerate.isIndex());
			indexRowGenerator = conglomerate.getIndexDescriptor();
			supportsConstraint = Boolean.valueOf(conglomerate.isConstraint());
			/* (original code)
...
// GemStone changes END
		...
		*  (user type com.pivotal.gemfirexd.internal.catalog.IndexDescriptor)
		*/
		row.setColumn(6,
			dvf.getDataValue(
...
// GemStone changes BEGIN
    ...
    if (td != null) {
      ...
      if (sd == null) {
        try {
          sd = td.getDataDictionary().getSchemaDescriptor(
              conglomerate.getSchemaID(), null);
    ...
    row.setColumn(SYSCONGLOMERATES_SCHEMANAME, new SQLVarchar(schemaName));
// GemStone changes END
		return row;
	}
        ...
        throws StandardException
	{
            return new SystemColumn[] {
               ...
               SystemColumnImpl.getJavaColumn("DESCRIPTOR",
                       "com.pivotal.gemfirexd.internal.catalog.IndexDescriptor", true),
               SystemColumnImpl.getColumn("ISCONSTRAINT", Types.BOOLEAN, true),
// GemStone changes BEGIN
               SystemColumnImpl.getUUIDColumn("CONGLOMERATEID", false),
               SystemColumnImpl.getIdentifierColumn("CONGLOMSCHEMANAME", true)
               /* SystemColumnImpl.getUUIDColumn("CONGLOMERATEID", false) */
// GemStone changes END
           };
	}
