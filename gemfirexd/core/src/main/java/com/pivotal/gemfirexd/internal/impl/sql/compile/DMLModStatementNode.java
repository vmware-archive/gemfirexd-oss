/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.DMLModStatementNode
...
//	protected DataDictionary	dataDictionary;
	public /* GemStone made public; original: protected */ FromVTI			targetVTI;
		...
		if (targetTableName != null)
		{
			...
			*/
			...
			if (targetTableDescriptor == null)
			{
				...
				if (synonymTab == null)
					throw StandardException.newException(SQLState.LANG_TABLE_NOT_FOUND, targetTableName.getFullTableName());
			 ...
			 */
// GemStone changes BEGIN
			// no exclusive lock for DML mod statements since that
			// will end up taking distributed write lock
			targetTableDescriptor = lockTableForCompilation(
			    targetTableDescriptor, false);
			/* (original derby code)
		...
		FromBaseTable	fbt =
			(FromBaseTable)
				(getNodeFactory().getNode(
										...
										/* for derby bug#4110 patch applied*/
...
// GemStone changes BEGIN
						...
						refActions.addElement(Integer.valueOf(raRules[inner]));
						/* (original code)
...
// GemStone changes BEGIN
						...
						refIndexConglomNum.addElement(Long.valueOf(conglomNumbers[inner]));
						/* (original code)
...
// GemStone changes BEGIN
		//[sb] don't use optimizedLiterals here
...
// GemStone changes END
   ...
   */
	...
	//	long primaryKeyIndexID=baseTable.getPrimaryKeyIndexID();
		//GemStone Change END
		for (int index = 0; index < cds.length; index++)
		{
			ConglomerateDescriptor cd = cds[index];
			//GemStone Change BEGIN
			if (!cd.isIndex()) { continue; } //derby code
		//	if(!cd.isIndex() 
		//	    || cd.getConglomerateNumber()==primaryKeyIndexID) {
		//	   continue;
		//	}
			//GemStone Change END
			/*
