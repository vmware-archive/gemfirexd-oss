/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.execute.DMLWriteResultSet
...
// GemStone changes BEGIN
		if (GemFireXDUtils.TraceQuery) {
		  SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_QUERYDISTRIB,
		      "DMLWriteResultSet::getNextRowCore: ExecRow retrieved = "
		      + row + ". Source of retrieval = " + source);
		}
// GemStone changes END
     ...
     * @see com.pivotal.gemfirexd.internal.impl.sql.compile.FromBaseTable#updateTargetLockMode
     */
...
// GemStone changes BEGIN
  protected void distributeBulkOpToDBSynchronizer() throws StandardException {
    // Asif:If the activation is derby activation & the
    // boolean createQueryInfo is true, then we have to execute the
    // update locally for a PR based or replicated region.
    final GenericPreparedStatement gps = (GenericPreparedStatement)((BaseActivation)this.activation)
        .getPreparedStatement();
    if (GemFireXDUtils.TraceDBSynchronizer) {
      SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_DB_SYNCHRONIZER,
          "DMLWriteResultset::distributeBulkOpToDBSynchronizer: DML under "
              + "consideration for hub distribution: " + gps.getUserQueryString(this.lcc));
    }

    boolean dbSyncToBeDone = lcc.dbSyncToBeDone();
    if (gps.createQueryInfo() || dbSyncToBeDone) {
      if (GemFireXDUtils.TraceDBSynchronizer) {
        SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_DB_SYNCHRONIZER,
            "DMLWriteResultset::distributeBulkOpToDBSynchronizer: eligible "
                + "for distribution: " + gps.getUserQueryString(lcc)
                + " and dbSync flag in lcc: " + lcc.dbSyncToBeDone()
                + ", gps.getpartitionedregion: " + gps.getPartitionedRegion());
      }
      LanguageConnectionContext lcc = this.activation
          .getLanguageConnectionContext();
      if (GemFireXDUtils.TraceDBSynchronizer) {
        SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_DB_SYNCHRONIZER,
            "DMLWriteResultset::distributeBulkOpToDBSynchronizer: activation is: "
                + this.activation + ", activation param count: "
                + this.activation.getParameterValueSet().getParameterCount()
                + ", skip listeners: " + lcc.isSkipListeners());
      }
      LocalRegion region = gps.getLocalRegion();
      if (region == null) {
        // Since this will happen only for single hop and
        // single hop is done only for PRs so no need to
        // worry about replicated region here
        assert dbSyncToBeDone == true;
        region = gps.getPartitionedRegion();
      }
      this.activation.distributeBulkOpToDBSynchronizer(region, this.activation
          .getParameterValueSet().getParameterCount() > 0,
          (GemFireTransaction)lcc.getTransactionExecute(), lcc
              .isSkipListeners(), null);
    }
    else {
      if (GemFireXDUtils.TraceDBSynchronizer) {
        SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_DB_SYNCHRONIZER,
            "DMLWriteResultset::distributeBulkOpToDBSynchronizer:: NOT "
                + "eligible for distribution: " + gps.getUserQueryString(lcc));
      }
    }
  }
// GemStone changes END
