/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.jdbc.EmbedBlob
...
// GemStone changes END
{
    ...
    */
    public /* GemStone changed to public: protected */ EmbedBlob(DataValueDescriptor dvd, EmbedConnection con)
        throws StandardException
    {
        ...
        else
        {
            ...
            try {
                ...
                tmpStream.close();
// GemStone changes BEGIN
                this.finalizer = new FinalizeEmbedBlob(this, myStream);
            } catch (DerbyIOException dioe) {
              if (dioe.getSQLState() != null) {
                throw StandardException.newPreLocalizedException(
                    dioe.getSQLState(), dioe, dioe.getLocalizedMessage());
              }
              else {
                throw StandardException.newException(
                    SQLState.LANG_STREAMING_COLUMN_I_O_EXCEPTION, dioe);
              }
// GemStone changes END
        ...
        throws SQLException
    {
        ...
        try
        {
           // we have a stream
            ...
            {
                pushStack = getEmbedConnection().isActive();
                if (pushStack)
...
// GemStone changes BEGIN
   // now using FinalizeEmbedBlob instead of finalizer
   /* (original code)
   /*
...
// GemStone changes BEGIN
    ...
    public Object replace() throws IOException {
      // length() can never exceed 2GB
      try {
        byte[] bytes = getBytes(1L, (int)length());
    ...
    private final void clearFinalizer() {
      final FinalizeEmbedBlob finalizer = this.finalizer;
      if (finalizer != null) {
        finalizer.clearAll();
        this.finalizer = null;
      }
    ...
    static final class FinalizeEmbedBlob extends FinalizeObject {

      private PositionedStoreStream myStream;

      public FinalizeEmbedBlob(final EmbedBlob blob,
          final PositionedStoreStream myStream) {
        super(blob, true);
      ...
      protected final boolean doFinalize() {
        final PositionedStoreStream stream = this.myStream;
        if (stream != null) {
          stream.closeStream();
          this.myStream = null;
        }
