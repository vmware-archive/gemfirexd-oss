/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.diag.ErrorLogReader
...
/**
	...
	<PRE>SELECT vti.ts, threadid, cast(xid as int) as xid_int, cast(lccid as int) as lccid_int, logtext 
		 FROM new com.pivotal.gemfirexd.internal.diag.ErrorLogReader() vti, 
			(VALUES timestampConstant) t(ts)
		 WHERE vti.ts <= t.ts AND 
				vti.ts >
					(SELECT MAX(ts) IS NULL ? '2000-01-01 00:00:00.1' : MAX(ts)
					 FROM new com.pivotal.gemfirexd.internal.diag.ErrorLogReader() vti_i
...
public class ErrorLogReader extends VTITemplate
{
	...
	*/
	...
	/**
		ErrorLogReader() accesses the gemfirexd.log in
		gemfirexd.system.home, if set, otherwise it looks in the current directory.
		ErrorLogReader('filename') will access the specified
		file name.
	 */
	public ErrorLogReader()
	{
		String home = System.getProperty("gemfirexd.system.home");

		inputFileName = "gemfirexd.log";
