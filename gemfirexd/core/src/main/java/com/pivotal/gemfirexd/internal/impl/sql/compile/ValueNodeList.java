/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.ValueNodeList
...
public class ValueNodeList extends QueryTreeNodeVector
{
	...
	// GemStone Changes begin		
			if ( valueNode.requiresTypeFromContext() && !valueNode.isParameterizedConstantNode() )
				continue;
...
//              GemStone changes begin		  
			if (((ValueNode) elementAt(index)).requiresTypeFromContext()
			   // && !((ValueNode) elementAt(index)).isParameterizedConstantNode()
			    )
			{
				return true;
			}
