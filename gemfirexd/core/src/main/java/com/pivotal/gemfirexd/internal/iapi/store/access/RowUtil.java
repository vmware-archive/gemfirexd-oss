/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.store.access.RowUtil
...
public class RowUtil
{
	...
	*/
     ...
     * @param allowPartialRow TODO
     * @exception  StandardException  Standard exception policy.
     **/
	public static final boolean qualifyRow(
    DataValueDescriptor[]        row, 
// GemStone changes BEGIN
    // allow for single DVD index key
    DataValueDescriptor dvd,
// GemStone changes END
    Qualifier[][]   qual_list, boolean allowPartialRow)
		 throws StandardException
	{
        ...
        {
// GemStone changes BEGIN
          SanityManager.ASSERT((row != null) | (dvd != null));
          /* (original code)
...
// GemStone changes BEGIN
            final DataValueDescriptor columnValue;
            if (dvd != null) {
              SanityManager.ASSERT(q.getColumnId() == 0);
              columnValue = dvd;
            }
            else {
...
// GemStone changes END
            ...
            row_qualifies = allowPartialRow && columnValue == null ? !q.negateCompareResult() :
                columnValue.compare(
                    q.getOperator(),
                    q.getOrderable(),
                    q.getOrderedNulls(),
...
// GemStone changes BEGIN
                    {
                      ...
                      SanityManager.THROWASSERT("1:row = " + rowStr
                          + "row.length = " + rowLen + ";q.getColumnId() = "
                          + q.getColumnId());
                    }
                      /* (original code)
                        SanityManager.THROWASSERT(
...
// GemStone changes END
	...
	// GemStone changes BEGIN
	/**
	 * Compare tow dvd arrays.
	 * @return  <0 arg1 is less than arg2,  0 arg1 equals arg2, >0 arg1 is greater than arg2.
	 */
	public static final int compare(DataValueDescriptor[] arg1,
      DataValueDescriptor[] arg2) throws StandardException {
   ...
   *          Any index scan controller to use for additional qualification
   *          before returning the result.
   * @param indexKey
   *          The index key corresponding to the destRow.
   * 
   ...
   *      boolean)
   */
  public static RowLocation fetch(RowLocation loc, ExecRow destRow,
      FormatableBitSet validColumns, boolean faultIn,
      GemFireContainer container, SortedMap2IndexScanController scanController,
      Object indexKey, int nodeVersion, OffHeapResourceHolder owner)
          throws StandardException {
    if (loc == null || loc.isNull()) {
      return null;
    }

    // get pointer to row in heap
    final ExecRow rowFromLocation = faultIn ? loc.getRow(container) : loc
        .getRowWithoutFaultIn(container);
    boolean rowQualified = false;
    if (rowFromLocation != null) {
      try {
        // re-qualify the row if required
        if (scanController == null
            || scanController.qualifyRow(loc, rowFromLocation, indexKey,
                nodeVersion)) {
          rowQualified = true;
        ...
        if (container.isOffHeap()) {
          @Unretained Object bs = rowFromLocation.getByteSource();
          if (bs instanceof OffHeapByteSource) {
            ...
            if (rowQualified) {
              ...
              // TODO: asif: is this an extra release?
              obs.release();
            }
          }
