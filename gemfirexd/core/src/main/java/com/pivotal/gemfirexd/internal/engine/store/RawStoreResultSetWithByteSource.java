...
public class RawStoreResultSetWithByteSource extends NonUpdatableRowsResultSet
    implements ResultSet, ResultWasNull {

  private OffHeapRow currentRowBytes;
  private OffHeapRowWithLobs currentRowByteArrays;
  ...
  public RawStoreResultSetWithByteSource(final OffHeapByteSource row,
      final RowFormatter rf) {
    if (row != null && row.isWithLobs()) {
      this.currentRowByteArrays = (OffHeapRowWithLobs)row;
    }
    else {
      this.currentRowBytes = (OffHeapRow)row;
    }
    this.formatter = rf;
  ...
  public RawStoreResultSetWithByteSource(final OffHeapByteSource row,
      final RowFormatter rf, final int[] changedColumns,
