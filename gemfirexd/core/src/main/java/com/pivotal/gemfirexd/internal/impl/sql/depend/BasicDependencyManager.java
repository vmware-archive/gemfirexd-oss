/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.depend.BasicDependencyManager
...
public class BasicDependencyManager implements DependencyManager {
     ...
     */
// GemStone changes BEGIN
    private final ConcurrentHashMap<UUID, List<Dependency>> dependents =
      new ConcurrentHashMap<UUID, List<Dependency>>();
    /* (original code) private final Hashtable dependents = new Hashtable(); */
// GemStone changes END
     ...
     */    
// GemStone changes BEGIN
    private final ConcurrentHashMap<UUID, List<Dependency>> providers =
      new ConcurrentHashMap<UUID, List<Dependency>>();
    /* (original code) private final Hashtable providers = new Hashtable(); */
// GemStone changes END
	...
	private void addDependency(Dependent d, Provider p, ContextManager cm, 
		...
		synchronized(this)
		{
			...
			if (! d.isDescriptorPersistent() || ! p.isDescriptorPersistent())
			{
			 	...
			 	*/
				StatementContext sc = (StatementContext) cm.getContext(com.pivotal.gemfirexd.internal.iapi.reference.ContextId.LANG_STATEMENT);
				// Gemstone changes BEGIN
				//sc.addDependency(dy);
				if (sc != null && !Dependable.TRIGGER_ACTION_MAP
				    .equals(d.getClassType()) && sc.inUse()) {
				  sc.addDependency(dy);
				}
				// Gemstone changes END
			}
	 ...
	 */
//GemStone changes BEGIN
	public List invalidateFor(Provider p, int action,
//GemStone changes END
		 ...
		 throws StandardException
	{
		...
		*/
		if (p.isDescriptorPersistent())
			return coreInvalidateFor(p, action, lcc);
		else {
			synchronized (this) {
				return coreInvalidateFor(p, action, lcc);
			}
	 ...
	 */
	private List coreInvalidateFor(Provider p, int action, LanguageConnectionContext lcc)
		throws StandardException
	{
...
*/
							...
							throws StandardException
	{
					...
					depsIterator.hasNext(); ) 
		{
			...
			if (dep.getProvider().isDescriptorPersistent())
			{
							...
							throws StandardException
	{
		...
		*/
		while (e != null && e.hasMoreElements())
		{
			...
			if (prov.isDescriptorPersistent())
			{
				numProviders++;
			}
		}
		...
		throws StandardException
	{
		...
		for (ListIterator depsIterator = list.listIterator(); depsIterator.hasNext(); ) 
		{
			...
			if (!persistentOnly || provider.isDescriptorPersistent())
			{
	 ...
	 */
// GemStone changes BEGIN
	private boolean addDependencyToTable(Map table,
...
// GemStone changes END
	...
	/**
		...
		@exception StandardException thrown if something goes wrong
	 */
	public List getDependents (Provider p) 
			throws StandardException {
...
// GemStone changes BEGIN
	public boolean isDependent(Provider p, Dependent d)
	    throws StandardException {
	  @SuppressWarnings("unchecked")
	  List<Dependency> deps = getDependents(p);
	  for (Dependency dy : deps) {
	    if (d.equals(dy.getDependent())) {
	      return true;
	    }
	  }
	  return false;
