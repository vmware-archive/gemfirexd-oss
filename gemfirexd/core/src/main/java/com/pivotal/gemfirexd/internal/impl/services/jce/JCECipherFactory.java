/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.services.jce.JCECipherFactory
...
package com.pivotal.gemfirexd.internal.impl.services.jce;
...
import com.pivotal.gemfirexd.internal.iapi.services.crypto.CipherFactory;
import com.pivotal.gemfirexd.internal.iapi.services.crypto.CipherProvider;
...
public final class JCECipherFactory implements CipherFactory, java.security.PrivilegedExceptionAction
{
		 ...
		 throws StandardException
	{
		...
		return com.pivotal.gemfirexd.internal.iapi.util.StringUtil.toHexString(result, 0, result.length);
	...
	private byte[] decryptKey(String encryptedKey, int encodedKeyCharLength, byte[] bootPassword)
		 throws StandardException
	{
		byte[] secretKey = com.pivotal.gemfirexd.internal.iapi.util.StringUtil.fromHexString(encryptedKey, 0, encodedKeyCharLength);
		// In case of AES, care needs to be taken to allow for 16 bytes muck as well
		// as to have the secretKey that needs encryption to be a aligned appropriately
		// AES supports 16 bytes block size
		int muckLength;
		...
		throws StandardException
	{
		...
		try
		{
			...
			if (externalKey != null) {
				...
				generatedKey = 
                    com.pivotal.gemfirexd.internal.iapi.util.StringUtil.fromHexString(externalKey, 
                                                                        0, 
                                                                        externalKey.length());
