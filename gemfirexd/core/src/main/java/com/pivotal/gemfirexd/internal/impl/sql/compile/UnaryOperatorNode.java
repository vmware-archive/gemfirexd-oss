/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.UnaryOperatorNode
...
// GemStone changes BEGIN
	// made visibility to public from package
...
// GemStone changes END
	...
	public ValueNode getParameterOperand() throws StandardException
	{
		if (requiresTypeFromContext() == false)
			...
			Class<?> opclass;
			while ((opclass = (operand = tempUON.getOperand()).getClass()) !=
			    ParameterNode.class && opclass != ParameterizedConstantNode.class) {
				...
				throws StandardException
	{
		operand = operand.bindExpression(fromList, subqueryList,
								aggregateVector);
//      Gemstone Changes Begin
		if (operand.requiresTypeFromContext() && !operand.isParameterizedConstantNode() ) {
			bindParameter();
//		      Gemstone Changes End
       ...
       throws StandardException 
    {
        ...
        operand = operand.genExpressionOperands(outerResultColumns 
                                                       ,null
                                                       ,remapToNew);
        return this;
    }
