/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.conn.TempTableInfo
...
final class TempTableInfo
{
  ...
  // GemStone changes BEGIN
    ...
    this.td = td;
    // GemStone changes BEGIN
    final ConglomerateDescriptorList conglomList = td.getConglomerateDescriptorList();
    assert conglomList.size() == 1 : "we don't expect multiple for temp tables";
    final ConglomerateDescriptor conglomDesc = ((ConglomerateDescriptor)conglomList.get(0));
    conglomId = conglomDesc.getConglomerateNumber();
    try {
      conglomerate = ((GemFireTransaction)tran).findConglomerate(conglomId);
