/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.ColumnDefinitionNode
...
// GemStone changes BEGIN
				hasAutoIncrementInc = aii[QueryTreeNode.HAS_AUTOINCREMENT_INC] != 0;
				setAutoIncrement = aii[QueryTreeNode.SET_AUTOINCREMENT] != 0;
				isGeneratedByDefault = aii[QueryTreeNode.AUTOINCREMENT_IS_GENERATED_BY_DEFAULT] != 0;
// GemStone changes END
	...
	// GemStone changes BEGIN
	/**
         * Is this an auto generated column?
         *
         * @return Whether or not this is an auto generated column.
         */
        boolean isGeneratedByDefault()
        {
	     ...
	     throws StandardException
	{
		...
		{
// GemStone changes BEGIN
		// GemFireXD does not support TINYINT or SMALLINT in autoincrement
		/*
		case Types.TINYINT:
		...
		try
		{
								...
								storable(defaultTypeId, getClassFactory()))
			{
				throw StandardException.newException(SQLState.LANG_NOT_STORABLE, 
					...
					defaultNode.getDefaultText() /* GemStoneAddition */);
			}

// GemStone changes BEGIN
			// TODO: we need more changes to skip defaultTree creation
			// in ResultSetNode and ResultColumnList
			//if (defaultTree instanceof ConstantNode) {
			//  this.defaultValue = ((ConstantNode)defaultTree).getValue();
			//}
// GemStone changes END
