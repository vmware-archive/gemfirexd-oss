/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.ValueNode
		...
		if(dtd.getTypeId().getBaseTypeId().getSQLTypeName().endsWith(DVDSet.class.getSimpleName())) {
		  ...
		  if(this instanceof UnaryOperatorNode) {
	           jtsvn.setType(DataTypeDescriptor.getSQLDataTypeDescriptor(stjvn.getJavaTypeName(), 
                ...
                jtsvn.setType(DataTypeDescriptor.getSQLDataTypeDescriptor(stjvn.getJavaTypeName(), 
                    ((UserDefinedTypeIdImpl)dtd.getTypeId().getBaseTypeId()).getClassParam(), true));
		  }
		}
		else
...
// GemStone changes BEGIN
	/**
	 * To expand the operands of an expression 
	 * 
	 * @param outerResultColumns - operands are added to this list.
	 * @param parentRC - ResultColumn containing this node as part of 'expression' variable of it.
	 *                   null indicates no immediate RC wraps this node. 
	 * @param remapToNew - this is true only in QueryNode whereby current node will point to 
   * the new RC if created. @see ColumnReference
	 * @return new node if remapToNew is true if required otherwise self.
	 * 
	 */
	...
	{
	   if(SanityManager.DEBUG) {
	     //skip for the following as its okay with default handling.
	     if(   !ConstantNode.class.isInstance(this)
	        && !VirtualColumnNode.class.isInstance(this)
                && !TernaryOperatorNode.class.isInstance(this)
                && !ParameterNode.class.isInstance(this)
                   //TODO soubhik support subquery in expressions
                && !SubqueryNode.class.isInstance(this)
	       ) 
	     {
                SanityManager.THROWASSERT("ValueNode::genExpressionOperands() - UnExpected Call in " + this.getClass() + ".. no action will be taken for it.");
	     }
	   }
	   ...
	   return this;
        ...
        /**
         * Returns TRUE if this is a parameter wrapped constant node. Used for generic statement's constant value set.
         * This is exactly same to requiresTypeFromContext() function, only thing is 'requiresTypeFromContext' is used
         * in many other situations applicable than type alignment during conditions. 
         */
        ...
        /**
         * Expression is defined as this node being anything other than single column
         * reference i.e ColumnReference.
         * 
         * @return
         */
        ...
        /**
         * Check to see if the node refers to a base table column
         * Used during GFE get-convertable checks in SelectQueryInfo to determine
         * whether the node can be converted to a get
         * Only base table ColumnReferences should return true here
         * 
         * @return
         */
        ...
        {
          // By default, return nothing, should be overridden by other subclasses
          return "";
        }
        
      /**
       * NCJ Convert this object to a SQL.
       * 
       * @return String
       */
      public String ncjGenerateSql() {
        SanityManager.THROWASSERT("Not added support yet for "
      ...
      public int setColocatedWith(Optimizable src, Optimizable tgt, JBitSet srcPCols, JBitSet tgtPCols) throws StandardException {
        if (SanityManager.ASSERT) {
          SanityManager.THROWASSERT("colocation not handled for " + getClass().getSimpleName());
        }
        return UNKNOWN;
      }

      /**
       * Recursively traverses through all nodes until BaseColumnNode is
       * reached to find the base schema name. This will by pass derived
       * column definitions/aliases/views etc. to find the underlying
       * schema name involved.
       */
      ...
      /**
       * Recursively traverses through all nodes until BaseColumnNode is
       * reached to find the base table name. This will by pass derived
       * column definitions/aliases/views etc. to find the underlying
       * table name involved.
       */
      ...
      /**
       * Recursively traverses through all nodes until BaseColumnNode is
       * reached to find the base column name. This will by pass derived
       * column definitions/aliases/views etc. to find the underlying
