/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.CoalesceFunctionNode
...
public class CoalesceFunctionNode extends ValueNode
{
					...
					throws StandardException
	{
		...
		{
                        if (!(((ValueNode)argumentsList.elementAt(index))
                            .requiresTypeFromContext() && !((ValueNode)argumentsList
                            .elementAt(index)).isParameterizedConstantNode()))
			{
...
// GemStone changes BEGIN
	...
	public ValueNode genExpressionOperands(ResultColumnList outerResultColumns,
		...
		int argumentsListSize = argumentsList.size();
		for (int i = 0; i < argumentsListSize; i++) {
                        // refer CollectExpressionOperandsVisitor
		        ResultColumn paramRC = parentRC;
                        if (ColumnReference.class.isInstance(argumentsList.elementAt(i))) {
                          ColumnReference cr = (ColumnReference)argumentsList.elementAt(i);
                          boolean isAggregateFound = cr.getColumnName().startsWith(
                              "##aggregate result")
                              || cr.getGeneratedToReplaceAggregate();
                          if (isAggregateFound) {
                            paramRC = null;
                          }
                        }
		  
                        argumentsList.setElementAt((ValueNode)((ValueNode)argumentsList
                            .elementAt(i)).genExpressionOperands(outerResultColumns, paramRC,
                            remapToNew), i);
		}
