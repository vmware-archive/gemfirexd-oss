/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.types.TypeId
...
public final class TypeId
{
	...
	// in decimal digits, rather than number of bits, so need a separate constant.
        ...
        */
        ...
        public static final TypeId VARCHAR_ID = create(
        ...
        public static final TypeId JSON_ID = create(
            StoredFormatIds.JSON_TYPE_ID, StoredFormatIds.JSON_TYPE_ID_IMPL);
...
// GemStone changes BEGIN
        public static TypeId getUserDefinedTypeId(String className,
            Object classParam, boolean delimitedIdentifier)
                throws StandardException {
          return new TypeId(StoredFormatIds.USERDEFINED_TYPE_ID_V3,
              new UserDefinedTypeIdImpl(className, classParam),
              delimitedIdentifier);
        }
...
// GemStone changes BEGIN
        {
            return getSQLTypeForJavaType(javaTypeName, null);
        }

        public static TypeId getSQLTypeForJavaType(String javaTypeName, Object typeParam)
            throws StandardException
// GemStone changes END
        {
                ...
                else if (javaTypeName.equals("com.pivotal.gemfirexd.internal.iapi.types.XML"))
                {
                    return XML_ID;
                }
                else
...
// GemStone changes BEGIN
                                try {
                                  this.zeroForFixedType = new SQLDate(
                                      new java.sql.Date(0));
...
// GemStone changes END
                        ...
                        case StoredFormatIds.SMALLINT_TYPE_ID:
                                ...
                                isNumericTypeId = true;
// GemStone changes BEGIN
                                this.zeroForFixedType = new SQLSmallint((short)0);
// GemStone changes END
                                break;
...
// GemStone changes BEGIN
                                try {
                                  this.zeroForFixedType = new SQLTime(
                                      new java.sql.Time(0));
                                } catch (StandardException se) {
                                  throw new GemFireXDRuntimeException(se);
                                }
...
// GemStone changes BEGIN
                                try {
                                  this.zeroForFixedType = new SQLTimestamp(
...
// GemStone changes END
                        ...
                        case StoredFormatIds.TINYINT_TYPE_ID:
                                ...
                                isNumericTypeId = true;
// GemStone changes BEGIN
                                this.zeroForFixedType = new SQLTinyint((byte)0);
// GemStone changes END
                                break;
...
// GemStone changes END
                      ...
                      case StoredFormatIds.JSON_TYPE_ID:
                        typePrecedence = CLOB_PRECEDENCE;
//                        javaTypeName = "java.sql.Clob";
                        javaTypeName = "com.pivotal.gemfirexd.internal.iapi.types.JSON";
                        maxMaxWidth = TypeId.CLOB_MAXWIDTH;
                        isStringTypeId = true;
                        isConcatableTypeId = true;
                        isLongConcatableTypeId = true; // ??
...
//GemStone changes END
                      ...
                      case StoredFormatIds.XML_TYPE_ID:
                              ...
                              javaTypeName = "com.pivotal.gemfirexd.internal.iapi.types.XML";
...
// GemStone changes BEGIN
                if (baseUserTypeId.getSQLTypeName().endsWith(DVDSet.class.getSimpleName())) {
                    DVDSet dvdset = (DVDSet) getNull();
                    typePrecedence = dvdset.resultTypePrecedence();
                    isNumericTypeId = true;
                }
// GemStone changes END
...
// GemStone changes END
        ...
        /**
         * Is this a type id for a json type?
         *
         * @return Whether or not this a type id for a json type.
         */
        ...
        public DataValueDescriptor getNull()
        {
                switch (formatId)
                {
                        ...
                        case StoredFormatIds.USERDEFINED_TYPE_ID_V3:
// GemStone changes BEGIN
                                if (baseTypeId != null && baseTypeId.getSQLTypeName()
                                      .endsWith(DVDSet.class.getSimpleName())) {
                                    return new DVDSet(
                                        ((UserDefinedTypeIdImpl)baseTypeId).getClassParam());
                                }
// GemStone changes END
                                return new UserType();
  ...
  // GemStone changes BEGIN
  public final void assertTypeEquivalent(final DataValueDescriptor dvd, final int colIdx) {
    
    Class<?> c = null;
    // switch case picked from getNull() implementation above.
    switch (formatId) {
      case StoredFormatIds.BIT_TYPE_ID:
        c = SQLBit.class;
        break;
      ...
      case StoredFormatIds.USERDEFINED_TYPE_ID_V3:
        // GemStone changes BEGIN
        if (baseTypeId != null
            && baseTypeId.getSQLTypeName().endsWith(
                DVDSet.class.getSimpleName())) {
          c = DVDSet.class;
          break;
        }
        // GemStone changes END
      ...
      default:
        if (SanityManager.DEBUG) {
          SanityManager.THROWASSERT("unexpected formatId in assertTypeEquivalent() - "
              + formatId);
        }
        c = null;
    }
    ...
    if (!(CloneableObject.class.isAssignableFrom(c))) {
      SanityManager.THROWASSERT("col[" + colIdx + "] ("
          + (c != null ? c.getName() : c) + ") is not a CloneableObject.");
    }

    if (!c.isInstance(dvd)) {
      SanityManager.THROWASSERT("col1.getClass() (" + c
          + ") expected to be the same as col2.getClass() (" + dvd.getClass()
          + ")");
    }
  }
  // GemStone changes END
