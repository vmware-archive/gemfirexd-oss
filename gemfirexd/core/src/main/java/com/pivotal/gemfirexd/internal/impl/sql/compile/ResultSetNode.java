/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.ResultSetNode
...
public abstract class ResultSetNode extends QueryTreeNode
{
	...
	// GemStone changes BEGIN
         ...
         * Copy Constructor
         * 
         * For use in Non Collocated Join Copy everything used till preprocess phase
        ...
        public ResultSetNode(ResultSetNode other) {
          ...
          try {
            ...
            if (other.getReferencedTableMap() != null) {
              this.referencedTableMap = (JBitSet)other.getReferencedTableMap()
                  .clone();
            }
	...
	public void assignResultSetNumber(int rsNum) throws StandardException
        {
                ...
                resultColumns.setResultSetNumber(rsNum);
        }
        ...
        public void ncjAssignResultSetNumber(int rsNum) throws StandardException {
          resultSetNumber = rsNum;
          resultColumns.ncjSetResultSetNumber(rsNum);
        }
...
// GemStone changes BEGIN
								...
								null, Boolean.valueOf(forUpdate),
								/* (original code)
...
// GemStone changes END
	...
	protected static final int numDistinctAggregates(Vector aggregateVector)
	{
		...
		for (int index = 0; index < size; index++)
		{
			count += (((AggregateNode) aggregateVector.elementAt(index)).isDistinct() == true) ?
						1 : 0;
      // GemStone changes BEGIN
			/* Because AVG( distinct ..) splits to SUM/COUNT in GFXD we need to  
			 * accommodate that into the check.
			 * if two consecutive aggregates are distinct and have the operands are same
			 * verbatim by reference, we can conclude those two aggregates to be of AVG. 
			 */
			 if(count > 1) {
			   count = numDistinctAvgAggregate(aggregateVector, index, count);
			 }
      // GemStone changes END
		}
				...
				throws StandardException
	{
        ...
        public ResultSetNode getInnerMostPRN() {
          ...
          {
                  SanityManager.THROWASSERT(
                          "getInnerMostPRN() not expected to be called for " +
                          getClass().getName());
          }
          return null;
        }
        ...
        public void eliminateExtraColumns() {
          eliminateExtraColumns = true;
        }

        /**
         * NCJ Usage
         * 
         * Collect HashKeyColumns
         * 
         * @throws StandardException
         */
        public void collectHashKeyColumns(Vector hashKeyVector, int tabNum, int colNum)
            throws StandardException {
          SanityManager
              .THROWASSERT("collectHashKeyColumns not expected to be called for "
                  + getClass().getName());
        }
         ...
         * NCJ Usage
         * 
         * Returns the underlying table FromBaseTable.
         * Should be Only One Under Given Node, mostly ProjectRestrictNode
         * 
        ...
        protected FromBaseTable ncjGetOnlyOneFBTNode() throws StandardException {
          SanityManager
              .THROWASSERT("ncjGetOnlyOneFBTNode not expected to be called for "
                  + getClass().getName());
          return null;
        }
         ...
         * NCJ Usage
         * 
         * Verify given node is of type ProjectRestrictNode.
         * 
         * @return boolean
         * @throws StandardException
         */
        protected boolean assertProjectRestrictNode() {
          SanityManager.THROWASSERT("Expected ProjectRestrictNode but got class "
              + getClass().getName());
          return false;
        }
         ...
         * NCJ Usage
         * 
         * Verify given node is of type JoinNode.
         * 
         * @return boolean
         */
