/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.NumericTypeCompiler
							...
							throws StandardException
	{
		...
		*/
		...
		if( DVDSet.class.getName().equals(rightTypeId.getBaseTypeId().getSQLTypeName()) ) {
		  rightTypeId = ( (DataTypeDescriptor) ((UserDefinedTypeIdImpl)rightTypeId.getBaseTypeId())
		                                     .getClassParam() ).getTypeId() ;
		}
...
// GemStone changes BEGIN
		// if precision exceeds MAX then adjust both precision and
		// scale to prefer whole digits like in other DBs
		if (precision > Limits.DB2_MAX_DECIMAL_PRECISION_SCALE) {
		  final int lscale = leftType.getScale();
		  final int rscale = rightType.getScale();
		  final int lprec = leftType.getPrecision();
		  final int rprec = rightType.getPrecision();
		  int wholeDigits;
		  if (TypeCompiler.TIMES_OP.equals(operator)) {
		    wholeDigits = (lprec - lscale) + (rprec - rscale);
		    if (wholeDigits > Limits.DB2_MAX_DECIMAL_PRECISION_SCALE) {
		      wholeDigits = Limits.DB2_MAX_DECIMAL_PRECISION_SCALE;
		    }
		  }
		  else if (TypeCompiler.DIVIDE_OP.equals(operator)) {
		    // worst case 999999.xxx / .00001 where digits is
		    // left digits + rscale, but also give minimum
		    // scale so check against that
		    ...
		    if (operator != null && !TypeCompiler.AVG_OP.equals(
		        operator) && !TypeCompiler.MOD_OP.equals(operator)) {
		      wholeDigits++;
		    }
		    if (wholeDigits > Limits.DB2_MAX_DECIMAL_PRECISION_SCALE) {
		      wholeDigits = Limits.DB2_MAX_DECIMAL_PRECISION_SCALE;
...
// GemStone changes BEGIN
		  ...
		  // for SUM operation, the number of entries in the
		  // table will actually determine the precision but
		  // in absence of that assume Integer.MAX_VALUE entries
		  val = this.getScale(operator, leftType, rightType) +
		      Math.max(lprec - lscale, rprec - rscale) + 10;
		}
		// for MOD/AVG operations don't add 1 as in the default handling
		else if (operator.equals(TypeCompiler.MOD_OP)
		    || operator.equals(TypeCompiler.AVG_OP)) {
		  val = this.getScale(operator, leftType, rightType) +
		      Math.max(lprec - lscale, rprec - rscale);
		}
		/* (original code)
...
// GemStone changes END
		...
		{
// GemStone changes BEGIN
			// worst case is 9999999.xxxx / .0000001, so the number
			// of whole digits is leftDigits + rscale, so for
			// precision add scale to this
			val = lprec - lscale + rscale +
			    this.getScale(operator, leftType, rightType);
			/* (original code)
...
// GemStone changes BEGIN
		// adjust the scale and precision w.r.t. MAX allowed at higher
		// level since both have to be adjusted appropriately
		}
		/* (original code)
							...
							DataTypeDescriptor rightType)
	{
		...
		*/
		...
		else if (TypeCompiler.DIVIDE_OP.equals(operator))
		{
			...
			*/
// GemStone changes BEGIN
			// normally expect as many digits after decimal as the
			// precision of divisor, but also check the left side
			// scale for the case when right side has very
			// small precision
			val = Math.max(Math.max(lscale, rprec),
			    NumberDataValue.MIN_DECIMAL_DIVIDE_SCALE);
			/* (original code)
