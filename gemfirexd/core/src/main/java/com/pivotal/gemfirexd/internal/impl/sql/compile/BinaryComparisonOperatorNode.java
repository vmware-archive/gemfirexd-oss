/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.BinaryComparisonOperatorNode
...
public abstract class BinaryComparisonOperatorNode extends BinaryOperatorNode
{
			...
			throws StandardException
	{
		...
		if (this.leftOperand instanceof ColumnReference && this.rightOperand instanceof ConstantNode) {
		  String colName = ((ColumnReference)this.leftOperand).getColumnName();
		  int colNumber = ((ColumnReference)this.leftOperand).getColumnNumber();
		  ResultColumn rc;
		  ColumnDescriptor cd;
		  DataTypeDescriptor dtd;
		  if ((rc = ((ColumnReference)this.leftOperand).getSource()) != null
		      && (cd = rc.columnDescriptor) != null
		      && (dtd = cd.getType()) != null) {
		    ((ConstantNode)this.rightOperand).setCorrespondingColumnInfo(
		        colName, colNumber, dtd.getMaximumWidth());
		  }
		}
