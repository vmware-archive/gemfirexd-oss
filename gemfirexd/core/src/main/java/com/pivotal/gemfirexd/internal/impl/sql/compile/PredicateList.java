/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.PredicateList
...
// GemStone changes BEGIN
	public List<Predicate> redundantPredicates = new ArrayList<Predicate>();
// GemStone changes END
	...
	/**
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.compile.OptimizablePredicateList#getOptPredicate
	 */
	@Override
  public OptimizablePredicate getOptPredicate(int index)
	...
	/**
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.compile.OptimizablePredicateList#removeOptPredicate
	 *
...
// GemStone changes BEGIN
		/*
		 * For any kind of queries global index should never be consulted.
		 * Scans on global indexes are opened from inside directly in case
		 * of lookups but that path will never hit this plan selection code.
		 */
                final boolean isGlobalHashIndex = GfxdConstants.GLOBAL_HASH_INDEX_TYPE
                    .equals(cd.getIndexDescriptor().indexType());
                if (isGlobalHashIndex) {
                  return retval;
                }
                final boolean isLocalHashIndex = GfxdConstants.LOCAL_HASH1_INDEX_TYPE
                    .equals(cd.getIndexDescriptor().indexType());
// GemStone changes END
		/*
...
// GemStone changes BEGIN
			// don't try to use anything other than == or IN with
			// Hash1Index
			if ((relop == null || relop.getOperator() != RelationalOperator
			    .EQUALS_RELOP) && isLocalHashIndex) {
			  continue;
			}
// GemStone changes END
			/*
	 ...
	 */
	@Override
  public void pushUsefulPredicates(Optimizable optTable, boolean pushDeemedNonpushables)
						throws StandardException
	{
		...
		orderUsefulPredicates(optTable,
							...
							ap.getCoveringIndexScan(), pushDeemedNonpushables);
	}
	//GemStone changes END
  ...
  public void classify(Optimizable optTable, ConglomerateDescriptor cd)
			throws StandardException
	{
		...
		*/
	//GemStone changes BEGIN
		orderUsefulPredicates(optTable, cd, false, false, false, false /* push deemed non pushables*/);
	//GemStone changes END
	}
...
// GemStone changes BEGIN
			// for OR list always assume no equality
			if (andNode instanceof OrListNode) {
			  return false;
			}
// GemStone changes END
  ...
  public void putOptimizableEqualityPredicateFirst(Optimizable optTable,
	...
	private void orderUsefulPredicates(Optimizable optTable,
										...
										boolean pushDeemedNonpushables)
						throws StandardException
	{
		boolean[]	deletes;
...
// GemStone changes BEGIN
                //Asif: Changes to fix bug 40879
                Set<Integer> hashIndexablePreds = null;
                int [] positionInUsefulPredList = null;
		int slotsFilled = 0;
		final boolean isLocalHashIndex = id.indexType().equals(
		    GfxdConstants.LOCAL_HASH1_INDEX_TYPE);
                if (isLocalHashIndex) {
                  hashIndexablePreds = new HashSet<Integer>(
                      baseColumnPositions.length);
                  positionInUsefulPredList = new int[size];
                }
// GemStone changes END
		...
		for (int index = 0; index < size; index++)
		{
				...
				indexPosition++)
			{
				...
				{
              // GemStone changes BEGIN
              indexCol = pred.andNode instanceof OrListNode ? ((OrListNode)pred.andNode)
                         .getColumnOperand(optTable, baseColumnPositions[indexPosition])
                        : relop.getColumnOperand(optTable, baseColumnPositions[indexPosition]);
                                        // GemStone changes END
...
// GemStone changes BEGIN
				...
				if (indexCol != null) {
  				//GemStone changes BEGIN
                                  if (!pred.pushable && pushDeemedNonpushables) {
                                    // get the other side operand
                                    ValueNode otherOp = relop.getOtherOperand(indexCol);
                                    // If the otherOp is also dependent on table same as indexCol , then
                                    // it cannot
                                    // be used as a useful predicate
                                    JBitSet referencedTables = new JBitSet(pred.referencedSet.size());
                                    otherOp.categorize(referencedTables, false);
                                    if (referencedTables.get(optTable.getTableNumber())) {
                                      indexCol = null;
                                      continue;
                                    }else {
                                      optTable.delayScanOpening(true);
                                    }
                                  }
  		                //GemStone changes END
				  if (isLocalHashIndex) {
				    if ((relop != null && relop.getOperator()
				        == RelationalOperator.EQUALS_RELOP)
				        || (pred.andNode instanceof OrListNode)) {
				      hashIndexablePreds.add(indexPosition);
				      positionInUsefulPredList[slotsFilled++] = usefulCount;
				    }
				    else {
				      // force skip of this predicate
				      indexCol = null;
				    }
				  }
				  break;
...
// GemStone changes BEGIN
		//Asif: Changes to fix bug 40879
		if (isLocalHashIndex) {
		  if (hashIndexablePreds.size() != baseColumnPositions.length) {
		    usefulCount -= slotsFilled;
		    for(int i = 0;i < slotsFilled ;++i) {
		      usefulPredicates[positionInUsefulPredList[i]] = null;
		    }
		  }
		}
// GemStone changes END
...
// GemStone changes BEGIN
			//Asif: Changes to fix bug 40879
			int j = 0;
			for (int i = 0; i < usefulPredicates.length; i++) {
			  if (usefulPredicates[i] != null) {
			    shrink[j++] = usefulPredicates[i];
			  }
			}
			/* (original code)
...
// GemStone changes BEGIN
				if (isIn && !thisPred.isInListProbePredicate() &&
				    !(thisPred.getAndNode() instanceof OrListNode))
				/* (original code)
				if (isIn && !thisPred.isInListProbePredicate())
				*/
// GemStone changes END
		...
		throws StandardException
	{
		...
		// GemStone changes BEGIN
                for (int index = 0; index < redundantPredicates.size(); index++) {
                  redundantPredicates.get(index).categorize();
                }
		// GemStone changes END
	}
  ...
  public void printSubNodes(int depth)
	 ...
	 */
	//Gemstone changes Begin       
	 public         void pullExpressions(int numTables,
	...
	public void xorReferencedSet(JBitSet fromMap)
	{
		...
		for (int index = 0; index < size; index++)
		{
			...
			{
// GemStone changes BEGIN
			  if (fromMap.size() != predicate.getReferencedSet().size())
// GemStone changes END
		...
		throws StandardException
	{
		 ...
		 throws StandardException
	 {
		...
		for (int index = size() - 1; index >= 0; index--)
		{
			...
			{
				removeElementAt(index);
                                redundantPredicates.add(predicate);
			}
		}
	 }
  ...
  public void transferPredicates(OptimizablePredicateList otherList,
		...
		throws StandardException
	{
		...
		theOtherList.orderUsefulPredicates(
									...
									//GemStone changes BEGIN
									true /* push deemed non pushables*/
									//GemStone changes END
		                                                      );
...
// GemStone changes END
	 ...
	 */
        public void moveJoinPredicatesToOtherList(OptimizablePredicateList otherList,
            Optimizable optTable) throws StandardException {
          for (int idx = size() - 1; idx >= 0; idx--) {
            Predicate pred = (Predicate)elementAt(idx);
            if (pred.referencedSet.get(optTable.getTableNumber())) {
  ...
  public boolean isRedundantPredicate(int predNum)
	{
		...
		//GemStone Changes BEGIN
                if(!pred.getAndNode().isUsableInSelectivity()) {
                  return true;
                }
                //GemStone Changes END
        ...
        throws StandardException
    {
        ...
        */
        ...
        consMB.callMethod(
            ...
            //Gemstone changes BEGIN
            // Extra column name parameter needed
            "getQualifier", ClassName.Qualifier, 9);
...
//Gemstone changes End         
	{
		for (int index = size() - 1; index >= 0; index--)
		{
			...
			InListOperatorNode ilon = pred.getSourceInList();
                        if (ilon.getIsRightOperandArray()) {
                          ilon.ncjGenerateListAsArray(acb, mb);
                        }
                        else {
...
// GemStone changes BEGIN
        public int getStartPredicatesNum() {
             return this.numberOfStartPredicates;
        }

        @Override
        public boolean hasOrList() {
          for (int i = 0, size = size(); i < size; i++) {
            Predicate pred = (Predicate)elementAt(i);
        ...
        public int getStopPredicatesNum() {
             return this.numberOfStopPredicates;
        }
        
        public void copyPredicatesToOtherListSkipPushedPredicates(OptimizablePredicateList otherList) throws StandardException {
          
            for (int i = 0, size = size(); i < size; i++)
            {
                  Predicate pred = ((Predicate) getOptPredicate(i));
                  
                  if ( pred.startKey) {
                     if(pred.stopKey || pred.isQualifier) {
                       continue;
                     }
                  }
                  else if(pred.stopKey) {
                    if(pred.startKey || pred.isQualifier) {
                      continue;
                    }
                  }
                  otherList.addOptPredicate(pred);
        ...
        /**
         * NCJ Note: For NCJ Only. Mark and store all Equi Join Pairs.
         * 
         * @see joinClauseTransitiveClosure
         */
        public void setEquiJoins(THashMap equiJoinMap,
          ...
          for (int index = 0; index < size; index++) {
            ...
            if ((left instanceof ColumnReference && right instanceof ColumnReference)) {
              ...
              if (leftCR.getSourceLevel() == rightCR.getSourceLevel()
                  && leftCR.getTableNumber() != rightCR.getTableNumber()
                  && tableNumberList.contains(leftCR.getTableNumber())
                  && tableNumberList.contains(rightCR.getTableNumber())) {
                if (!equiJoinMap.containsKey(leftCR.getTableNumber())) {
                  equiJoinMap.put(leftCR.getTableNumber(), leftCR);
                }
                if (!equiJoinMap.containsKey(rightCR.getTableNumber())) {
                  equiJoinMap.put(rightCR.getTableNumber(), rightCR);
                }
              }
              continue;
            }
        ...
        protected void optimizeForOffHeap(boolean shouldOptimize) {
          ...
          {
            Predicate pred = (Predicate) elementAt(index);
            pred.optimizeForOffHeap(shouldOptimize);
          }

        }

        @Override
        public int setColocatedWith(Optimizable src, Optimizable target)
            throws StandardException {
          
          if (!src.isColocatedWith(target)) {
            return ValueNode.NOTCOLOCATED;
          }
      
          final int srcNumPCols = src.getNumPartitioningCols(), tgtNumPCols = target
              .getNumPartitioningCols();
      
          if (srcNumPCols != tgtNumPCols) {
            return ValueNode.NOTCOLOCATED;
          }
      
          final JBitSet srcPCols = new JBitSet(srcNumPCols), tgtPCols = new JBitSet(
              tgtNumPCols);
          ArrayList<Predicate> partColPred = new ArrayList<Predicate>();
      
          for (int index = 0; index < size(); index++) {
            final Predicate pred = (Predicate)elementAt(index);
            if (!pred.referencedSet.contains(src.getReferencedTableMap())
                || !pred.referencedSet.contains(target.getReferencedTableMap())) {
              continue;
            }
      
            int ret = -1;
            if ((ret = pred.setColocatedWith(src, target, srcPCols, tgtPCols)) == ValueNode.UNKNOWN) {
              if (SanityManager.DEBUG) {
                SanityManager.DEBUG_PRINT("warning:Colocation",
                    "Ignoring colocation criteria due to insufficient code for pred : "
                        + pred);
              }
              return ret;
            }
            ...
            else if (ret == ValueNode.PARTLYCOLOCATED) {
              partColPred.add(pred);
            }
          }
      
          if (srcPCols.hasAllBitSet() && tgtPCols.hasAllBitSet()
              && srcPCols.equals(tgtPCols)) {
            for (Predicate p : partColPred) {
              // remove predicates that in conjunction with other predicates fulfills
            ...
            return ValueNode.COMPLETELYCOLOCATED;
          }
