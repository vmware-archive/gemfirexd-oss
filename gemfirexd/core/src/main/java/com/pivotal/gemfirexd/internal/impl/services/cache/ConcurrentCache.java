/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.services.cache.ConcurrentCache
...
public class ConcurrentCache implements CacheManager {
  /** Map with all the cached objects. */
  protected final ConcurrentHashMap<Object, CacheEntry> cache;
  /** Factory which creates <code>Cacheable</code>s. */
  protected final CacheableFactory holderFactory;
  /** Name of this cache. */
...
// GemStone changes END
    ...
    void evictEntry(Object key) {
        CacheEntry entry = cache.remove(key);
// GemStone changes BEGIN
        // entry can be different from that locked due to Holder.switchEntry()
        // call, so let the caller clear identity (UseCase8's support issue #6131)
        if (entry != null) {
...
// GemStone changes END
     ...
     */
// GemStone changes BEGIN
  protected final Cacheable insertIntoFreeSlot(Object key, CacheEntry entry)
...
// GemStone changes END
        ...
        try {
            ...
            removeEntry(key);
            throw se;
        } catch (GemFireXDRuntimeException ge) {
            removeEntry(key);
            throw ge;
        }
    ...
    public void release(Cacheable item) {
        ...
        //GemStone changes BEGIN
        /*[sb] do null handling to fix #42544. 
         * call between cc.find() and cc.release() the entry can vanish from 
         * cache due to atleast one reason (EvictEntry) where its independent 
         * of DD activity i.e. DD locks. So, DataDictionary seek occurring 
         * concurrently (DD read locks) might result into entry getting evicted
         * in between find() and release(). So guarding NPE.
         */
        if(entry == null) {
          //nothing to be done.
          return;
 ...
 // GemStone changes BEGIN
    public ConcurrentHashMap<Object, CacheEntry> getCache() {
      return cache;
    }
 // GemStone changes END
}
