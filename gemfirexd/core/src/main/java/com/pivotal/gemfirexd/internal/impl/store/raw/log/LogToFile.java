/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.store.raw.log.LogToFile
...
import com.pivotal.gemfirexd.internal.iapi.services.diag.Performance;
								  ...
								  Serviceable, java.security.PrivilegedExceptionAction
{
	...
	*/
	...
	public static final String DUMP_LOG_FROM_LOG_FILE = 
		SanityManager.DEBUG ? "gemfirexd.storage.logDumpStart" : null;
    ...
    /**
     * When the gemfirexd.system.durability property is set to 'test', the store 
     ...
     * @see com.pivotal.gemfirexd.internal.iapi.reference.Property#DURABILITY_PROPERTY
    ...
    // disable syncing of log file when running in gemfirexd.system.durability=test
				...
				if (SanityManager.DEBUG)
				{
					if (SanityManager.DEBUG_ON("setCheckpoint"))
					{
						...
						String checkpointStartLogStr =
							PropertyUtil.getSystemProperty(
                                "gemfirexd.storage.checkpointStartLog");

						String checkpointStartOffsetStr =
							PropertyUtil.getSystemProperty(
                                "gemfirexd.storage.checkpointStartOffset");
                        ...
                        else
                        {
                            SanityManager.THROWASSERT(
                                "must set gemfirexd.storage.checkpointStartLog and gemfirexd.storage.checkpointStartOffset, if setting setCheckpoint.");
                        }
	...
	*/
		 ...
		 throws IOException, StandardException
	{
        ...
        // When database is booted with gemfirexd.system.durability=test,
		...
		try
		{
			...
			if ( onDiskChecksum == checksum.getValue() || onDiskChecksum ==0)
			{
                ...
                if (SanityManager.DEBUG) {
                    if (SanityManager.DEBUG_ON(LogToFile.DBG_FLAG))
                        SanityManager.DEBUG(LogToFile.DBG_FLAG,
                        "log control file, was gemfirexd.system.durability set to test = " +
                        wasDBInDurabilityTestModeNoSync);
                }
	...
	*/
	public void	boot(boolean create, Properties startParams) throws StandardException
	{
		...
		logBufferSize =  PropertyUtil.getSystemInt(com.pivotal.gemfirexd.internal.iapi.reference.Property.LOG_BUFFER_SIZE, 
												   LOG_BUFFER_SIZE_MIN, 
												   LOG_BUFFER_SIZE_MAX, 
		...
		//get log factorty properties if any set in gemfirexd.properties
		getLogFactoryProperties(null);

		/* check if the storage factory supports write sync (rws and rwd). If
		 * so, use it unless gemfirexd.storage.fileSyncTransactionLog property is
		 * set true by user.
		 */
		...
		if (logStorageFactory.supportsWriteSync())
        {
			...
			//disable write sync if gemfirexd.storage.fileSyncTransactionLog is true
			isWriteSynced =
        ...
        // If gemfirexd.system.durability=test is set,then set flag to 
		...
		try
		{
			...
			if (!createNewLog)
			{
                if (privExists(logControlFileName))
				{
                    ...
                    // gemfirexd.system.durability=test then print a message 
                    // to the derby log
	...
	*/
		 ...
		 throws StandardException
	{
		...
		if(set == null)
		{
			lsInterval=PropertyUtil.getSystemProperty(com.pivotal.gemfirexd.internal.iapi.reference.Property.LOG_SWITCH_INTERVAL);
			cpInterval=PropertyUtil.getSystemProperty(com.pivotal.gemfirexd.internal.iapi.reference.Property.CHECKPOINT_INTERVAL);
		}else
		{
			lsInterval = PropertyUtil.getServiceProperty(set, com.pivotal.gemfirexd.internal.iapi.reference.Property.LOG_SWITCH_INTERVAL);
			cpInterval = PropertyUtil.getServiceProperty(set, com.pivotal.gemfirexd.internal.iapi.reference.Property.CHECKPOINT_INTERVAL);
		}
	...
	*/
     ...
     * with gemfirexd.system.durability=test or was previously at any time booted in
     * this mode, mention in the error message that the error is probably 
     * because the gemfirexd.system.durability was set. 
	...
	*/
     ...
     * @see com.pivotal.gemfirexd.internal.impl.db.SlaveDatabase
     */
