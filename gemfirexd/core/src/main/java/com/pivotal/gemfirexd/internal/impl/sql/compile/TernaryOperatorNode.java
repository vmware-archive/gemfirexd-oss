/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.TernaryOperatorNode
...
//  GemStone changes BEGIN
    /**
     * It may look like substring() etc doesn't need expansion of its operands. For simplicity
     * we expand this and transport the complete column, otherwise we have to tweak with the
     * generateExpression to point to shorter value column and have to evaluate this at the 
     * data store node. In case of an expression this selective evaluation seems to complicate
     * the implementation.
     * 
     * e.g. select .... having address like ?
     ...
     * 
     * TODO We might look for better solution when we try to re-utilize the same source column(s) 
     * in different expressions later.
     */
    @Override
       ...
       throws StandardException 
    {
        ...
        receiver = receiver.genExpressionOperands(outerResultColumns, null, remapToNew);
        leftOperand = leftOperand.genExpressionOperands(outerResultColumns, null, remapToNew);
        // right operand can be null at times ... e.g. SELECT 'xx' || TRIM(CHAR(MAX(xxx)/1000))) from ... 
        if(rightOperand != null)
          rightOperand = rightOperand.genExpressionOperands(outerResultColumns, null, remapToNew); 
    ...
    {
      // Ternary ops have a left op, method name and optional right op and receiver
      String explainString = getMethodName();
      explainString += "("+ leftOperand.printExplainInfo();
      if (rightOperand != null)
      {
      ...
      if (receiver != null)
      {
        explainString += " "+receiver.printExplainInfo();
      }
