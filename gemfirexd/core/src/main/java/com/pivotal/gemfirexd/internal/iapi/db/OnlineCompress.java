/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.db.OnlineCompress
...
Implementation of SYSCS_UTIL.INPLACE_COMPRESS_TABLE().
<p>
...
<p>
Use the SYSCS_UTIL.INPLACE_COMPRESS_TABLE system procedure to reclaim 
...
destroyed. SYSCS_UTIL.INPLACE_COMPRESS_TABLE allows you to return unused 
...
SYSCS_UTIL.COMPRESS_TABLE() all work is done in place in the existing
table/index.
<p>
Syntax:
SYSCS_UTIL.INPLACE_COMPRESS_TABLE(
    IN SCHEMANAME        VARCHAR(128),
    IN TABLENAME         VARCHAR(128),
    IN PURGE_ROWS        SMALLINT,
    IN DEFRAGMENT_ROWS   SMALLINT,
...
call SYSCS_UTIL.INPLACE_COMPRESS_TABLE('US', 'CUSTOMER', 1, 1, 1);
...
call SYSCS_UTIL.INPLACE_COMPRESS_TABLE('US', 'CUSTOMER', 0, 0, 1);
...
("CALL SYSCS_UTIL.COMPRESS_TABLE(?, ?, ?, ?, ?)");
cs.setString(1, "US");
...
<p>
It is recommended that the SYSCS_UTIL.COMPRESS_TABLE procedure is 
...
// added the last argument null for activation
                    0, null);		//
