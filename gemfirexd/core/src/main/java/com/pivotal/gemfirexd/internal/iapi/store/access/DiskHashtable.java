/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.store.access.DiskHashtable
...
//      GemStone changes Begin
        // insert the row into the "base" conglomerate.
         btreeRow[1] = rowConglomerate.insertAndFetchLocation( 
...
//       GemStone changes END
        ...
        throws StandardException
    {
        ...
        try
        {
            ...
            {
                RowLocation rl = rowConglomerate.fetch(
                    (RowLocation) btreeRow[1], 
                    row, 
                    (FormatableBitSet) null /* all columns */, false);
                
                 
                if (rl != null && rowMatches( row, key))
                {
                  btreeRow[1] = rl;
                    if( existenceOnly)
                        return this;
...
//                  GemStone changes Begin
                    rowloc = scan.fetchLocation(rowloc);
//                  GemStone changes End
                }
            }
