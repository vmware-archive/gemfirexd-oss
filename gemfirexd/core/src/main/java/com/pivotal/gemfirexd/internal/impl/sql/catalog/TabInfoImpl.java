/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.catalog.TabInfoImpl
...
// GemStone changes END
		...
		drivingIndexRow = getIndexRowFromHeapRow(
			...
			crf.makeEmptyRow());
            //    System.out.println("XXXXTabInfoImpl:");
           //     ((MemIndexScanController)drivingScan).dumpIndex();
		while (drivingScan.fetchNext(drivingIndexRow))
		{
			baseRowLocation = (RowLocation)
						drivingIndexRow.getLastColumn();

// GemStone change BEGIN
                baseRowLocation = heapCC.fetch(
                    baseRowLocation, baseRow, (FormatableBitSet)null, false);
                boolean base_row_exists = baseRowLocation != null;
                if (base_row_exists) {
                  drivingIndexRow.setColumn(drivingIndexRow.nColumns(),
                      baseRowLocation);
                }
// GemStone change END
            if (SanityManager.DEBUG)
...
// GemStone change BEGIN
			if (drivingScan.fetchNext(drivingIndexRow))
			{
				rl[0] = baseRowLocation = (RowLocation)
					drivingIndexRow.getLastColumn();
				baseRowLocation = heapCC.fetch(
				    baseRowLocation, baseRow, (FormatableBitSet)null, true);
				boolean base_row_exists = baseRowLocation != null;
				if (base_row_exists) {
