/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.services.monitor.Monitor
...
/**
	...
	or the system (JVM) set.
	<PRE>
	gemfirexd.service.<EM>service name</EM>=<EM>persistent storage type</EM>
	e.g.
	gemfirexd.service.mydatabase=serviceDirectory
	</PRE>
  ...
  <P><B>Module Implementations</B><BR>
	...
	<PRE>
	gemfirexd.module.<EM>tag</EM>=<EM>java class name</EM>
	</PRE>
	...
	gemfirexd.module.lockManager for an implementation of a lock manager.
	<BR>
	...
	<LI>The application properties set (i.e. obtained from the gemfirexd.properties file).
	<LI>The default implementation properties set (i.e. obtained from the
	/com/pivotal/gemfirexd/internal/modules.properties resource).
	</OL>
...
// GemStone changes BEGIN
        // now moved to SanityManager to enable use by clients too and
        // also made available in insane builds
        public static final String DEBUG_TRUE = SanityManager.DEBUG_TRUE;
        public static final String DEBUG_FALSE = SanityManager.DEBUG_FALSE;
        /* (original code)
	public static final String DEBUG_TRUE = SanityManager.DEBUG ? "gemfirexd.debug.true" : null;
	public static final String DEBUG_FALSE = SanityManager.DEBUG ? "gemfirexd.debug.false" : null;
	*/
...
// GemStone changes BEGIN
		...
		com.gemstone.gemfire.internal.cache.GemFireCacheImpl.setGFXDSystem(true);
		new com.pivotal.gemfirexd.internal.impl.services.monitor.FileMonitor(bootProperties, logging);
// GemStone changes END
	}
	/**
	...
	*/
	...
	public static ModuleFactory getMonitorLite() {
		...
		// for lookup of gemfirexd.system.home.
		// This instance will be discarded once it is used.				;

                return new com.pivotal.gemfirexd.internal.impl.services.monitor.FileMonitor();
	}
	
// GemStone changes BEGIN
	public static boolean inShutdown() {
	  final ModuleFactory m = monitor;
	...
	public static ModuleFactory getCachedMonitorLite(boolean clear) {
          ...
          // [sb] caching monitorlite so that anything reported on it via tmpWriter
          // gets flushed out to the log eventually after boot.
          if (!clear && monitorlite == null) {
            monitorlite = new com.pivotal.gemfirexd.internal.impl.services.monitor.FileMonitor();
          }
          
          final ModuleFactory retVal = monitorlite;
...
// GemStone changes END
	...
	public static HeaderPrintWriter getStream() {
            return monitor != null ? monitor.getSystemStreams().stream()
                : null;
	}
