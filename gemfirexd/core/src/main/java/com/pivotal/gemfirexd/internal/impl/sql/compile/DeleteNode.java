/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.DeleteNode
...
// GemStone changes BEGIN
					...
					graphHashTable.put(currentTargetTableName,
					    Integer.valueOf(noDependents));
					/* (original code)
...
// GemStone changes END
	...
	private boolean hasReferentialConstraint() throws StandardException{
	  ...
	  if(this.targetTableDescriptor != null) {
	    ...
	    StaticCompiledOpenConglomInfo scoc =tc.getStaticCompiledConglomInfo(heapConglomId);
	    GemFireContainer baseContainer = ((MemConglomerate)scoc
	        .getConglom()).getGemFireContainer();
	       return  (!baseContainer.isTemporaryContainer())
	                    && baseContainer.getExtraTableInfo().getReferencedKeyColumns() != null;
	  }
	  return false;
	}
							...
							throws StandardException
	{
	  ...
	  boolean optimizeforOffHeap = true;
	  
	  if((this.deferred || this.cascadeDelete) 
	      && (this.getFKInfo() != null || this.getTriggerInfo() != null)
	      && !hasReferentialConstraint()) {
	    optimizeforOffHeap = false;
	  }
		this.optimizeForOffHeap(optimizeforOffHeap);	
...
//Gemstone changes Begin
		/*
		DataDictionary dd=this.targetTableDescriptor.getDataDictionary();
        this.hasReferencedKey=this.hasDependentTables(dd, this.targetTableDescriptor);
        if(this.hasReferencedKey){ 

           getCompilerContext().setLocalScope();
        //  getCompilerContext().disableParseNode();
        }
        */
//Gemstone changes End       
...
//      GemStone changes BEGIN
	       ...
	       @Override
	        public  QueryInfo computeQueryInfo(QueryInfoContext qic)throws StandardException 
	        {
	           ...
	           DeleteQueryInfo info = new DeleteQueryInfo(qic);	           
	           //this.accept(info);
	           info.visit(this);	           
	           info.init();
	           return info;
	       ...
	       private boolean hasDependentTables(DataDictionary dd, TableDescriptor td) 
	           throws StandardException{
	    		...
	    		for (int index = 0; index < cdlSize; index++)
	    		{
	    				...
	    				(refcd.getForeignKeyConstraints(ConstraintDescriptor.ENABLED) );
