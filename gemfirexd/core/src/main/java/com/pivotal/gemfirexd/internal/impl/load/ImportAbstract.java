/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.load.ImportAbstract
...
// GemStone changes BEGIN
                      hexData, lineNumber, columnIndex));
                      /* (original code)
                      hexData));
                      */
// GemStone changes END
	...
	private boolean isColumnInExtFile(int colIndex) 
	{
		if (lobsInExtFile && 
            (tableColumnTypes[colIndex -1] == java.sql.Types.BLOB || 
             ...
             tableColumnTypes[colIndex -1] == JDBC40Translation.JSON 
             ))
			return true;
		else 
			return false;
