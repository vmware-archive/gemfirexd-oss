/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.tools.sysinfo.Main
...
package com.pivotal.gemfirexd.internal.impl.tools.sysinfo;
...
import com.pivotal.gemfirexd.internal.iapi.services.info.PropertyNames;
...
/**
  ...
  <P>
  Sysinfo looks for properties files in com.pivotal.gemfirexd.internal.info named after
  the genus names in com.pivotal.gemfirexd.internal.tools.sysinfo, and gets their location
  ...
  <PRE>
    java com.pivotal.gemfirexd.internal.tools.sysinfo
  </PRE>
...
public final class Main {
    ...
    private final static String MESSAGE_FILE =
        "com.pivotal.gemfirexd.internal.loc.sysinfoMessages";
     ...
     * the locale and codeset specified by gemfirexd.ui.locale and gemfirexd.ui.codeset
     * if they are set.
     *
...
public static void getMainInfo (java.io.PrintWriter aw, boolean pause) {
  ...
  */
  ...
  private static void reportDerby (java.io.PrintWriter localAW) {
	  ...
	  localAW.println("JRE - JDBC: " + com.pivotal.gemfirexd.internal.iapi.services.info.JVMInfo.derbyVMLevel());
  ...
  private static void reportLocales (java.io.PrintWriter localAW) {
    ...
    for (int i = 0; i < stringLocales.length; i++) {
      ...
      String localeResource =
         "/com/pivotal/gemfirexd/internal/info/locale_" + stringLocales[i] + ".properties";
...
//           localAW.println("resource is null: " + localeResource);
	...
	String localeName = p.getProperty("gemfirexd.locale.external.name");
	...
	localeName = localeName.substring(0,localeName.indexOf("]"));
			...
			int major = Integer.valueOf(p.getProperty ("gemfirexd.locale.version.major")).intValue();
			int minor = Integer.valueOf(p.getProperty ("gemfirexd.locale.version.minor")).intValue();
			int maint = Integer.valueOf(p.getProperty ("gemfirexd.locale.version.maint")).intValue();
			String build = p.getProperty ("gemfirexd.locale.build.number");
	...
	private static final String MAINUSAGESTRING = "java com.pivotal.gemfirexd.internal.tools.sysinfo -cp";
  ...
  static  void useMe(String[] args, java.io.PrintWriter pw) {
	  ...
	  private static void tryAllClasspaths(java.io.PrintWriter localPW) throws Throwable {
		  ...
		  StringBuilder successes = new StringBuilder(Main.getTextMessage("SIF08.D")+ crLf());
		  StringBuilder failures = new StringBuilder(crLf() + Main.getTextMessage("SIF08.E") + crLf());
	...
	private static void tryCoreClasspath(StringBuilder successes, StringBuilder failures) {
		tryMyClasspath("com.pivotal.gemfirexd.internal.database.Database", Main.getTextMessage("SIF08.J","gemfirexd.jar" ), successes, failures);
	}
	private static void tryNetClasspath(StringBuilder successes, StringBuilder failures) {
		tryMyClasspath("com.pivotal.gemfirexd.internal.database.Database", Main.getTextMessage("SIF08.J","gemfirexd.jar" ), successes, failures);
	...
	private static void tryUtilsClasspath(StringBuilder successes, StringBuilder failures) {
		tryMyClasspath("com.pivotal.gemfirexd.internal.tools.ij", Main.getTextMessage("SIF08.Q", "gemfirexd-tools.jar"), successes, failures);
	}

	private static void tryMyClasspath(String cn, String library, StringBuilder successes, StringBuilder failures) {
	...
	private static void tryAsResource(String cn, String library, StringBuilder successes, StringBuilder failures) {
	...
	*/
    ...
    private static final String infoNames[] = {
                                    ...
                                    com.pivotal.gemfirexd.internal.iapi.services.info.ProductGenusNames.DBMS +
                                    ".properties",


                                    "com/pivotal/gemfirexd/internal/info/" +
                                    com.pivotal.gemfirexd.internal.iapi.services.info.ProductGenusNames.TOOLS +
                                    ".properties",

                                    "com/pivotal/gemfirexd/internal/info/" +
                                    com.pivotal.gemfirexd.internal.iapi.services.info.ProductGenusNames.NET +
                                    ".properties",

                                   "com/pivotal/gemfirexd/internal/info/" +
                                    com.pivotal.gemfirexd.internal.iapi.services.info.ProductGenusNames.DNC +
                                    ".properties"
                                };
    ...
    public static ZipInfoProperties[] getAllInfo(String classpath)
    {
        ...
        if (zips == null)
        {
            ...
            ZipInfoProperties zip = new ZipInfoProperties(ProductVersionHolder.getProductVersionHolderFromMyEnv(com.pivotal.gemfirexd.internal.tools.sysinfo.TOOLS));
     ...
     *  com/pivotal/gemfirexd/internal/info inside the directory.
     *
	...
	if (filename.indexOf("db2jcc") >= 0)
	{
                ...
                try 
		{
		    ...
		    m = c.getMethod("getJCCBuildNumber");
		    o = c.newInstance();
		    build = (Integer)m.invoke(o);
		} catch (ClassNotFoundException cnfe) {
		    c = Class.forName("com.ibm.db2.jcc.DB2Version");
		    m = c.getMethod("getBuildNumber");
		    o = c.newInstance();
		    build = (Integer)m.invoke(o);
	        } 
		m = c.getMethod("getMajorVersion");
		major = (Integer)m.invoke(o);
		m = c.getMethod("getMinorVersion");
		minor = (Integer)m.invoke(o);
	...
	*/
            ...
            if (zip1[i] != null && zip1.length > 1)
            {
                for (int j = i + 1; j < zip1.length; j++)
                {
                  //GemStone changes BEGIN
                    if (zip1[j] != null && zip1[i].getLocation().equals(zip1[j].getLocation()))
                    /*original code if (zip1[i].getLocation().equals(zip1[j].getLocation())) */
    ...
    private static String formatURL(URL loc)
    {
        ...
        if (filename.indexOf("/com/pivotal/gemfirexd/internal") > -1) { 
            filename = filename.substring(0, filename.indexOf("/com/pivotal/gemfirexd/internal")); 
        }
