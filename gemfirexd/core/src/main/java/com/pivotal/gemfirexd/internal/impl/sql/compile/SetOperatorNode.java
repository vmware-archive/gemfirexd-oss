/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.SetOperatorNode
...
abstract class SetOperatorNode extends TableOperatorNode
{
	...
	*/
	...
	public void pullOptPredicates(
		OptimizablePredicateList optimizablePredicates, /*GemStone changes BEGIN*/JBitSet outerTables /*GemStone changes END*/)
		throws StandardException
	{
		if (pushedPredicates == null)
	 ...
	 throws StandardException
	{
		...
		for (int i = 0; i < types.length; i++)
		{
			if (types[i] == null)
			{
				...
				if ( ! (rc.getExpression().requiresTypeFromContext() && !rc.getExpression().isParameterizedConstantNode()))
				{
		...
		throws StandardException
	{
		...
		ResultSetNode result =
			(ResultSetNode) getNodeFactory().getNode(
				...
				null,     // GROUP BY list
				null,     // having clause
                Boolean.valueOf(false), /* Dml op as this is a SELECT */
				getContextManager());
...
// GemStone changes BEGIN
	      ...
	      throws StandardException 
	  {
          ...
          public void eliminateUnUsedColumns(ResultColumnList parentResultColumn,
            ...
            // Do nothing #45876
            // Union query will any be provided with equal number of columns in projection list
            // so eliminating unnecessary columns is not required.
          }
