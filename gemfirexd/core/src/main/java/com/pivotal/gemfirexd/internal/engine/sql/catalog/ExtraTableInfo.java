...
public final class ExtraTableInfo extends ExtraInfo implements Dependent {
  ...
  private LongObjectHashMap<ColumnDescriptor> autoGenColumns;
...
//    }
    ...
    for (int index = 0; index < cdl.size(); ++index) {
      ...
      if (cd.isAutoincrement()) {
        if (this.autoGenColumns == null) { // user provided column indexes
          this.autoGenColumns = LongObjectHashMap.withExpectedSize(4);
  ...
  public final int[] getAutoGeneratedColumns() {
    if (this.autoGenColumns != null) {
      ...
      this.autoGenColumns.forEachWhile(new LongObjPredicate<ColumnDescriptor>() {
        private int index;

        @Override
        public boolean test(long k, ColumnDescriptor cd) {
          keys[index++] = (int)k;
          return true;
        }
      });
