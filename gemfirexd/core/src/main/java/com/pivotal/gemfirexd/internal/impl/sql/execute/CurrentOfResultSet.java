/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.execute.CurrentOfResultSet
	        ...
	        if ( ! next ) {
				...
				if (scan != null)
				{
					if (target instanceof IndexRowToBaseRowResultSet)
						scan.setCompareToLastKey(((IndexRowToBaseRowResultSet) target).currentRowPrescanned);
					else if (target instanceof TableScanResultSet)
						scan.setCompareToLastKey( ((TableScanResultSet) target).currentRowPrescanned());
				}
...
// GemStone changes BEGIN
  ...
  public void updateRowLocationPostRead() throws StandardException {
    upgradeReadLockToWrite(this.rowLocation, null);
  }

  @Override
  public void filteredRowLocationPostRead(TXState localTXState) {
    if(localTXState != null) {
      releaseRowLocationLock(this.rowLocation, null);
    }
  }
  ...
  public StringBuilder buildQueryPlan(StringBuilder builder, PlanUtils.Context context) {
    ...
    PlanUtils.xmlTermTag(builder, context, PlanUtils.OP_CURRENT_OF);
