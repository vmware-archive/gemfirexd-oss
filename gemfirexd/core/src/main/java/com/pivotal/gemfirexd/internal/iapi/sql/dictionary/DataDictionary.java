/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.sql.dictionary.DataDictionary
...
{
	String MODULE = "com.pivotal.gemfirexd.internal.iapi.sql.dictionary.DataDictionary";
	...
	// general info
	public	static	final	String	DATABASE_ID = "gemfirexd.distributedsystemID";
	...
	/**
	 * gemfirexd.softDataDictionaryVersion property indicates the soft upgrade level of the system catalogs.
	 ...
	 * Stored as a database property. Set to an instance of DD_Version.
	 */
	public  static  final   String  SOFT_DATA_DICTIONARY_VERSION = "gemfirexd.softDataDictionaryVersion";
...
//      GemStone changes Begin
        public static final int SYSDISKSTORES_CATALOG_NUM = 20;
        public static final int GATEWAYSENDERS_CATALOG_NUM = 21;
        public static final int ASYNCEVENTLISTENERS_CATALOG_NUM = 22;
        public static final int GATEWAYRECEIVERS_CATALOG_NUM = 23;
        public static final int SYSHDFSSTORES_CATALOG_NUM = 24;
...
// GemStone changes BEGIN
	...
	/**
	 * Returns true if the given routine in a schema requires explicit
	 * privileges to execute when authorization has been turned on.
	 * 
	 * This method is only invoked for system schemas like SYS, SYSCS_UTIL
	 * etc. while all routines in other schemas are always restricted.
	 */
	public boolean isRestrictedRoutine(String schemaName, UUID routineID);

	/**
	 * For some special procedures like CHANGE_PASSWORD, we want to bypass
	 * the regular authorization, and instead decide it during execution
	 * in the procedure body itself (#48372)
	 */
	public boolean bypassRoutineAuthorization(UUID routineUUID);
	 ...
	 * 
	 * @param rl		RowLocation array having the entries in SYSCOLUMNS.
	 * @param index        int refers to the RowLocation object for the entry in the array
	 * @param tc		TransactionController to use.
	 ...
	 * holds the lock.
	 */
	public NumberDataValue 	getSetAutoincrementValue(RowLocation[] rl,int index,
											 TransactionController tc,
											 boolean doUpdate,
	...
	public String getBuiltinVTIClass(String schemaName,
	 ...
	 * 
	 * @param lcc        The LanguageConnectionContext to use.
	 * @param localOnly  If true then acquire the lock on DataDictionary only
	 *                   locally, else globally.
	 * 
	 * @return true if the DD write lock was acquired the first time
	 *              and false if it was re-entered
	 * 
	 * @exception StandardException         Thrown on error
         */
	public boolean startWriting(LanguageConnectionContext lcc,
	    boolean localOnly) throws StandardException;

	/**
	 * Take the write lock on DataDictionary before writing which can be a
	 * local lock or a distributed one.
	 * 
	 * @return true if the DD write lock was acquired the first time and
	 *         false if it was re-entered
	 * 
	 * @throws StandardException
	 *           if a retry has to be attempted at higher level due to
	 *           possibility of deadlock
	 */
	public boolean lockForWriting(TransactionController tc,
	    boolean localOnly) throws StandardException;

	/**
	 * Get the existing lock type being held on DataDictionary, if any.
	 */
	public GfxdLockSet.LockType getLockType(TransactionController tc);

	/**
	 * Unlock the DataDictionary explicitly after a
	 * <code>startWriting</code> or <code>lockForWriting</code>. Normally
	 * locks are released at the end of a transaction at commit or abort.
	 * 
	 * Note that in case a read lock was already obtained before
	 * <code>lockForWriting</code> was invoked then a write lock will
	 * upgrade the read lock to write and in that case the unlock will
	 * end up releasing both the locks which may not be intention. So code
	 * calling this explicitly should make sure to avoid releasing the write
	 * lock prematurely by checking the existing lock type, if any, by
	 * invoking <code>getLockType</code>.
	 */
	public void unlockAfterWriting(TransactionController tc,
	    boolean localOnly) throws StandardException;

	/**
	 * Take the read lock on DataDictionary (works in a distributed manner)
	 * before reading.
	 * 
	 * @return true if the DDL read lock acquisition was successful and
	 *         needs to be released, and false if the lock was reentered
	 *         or otherwise acquired before and does not need to be released
	 */
	public void lockForReading(TransactionController tc)
	    throws StandardException;

        /**
         * Unlock the DataDictionary explicitly after a
         * <code>startReading</code> or <code>lockForReading</code>. Normally
         * locks are released at the end of a transaction at commit or abort.
         * 
         * Note that the result of <code>lockForReading</code> should be used
...
// GemStone changes END
	 ...
	 * 
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.depend.DependencyManager
	 */
	 ...
	 * tuple and parent.
	 * @see com.pivotal.gemfirexd.internal.impl.sql.catalog.DataDictionaryImpl#duplicateDescriptorException
...
// GemStone changes BEGIN
	...
	/**
	 * Get the list of user descriptors belonging to a given LDAP group
	 * having defined permissions for a table or routine.
	 *
	 * @param perm the descriptor used for search
	 * @param catalogNum the catalog number of the permissions table to look
	 * @param indexNum the index number of the permissions table to scan
	 * @param groupColumnNum the LDAP group column number in the table
	 * @param tc the TransactionController to use
	 *
	 * @return list of descriptors that have been granted given permission
	 */
	public List<PermissionsDescriptor> getLDAPDescriptorsHavingPermissions(
	    PermissionsDescriptor perm, int catalogNum,
	    int indexNum, int groupColumnNum,
	    TransactionController tc) throws StandardException;

	/**
	 * Get the list of all user descriptors belonging to a given LDAP group
	 * having any permission for a table or routine.
	 *
	 * @param ldapGroup the LDAP group to search
	 * @param catalogNum the catalog number of the permissions table to look
	 * @param groupColumnNum the LDAP group column number in the table
	 * @param tc the TransactionController to use
	 * @param outDescriptors the output list of descriptors of LDAP group
	 *                        that have some permission in given table
	 */
