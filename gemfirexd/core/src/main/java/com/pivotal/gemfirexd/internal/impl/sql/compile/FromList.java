/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.FromList
...
public class FromList extends QueryTreeNodeVector implements OptimizableList
{
	...
	boolean 	useStatistics = true;
        ...
        /**
         * Captures an alias provided in the query hint to refer in statistics
         * display/retrieval.
         */
        String statementAlias = null;
	...
	/**
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.compile.OptimizableList#getOptimizable
	 */
	...
	/**
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.compile.OptimizableList#setOptimizable
	 */
	...
	// GemStone changes BEGIN
        public void eliminateUnUsedColumns(ResultColumnList parentResultColumn,
          ...
          for (int index = 0; index < size; index++) {
             ...
             * be bound based on the outer query's FROM list.
             */
            fromTable.eliminateUnUsedColumns(parentResultColumn, finder);
          }
        }
				...
				throws StandardException
	{
	 ...
	 /**
   * Returns true if any joins present. 
   *
   * @return  True if has any joins. False otherwise.
   */
		...
		for (int index = 0; index < size; index++)
		{
			...
			if (resultColumn != null)
			{
				if (! columnNameMatch)
				{
					 ...
					 * This is important for being able to push predicates 
...
// GemStone changes BEGIN
					...
					flattenedTableNumbers.addElement(
					    Integer.valueOf(ft.getTableNumber()));
					/* (original code)
...
// GemStone changes END
		 ...
		 */
		{
                        FromBaseTable ncjJoinOrderAtOneIdx = null;
                        FromBaseTable ncjJoinOrderAtTwoIdx = null;
                        FromBaseTable[] ncjJoinOrder = null;
                        if (cc.isNCJoinOnRemote()) {
                          int highJnSize = NcjHashMapWrapper.sizeOfHigherLevelJoinOrder(cc
                              .getNCJMetaDataOnRemote());
                          if (highJnSize > 0) {
                            ncjJoinOrder = new FromBaseTable[highJnSize];
                          }
                        }
				...
				if (ft instanceof ProjectRestrictNode)
				{
					ResultSetNode rst = ((ProjectRestrictNode)ft).getChildResult();
                                        if (rst instanceof FromBaseTable) {
                                          if (cc.isNCJoinOnRemote()) {
                                            {
                                              final String corrName = ((FromBaseTable)rst)
                                                  .ncjGetCorrelationName();
                                              if (NcjHashMapWrapper.isTabAtFirstPosition(
                                                  cc.getNCJMetaDataOnRemote(), corrName)) {
                                                ncjJoinOrderAtOneIdx = (FromBaseTable)rst;
                                              }
                                              else if (NcjHashMapWrapper.isTabAtSecondPosition(
                                                  cc.getNCJMetaDataOnRemote(), corrName)) {
                                                ncjJoinOrderAtTwoIdx = (FromBaseTable)rst;
                                              }
                                              else if (ncjJoinOrder != null) {
                                                int idx = NcjHashMapWrapper.indexOfHigherLevelJoinOrder(
                                                    cc.getNCJMetaDataOnRemote(), corrName);
                                                if (idx > NcjHashMapWrapper.MINUS_ONE) {
                                                  ncjJoinOrder[idx] = (FromBaseTable)rst;
                                                }
                                              }
                                            }
                                          }
                        ...
                        /**
                         * NCJ Note: Mark each table to be dependent upon Remote table, and also
                         * mark all table except Remote table to be dependent upon Driver table.
                         * Thus when join order is considered, we will get desired join order of
                         * (RemoteTable -> Driver Table -> Other tables) in a left deep join tree
                         * hierarchy.
                         * 
                         * @see com.pivotal.gemfirexd.internal.impl.sql.compile.FromBaseTable.
                         *      legalJoinOrder(JBitSet)
                         */
                        if (cc.isNCJoinOnRemote()) {
                          if (ncjJoinOrder != null) {
                            final int totalTabs = ncjJoinOrder.length;
                            for (int i = totalTabs - 1; i >= 0; i--) {
                              FromBaseTable currentTab = ncjJoinOrder[i];
                              if (currentTab != null) {
                                if (ncjJoinOrderAtOneIdx != null) {
                                  currentTab.setDependency(ncjJoinOrderAtOneIdx.getTableNumber(),
                                      cc.getNumTables());
                                }
                                if (ncjJoinOrderAtTwoIdx != null) {
                                  currentTab.setDependency(ncjJoinOrderAtTwoIdx.getTableNumber(),
                                      cc.getNumTables());
                                }
                                for (int j = i - 1; j >= 0; j--) {
                                  FromBaseTable dependentTab = ncjJoinOrder[j];
                                  if (dependentTab != null) {
                                    if (currentTab != dependentTab) {
                                      currentTab.setDependency(dependentTab.getTableNumber(),
                                          cc.getNumTables());
                                    }
                                  }
                                }
                          ...
                          if (ncjJoinOrderAtOneIdx != null && ncjJoinOrderAtTwoIdx != null) {
                            ncjJoinOrderAtTwoIdx.setDependency(
                                ncjJoinOrderAtOneIdx.getTableNumber(), cc.getNumTables());
                          }
                        }
		}
	}
	...
	public FromTable getFromTableByResultColumn(ResultColumn rc)
	{
		...
		for (int index = 0; index < size; index++)
		{
			...
			// TODO: PERF: create THashSet of columns in ResultColumnList
  ...
  // GemStone changes BEGIN
  final static String validQueryHintProperites = Constants.QueryHints.joinOrder
      + "," + Constants.QueryHints.statementAlias;
  ...
  // GemStone changes END
	...
	public void setProperties(Properties props) throws StandardException
	{
		...
		*/
		...
		while (e.hasMoreElements())
		{
			...
			/*(original code) if (key.equals("joinOrder"))*/
			if (key.equals(Constants.QueryHints.joinOrder.name()))
			{
			...
			else if (key.equals("useStatistics"))
			{
				...
				else
				{
					throw StandardException.newException(SQLState.LANG_INVALID_STATISTICS_SPEC, value);
                        ...
                        // GemStone changes BEGIN
                        else if (key.equals(Constants.QueryHints.statementAlias.name())) {
                          statementAlias = value;
                          getCompilerContext().setStatementAlias(value);
                        }
                        // GemStone changes END
			else
			{
	...
	boolean hashJoinSpecified()
	{
		...
		// Special handling for NCJ Usage
                if (size > 0) {
                  CompilerContext cc = getCompilerContext();
                  if (cc != null && cc.isNCJoinOnRemote()) {
                    if (cc.getNCJMetaDataOnRemote().size() > 0) {
                      return true;
                    }
                  }
                }
		...
		for (int index = 0; index < size; index++)
		{
			...
			String joinStrategy = ft.getUserSpecifiedJoinStrategy();
                        ...
                        if (joinStrategy != null
                            && (StringUtil.SQLToUpperCase(joinStrategy).equals("HASH")))
			{
				return true;
			}
		}
