/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.conn.GenericStatementContext
 ...
 *
 */
public final class GenericStatementContext 
	extends ContextImpl implements StatementContext
{
	...
	*/
	...
	private boolean isContextPushedForLigthWeightNext = false;
	...
	*/
	...
	{
		super(lcc.getContextManager(), com.pivotal.gemfirexd.internal.iapi.reference.ContextId.LANG_STATEMENT);
		this.lcc = lcc;
            ...
            TimerTask
    {
         ...
         * activation object by calling cancelOnTimeOut();
...
//                    statementContext.cancel();
                    ResultSet rs = statementContext.getTopResultSet();
                    if ((rs != null) && (rs.getActivation() != null)) {
                      rs.getActivation().cancelOnTimeOut();
                    }
                }
            }
        }
	...
	public void setSavePoint() throws StandardException {
		...
		if (SanityManager.DEBUG)
		{
			if (GemFireXDUtils.TraceSavePoints)
			{
...
//		pleaseBeOnStack();
	...
	{
// GemStone changes BEGIN
		if (this.topResultSet == topResultSet) {
		  return;
		}
// GemStone changes END
		pleaseBeOnStack();
	...
	public void cleanupOnError(Throwable error) throws StandardException
	{
        ...
        */
		...
		if (topResultSet != null)
		{
			topResultSet.cleanUp(true);
// GemStone changes BEGIN
			// also release any locks for non-transactional case
			EmbedResultSet.releaseNonTXLocks(false, lcc, topResultSet);
// GemStone changes END
		}
      ...
      tran.release();
		...
		if (subqueryTrackingArray != null)
		{
			for (int index = 0; index < subqueryTrackingArray.length; index++)
			{
				...
				{
					subqueryTrackingArray[index].cleanUp(false);
				}
			}
		}
...
// GemStone changes END
    ...
    public ResultSet getTopResultSet() {
      return this.topResultSet;
    }
	...
	public short getSQLAllowed() {
		if (!inUse)
			return com.pivotal.gemfirexd.internal.catalog.types.RoutineAliasInfo.NO_SQL;
	...
	*/
	...
	//GemStone changes BEGIN
	public final void setLightWeight() {
	   isContextPushedForLigthWeightNext = true;
	}
	
	public final boolean isLightWeight() {
	  return isContextPushedForLigthWeightNext;
	}
	//GemStone changes END
}
