...
// GemStone changes BEGIN
		  	is = SharedUtils.openURL(externalPath);
		  	/* (original code)
...
// GemStone changes BEGIN
	public static long install(LanguageConnectionContext lcc,
	    String schemaName, String sqlName, java.sql.Blob jarBlob)
...
// GemStone changes END
	...
	private long add(final InputStream is) throws StandardException
	{
        ...
        try {
            // Gemstone changes BEGIN
            notifyClassPathChange(this.schemaName + "." + this.sqlName, ClassFactory.JAR_ADD_OP_TYPE);
            // Gemstone changes END
            notifyLoader(false);
            ...
            */
            final String jarExternalName = this.schemaName + "." + this.sqlName;
            long generationId = setJar(jarExternalName, is, true, 0L);
            // Gemstone changes END
...
// GemStone changes BEGIN
		       ...
		       JarLoader oldLoader = closeOldJarLoader(this.schemaName, this.sqlName);
		       String fullName = this.schemaName + '.' + this.sqlName;
                       notifyClassPathChange(fullName,
...
//				fid.getGenerationId());
			fr.remove(fullName, fid.getGenerationId(), lcc);
		} finally {
			notifyLoader(true);
		}
	}
...
// GemStone changes BEGIN
    ...
    // now using the common copy in SharedUtils
    /* (original code)
    /**
