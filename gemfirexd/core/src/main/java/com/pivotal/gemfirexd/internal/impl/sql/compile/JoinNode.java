/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.JoinNode
...
public class JoinNode extends TableOperatorNode
{
			...
			throws StandardException
	{
		...
		else if (joinResultSetString == null)
		{
			joinResultSetString = 
				((Optimizable) rightResultSet).getTrulyTheBestAccessPath().
							...
							throws StandardException
	{
		...
		if(SanityManager.DEBUG) {
		  if(SanityManager.DEBUG_ON("PrintRCList")) {
                    mb.push(acb.addItem(getTableColumnDetails(leftResultSet)));
                    mb.push(acb.addItem(getTableColumnDetails(rightResultSet)));
				...
				throws StandardException
	{
	   ...
	   */
	@Override
	  public void collectHashKeyColumns(Vector hashKeyVector, int tabNum,
         ...
         * NCJ
         * (non-Javadoc)
         * @see com.pivotal.gemfirexd.internal.impl.sql.compile.FromTable#convertAbsoluteToRelativeColumnPosition(int, com.pivotal.gemfirexd.internal.impl.sql.compile.ColumnReference)
         */
        @Override
	...
	@Override
        public void eliminateUnUsedColumns(ResultColumnList parentResultColumn, CollectAndEliminateColumnsVisitor finder)
            ...
            super.eliminateUnUsedColumns(resultColumns.mergeColumnList(
                parentResultColumn, leftPredicateList, rightPredicateList,
                joinPredicates, usingClause), finder);
        }
	
	@Override
  protected void optimizeForOffHeap( boolean shouldOptimize) {
    //Irrespective of shouldOptimize flag, the join node will ensure that
	  // left & right are optimizable for offheap
