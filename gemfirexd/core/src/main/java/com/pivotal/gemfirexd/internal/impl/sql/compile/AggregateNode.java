/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.AggregateNode
...
public class AggregateNode extends UnaryOperatorNode
{
	...
	*/
	...
	public ResultColumn getGeneratedRC()
	{
		if (SanityManager.DEBUG)
		{
			SanityManager.ASSERT(generatedRC != null, 
				"generatedRC is null.  replaceAggregatesWithColumnReferences() "+
			...
			throws StandardException
	{
		...
		/* GemStone changes BEGIN
		 * avoiding duplicate object reference to be added
		 * e.g. select 
		 *        case when sum(amount) > 0 then cast( sum(amount) as REAL) else 1 end 
		 *      from Table;
		 */
		...
		** Ask the aggregate definition whether it can handle
	 	** the input datatype.
	 	*/
        aggregatorClassName = new StringBuilder();
        /// Gemstone changes : Added isAvg argument to getAggregator method.
        DataTypeDescriptor resultType = uad.getAggregator(dts, aggregatorClassName, isAvg);

		if (resultType == null)
		{
			// Report error with "AVG" if binding error happens during SUM/COUNT binding for AVG
			throw StandardException.newException(SQLState.LANG_USER_AGGREGATE_BAD_TYPE, 
						(isAvg?"AVG":aggregateName),       // Gemstone change, report AVG as error if avg rewrite
	...
	*/
	private void checkAggregatorClassName(String className) throws StandardException
	{
		...
		if (!classInspector.assignableTo(className, "com.pivotal.gemfirexd.internal.iapi.sql.execute.ExecAggregator"))
		{
	...
	*/
		...
		if (instance instanceof SumAvgAggregateDefinition)
		{
			...
			if(distinct) {
			  temp1.markDistinct();
                ...
                //GemStone changes BEGIN
                if(instance instanceof CountAggregateDefinition && distinct) 
                {
	...
	public boolean isCountStarAggregate() {
	  return isCountStar;
        ...
        @Override
        /**
         *  This function will get called only at the Data Store end because in Query node we 
         *  call on ParentRCL post optimization (GroupByQueryInfo) where only result ColumnReferences
         *  are found. underlying resultColumns too have only CRs and no AggregateNode.
         *  
         *  So, we have to take care of Query Node differently. 
         *  @see GroupByQueryInfo for more details.
         */
