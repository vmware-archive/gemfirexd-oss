/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.services.io.FormatableBitSet
 ...
 */
// Gemstone changes BEGIN
public final class FormatableBitSet implements Formatable, Cloneable, DataSerializableFixedID
//Gemstone changes END
{
	...
	*/
	...
	public static int udiv8(int i) { return (i>>3); }
	public static byte umod8(int i) { return (byte)(i&0x7); }
...
// GemStone changes BEGIN
		  // using StringBuilder and not using braces to avoid
		  // getting caught by grepLogs
		  StringBuilder str = new StringBuilder(getLength() * 8 * 3);
		  str.append('(');
		  /* (original code)
			StringBuilder str = new StringBuilder(getLength()*8*3);
			str.append("{");
...
// GemStone changes BEGIN
  ...
  /**
   * If any bit is set, return the zero-based bit index of the last bit that is
   * set. If no bit is set, return -1.
   * 
   * @return the zero-based index of the last bit that is set, or -1 if no bit
   *         is set
   */
  public int lastSetBit() {
    final int numBytes = getLengthInBytes();
    for (int i = numBytes - 1; i >= 0; i--) {
      final byte v = this.value[i];
      if (v == 0) {
        continue;
      }
      return (umul8(i) + lastSet(v));
    }
    return -1;
  }

  /**
   * A utility method which treats the byte argument as an 8-bit bitset and
   * finds the last set bit in that byte. Assumes that at least one bit in v is
   * set (v != 0).
   * 
   * @param v
   *          a non-zero byte to check for set bits
   * @return the zero-based index of the last set bit in the argument byte
   */
  ...
  public void toData(final DataOutput out) throws IOException {
    InternalDataSerializer.writeArrayLength(getLength(), out);
    int byteLen = getLengthInBytes();
    if (byteLen > 0) {
  ...
  public void fromData(final DataInput in) throws IOException,
    ...
    final int lenInBits = InternalDataSerializer.readArrayLength(in);
    final int lenInBytes = numBytesFromBits(lenInBits);
    if (lenInBytes > 0) {
      this.value = DataSerializer.readByteArray(in);
    }
    this.bitsInLastByte = numBitsInLastByte(lenInBits);
    this.lengthAsBits = lenInBits;
  }

  public int getDSFID() {
    return DataSerializableFixedID.GFXD_FORMATIBLEBITSET;
  }
