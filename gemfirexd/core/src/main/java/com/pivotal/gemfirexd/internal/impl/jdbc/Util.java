/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.jdbc.Util
...
// GemStone changes BEGIN
	public static SQLException newEmbedSQLException(String messageId,
	  ...
	  return newEmbedSQLException(messageId, args, (SQLException)null,
	      StandardException.getSeverityFromIdentifier(messageId), cause);
	}

	static SQLException seeNextException(StandardException se,
	    SQLException next) {
	  return exceptionFactory.getSQLException(
	      se.getMessage(), se.getMessageId(), next,
	      se.getSeverity(), null, se.getArguments());
	}
...
// GemStone changes BEGIN
				SanityManager.ASSERT(se instanceof DerbySQLException);
