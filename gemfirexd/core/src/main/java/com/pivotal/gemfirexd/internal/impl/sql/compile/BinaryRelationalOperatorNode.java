/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.BinaryRelationalOperatorNode
	...
	implements RelationalOperator
{
  ...
  public ColumnReference getColumnOperand(
	...
	//GemStone changes BEGIN
        ...
        public ValueNode getOtherOperand( ValueNode thisOperand) {
          ...
          }else if(this.leftOperand== thisOperand) {
  ...
  public boolean usefulStopKey(Optimizable optTable)
	...
	//Gemstone changes BEGIN
	// Retrieve column name to be passed to Qualifier
	// and returned during EXPLAIN formatting of qualifier nodes
  ...
  public void generateAbsoluteColumnId(MethodBuilder mb,
											   Optimizable optTable)
	{
		...
		//Gemstone changes BEGIN
                ...
                // for EXPLAIN output
		ColumnReference columnRef = getColumnReference(optTable);
	...
	private int getAbsoluteColumnPosition(Optimizable optTable)
	{
		...
		*/
		columnPosition = cr.getSource().getColumnPosition();
                 ...
                 * Specific NCJ Usage
                 * 
                 * This path will not be chosen for actual execution 
                 * and thus ignore it.
                 */
                if (getCompilerContext().isNCJoinOnRemote()) {
                  if (optTable instanceof NcjPullScanNode) {
                    if (((NcjPullScanNode)optTable).isAtSecondPosition()) {
                      bestCD = null;
                    }
                  }
                }
...
//      GemStone changes begin		 
		  ...
		  return node instanceof ConstantNode ||
		      node instanceof ParameterizedConstantNode ||
...
//	              
  ...
  public ValueNode genSQLJavaSQLTree() throws StandardException
	...
	//Asif: Increasing the visibility to make use in OrListNode
	public boolean valNodeReferencesOptTable(ValueNode valNode,
...
//      GemStone changes BEGIN
        ...
        public QueryInfo computeQueryInfo(QueryInfoContext qic) throws StandardException {
          //Asif :In case one of the operand is a constant of type boolean with value true
          // then return only a single condition info object rather than a
          // junction info object
          //In case of Like Predicate, a CastNode may be seen. Since Like is not being
          // used for nodes pruning , return null. This needs to change later when Like is used for
          // nodes pruning somehow
          if( this.inListProbeSource != null ) {
            return this.inListProbeSource.computeQueryInfo(qic);  
          }else {
            if(this.leftOperand instanceof VirtualColumnNode || this.rightOperand instanceof VirtualColumnNode
                || this.leftOperand instanceof CastNode || this.rightOperand instanceof CastNode) {
              return null;
            }
        ...
        {
          // Print parenthesis for readability of nested binary ops
          String explainString = "("+leftOperand.printExplainInfo() + " " + operator;
          if (inListProbeSource != null)
          {
            // There's an IN list on the RHS. Print it out
            explainString += " "+inListProbeSource.printExplainInfo();
          }
          else
          {
            explainString += " "+ rightOperand.printExplainInfo()+")";
          }
          return explainString;
        }

        /**
         * Returns which side column reference exists in the complete tree.<br>
         * <br>
         * 
         * <b>NOTE:</b> This is different from {@link #columnOnOneSide(Optimizable)}
         * and {@link #keyColumnOnLeft(Optimizable)} as it walks the left deep tree
         * (complete CNF expression) to investigate all the conditions.<br>
         * <br>
         * 
         * @param optTable
         *          the table for which col. ref match is to found.
         * @return LEFT, RIGHT or NEITHER
         */
...
//          leftOperand.accept(v);
        ...
        public int setColocatedWith(Optimizable src, Optimizable tgt, JBitSet srcPCols, JBitSet tgtPCols) throws StandardException {
          ...
          final int leftRet = leftOperand.setColocatedWith(src, tgt, srcCols, tgtCols);
          assert srcCols.isEmpty() || tgtCols.isEmpty(): "Only one table's partitioning information is expected to be set. srcCols="
              + srcCols + " tgt=" + tgtCols;

          if (leftRet == UNKNOWN || leftRet == NOTCOLOCATED) {
            return leftRet;
          }
          ...
          final int rightRet = rightOperand.setColocatedWith(src, tgt, srcCols, tgtCols);
          
          if (rightRet == UNKNOWN || rightRet == NOTCOLOCATED) {
            return rightRet;
          }
          ...
          if (srcCols.equals(tgtCols)) {
            srcPCols.or(srcCols);
            tgtPCols.or(tgtCols);
            return COMPLETELYCOLOCATED;
          }
          else if (leftRet == CONTINUE && rightRet == CONTINUE) {
            srcPCols.or(srcCols);
            tgtPCols.or(tgtCols);
            return PARTLYCOLOCATED;
          }
