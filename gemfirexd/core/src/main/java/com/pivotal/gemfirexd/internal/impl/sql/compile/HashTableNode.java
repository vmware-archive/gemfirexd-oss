/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.HashTableNode
...
public class HashTableNode extends SingleChildResultSetNode
{
									...
									throws StandardException
	{
		 ...
		 */
                if (isNCJCase) {
                  /*
                   * This fixes Derby's handling of rows with as per result number assigned to different nodes.
                   * 
                   * For some cases, added as DUnits with this checkin, that were resulting in NPE,
                   * I noticed that a scan operator node at right side of join node should expect 
                   * qualifier rows corresponding to its sibling and not of any other node.
                   */
                  ncjAssignResultSetNumber(getCompilerContext().getNextResultSetNumber());
                }
                else {
...
// GemStone changes BEGIN
	...
	@Override
        public void eliminateUnUsedColumns(ResultColumnList parentResultColumn, CollectAndEliminateColumnsVisitor finder)
          ...
          resultColumns = resultColumns.eliminateUnUsedColumns(resultColumns
              .mergeColumnList(parentResultColumn, joinPredicateList,
                  searchPredicateList), finder);
          
          childResult.eliminateUnUsedColumns(resultColumns, finder);
        }
