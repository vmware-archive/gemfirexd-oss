/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.types.DataValueFactoryImpl
...
abstract class DataValueFactoryImpl implements DataValueFactory, ModuleControl
{
    	...
    	/* (non-Javadoc)
    	 * @see com.pivotal.gemfirexd.internal.iapi.services.monitor.ModuleControl#boot(boolean, java.util.Properties)
    	 */
...
// GemStone changes END
    	public void boot(boolean create, Properties properties) throws StandardException {
    		...
    		FabricDatabase database = (FabricDatabase)properties.get(
    		    FabricDatabase.PROPERTY_NAME);
    		databaseLocale = database.getLocale();
	    	...
	    	if (create) {
	    		...
	    		String userDefinedCollation = 
	    			properties.getProperty(com.pivotal.gemfirexd.Attribute.COLLATION);
        ...
        /**
         * Return a StringDataValue to represent a SQL JSON
         * with the given collation re-using previous if not null.
         */
        public StringDataValue getJSONDataValue(String value,
                StringDataValue previous, int collationType)
            throws StandardException
        {
            return getJSONDataValue(value, previous);
          /*
            if (collationType == StringDataValue.COLLATION_TYPE_UCS_BASIC)
                return getClobDataValue(value, previous);
         ...
         * otherwise set it to null and return that value.
         */
        public StringDataValue getNullJSON(StringDataValue previous,
                int collationType)
        throws StandardException
        {
          return getNullJSON(null);
    ...
    int formatId) {
        ...
        case StoredFormatIds.SQL_JSON_ID: return new JSON();
        ...
        // a DTD.  
             return(
                 new com.pivotal.gemfirexd.internal.impl.store.access.heap.HeapRowLocation());
//GemStone Changes BEGIN
        case StoredFormatIds.ACCESS_MEM_HEAP_ROW_LOCATION_ID:
          //TODO:Asif: Revisit this code
             return DUMMY;
//GemStone Changes END             
        default:return null;
        }
        ...
        //GemStone changes BEGIN
        /**
         * Called based on {@link UserDefinedTypeCompiler#nullMethodName()} return value.
         */
        public UserDataValue getNullDVDSet(UserDataValue dataValue) 
        {
          if(SanityManager.DEBUG) {
            if(dataValue != null && ! DVDSet.class.isInstance(dataValue) ) {
               SanityManager.THROWASSERT("UserDataValue is not of DVDList type " + dataValue.getClass().getName());
            }
          }
          
          if (dataValue == null)
          {
                  return new DVDSet(DVDSet.DEFAULT_SIZE);
          }
          else
          {
                  dataValue.setToNull();
