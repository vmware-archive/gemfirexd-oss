/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.ColumnReference
...
// GemStone changes BEGIN
	...
	@Override
	public int hashCode() {
	  int h = this.tableName != null ? this.tableName.hashCode()
	      : this.tableNumber;
	  h ^= (this.columnName != null ? this.columnName.hashCode()
	      : this.columnNumber);
	  return h;
	}
	...
	public boolean equals(Object other) {
	  if (other instanceof ColumnReference) {
	    ...
	    if (this.columnName != null) {
	      if (!this.columnName.equals(o.columnName)) {
	        return false;
	      }
	    }
	    else {
	      if (this.columnNumber != o.columnNumber) {
	        return false;
	      }
	    }
	...
	public QueryInfo computeQueryInfo(QueryInfoContext qic)
	  ...
	  return new ColumnQueryInfo(this,qic);
           ...
           throws StandardException 
        {
            ...
            /* Point ourselves to the new RC of intermediate resultColumnList
             * which will be received from DataStore node.
             */
            if(remapToNew) {
            ...
            throws StandardException
        {
            ...
            if (v instanceof CollectAndEliminateColumnsVisitor && this.source != null
                && !v.stopTraversal()) {
        ...
        public boolean hasExpression() {
          // If source is null (i.e. this is a constant with no table), it is an expression
          return (source!=null?source.hasExpression():true);
        }
        ...
        {
          // Print table.columnname for clarity if table name is known
          return ((tableName!=null)?tableName.toString()+".":"") + columnName;
        }
        ...
        @Override
        public int setColocatedWith(Optimizable src, Optimizable tgt,
            JBitSet srcPCols, JBitSet tgtPCols) throws StandardException {

          String baseTableName = getSourceTableName();
          String colName = getColumnName();
          int pindex = -1;
          Optimizable opt = src.getBaseTable();
          if (!opt.isBaseTable()) {
            return ValueNode.UNKNOWN;
          }

          if (opt.getBaseTableName().equals(baseTableName)
              && ((pindex = opt.getTableDescriptor().getDistributionDescriptor()
                  .getPartitioningColumnIdx(colName)) != -1)) {
            srcPCols.set(pindex);
            return CONTINUE;
          }
        ...
        public String findSourceSchemaName() {
          return source != null ? source.findSourceSchemaName() : getSchemaName();
        }
        ...
        public String findSourceTableName() {
          return source != null ? source.findSourceTableName() : getTableName();
        }
        ...
        public String findSourceColumnName() {
          return source != null ? source.findSourceColumnName() : getColumnName();
        }
// GemStone changes END
}
