/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.services.property.PropertyValidation
...
public class PropertyValidation implements PropertyFactory
{
 		...
 		if (notifyOnSet != null) {
			synchronized (this) {
				...
				for (int i = 0; i < notifyOnSet.size() ; i++) {
					...
					if (!dbOnlyProperty && key.startsWith("gemfirexd.")) {
	...
	public synchronized void addPropertySetNotification(PropertySetCallback who){
		...
		for(int i = notifyOnSet.size() - 1; i > 0 ; i--) {
		  if(notifyOnSet.get(i) == who) {
		    return;
		  }
		}
