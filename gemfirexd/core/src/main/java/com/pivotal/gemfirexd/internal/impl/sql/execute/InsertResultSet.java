/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.execute.InsertResultSet
...
// GemStone changes BEGIN
								  ...
								  Long.valueOf(aiCache[i].getLong()));
								/* (original code)
		...
		autoincrementGenerated = true;
  		...
  		if (bulkInsert)
  		{
			...
			if (aiCache[index].isNull())
			{
				...
				{
//		                      Gemstone changes Begin				       
					dvd = dd.getSetAutoincrementValue( constants.autoincRowLocation,index,
...
// GemStone changes END
		...
		else
		{
			...
			try 
			{
				...
				*/
//	                      Gemstone changes Begin
				newValue = dd.getSetAutoincrementValue(constants.autoincRowLocation,index,
...
//		                      Gemstone changes End
		...
		throws StandardException
	{
		...
		*/
		...
		{
// GemStone changes BEGIN
      // make a copy of Properties since those are changed inside
      // TemporaryRowHolderImpl
      Properties tmpProps = new Properties();
      tmpProps.putAll(properties);
      rowHolder = new TemporaryRowHolderImpl(activation, tmpProps);
		/*
			rowHolder =
				new TemporaryRowHolderImpl(activation, properties);
		*/
...
// GemStone changes BEGIN
						...
						fkConglom = ((Long)indexConversionTable.get(
										Long.valueOf(fkInfo.fkConglomNumbers[index]))).longValue();
										/* (original code)
...
// GemStone changes BEGIN
										// changed to use valueOf()
										Long.valueOf(fkInfo.fkConglomNumbers[0]));
										/* (original code)
...
// GemStone changes BEGIN
						com.pivotal.gemfirexd.internal.engine.GfxdConstants
						    .SCAN_OPENMODE_FOR_REFERENCED_PK, // for FK reference
						/* (original code)
						0, 								// read only
						*/
// GemStone changes END
				...
				*/	
				...
				//TODO:Asif: Check if special checks for reference key & table are required here also
				RIBulkChecker riChecker = new RIBulkChecker(refScan, 
											fkScan, 
											template, 	
											true, 				// fail on 1st failure
											(ConglomerateController)null,
											firstFailedRow, null, null);
		...
		for (int index = 0; index < numIndexes; index++)
		{
			...
			sortIds[index] = 
                tc.createSort(
                    ...
                    0  // maxSortLimit for FETCH FIRST/NEXT is not set
                    );
...
// GemStone changes BEGIN
			// changed to use valueOf()
			indexConversionTable.put(Long.valueOf(constants.indexCIDS[index]),
			    Long.valueOf(newIndexCongloms[index]));
			/* (original code)
...
// GemStone changes END
	...
	public void	cleanUp(boolean cleanupOnError) throws StandardException
	{
		...
		if (tableScan != null)
		{
			tableScan.close(cleanupOnError);
			tableScan = null;
		}
		...
		if (tableScan == null)
		{
			tableScan = 
                new BulkTableScanResultSet(
                    ...
                    GemFireXDUtils.DML_BULK_FETCH_SIZE,	// rows per read
