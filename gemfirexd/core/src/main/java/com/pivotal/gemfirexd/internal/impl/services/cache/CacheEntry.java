/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.services.cache.CacheEntry
...
//        }
    ...
    void keep(boolean accessed) {
        ...
        if (accessed) {
// GemStone changes BEGIN
          final ReplacementPolicy.Callback temp = this.callback;
          if (temp != null) {
            temp.access();
          }
          /* (original code)
...
// Gemstone changes END
     ...
     *
     * @see com.pivotal.gemfirexd.internal.iapi.services.cache.CacheManager#remove
     */
...
// GemStone changes BEGIN
      // GfxdConcurrentCache now creates the entry completely first before
      // putting in cache so does not need to acquire the lock
      /*
        if (SanityManager.DEBUG) {
            SanityManager.ASSERT(mutex.isHeldByCurrentThread());
        }
