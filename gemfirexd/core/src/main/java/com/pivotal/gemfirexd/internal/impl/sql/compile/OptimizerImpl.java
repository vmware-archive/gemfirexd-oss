/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.OptimizerImpl
...
public class OptimizerImpl implements Optimizer 
{
	...
	/**
	 * Segregated colocation grouping of tables in proposedJoinOrder
	 * based on predicates supplied in the user query.
	 */
	protected ArrayList<JBitSet> colocatedTableGrp;
	protected JBitSet replicatedTableGrp;
	/**
	 * index to the proposedJoinOrder pointing to first PR 
	 * indicating driverTable.
	 */
...
// GemStone changes END
	protected  OptimizerImpl(OptimizableList optimizableList, 
				  ...
				  DistributedJoinStrategy[] distJoinStrategies,
		...
		throws StandardException
	{
		...
		*/
		...
		// GemStone changes BEGIN
                CompilerContext cc = (CompilerContext)ContextService
                    .getContextOrNull(CompilerContext.CONTEXT_ID);
                boolean doDistJoinStrategy = false;
                if (cc != null) {
                  createQueryInfo = cc.createQueryInfo();
                  // If all tables are replicated, no special processing on Data Node
                  allReplicatedTables = cc.allTablesAreReplicatedOnRemote();
                ...
                // Lets not use DistributedJoinStrategy on Remote Nodes for now
                // TODO Later we would review this strategy
                if (doDistJoinStrategy) {
                  this.distJoinStrategies = distJoinStrategies;
                }
                else {
                  this.distJoinStrategies = null;
                }
                colocatedTableGrp = new ArrayList<JBitSet>(numOptimizables);
                replicatedTableGrp = new JBitSet(numOptimizables);
                bestJoinOrderColocatedTableGrp = new ArrayList<JBitSet>(numOptimizables);
                bestJoinOrderColocatedPrCorrName = new ArrayList<THashSet>();
		driverTableIdx = 0;
	        isLoner = Misc.getDistributedSystem().isLoner();
		// GemStone changes END
  ...
  public boolean getNextPermutation()
		...
		*/
		while (joinPosition >= 0)
		{
			...
			*/
			...
			//GemStone changes BEGIN
	                //Based on the join order starting from right most table, identify the 
	                // tables which have no correlation restriction with left tables
	                JBitSet tablesNoCorr = identifyTablesForSubqueryPush(proposedJoinOrder, joinPosition); 
	                int tableNum = getTableNumber(nextOpt);
	                //GemStone changes END
			pushPredicates(
				optimizableList.getOptimizable(nextOptimizable),
				assignedTableMap, 
				//GemStone changes BEGIN
				tableNum >=0 && tablesNoCorr.get(tableNum) /* try to push non pushable predicates too*/ /* push deemed nonpushables*/,
				false /* final remap*/
				//GemStone changes END
			    );
		...
		throws StandardException
	{
		for (; ; joinPosition--)
		{
			Optimizable pullMe =
				optimizableList.getOptimizable(
									proposedJoinOrder[joinPosition]);
			pullMe.pullOptPredicates(predicateList, this.assignedTableMap);
			if (reloadBestPlan)
	...
	*/
	void pushPredicates(Optimizable curTable, JBitSet outerTables, 
	    boolean pushDeemedNonpushables, final boolean finalRemap)
			throws StandardException
	{
		...
		*/
		...
		for (int predCtr = numPreds - 1; predCtr >= 0; predCtr--)
		{
			...
			//GemStone changes BEGIN
			/* Skip over non-pushable predicates  conditionally*/
			if (! isPushable(pred, pushDeemedNonpushables))
			{
				continue;
			}
			//GemStone changes END
			...
			//GemStone changes BEGIN
			// remove the tables which are not at current level.
			/* Those tables which are above this level, they are already optimized
			 * and the predicate would already have moved towards the most right 
			 * satisyfing the criteria. And for those tables which are below the 
			 * current level, implies that they are self contained and can be ignored
			 * 
			 */
			//TODO: check if we get the correct list of tables from this
			int numTablesInPred = predMap.size();
			int numTablesInQuery = this.nonCorrelatedTableMap.size();
			for(int index = 0 ; index < numTablesInPred; ++index  ) {			  
			  if(predMap.get(index)) {
			    // check at this index a table exists in thequery or not
			    if(numTablesInQuery <= index || !this.nonCorrelatedTableMap.get(index)) {
			       predMap.clear(index);
			    }
			  }
			}
			...
			*/
			if (pushPredNow)
			{
				...
				{
				  //GemStone changes BEGIN
				  //if the predicate pushed is a subquery predicate & we are at final step 
				  // of optmization, we need to remap the column reference which are refering to
				  // current result column, to the store level result column for only those column reference
				  // which are derived from the table to which we are pushing the subquery predicate
				 if( pred.hasSubquery()) { 
...
//GemStone changes BEGIN	
  ...
  private void remapSubqueryPredicate( Optimizable optimizable,
      final Predicate pred, final JBitSet outerTables, final boolean finalRemap) throws StandardException {
    // retrieve subquery node
   
    
    Visitor subqueryColRemapper = new RemapSubqueryVisitor(true, outerTables, finalRemap);
				...
				if (requiredRowOrdering != null)
				{
					...
					else if (requiredRowOrdering.getSortNeeded())
					{
						if (bestCost.rowCount() > currentCost.rowCount()
						    // GemStone change:
						    // bestPlanIsSortAvoidance is needed for 
						    // query node as getSortNeeded() returns true on query 
						    // node always (even for sort avoidance plan) which 
						    // may result in assert failure as cost may become 
						    // -ve after the calulcation below as sort cost might 
						    // be subtracted from the sort avoidance plan (where 
						    // it should not be).     
						    && !bestPlanIsSortAvoidance
						    )
						{
							// adjust bestCost
  ...
  /**
		...
		throws StandardException
	{
		...
		*/
		...
		// GemStone changes BEGIN
                if (bestJoinOrderColocatedTableGrp != null && colocatedTableGrp != null) {
                  bestJoinOrderColocatedTableGrp.clear();
                  bestJoinOrderColocatedTableGrp.addAll(colocatedTableGrp);
                  bestJoinOrderDriverTableIdx = driverTableIdx;
                }
		// GemStone changes END
		...
		bestPlanIsSortAvoidance = (planType == Optimizer.SORT_AVOIDANCE_PLAN);
		/* Remember if a sort is not needed for this plan */
		if (requiredRowOrdering != null)
		{ 
// GemStone changes BEGIN
			// don't eliminate sort on query node
			if (((!createQueryInfo && !allReplicatedTables) || isLoner)
			    && planType == Optimizer.SORT_AVOIDANCE_PLAN)
			/* (original code)
...
// GemStone changes END
	...
	/**
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.compile.Optimizer#costPermutation
	 *
	 * @exception StandardException		Thrown on error
	 */
	@Override
  public void costPermutation() throws StandardException
	...
	/**
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.compile.Optimizer#costOptimizable
	 *
	 * @exception StandardException		Thrown on error
	 */
	@Override
  ...
  public void	considerCost(Optimizable optimizable,
	...
	/**
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.compile.Optimizer#getDataDictionary
	 */
  ...
  public void modifyAccessPaths() throws StandardException
	{
		...
		//unmodifiedOptimizableList
		optimizableList.reOrder(bestJoinOrder);
		//GemStone changes BEGIN
		//Based on the join order starting from right most table, identify the 
		// tables which have no correlation restriction with left tables
		JBitSet tablesNoCorr = identifyTablesForSubqueryPush(this.bestJoinOrder, this.bestJoinOrder.length -1); 
		//GemStone changes END
		...
		for (int ictr = 0; ictr < numOptimizables; ictr++)
		{
			...
			*/
			//GemStone changes BEGIN
			int tableNum = getTableNumber(optimizable);
			
			 pushPredicates(optimizable, outerTables, 
			     tableNum >=0 && tablesNoCorr.get(tableNum) /* try to push non pushable predicates too*/,
			     true /* final remap*/);
	...
	private JBitSet identifyTablesForSubqueryPush(int [] joinOrder, int traverseFrom) {
          ...
          //int len = joinOrder.length - traverseTill;
          
          for(int i= traverseFrom; i >= 0; --i) {
            Optimizable opt = this.optimizableList.getOptimizable(joinOrder[i]);      
            int tableNum = getTableNumber(opt);
            if( tableNum != -1) {
              tables.set(tableNum);
            }
          }
          if (tables.getFirstSetBit() != -1) {
            ...
            for(int i= traverseFrom ; i >= 0  ; --i) {
              Optimizable opt = this.optimizableList.getOptimizable(joinOrder[i]);
              int optTableNum = getTableNumber(opt);
              if(optTableNum == -1) {
                continue;
              }
              ...
              for(int j = i-1;j >= 0 ; --j) {
                ...
                int tableNum = getTableNumber(temp);
                if(tableNum == -1) {
                  continue;
                }
                if(set.get(tableNum)) {
  ...
  public int getNumberOfJoinStrategies()
	{
	  ...
	  // GemStone changes BEGIN
          final boolean isOptColocated;
          if (distJoinStrategies == null) {
            return joinStrategies.length;
          }
          else {
            return getColocatedTableGroup() == driverTableIdx ? joinStrategies.length
                : distJoinStrategies.length;
          }
	  /*(original code) return joinStrategies.length; */
	  // GemStone changes END
	}
  ...
  public JoinStrategy getJoinStrategy(int whichStrategy) {
	  // GemStone changes BEGIN
          ...
          if (distJoinStrategies == null) {
            ...
            isOptColocated = getColocatedTableGroup() == driverTableIdx;
          }
	  // GemStone changes END
		if (SanityManager.DEBUG) {
			if (whichStrategy < 0 || whichStrategy >= (isOptColocated ? joinStrategies.length : distJoinStrategies.length)) {
				SanityManager.THROWASSERT("whichStrategy value " +
                        ...
                        if (isOptColocated ? joinStrategies[whichStrategy] == null
                            : distJoinStrategies[whichStrategy] == null) {
				SanityManager.THROWASSERT("Strategy " + whichStrategy +
											" not filled in.");
			}
		}

		return isOptColocated ? joinStrategies[whichStrategy] : distJoinStrategies[whichStrategy];
	}
  ...
  public JoinStrategy getJoinStrategy(String whichStrategy) {
		...
		for (int i = 0; i < joinStrategies.length; i++) {
			if (upperValue.equals(joinStrategies[i].getName())) {
                ...
                // GemStone changes BEGIN
                if (distJoinStrategies != null) {
                  for (int i = 0; i < distJoinStrategies.length; i++) {
                    if (upperValue.equals(distJoinStrategies[i].getName())) {
                      retval = distJoinStrategies[i];
                    }
                  }
                }
                // GemStone changes END
  ...
  public double uniqueJoinWithOuterTable(OptimizablePredicateList predList)
	...
	private boolean isPushable(OptimizablePredicate pred, boolean pushDeemedNonpushables)
	{
		...
		if (pred.hasSubquery())
		{
		  ...
		  //If pred has no reference table at all , then it cannot be pushed 
  ...
  public boolean useStatistics() { return useStatistics && optimizableList.useStatistics(); }
		...
		throws StandardException
	{
        ...
        public void determineColocation()
          ...
          if (optimizableList.size() <= 1) {
            return;
          }
          
          assert predicateList instanceof PredicateList;
          final PredicateList src = ((PredicateList)predicateList);
          
          if (src.size() == 0 && src.redundantPredicates.size() == 0) {
            return;
          }
          
          final OptimizablePredicateList predicateCopy = new PredicateList(); 
          predicateList.copyPredicatesToOtherList(predicateCopy);
          
          for (int i = 0; i < src.redundantPredicates.size(); i++) {
            predicateCopy.addOptPredicate(src.redundantPredicates.get(i));
          }

          for (int optIdx = 0; optIdx < optimizableList.size(); optIdx++) {
              final Optimizable lookupOpt = optimizableList.getOptimizable(optIdx);
              
              if (((FromTable)lookupOpt).getNumPartitioningCols() == -1 ) {
                trace(Optimizer.SETTING_REPLICATED_TABLE_GROUP, optIdx, 0, 0, lookupOpt);
                replicatedTableGrp.set(optIdx);
                continue;
              }

              //ignoring the first group (replicated tables), there aren't any partitioned tables yet.
              if (colocatedTableGrp.size() == 0) {
                JBitSet set = new JBitSet(numOptimizables);
                set.set(optIdx);
                colocatedTableGrp.add(set);
                continue;
              }
              
              
              int groupIdx = setColocatedTableInGroup(lookupOpt, optIdx, 0, predicateCopy);
                ...
                if (GemFireXDUtils.TraceNCJ) {
                  if (groupIdx >= 0) {
                    SanityManager.DEBUG_PRINT(
                        ...
                        "Optimizable " + lookupOpt.shortString() + " is colocated in groupIdx "
                            + groupIdx + ", group info : "
                            + colocatedTableGrp.get(groupIdx));
                  }
                  else if (groupIdx == -2) {
                    SanityManager
                        .DEBUG_PRINT(
                            "warning:"+GfxdConstants.TRACE_NON_COLLOCATED_JOIN,
                            "Couldn't determine colocation for optimizable "
                                + lookupOpt.getClass().getSimpleName()
                                + " opt: ["
                                + lookupOpt.shortString() 
                                + "] due to insufficient code. About to clear ColocatedTableGrp : "
                                + colocatedTableGrp + " predList : " + predicateList);
                    colocatedTableGrp.clear();
                    return;
                  }
                  else {
                    SanityManager
                        .DEBUG_PRINT(
                            ...
                            "Optimizable "
                                + lookupOpt.shortString() 
                                + " not colocated with any existing table. ColocatedTableGrp : "
                                + colocatedTableGrp);
      ...
      public boolean hasNonColocatedTables() {
        return bestJoinOrderColocatedTableGrp.size() > 1;
      }
        
      private int setColocatedTableInGroup(final Optimizable lookupOpt,
          final int optimizableIdx, final int beginOffset,
          final OptimizablePredicateList predList) throws StandardException {
      
          int lookupOptColocatedWith = -1;
          for (int colocIdx = beginOffset; colocIdx < colocatedTableGrp.size(); colocIdx++) {
            final JBitSet groups = colocatedTableGrp.get(colocIdx);
            int ret = -4;
            if ((ret = iterateGroupToSetColocation(groups, lookupOpt, predList)) < 0) {
              if (ret == -1) {
                continue;
              }
              else {
            ...
            // lookupOpt is colocated.
            lookupOptColocatedWith = colocIdx;
      
            if (optimizableIdx != -1) {
              groups.set(optimizableIdx);
              // lets find out tables that are deemed non-colocated earlier
              // due to absence of this table.
              setColocatedTableInGroup(lookupOpt, -1, colocIdx + 1, predList);
              return lookupOptColocatedWith;
            }
      
            // okay, complete group needs to be merged
            // as newly arrived lookupOpt is
            // completing the missing colocation chain.
            assert beginOffset != -1 && (beginOffset - 1) < colocatedTableGrp.size();
            // as we recurse only 1 level, beginOffset-1 is the group where new
            // optimizable got introduced.
            colocatedTableGrp.get(beginOffset - 1).or(groups);
            final JBitSet mergedGroup = colocatedTableGrp.remove(colocIdx);
      
            if (SanityManager.DEBUG) {
              SanityManager.DEBUG_PRINT("TraceDisJoin", "Merged " + mergedGroup
                  + " with " + (beginOffset - 1) + " by induction. group info: "
                  + colocatedTableGrp.get(beginOffset - 1));
            }
          } // end for
      
          // non-colocated, note the fact but exclude during recursive call.
          if (lookupOptColocatedWith == -1 && optimizableIdx != -1) {
            final JBitSet set = new JBitSet(numOptimizables);
            set.set(optimizableIdx);
            colocatedTableGrp.add(set);
            return ValueNode.NOTCOLOCATED;
          }
          
          return -2;
        }

        private int iterateGroupToSetColocation(JBitSet groups, final Optimizable lookup,
            OptimizablePredicateList predList) throws StandardException {
          int g = -1;
          while ((g = groups.getNextSetBit(g + 1)) != -1) {
            final Optimizable driverTable = optimizableList.getOptimizable(g);
            int ret = ValueNode.UNKNOWN;
            if ((ret = predList.setColocatedWith(driverTable, lookup)) == ValueNode.UNKNOWN) {
              return -2;
            }
            else if (ret == ValueNode.COMPLETELYCOLOCATED) {
              return g;
            }
          }
        ...
        private final int getColocatedTableGroup() {
          ...
          if (colocatedTableGrp.size() == 0) {
            return driverTableIdx;
          }
          
          final int lookupOptIdx = proposedJoinOrder[joinPosition];
          
          // specially handle replicated table colocation.
          if (replicatedTableGrp.get(lookupOptIdx)) {
            return driverTableIdx;
            /*
            // if first replicated table, its colocated with whatever.
            if (joinPosition == 0) {
              return driverTableIdx;
            }
            
            int priorTabIdx = -1, pos = joinPosition-1;
            //search the previous list of tables for the nearest partitioned table with which
            //this replicated table is going to join with by transition.
            while ( pos > 0 && (priorTabIdx = searchColocatedGroup(proposedJoinOrder[pos--])) < 0);
            return priorTabIdx;
            */
          }
          
          final int ret = searchColocatedGroup(lookupOptIdx);
          if (joinPosition == 0 || driverTableIdx < 0) {
            if (ret < 0) {
              trace(Optimizer.RESETTING_DRIVER_TABLE_INDEX, ret, 0, 0, null);
              driverTableIdx = ret;
            }
            return driverTableIdx;
          }
        ...
        private final int searchColocatedGroup(int lookupOptIdx) {
          for (int index = 0; index < colocatedTableGrp.size(); index++) {
            if (colocatedTableGrp.get(index).get(lookupOptIdx)) {
              return index;
            }
          }
        ...
        public void saveBestJoinOrderPrIDs() throws StandardException {
          if (bestJoinOrder.length == 0 || bestJoinOrderColocatedTableGrp == null
              || bestJoinOrderColocatedTableGrp.size() <= 1) {
            return;
          }
          
          Optimizable opt = optimizableList
              .getOptimizable(bestJoinOrderColocatedTableGrp.get(
                  bestJoinOrderDriverTableIdx).getFirstSetBit());
          bestJoinOrderDriverTablePrID = getPrID(opt);
          
          for (int colocIdx = 1/*skip replicated group*/; colocIdx < bestJoinOrderColocatedTableGrp.size(); colocIdx++) {
            final JBitSet groups = bestJoinOrderColocatedTableGrp.get(colocIdx);
            THashSet prNameGrp = new THashSet();
            bestJoinOrderColocatedPrCorrName.add(prNameGrp);
            int g = -1;
            String prName = null;
            while ((g = groups.getNextSetBit(g + 1)) != -1) {
              opt = optimizableList.getOptimizable(g);
              if ((prName = getPrCorrName(opt)) != null) {
                prNameGrp.add(prName);
              }
            }
          }
        }

        //TODO: sb:DJ: need to take care of multiple tables in an Optimizable.
        private int getPrID(final Optimizable opt) throws StandardException {
          assert opt != null: "opt is not expected to be null";
          final Optimizable tab = opt.getBaseTable();
        ...
        //TODO: sb:DJ: need to take care of multiple tables in an Optimizable.
        private String getPrCorrName(final Optimizable opt) throws StandardException {
          assert opt != null: "opt is not expected to be null";
          final Optimizable tab = opt.getBaseTable();
          assert tab != null: "tab is not expected to be null";
          ...
          if (r.getAttributes().getPartitionAttributes() != null) {
            if (!(tab instanceof FromBaseTable)) {
              SanityManager
                  .THROWASSERT("tab expected instanceof FromBaseTable but is "
        ...
        public final ArrayList<THashSet> getBestJoinOrderColocationPrCorrName() {
          return bestJoinOrderColocatedPrCorrName;
        }
        
        public final int getBestJoinOrderDriverTablePrID() {
          return bestJoinOrderDriverTablePrID;
        }
        // GemStone changes END
}
