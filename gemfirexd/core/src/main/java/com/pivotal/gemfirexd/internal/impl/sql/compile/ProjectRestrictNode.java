/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.ProjectRestrictNode
...
//	GemStone Changes End
					...
					throws StandardException
	{
		...
		if (childResult instanceof Optimizable)
		{
			...
			if (childResult instanceof UnionNode)
				((UnionNode)childResult).pullOptPredicates(restrictionList, /*GemStone Changes BEGIN*/null /*GemStone Changes END*/ );
  ...
  public boolean pushOptPredicate(OptimizablePredicate optimizablePredicate)
			throws StandardException
	{
		...
		{
		  //GemStone changes BEGIN
		    //Asif: Predicates having subquery are remapped in Optimzer after predicate push.
		        if(!pred.hasSubquery()) {
					...
					throws StandardException
	{
...

		{
			...
			if (childResult instanceof UnionNode)
				((UnionNode)childResult).pullOptPredicates(restrictionList, outerTables);
		...
		throws StandardException
	{
		...
		*/
		...
		boolean hashJoinWithThisPRN = hasTrulyTheBestAccessPath &&
			...
			trulyTheBestAccessPath.getJoinStrategy().isHashJoin();
                ...
                if (cc.isNCJoinOnRemote()) {
                  FromBaseTable leftrs = this.ncjGetOnlyOneFBTNode();
                  if (leftrs != null) {
                    if (NcjHashMapWrapper.isTabForPull(cc.getNCJMetaDataOnRemote(),
                        leftrs.ncjGetCorrelationName())
                        || NcjHashMapWrapper.isTabAtSecondPosition(
                            cc.getNCJMetaDataOnRemote(), leftrs.ncjGetCorrelationName())) {
		...
		if ((restrictionList != null) && !alreadyPushed && !hashJoinWithThisPRN
	                //GemStone changes BEGIN
		        && !isLeftChildOfNCJoinOnRemote
		        //GemStone changes END
		    )
		{
		//GemStone changes BEGIN
			restrictionList.pushUsefulPredicates((Optimizable) childResult, true /* push deemed non pushables*/);
		//GemStone changes END	
		}
...
// GemStone changes BEGIN
	public void handleSubqueryProcessing(SubqueryNode sqn,ExpressionClassBuilder acb, MethodBuilder mb) throws StandardException {
          ...
          if (nopProjectRestrict())
          {
                 ...
                 sqn.generateGFEResultSet(acb, mb,-1, getCompilerContext());
          ...
          sqn.generateGFEResultSet(acb, mb, resultSetNumber, getCompilerContext());
          //Bug #42673
          if(this.childResult instanceof ProjectRestrictNode) {
            ((ProjectRestrictNode)childResult).assignResultSetNumber(resultSetNumber);
          }
          ...
          // Does EXPLAIN print out the PRN for the subquery? 
          // TODO : fix EXPLAIN formatting for subquery PRN - for now, 
          // just save the column list and restrictions for future use
          // (see generateMinion for main query usage of this string)
          StringBuilder projectedColumns = new StringBuilder();
          projectedColumns.append("SELECT :");
          for (int i = 0; i < resultColumns.size(); i++)
          {
              ResultColumn col =  (ResultColumn)resultColumns.elementAt(i); 
              // May need to use getSourceColumnName for expressions
              // Do not print out generated col names (agg columns, etc)
              //  which have ## at start of name
              // TODO : handle these generated columns and display to user in some form
          ...
          else
          {
                  ...
                  // Generate a string from the ValueNodes under restrict
                  // which can be used during EXPLAIN of this PROJECT/FILTER op
                  // Use printExplainInfo which is similar to printSubNodes,
                  //  but printExplainInfo returns a String and writes SQL-like
                  //  syntax for restrict ValueNodes
                  projectedColumns.append("WHERE :");
                  projectedColumns.append(restriction.printExplainInfo());
...
// GemStone changes END
  ...
  public void setRefActionInfo(long fkIndexConglomId, 
	...
	//GemStone changes BEGIN
        ...
        public String toString() {
          if (SanityManager.DEBUG) {
              return childResult != null ? childResult + super.toString() : super
                  .toString();
          }
          else {
            return "";
	...
	@Override
	  ...
	  if (childResult instanceof Optimizable) {
             ((Optimizable)childResult).getTablesReferencedByRestrictionLists(referencedTables);
	  }
        }
	/**
	...
	@Override
	   ...
	   if(childResult instanceof OrderByNode || childResult instanceof ProjectRestrictNode || childResult instanceof DistinctNode) {
	      ...
	      throws StandardException 
	  {
	    return ProjectRestrictNode.getSelectQueryInfo(qic, this);
          ...
          public static SelectQueryInfo getSelectQueryInfo(QueryInfoContext qic, 
              QueryTreeNode qtNode) throws StandardException 
          {
	    SelectQueryInfo info = new SelectQueryInfo(qic, null);
	    try {
	      // TODO: Avoid escaping the reference. But cannot think of a better way
	      // at this point
    ...
    protected void optimizeForOffHeap( boolean shouldOptimize) {
        shouldOptimize = shouldOptimize 
            || (reflectionNeededForProjection() && !nopProjectRestrict());
    ...
    public Optimizable getBaseTable() throws StandardException {
      if (childResult instanceof Optimizable) {
        return ((Optimizable)childResult).getBaseTable();
      }

      //TODO: sb:DJ: handle SelectNode and alike.
      return this;
    }
    
        /*
        public  SubqueryList getRestrictionSubqueryList() {
          return this.restrictSubquerys;
        }*/
   ...
   */
  @Override
  public void collectHashKeyColumns(Vector hashKeyVector, int tabNum, int colNum)
      throws StandardException {
    ...
    for (int colCtr = 0; colCtr < colLength; colCtr++) {
      ...
      if (resCol.getTableNumber() == tabNum
          && resCol.getColumnPosition() == colNum) {
        hashKeyVector.addElement(Integer.valueOf(colCtr));
      }
    }
  }

  public ArrayList<String> getRemoteInListCols() {
    return remoteInListCols;
  }
