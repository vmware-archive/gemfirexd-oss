/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.DDLStatementNode
...
// GemStone changes END
{
	...
	*/
    ...
    if (ownerCheck && isPrivilegeCollectionRequired()
        && !(this instanceof AlterTableNode || 
            this instanceof GfxdAlterTableNode)) {
      cc.addRequiredSchemaPriv(sd.getSchemaName(), null,
		...
		*/	 
		if (!getLanguageConnectionContext().getExplainSchema() && sd.isSystemSchema())
		{
...
// GemStone changes BEGIN
			    // use table name string to allow serialization
                            statementToString(), (sd != null ? sd.getSchemaName() + "." : "")
...
// GemStone changes END
		...
		throws StandardException
	{
		...
		case TableDescriptor.COLUMN_TABLE_TYPE:
			 ...
			 */
// GemStone changes BEGIN
        // use non-exclusive mode to avoid possible deadlocks and to avoid
        // unnecessary distributed write locks
        return lockTableForCompilation(td, false);
