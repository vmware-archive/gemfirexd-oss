/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.GroupByNode
		...
		*/
		...
		if (! addDistinctAggregate && groupingList != null)
		{
			...
			if (index == glSize) {				
                ...
                // GemStone changes BEGIN
                if (isInSortedOrder && !aggInfo.hasDistinct()
                    && !aggInfo.hasExpressionInAggregate()) {
                  ((ProjectRestrictNode)childResult).markChildOptimized();
                }
                // GemStone changes END
		...
		for (int i = 0; i < sz; i++) 
		{
			...
			if (!rc.isGenerated()) {
				rclNew.addElement(rc);
	                ...
	                //GemStone changes BEGIN
			else if( !getCompilerContext().createQueryInfo()
...
// GemStone changes BEGIN
					...
					Integer.valueOf(groupByRCL.size()),
					/* (original code)
...
// GemStone changes END
	...
	private void addAggregateColumns() throws StandardException
	{
        ...
        //GemStone changes BEGIN
        boolean createQueryInfo = getCompilerContext().createQueryInfo();
        //GemStone changes END
		...
		for (int index = 0; index < alSize; index++)
		{
			...
			*/
			aggInfo.addElement(new AggregatorInfo(
					...
					// GemStone changes BEGIN
					, (!createQueryInfo && !getCompilerContext().allTablesAreReplicatedOnRemote() && aggregate.isDistinct())
					, (aggregate.getOperand() != null && aggregate.getOperand().hasExpression())
                                        // GemStone changes END
			));
		}
	}
...
// GemStone changes BEGIN
			...
			/* (original code) if (SanityManager.DEBUG_ON("AggregateTrace")) */
// GemStone changes END
			{
...
//      GemStone changes BEGIN        
          /**
           * Returns the QueryInfo object reprsenting the queries grouped by
           * list.
           */
           
          @Override
          public QueryInfo computeQueryInfo(QueryInfoContext qic) throws StandardException {
            if (qic.getRootQueryInfo().hasUnionNode()
                || qic.getRootQueryInfo().hasIntersectOrExceptNode()) {
              if (qic.getPRTableCount() == 0) {
                return QueryInfoConstants.DUMMYGROUPBYQI;
              }
              throw StandardException.newException(SQLState.NOT_IMPLEMENTED,
                  " Currently Set operators are not supported with Group By for Partitioned tables");
            }
            return new GroupByQueryInfo(qic, this, aggInfo, groupingList, resultColumns, parent.resultColumns, havingClause, isInSortedOrder);
          }
        ...
        //TODO :Asif : Check if this can be optimized for offheap
          ...
          protected void optimizeForOffHeap( boolean shouldOptimize) {
            /*if(shouldOptimize) {
              //check if it is optimizable
              shouldOptimize= (isInSortedOrder && !aggInfo.hasDistinct()) ;
            }*/
