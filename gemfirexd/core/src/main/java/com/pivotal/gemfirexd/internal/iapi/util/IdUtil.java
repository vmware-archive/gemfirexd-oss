  ...
  <PRE>
	...
	*/
	...
	public static String getUserNameFromURLProps(Properties params)
	{
		...
		*/
                String userName = params.getProperty(com.pivotal.gemfirexd.Attribute.USERNAME_ATTR);
                userName = userName == null ? params.getProperty(com.pivotal.gemfirexd.Attribute.USERNAME_ALT_ATTR,
                    Property.DEFAULT_USER_NAME) : userName;
...
// GemStone changes BEGIN
	...
	public static String getDBUserId(String userID, boolean isBUILTIN)
	    throws StandardException {
	  if (isBUILTIN) {
	    boolean isSQLFire = false;
	    if (userID.startsWith(Property.USER_PROPERTY_PREFIX)) {
	      userID = userID.substring(Property.USER_PROPERTY_PREFIX.length());
	    }
	    //SQLF:BC
            if (userID.startsWith(Property.SQLF_USER_PROPERTY_PREFIX)) {
              userID = userID.substring(Property.SQLF_USER_PROPERTY_PREFIX.length());
              isSQLFire = true;
            }
	    userID = getUserAuthorizationId(userID);
	    //SQLF:BC
	    userID = (isSQLFire ? Property.SQLF_USER_PROPERTY_PREFIX : Property.USER_PROPERTY_PREFIX) + userID;
	  }
	  else {
	    userID = getUserAuthorizationId(userID);
	  }
