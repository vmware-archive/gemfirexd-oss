/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.services.loader.ClassInspector
...
public final class ClassInspector
{
	...
	*/
		...
		if (! Modifier.isPublic(theClass.getModifiers())) {
                  if (GemFireXDUtils.TraceApplicationJars) {
                    SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_APP_JARS,
                        "class " + className + " is not defined with public modifier");
                  }
					...
					throws ClassNotFoundException, StandardException
	{
        ...
        // GemStone change: commented the code below, resolve the method in
        // the resolveMethod() call below which also takes care of
        // ProcedureExecutionContext arg
...
//Gemstone changes Begin
				//we need to remove the ProcedureExecutionContext from the method match;
				int numParameters=currentMethodParameters.length;
				if(numParameters>0) {
				   String lastParameterClassName=currentMethodParameters[numParameters-1].getCanonicalName();
				   if(lastParameterClassName.equals(ClassName.ProcedureExecutionContext)) {
				      Class[] realMethodParameters=new Class[numParameters-1];
				      System.arraycopy(currentMethodParameters, 0, realMethodParameters, 0, numParameters-1);
				      currentMethodParameters=realMethodParameters;
				   }
			       }		
