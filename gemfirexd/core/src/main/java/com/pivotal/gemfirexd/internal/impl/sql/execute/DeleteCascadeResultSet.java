/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.execute.DeleteCascadeResultSet
	...
	*/
	...
	public void cleanUp(boolean cleanupOnError) throws StandardException
	{
		...
		for(int i =0 ; i < noDependents; i++)
		{
			if(dependentResultSets[i] instanceof UpdateResultSet)
			{
				((UpdateResultSet) dependentResultSets[i]).cleanUp(cleanupOnError);
			}else
			{
				((DeleteCascadeResultSet) dependentResultSets[i]).cleanUp(cleanupOnError);
			}
		}
