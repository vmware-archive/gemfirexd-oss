/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.types.DataType
...
// GemStone changes BEGIN
		// cannot compare RowLocation against a real DVD
...
// GemStone changes END
  ...
  // GemStone changes BEGIN
  public static void writeDVDArray(DataValueDescriptor[] array, DataOutput out)
      throws IOException {
    ...
    if (SanityManager.DEBUG) {
      if (DataSerializer.DEBUG) {
        SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_TEMP_FILE_IO, 
            "Writing DataValueDescriptor array of length " + length);
      }
    }
    if(length > 0) {
  ...
  public static DataValueDescriptor[] readDVDArray(DataInput in)
    ...
    if (length == -1) {
      ...
      if (SanityManager.DEBUG) {
        if (DataSerializer.DEBUG || GemFireXDUtils.TraceTempFileIO) {
          SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_TEMP_FILE_IO, 
            "Read DataValueDescriptor array of length " + length);
        }
      }
      ...
      for (int i = 0; i < length; i++) {
        ...
        if (SanityManager.DEBUG) {
          if (GemFireXDUtils.TraceTempFileIO) {
            SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_TEMP_FILE_IO, "Read "
                + array[i] + "(" + array[i].getTypeName() + ")");
          }
        }
      }
  ...
  public static void writeDVDCollection(Collection<?> coll, DataOutput out)
      throws IOException {
    InternalDataSerializer.checkOut(out);
  ...
  public static <C extends Collection> C readDVDCollection(DataInput in,
      InternalDataSerializer.CollectionCreator<C> ctr) throws IOException,
    ...
    if (length == -1) {
      ...
      final C coll = ctr.newCollection(length);
  ...
  // register the DVD deserialization method
  ...
  public static void init() {
    ...
    DataSerializer ds = new DataSerializer() {
      ...
      public final int getId() {
        // must return zero to indicate WellKnownObjects
        return 0;
      }
      ...
      public final Object fromData(DataInput in) throws IOException,
          ClassNotFoundException {
        return readDVDArray(in);
      }

      @Override
      public final boolean toData(Object o, DataOutput out) throws IOException {
        DataValueDescriptor[] array = (DataValueDescriptor[])o;
        out.writeByte(DSCODE.GFXD_DVD_ARR);
        writeDVDArray(array, out);
        return true;
      }
    };

    InternalDataSerializer.getClassesToSerializers().put("[Lcom.gemstone"
        + ".gemfirexd.internal.iapi.types.DataValueDescriptor;", ds);
    InternalDataSerializer.registerDVDDeserializer(ds);

    DSFIDFactory.registerDVDDeserializer(new DSFIDFactory.DeserializeDVD() {
      ...
      public DataSerializableFixedID getGemFireKey(DataInput in)
          throws IOException, ClassNotFoundException {
        final CompositeRegionKey key = new CompositeRegionKey();
        key.fromData(in);
        return key;
      }
  ...
  public static DataValueDescriptor readNullDVD(byte typeId, DataInput in)
      throws IOException, ClassNotFoundException {
    switch (typeId) {
      case DSCODE.BOOLEAN:
      ...
      case DSCODE.TIME_UNIT:
        return new SQLTime();
      case DSCODE.INTEGER_TYPE:
        return new SQLTimestamp();
      case DSCODE.BYTE:
        return new SQLTinyint();
      case DSCODE.DOUBLE_TYPE:
        return new SQLDecimal();
      case DSCODE.BYTE_ARRAY:
        short formatId = in.readShort();
        return DataValueFactoryImpl.getNullDVDWithUCS_BASICcollation(formatId);
      case DSCODE.CHAR_ARRAY:
        return new SQLChar();
      case DSCODE.STRING:
        return new SQLVarchar();
      case DSCODE.HUGE_STRING:
        return new SQLLongvarchar();
      case DSCODE.HUGE_STRING_BYTES:
        return new SQLClob();
      case DSCODE.GFXD_JSON:
        return new JSON();
      case DSCODE.GFXD_XML:
        return new XML();
      case DSCODE.USER_DATA_SERIALIZABLE:
        return new UserType();
      case DSCODE.DS_FIXED_ID_BYTE:
        try {
          return new J2SEDataValueFactory().getNull(in.readShort(),
              StringDataValue.COLLATION_TYPE_UCS_BASIC);
        } catch (StandardException ex) {
          throw GemFireXDRuntimeException.newRuntimeException(
              "DataType#readNullDVD: unexpected exception", ex);
        }
        // case DSCODEStoredFormatIds.SQL_REF_ID: return new SQLRef();
        // case StoredFormatIds.SQL_USERTYPE_ID_V3: return new UserType();
      case DSCODE.NULL:
        return null;
      case DSCODE.HASH_SET:
        return new DVDSet();
      default:
        throw new GemFireXDRuntimeException(
            "DataType#readNullDVD: unexpected typeId=" + typeId);
    }
  }
  ...
  public static DataValueDescriptor readDVD(DataInput in) throws IOException,
      ClassNotFoundException {
    byte typeId = in.readByte();
    if (typeId == DSCODE.NULL) {
      return readNullDVD(in.readByte(), in);
    }
    else {
      DataValueDescriptor dvd = readNullDVD(typeId, in);
      dvd.fromData(in);
      return dvd;
    }
  }
  ...
  public static void readDVD(DataInput in, DataValueDescriptor dvd) throws IOException,
      ClassNotFoundException {
    byte typeId = in.readByte();
    ...
    if (typeId == DSCODE.BYTE_ARRAY || typeId == DSCODE.DS_FIXED_ID_BYTE) {
      in.readShort();
    }
    if (!isNull) {
      dvd.fromData(in);
  ...
  public final int getDSFID() {
    return DataSerializableFixedID.GFXD_DVD_OBJECT;
  }

  @Override
  public void toData(final DataOutput out) throws IOException {
    if (!isNull()) {
      out.writeByte(DSCODE.DS_FIXED_ID_BYTE);
  ...
  public boolean compare(int op, ExecRow row, boolean byteArrayStore,
      ...
      throws StandardException {
    if (byteArrayStore) {
      return BinarySQLHybridType.compare(op, (AbstractCompactExecRow)row, this,
          null, 0, logicalPosition, orderedNulls, unknownRV);
    }
    else {
      final DataValueDescriptor columnValue = row.getColumn(logicalPosition);
  ...
  public boolean compare(int op, CompactCompositeKey key, int logicalPosition,
    ...
    return BinarySQLHybridType.compare(op, key, this, null, 0, logicalPosition,
        orderedNulls, unknownRV);
  }
  ...
  public int equals(RowFormatter rf, byte[] bytes, boolean isKeyBytes,
      ...
      throws StandardException {
    throw new InternalGemFireError("needs implementation for class "
        + getClass().getName());
    /*
     * [sumedh] if this has to be used then a more efficient version should be
     * used that avoids creating other DVD; one option involving less work
     * could be to use getAs* methods from RowFormatter that avoids creating
     * DVDs, and optimize for strings and byte arrays separately
    final DataValueDescriptor other;
    if (isKeyBytes) {
      keyBytesPos = rf.getPrimaryKey(bytes, logicalPosition, keyBytesPos,
          outDVD);
      other = outDVD[0];
    }
    else {
      other = rf.getColumn(logicalPosition, bytes);
    }
    if (compare(ORDER_OP_EQUALS, other, true, false)) {
      return keyBytesPos;
    }
    return -1;
    */
