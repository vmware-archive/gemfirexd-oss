/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.store.access.sort.ExternalSortFactory
    ...
    SortFactory, ModuleControl, ModuleSupportable, SortCostController
{
	...
	**/
        ...
        throws StandardException
	{
		...
		if (!userSpecified)	
		{
			// gemfirexd.storage.sortBufferMax is not specified by the
		...
		{
			// if user specified gemfirexd.storage.sortBufferMax, use it.
				sortBufferMax = defaultSortBufferMax;
		}
		...
		if (SanityManager.DEBUG)
        {
            ...
            {
                SanityManager.DEBUG(GfxdConstants.TRACE_SORT_TUNING,
		...
		// GemStone changes BEGIN
                if (SanityManager.ASSERT) {
                  final GemFireXDQueryObserver observer = GemFireXDQueryObserverHolder
                      .getInstance();
                  if (observer != null) {
                    sortBufferMax = observer.overrideSortBufferSize(columnOrdering,
                        sortBufferMax);
                  }
                }
		// GemStone changes END
		...
		sort.initialize(
            tran, template, columnOrdering, 
            sortObserver, alreadyInOrder, estimatedRows, sortBufferMax, estimatedRowSize);
		return sort;
	}
	...
	*/
		...
		throws StandardException
	{
		...
		defaultSortBufferMax = PropertyUtil.getSystemInt("gemfirexd.storage.sortBufferMax",
