/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.sql.dictionary.ConglomerateDescriptor
...
// GemStone changes BEGIN
				" heap = " + Misc.getMemStoreBooting().findConglomerate(
				    ContainerKey.valueOf(ContainerHandle.TABLE_SEGMENT, conglomerateNumber)) +
...
// GemStone changes BEGIN
        // Since GfxdIndexManager registers with the table but not with all
        // the indices, explicitly invalidate the relevant GfxdIndexManager.
        // Handling this by registering as dependency with all the indices
        // is hard since changing the dependency registration for a new index
        // requires a statement context which is not available. Alternatively
        // updating the dependency registration in the makeInvalid/
        // prepareToInvalidate is somewhat problematic since the conglomerate
        // has not yet been updated in table descriptor list.
