/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.sql.dictionary.CatalogRowFactory
...
// GemStone changes BEGIN
		// [sumedh]: system tables are now local since now we have a hidden
		// GemFireStore.DDL_LISTENER_REGION region where all DDL statements
		// are put to distribute them.
		final AttributesFactory<?, ?> af =
		  new AttributesFactory<Object, Object>();
                af.setScope(Scope.LOCAL);
                af.setConcurrencyChecksEnabled(false);
                properties.put(GfxdConstants.REGION_ATTRIBUTES_KEY, af.create());
// GemStone changes END
		return properties;
	}
