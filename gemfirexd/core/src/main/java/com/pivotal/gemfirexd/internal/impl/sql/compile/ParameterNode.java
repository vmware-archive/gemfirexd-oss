/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.ParameterNode
...
public final class ParameterNode extends ValueNode
{
	...
	*/
	//GemStone changes BEGIN
	/*originally private DataValueDescriptor               defaultValue;*/
...
//Gemstone changes End  
	...
	public void setType(DataTypeDescriptor descriptor) throws StandardException
	{
	  //GemStone changes BEGIN
	    if( DVDSet.class.getName().equals(descriptor.getTypeId().getSQLTypeName()) ) {
	      assert ! getCompilerContext().createQueryInfo() : " DVDSet shouldn't have occured in QueryNode" ;
	      descriptor = (DataTypeDescriptor) 
	                      ((UserDefinedTypeIdImpl)descriptor.getTypeId().getBaseTypeId()).getClassParam();
	      assert descriptor != null : "DVDSet underlying result type is not expected to null" ;
	    }

	    /*
	     * Changes for NCJ
	     * Change the right side parameter descriptor, to make thet DVDSet.
	     * However, the left side parameter remained unchanged, roughly will
	     * be represented as (datatype in DVDSet(datatype)).   
	     */
            if (!getCompilerContext().createQueryInfo() && defaultValueIsArray) {
              descriptor = new DataTypeDescriptor(TypeId.getUserDefinedTypeId(
                  ClassName.DVDSet, descriptor, false), true);
            }
	  //GemStone changes END
    ...
    {
      //GemStone changes BEGIN
       //if by reference the same parameter, we can say there are equivalent
      if( this == o ) {
        return true;
      }
      //GemStone changes END
...
//  GemStone changes BEGIN
    ...
    public QueryInfo computeQueryInfo(QueryInfoContext qic) throws StandardException {
      //qic.addParameterIndex(this.parameterNumber);
      // its a constant generated as Parameter within an IN-list.
      if (valToGenerate != null) {
        return valToGenerate.computeQueryInfo(qic);
      }
