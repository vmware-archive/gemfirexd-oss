/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.Level2OptimizerImpl
...
public class Level2OptimizerImpl extends OptimizerImpl
{
		...
		throws StandardException
	{
		super(optimizableList, predicateList, dDictionary,
			  ...
			  joinStrategies, distJoinStrategies, tableLockThreshold, requiredRowOrdering,
			  numTablesInQuery);
					  ...
					  double doubleParam, Object objectParam1)
	{
		...
		switch (traceFlag)
		{
			...
			case MODIFYING_ACCESS_PATHS:
                                traceString = "Modifying access paths using optimizer "
                                    + this.hashCode() + " with bestJoinOrder " + Arrays.toString(bestJoinOrder)
                                    + " & bestJoinOrderColocatedTableGrp "
                                    + (bestJoinOrderColocatedTableGrp == null ? "null"
                                        : bestJoinOrderColocatedTableGrp);
				break;
		    ...
		    case COST_OF_CONGLOMERATE_SCAN7:
				...
				break;
// GemStone changes BEGIN
		    case HASH_INDEX_FULL_MATCH_ON_UNIQUE_KEY:
		      traceString = "Using hash index, but we have a full match"
		          + " on a unique key so using sort avoidance plan.";
		      break;
		    ...
		    case SETTING_REPLICATED_TABLE_GROUP:
                      traceString = "Setting Replicated Table Group at " + intParam1
                          + " for table " + objectParam1;
		      break;
		    case RESETTING_DRIVER_TABLE_INDEX:
                      traceString = "Resetting DriverTableIndex from " + driverTableIdx
                          + " to " + intParam1 + ", joinPosition=" + joinPosition
                          + " Colocated Table Group = " + colocatedTableGrp
                          + ", proposedJoinOrder = " + Arrays.toString(proposedJoinOrder);
		      break;
// GemStone changes END
		...
		if (addJoinOrderNumber)
		{
			joinOrderString = joinOrderString + " " + joinOrderNumber;
                ...
                return joinOrderString + " with assignedTableMap = " + assignedTableMap
                    + " colocatedTableGrp = " + colocatedTableGrp + "\n\n";
	}
