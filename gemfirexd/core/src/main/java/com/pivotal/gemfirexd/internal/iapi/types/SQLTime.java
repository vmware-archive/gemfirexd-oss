/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.types.SQLTime
        ...
        if( validSyntax)
        {
            if( amPm == 0) // AM
            {
                if( hour == 12)
                {
                	...
                	//Gemstone changes BEGIN
                    throw StandardException.newException( SQLState.LANG_DATE_RANGE_EXCEPTION, timeStr);
                    //Gemstone changes END
            }
            else if( amPm == 1) // PM
            {
        ...
        else
        {
            ...
            catch( ParseException pe)
            {
                ...
                catch( ParseException pe2)
                {
                    if( thrownSE != null)
                        throw thrownSE;
                    throw StandardException.newException( SQLState.LANG_DATE_SYNTAX_EXCEPTION,timeStr);
                }
            }
            valueString = timeStr;
        }
...
// GemStone changes END
        ...
        if( hour == 24)
        {
            if( minute != 0 || second != 0)
                ...
                throw StandardException.newException( SQLState.LANG_DATE_RANGE_EXCEPTION,
                		"Hour 24 but minute "+minute+" or second "+second+" is not zero");
                //Gemstone changes END
        }
     ...
     * @param minute
     * @param second
     * @param sb The resulting string is appended to this StringBuilder
     */
...
// GemStone changes BEGIN
  ...
  public int writeBytes(byte[] outBytes, int offset, DataTypeDescriptor dtd) {
    ...
    int bytesWritten = 0;
    bytesWritten += RowFormatter.writeInt(outBytes, this.encodedTime, offset);
    assert bytesWritten == (Integer.SIZE >>> 3);
    bytesWritten += RowFormatter.writeInt(outBytes, this.encodedTimeFraction,
        offset + bytesWritten);
    assert bytesWritten == 2 * (Integer.SIZE >>> 3);
    return bytesWritten;
  }
  ...
  public int readBytes(final byte[] inBytes, int offset,
    ...
    offset += 4;
    this.encodedTimeFraction = RowFormatter.readInt(inBytes, offset);
  ...
  public int computeHashCode(int maxWidth, int hash) {
    ...
    return ResolverUtils.addIntToBucketHash(this.encodedTimeFraction, hash,
        typeId);
  }

  static final java.sql.Time getAsTime(final byte[] bytes,
      int offset, Calendar cal) {
    int encodedTime = RowFormatter.readInt(bytes, offset);
    if (encodedTime == -1) return null;
    offset += 4;
    int encodedTimeFraction = RowFormatter.readInt(bytes, offset);
