/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.execute.CreateConstraintConstantAction
...
// GemStone changes BEGIN
			if (indexAction instanceof CreateIndexConstantAction) {
			  CreateIndexConstantAction createIndexAction =
			      (CreateIndexConstantAction)indexAction;
...
// GemStone changes END
		...
		switch (constraintType)
		{
			...
			case DataDictionary.FOREIGNKEY_CONSTRAINT:
				...
				if(!forCreateTable) {
				    TableDescriptor refTableDesc = otherConstraintInfo.getReferencedTableDescriptor(dd);
				    IndexRowGenerator refIndexDescriptor = referencedConstraint.getIndexConglomerateDescriptor(dd).
				        getIndexDescriptor();
				    DDUtils.validateRefConstraintCheckability(refTableDesc, refIndexDescriptor 
				    , td, lcc,this.columnNames );
				    isFKOnRR_PRRefIndexLocal = refTableDesc.getDistributionDescriptor().isPartitioned() 
				        && td.getDistributionDescriptor().getPolicy() == DistributionDescriptor.REPLICATE 
				            && refIndexDescriptor.indexType().equals(GfxdConstants.LOCAL_SORTEDMAP_INDEX_TYPE);
				    ...
				    if(isFKOnRR_PRRefIndexLocal) {
				      ...
				      partitioningColsIndexInRefKey = getPartitioningColsInRefKey(refTableDesc, refIndexDescriptor);			      
				    }
				}
...
// GemStone changes BEGIN
					 ...
					 // don't do the check during DDL replay
...
// GemStone changes END
					 dd.activeConstraint( conDesc ) )
				{
					validateFKConstraint(tc, 
										 ...
										 partitioningColsIndexInRefKey, refTablePR);
				}
  ...
  private int[] getPartitioningColsInRefKey(TableDescriptor refTableDesc,
      IndexRowGenerator refIndexDescriptor) throws StandardException {
  
    //idenitify the partitioning col indexes from the Ref Key columns( which will map to FK keys columns)
    ...
    int k = 0;
    for(int i = 0; i < refColsPos.length; ++i) {
      int  refColPos = refColsPos[i];
      for(int partColPos:partColsPos) {
        if(refColPos == partColPos ) {
...
// GemStone changes BEGIN
		// keep PK in table order due to serialized form in row
		if (constraintType == DataDictionary.PRIMARYKEY_CONSTRAINT) {
		  com.pivotal.gemfirexd.internal.engine.distributed.utils
		    .GemFireXDUtils.sortColumns(baseColumnPositions, columnNames);
		}
// GemStone changes END
...
// GemStone changes BEGIN
  ...
  public final java.util.Collection<CreateTableConstantAction.LoadIndexData>
      getLoadIndexData() {
