/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.sql.dictionary.DDUtils
	...
	**/
		...
		throws StandardException
	{
		...
		validateDeleteConnection(dd, td, refTd, 
								 ...
								 new StringBuilder(0), refTableName,
								 isSelfReferencingFk,
								 currentSelfRefValue);
	...
	public static void validateRefConstraintCheckability(TableDescriptor refTd, IndexRowGenerator refIndexDescriptor,
	    TableDescriptor fkTd, LanguageConnectionContext lcc, String[] fkColumnNames) throws StandardException {
	//If the FK table is partitioned & Referential table is also partitioned
          // with referential index constraint type is local, and the FK table is 
          // not colocated with the Referential table & FK table region size is > 0
          // then we throw exception for now.
          //Ref : Bug 47289
	  boolean checkFurther = refTd.getDistributionDescriptor().isPartitioned() && fkTd.getDistributionDescriptor().isPartitioned();
	  if(checkFurther) {
	    String prFKPath = Misc.getRegionPath(fkTd.getSchemaName(), fkTd.getName(), lcc);
	    String prRefPath = Misc.getRegionPath(refTd.getSchemaName(), refTd.getName(), lcc);
	    PartitionedRegion prFK =  (PartitionedRegion)Misc.getRegionByPath(prFKPath, true);
	    PartitionedRegion prRef =  (PartitionedRegion)Misc.getRegionByPath(prRefPath, true);
	    checkFurther = refIndexDescriptor.indexType().equals(GfxdConstants.LOCAL_SORTEDMAP_INDEX_TYPE) && prFK.size() > 0;
	    if(checkFurther) {
	      //check if the two tables are colocated && the FK columns contain the complete partitioning columns
	      GfxdPartitionResolver resolverFK = (GfxdPartitionResolver)prFK.getPartitionResolver();
	      String partitioningCols [] =  resolverFK.getColumnNames();
	      boolean fkColsContainPartitioningCols = doFKColsContainAllPartitioningCols(fkColumnNames, partitioningCols);
	      
	      GfxdPartitionResolver resolverRef = (GfxdPartitionResolver)prRef.getPartitionResolver();
	      
              if( !(fkColsContainPartitioningCols && resolverFK != null && resolverRef != null && resolverFK.getMasterTable(true).equals(resolverRef.getMasterTable(true)))) {
	        throw  StandardException.newException(SQLState.LANG_ALTER_TABLE_ADD_FK_CONSTRAINT_CHECK_FAIL,fkTd.tableName, refTd.tableName);
	      }
	    }
	...
	private static boolean doFKColsContainAllPartitioningCols(String[] fkCols, String[] partitioningCols) {
	  if(partitioningCols == null) {
	    return false;
	  }
	  boolean ok = true;
	  for(String partitionCol:partitioningCols) {
	    boolean foundMatch = false;
	    inner:for(String fkCol: fkCols) {
	      if(fkCol.equalsIgnoreCase(partitionCol)) {
	        foundMatch = true;
	        break inner;
	      }
	    }
...
// GemStone changes BEGIN
				// changed to use Integer.valueOf()
				if (isSelfReferencingFk && dch.contains(
				    Integer.valueOf(StatementType.RA_CASCADE)) &&
				/* (original code)
