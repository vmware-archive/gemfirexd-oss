/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.BitConstantNode
...
public class BitConstantNode extends ConstantNode
{
		...
		throws StandardException
	{
		...
		byte[] nv = com.pivotal.gemfirexd.internal.iapi.util.StringUtil.fromHexString(a1, 0, a1.length()); 
		...
		com.pivotal.gemfirexd.internal.iapi.types.BitDataValue dvd = getDataValueFactory().getBitDataValue(nv);
		...
		throws StandardException
	{
		...
		String hexLiteral = com.pivotal.gemfirexd.internal.iapi.util.StringUtil.toHexString(bytes, 0, bytes.length);

		mb.push(hexLiteral);
		mb.push(0);
		mb.push(hexLiteral.length());

		mb.callMethod(VMOpcode.INVOKESTATIC, "com.pivotal.gemfirexd.internal.iapi.util.StringUtil", "fromHexString",
						"byte[]", 3);
	}
}
