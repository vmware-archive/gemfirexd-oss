/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.execute.SetConstraintsConstantAction
		...
		for (Enumeration e = ht.elements(); e.hasMoreElements(); )
		{
			...
			*/
			...
			for (int index = 0; index < cdlSize; index++)
			{
				...
				if (text == null)
				{
					text = new StringBuilder("(").append(cd.getConstraintText()).append(") ");
					constraintNames = new StringBuilder(cd.getConstraintName());
				}
				else
				{
		...
		throws StandardException
	{
		...
		*/
		ConstraintConstantAction.validateFKConstraint(tc, dd, fk, 
							fk.getReferencedConstraint(), indexTemplateRow, null, null);
	}
