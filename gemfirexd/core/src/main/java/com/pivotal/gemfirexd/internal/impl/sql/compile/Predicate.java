/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.Predicate
														...
														Comparable
{
	...
	private int isColocated = ValueNode.UNKNOWN;
  ...
  public void init(Object andNode, Object referencedSet)
	...
	/**
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.compile.OptimizablePredicate#getReferencedMap
	 */
	@Override
  public JBitSet getReferencedMap()
	...
	/**
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.compile.OptimizablePredicate#hasSubquery
	 */
	@Override
  public boolean hasSubquery()
	...
	/**
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.compile.OptimizablePredicate#hasMethodCall
	 */
	@Override
  ...
  public double selectivity(Optimizable optTable)
	...
	{
// GemStone changes BEGIN
		return andNode.selectivity(optTable);
		/* (original code)
  ...
  public int compareTo(Object other)
	{
		...
		{
// GemStone changes BEGIN
			int thisOperator = andNode.getLeftOperatorForCompare();
			/* (original code)
...
// GemStone changes BEGIN
			...
			searchClauseHT.get(Integer.valueOf(ro.getOperator())) == null)
			/* (original code)
...
// GemStone changes BEGIN
		// changed to use Integer.valueOf()
		final Integer i = Integer.valueOf(ro.getOperator());
		/* (original code)
		Integer i = new Integer(ro.getOperator());
		*/
// GemStone chanhges END
  ...
  public Visitable accept(Visitor v) 
	...
	// TODO: SW: what to do about scoped predicates for OrListNode?
	/**
...
// GemStone changes END
  ...
  /**
   * Recursively traverse the predicate tree & for each AndNode, notice the
   * BinaryRelationalOperatorNode whereas for each OrNode take a note of both
   * left/right operand separately. Both operand's OrNode relation must
   * individually be considered in colocation determination.
   * 
   * @param src
   *          source optimizable (mainly FromBaseTable for now)
   * @param tgtPCols 
   * @param srcPCols 
   * @param target
   *          another optimizable (mainly FromBaseTable for now)
   * @return <list> 
   *           <li>COMPLETELYCOLOCATED if this predicate is sufficient to service
   *               colocated join b/w src & target.</li> 
   *           <li>PARTLYCOLOCATED if this contains
   *               some partitioning columns that may satisfy colocation requirement
   *               along with other predicates in the list.</li>
   *           <li>NOTCOLOCATED indicating predicate relation isn't 
   *               on partitioning column</li>
   *         </list>
   * @throws StandardException 
   */
  public int setColocatedWith(Optimizable src,
      Optimizable tgt, JBitSet srcPCols, JBitSet tgtPCols) throws StandardException {

    isColocated = ValueNode.UNKNOWN;
    final BinaryRelationalOperatorNode opNode = (BinaryRelationalOperatorNode)getRelop();

    if (isJoinPredicate()) {
      // okay simple case of a.col = b.col
      int ret = opNode.setColocatedWith(src, tgt, srcPCols, tgtPCols);
      
      if (ret == ValueNode.UNKNOWN) {
        isColocated = ret;
      }
      else if (srcPCols.equals(tgtPCols)) {
        if (srcPCols.hasAllBitSet() && tgtPCols.hasAllBitSet()) {
          isColocated = ValueNode.COMPLETELYCOLOCATED;
        }
        else {
          isColocated = ValueNode.PARTLYCOLOCATED;
        }
      }
      else {
        isColocated = ret;
    ...
    //TODO: sb:DJ: handle more complex join conditions.
