/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.store.access.RAMAccessManager
               ...
               PropertySetCallback
{
  ...
  public TransactionController getTransaction(ContextManager cm)
    ...
    ContextManager cm, boolean bootTime)
        throws StandardException
    {
       // Gemstone changes begin. 
	     return getAndNameTransaction(cm, AccessFactoryGlobals.USER_TRANS_NAME, bootTime);
	     // Gemstone changes end.
    }

    public TransactionController getAndNameTransaction(
    ContextManager cm, String transName, boolean bootTime)
        throws StandardException
    {
        if (cm == null)
    ...
    /**
     * @see com.pivotal.gemfirexd.internal.iapi.store.access.AccessFactory#failover(String dbname).
     */
		...
		throws StandardException
	{
		...
		Monitor.bootServiceModule(
            create, this, com.pivotal.gemfirexd.internal.iapi.reference.Module.PropertyFactory, 
            startParams);
        ...
        RAMTransaction tc =
            (RAMTransaction) getAndNameTransaction(
                ...
                AccessFactoryGlobals.USER_TRANS_NAME, true);
        // Gemstone changes end.
        ...
        pf = (PropertyFactory) 
            Monitor.findServiceModule(
                this, com.pivotal.gemfirexd.internal.iapi.reference.Module.PropertyFactory);
