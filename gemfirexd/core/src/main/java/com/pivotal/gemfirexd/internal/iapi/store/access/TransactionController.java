/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.store.access.TransactionController
...
@see com.pivotal.gemfirexd.internal.iapi.error.StandardException
@see PersistentSet
    ...
    extends PersistentSet
{
    ...
    **/
     ...
     *      gemfirexd.storage.initialPages
     *      gemfirexd.storage.minimumRecordSize
     *      gemfirexd.storage.pageReservedSpace
...
// GemStone changes END
   ...
   * Open a scan on a conglomerate, optionally providing compiled info.
   * <p>
   * Same as openScan(), except that a local data set is provided by function 
   * service to qualify the rows.
   * 
   * <p>
   *
   * @see TransactionController#openScan
 ...
 * @return The identifier to be used to open the conglomerate later.
   ...
   * @param dynamic_info      object returned from
   *                          getDynamicCompiledConglomInfo() call on this id.
   * @param localDataSet      local data set for GemFire Funciton execution.
   *
 * @exception  StandardException  Standard exception policy.
   **/
	ScanController openCompiledScan(
     ...
     * used by either explicit user call of the ONLINE_COMPRESS_TABLE()
	...
	*/
     ...
     * @see com.pivotal.gemfirexd.internal.iapi.services.locks.LockOwner#noWait()
     * @see com.pivotal.gemfirexd.internal.iapi.store.raw.Transaction#setNoLockWait(boolean)
     */
    ...
    @see com.pivotal.gemfirexd.internal.iapi.store.raw.Loggable
    @see com.pivotal.gemfirexd.internal.iapi.store.raw.Transaction#logAndDo
...
// GemStone changes BEGIN
	// added return of DatabaseInstant like in commitNoSync
...
// GemStone changes BEGIN
  ...
  /**
   * Get the {@link LanguageConnectionContext}, if any, associated with this
   * transaction.
   */
  public LanguageConnectionContext getLanguageConnectionContext();

  /**
   * Associate the given {@link LanguageConnectionContext} with this
   * transaction.
   */
  public void setLanguageConnectionContext(LanguageConnectionContext lcc);

  /** explicitly release all locks acquired in this transaction */
  public void releaseAllLocks(boolean force, boolean removeRef);

  /**
   * @return true if this is transaction should skip taking locks (e.g. since
   *         the query is from remote node)
   */
  public boolean skipLocks();

  /**
   * @param warnIfSkipLocksForConnection
   *          set a warning for current statement if
   *          <code>Attribute.SKIP_LOCKS</code> was set on the connection itself
   * 
   * @return true if this is transaction should skip taking locks (e.g. since
   *         the query is from remote node)
   * 
  ...
  /**
   * Specifies whether the locks should be skipped for this transaction (e.g.
   * due to being remote node).
   */
  public void setSkipLocks(boolean skip);

  /**
   * Specifies whether the <code>Attribute.SKIP_LOCKS</code> property has been
   * set for the connection itself explicitly.
   */
  public void setSkipLocksForConnection(boolean skip);

  /** Turn on logging for this Transaction. */
  public void enableLogging();

  /** Turn off logging for this Transaction. */
  ...
  /**
   * Commit this transaction indicating that this is due to a setIsolationLevel
   * call on the connection. All non-held conglomerates and scans are closed.
   */
  public DatabaseInstant commitForSetIsolationLevel() throws StandardException;

  /**
   * Start a new transaction at given isolation level. Previous transaction, if
   * any, will already have been committed or rolled back.
   */
  public void beginTransaction(IsolationLevel isolationLevel) throws StandardException;

  /**
   * Indicates an isolation-level greater than NONE.
   */
  public boolean isTransactional();

  /**
   * Returns true if the current transaction is in a closed state (i.e. just
   * after creation).
   */
  public boolean isClosed();

  long createSort(Properties implParameters, ExecRow template,
