/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.GenericColumnDescriptor
...
//        GemStone changes BEGIN
	...
	public GenericColumnDescriptor(String myName, String mySchemaName,
	    String myTableName, int myColPos, DataTypeDescriptor myDtd,
	    boolean isUpdtbl, boolean isautoIncrement) {
	  this.name = myName;
...
// GemStone changes BEGIN
	/**
	 * @see ResultColumnDescriptor#primaryKey(EngineConnection)
	 */
	public final short primaryKey(EngineConnection conn)
	    throws StandardException {
	  if (this.primaryKey == -1) {
	    if (this.tableName != null && this.tableName.length() > 0
	        && !GfxdDataDictionary.isSystemSchema(this.schemaName)) {
	      final GemFireContainer container = GemFireXDUtils
	          .getGemFireContainer(this.schemaName, this.tableName,
	              conn.getLanguageConnectionContext());
	      if (container != null) {
	        this.primaryKey = ResultColumn.checkPrimaryKey(container
	            .getExtraTableInfo(), this.name, null);
	      }
	      else {
	        this.primaryKey = 0;
	      }
