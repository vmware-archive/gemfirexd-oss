/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.execute.GenericConstantActionFactory
...
public class GenericConstantActionFactory
{
	 ...
	 * @param generatedSqlTextForCTAS If the query is of the from 'create table t1 as 
	 *                                select * from t2' this will contain a SQL text 
	 *                                containing actual column info instead of the 
	 *                                select clause
	 */
...
// GemStone changes END
	...
	{
// GemStone changes BEGIN
		return	new DropTableConstantAction( fullTableName, tableName,
		    sd, conglomerateNumber, tableId, onlyIfExists, behavior);
		/* (original code)
...
// GemStone changes END		    
    ...
    /**
	 * Make the ConstantAction for a CREATE ASYNCEVENTLISTENER statement.
	 *
	 */
	public ConstantAction getCreateAsyncEventListenerConstantAction(
			String id, ServerGroupsTableAttribute serverGroups,
			boolean manualStart, boolean enableBatchConflation,
			int batchSize, int batchTimeInterval,
			boolean enablePersistence, boolean diskSync, String diskStoreName,
			int maximumQueueMemory, int alertThreshold, boolean isParallel,
			String className, String initParams)
	{
		return new CreateAsyncEventListenerConstantAction(
				id, serverGroups,
				manualStart, enableBatchConflation,
				batchSize, batchTimeInterval,
				enablePersistence, diskSync, diskStoreName,
				maximumQueueMemory, alertThreshold,isParallel,
				className, initParams);
	}

    /**
	 * Make the ConstantAction for a DROP ASYNCEVENTLISTENER statement.
	 *
	 */
	public ConstantAction getDropAsyncEventListenerConstantAction(
			String id, boolean onlyIfExists)
	{
		return new DropAsyncEventListenerConstantAction(
				id, onlyIfExists);
	}
	
    /**
         * Make the ConstantAction for a CREATE HDFSSTORE statement.
         * 
         */
	public ConstantAction getCreateHDFSStoreConstantAction(
	    String hdfsStoreName, HDFSStoreFactory hsf)
	{
	        return new CreateHDFSStoreConstantAction(hdfsStoreName, hsf);
	}
	
  /**
   * Make the ConstantAction for a Alter HDFSSTORE statement.
   * 
   */
  public ConstantAction getAlterHDFSStoreConstantAction(String storeName,
      HDFSStoreMutator mutator) {
    return new AlterHDFSStoreConstantAction(storeName, mutator);
  }
	
    /**
	 * Make the ConstantAction for a DROP HDFSSTORE statement.
	 *
	 */
	public ConstantAction getDropHDFSStoreConstantAction(
			String id, boolean onlyIfExists)
	{
		return new DropHDFSStoreConstantAction(
				id, onlyIfExists);
	}
	
    /**
	 * Make the ConstantAction for a CREATE DISKSTORE statement.
	 *
	 */
	public ConstantAction getCreateDiskStoreConstantAction(
			String diskStoreName, List<String> dirPaths, List<Integer> dirSizes, Map otherAttribs)
	{
		return new CreateDiskStoreConstantAction(
				diskStoreName, dirPaths, dirSizes, otherAttribs);
	}
	
    /**
	 * Make the ConstantAction for a DROP DISKSTORE statement.
	 *
	 */
	public ConstantAction getDropDiskStoreConstantAction(
			String id, boolean onlyIfExists)
	{
		return new DropDiskStoreConstantAction(
				id, onlyIfExists);
	}
	
    /**
	 * Make the ConstantAction for a DROP GATEWAYSENDER statement.
	 *
	 */
	public ConstantAction getDropGatewaySenderConstantAction(
			String id, boolean onlyIfExists)
	{
		return new DropGatewaySenderConstantAction(
				id, onlyIfExists);
	}
	
    /**
	 * Make the ConstantAction for a DROP GATEWAYRECEIVER statement.
	 *
	 */
	public ConstantAction getDropGatewayReceiverConstantAction(
			String id, boolean onlyIfExists)
	{
		return new DropGatewayReceiverConstantAction(
				id, onlyIfExists);
	}

	/**
	 * Make the ConstantAction for a CREATE GATEWAYSENDER statement.
	 *
	 */
	public ConstantAction getCreateGatewaySenderConstantAction(
			String senderId, ServerGroupsTableAttribute serverGroups,
			int socketBufferSize, boolean manualStart, int socketReadTimeout, boolean enableBatchConflation,
			int batchSize, int batchTimeInterval, boolean enablePersistence, boolean diskSync, String diskStoreName,
			int maximumQueueMemory, int alertThreshold, int remoteDsId, boolean isParallel)
	{
		return new CreateGatewaySenderConstantAction(
				senderId, serverGroups, socketBufferSize, manualStart, socketReadTimeout, enableBatchConflation,
				batchSize, batchTimeInterval, enablePersistence, diskSync, diskStoreName,
				maximumQueueMemory, alertThreshold, remoteDsId, isParallel);
	}
	
    /**
	 * Make the ConstantAction for a CREATE GATEWAYRECEIVER statement.
     * @throws StandardException 
	 *
	 */
	public ConstantAction getCreateGatewayReceiverConstantAction(
			String id, ServerGroupsTableAttribute serverGroups,
					int startPort, int endPort, String bindAddress, int maxTimeBetPings,
					int socketBufferSize, String hostNameForSenders) throws StandardException
	{
		return new CreateGatewayReceiverConstantAction(
