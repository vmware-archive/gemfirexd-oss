/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.InListOperatorNode
...
public final class InListOperatorNode extends BinaryListOperatorNode
{
	...
	public String toString()
	{
		...
		{
                        return "isOrdered: " + isOrdered + " ,rightOperandIsArray: "
					...
					throws StandardException
	{
		 ...
		 */
                if (rightOperandList.size() == 1 && !this.rightOperandIsArray) 
                {
...
//Gemstone changes End					
	{
		...
		if (! underNotNode)
		{
			return this;
                ...
                if (SanityManager.DEBUG) {
                  SanityManager
                      .ASSERT(!this.rightOperandIsArray,
                          "Scenario of In-list as var param array under NOT is not supported yet");
                }
									...
									throws StandardException
	{
		...
		*/
		...
		resultTypeName = getTypeCompiler().interfaceName();
                ...
                if (this.rightOperandIsArray) {
                  ncjGenerateExpression(acb, mb, resultTypeName);
                  return;
                }

		// Generate the code to build the array
                final LocalField arrayField = generateListAsArray(acb, mb);
		...
		*/
		...
		mb.push(isOrdered); // third arg
                mb.callMethod(VMOpcode.INVOKEINTERFACE, receiverType, methodName,
                    resultTypeName, 3);
	}
	...
	{
                if (SanityManager.DEBUG) {
                  SanityManager.ASSERT(!this.rightOperandIsArray,
                      "Method ncjGenerateListAsArray should be called.");
                }
	...
	public void generateStartStopKey(boolean isAsc, boolean isStartKey,
									 ...
									 MethodBuilder mb)
											   throws StandardException
	{
                /* Since Inlist with ARRAY will have only one parameter, should not come here. 
                 * But if it does, we can disable calling this method based on rightOperandIsArray in PRN
                 */
                if (SanityManager.DEBUG) {
                  SanityManager
                      .ASSERT(
                          !this.rightOperandIsArray,
                          "Scenario of In-list as var param array with start/stop key is not supported yet");
                }
	...
	{
	        if (this.rightOperandIsArray) {
	          // TODO: alternatively sort values while returning
	          // Write test for same
	          return false;
	        }
...
//      GemStone changes BEGIN        
        ...
        public QueryInfo computeQueryInfo(QueryInfoContext qic) throws StandardException {          
          ...
          if (isDynamic != null) {
            final InQueryInfo inQI = new InQueryInfo(
                this.leftOperand.computeQueryInfo(qic), isDynamic.booleanValue(),
                qiArr);
            if (this.rightOperandIsArray) {
        ...
        public String ncjGenerateSql() {
          ...
          sqlStr.append(leftOperand.ncjGenerateSql());
          sqlStr.append(" ");
          sqlStr.append(operator);
          sqlStr.append(" (");
          for (int index = 0; index < rightOperandList.size(); index++) {
            ValueNode vNode = (ValueNode)rightOperandList.elementAt(index);
            if (index != 0) {
              sqlStr.append(", ");
            }
        ...
        public void setRightOperandArray() {
          ...
          if (SanityManager.DEBUG) {
            SanityManager.ASSERT(rightOperandList.size() == 1,
                "Right Child of IN Operator must have one and only one Parameter");
            SanityManager.ASSERT(
                rightOperandList.elementAt(0) instanceof ParameterNode,
                "Right Child of IN Operator must be of type ParameterNode, and not "
                    + rightOperandList.elementAt(0).getClass().getSimpleName());
          }
          ParameterNode rightNode = (ParameterNode)rightOperandList.elementAt(0);
        ...
        protected void handleUserDefinedTypes() throws StandardException {
          if (this.rightOperandIsArray) {
            // Do nothing - We do not expect User Defined Types
            return;
          }
          super.handleUserDefinedTypes();
        }

  private void ncjGenerateExpression(ExpressionClassBuilder acb,
      MethodBuilder mb, String resultTypeName) throws StandardException {
    LocalField arrayField = acb.newFieldDeclaration(Modifier.PRIVATE,
        ClassName.DataValueDescriptor + "[]");
    ParameterNode rightNode = (ParameterNode)rightOperandList.elementAt(0);
    rightNode.generateExpression(acb, mb);
    leftOperand.generateExpression(acb, mb);
    mb.upCast(ClassName.DataValueDescriptor); // first arg
    mb.getField(arrayField); // second arg - arrayField
    mb.push(false); // third arg - isOrdered
    mb.callMethod(VMOpcode.INVOKEVIRTUAL, ClassName.DVDSet, methodName,
        resultTypeName, 3);
  }

  protected void ncjGenerateListAsArray(ExpressionClassBuilder acb,
      MethodBuilder mb) throws StandardException {
    ParameterNode rightNode = (ParameterNode)rightOperandList.elementAt(0);
    rightNode.generateExpression(acb, mb);
    mb.callMethod(VMOpcode.INVOKEVIRTUAL, ClassName.DVDSet, "getValues",
        ClassName.DataValueDescriptor + "[]", 0);
  }
//    GemStone changes END
}
