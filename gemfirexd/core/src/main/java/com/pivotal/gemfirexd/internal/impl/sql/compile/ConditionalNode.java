/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.ConditionalNode
...
//      GemStone changes BEGIN
        ...
        public QueryInfo computeQueryInfo(QueryInfoContext qic) throws StandardException {
          //Since a "Case when then" type of statement cannot be used for nodes pruning 
          // just return null. If null is returned it will be automatically taken care of 
          // in the QueryInfo Where clause structure
           ...
           throws StandardException 
        {
            ...
            testCondition = testCondition.genExpressionOperands(outerResultColumns 
                                                               ,null
                                                               ,remapToNew);
            
            int tELsize = thenElseList.size();
            if( tELsize > 0 ) {
                ValueNodeList vnl = new ValueNodeList();
                for(int i = 0; i < tELsize; i++) {
                    ValueNode vn = ((ValueNode)thenElseList.elementAt(i)).
                                                           genExpressionOperands(outerResultColumns
                                                                                ,null
                                                                                ,remapToNew);
                    vnl.addElement(vn);
                }
