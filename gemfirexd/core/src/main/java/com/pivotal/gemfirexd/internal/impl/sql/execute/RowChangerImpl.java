/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.execute.RowChangerImpl
...
public class RowChangerImpl	implements	RowChanger
{
	...
	private final boolean isTransactional; // GemStone addition
	/**
		...
		*/
		if (changedColumnIdsInput != null)
		{
			...
			*/
			...
			//Asif: If transactional we will have to create for each update. The other option
			// would be to have SerializableDelta convert DVD[] into bytes, but that would I suppose
			// be waste in case of rollback, so better to create sparse row per operation
...
// GemStone change BEGIN
				baseRowLocation = baseCC.insertAndFetchLocation(
				    baseRow.getRowArray(), baseRowLocation);
				//isc.insert(baseRow, baseRowLocation); //rahul : index maintainance done from region.
// GemStone change END
			}
			else
			{
	...
	{	  
		if (isc != null)
		{
			// Todo : Rahul : have to revisit this.
		  // indexes should be maintained from region.
		  isc.delete(baseRow, baseRowLocation);
			
		}
		return baseCC.delete(baseRowLocation);
	}
	...
	/**
	  ...
	  @exception StandardException		Thrown on error
	  */
	public boolean updateRow(ExecRow oldBaseRow,
						  ExecRow newBaseRow,
...
// GemStone changes BEGIN
		if (changedColumnBitSet != null) {
		  if (this.isTransactional) {
		    this.sparseRowArray = new DataValueDescriptor[
		        changedColumnIds[changedColumnIds.length - 1] + 1];
		  }
		/* (original code)
		if (changedColumnBitSet != null)
		{
...
// GemStone changes BEGIN
				sparseRowArray[nextColumnToUpdate] = isTransactional
				    ? baseRowArray[copyFrom].getClone()
				    : baseRowArray[copyFrom];
				/* (original code)
...
// GemStone changes BEGIN
		  if (this.isTransactional) {
		    this.sparseRowArray = newBaseRow.getRowArrayClone();
		  }
		  else {
		    this.sparseRowArray = newBaseRow.getRowArray();
		  }
                    /* (original code)
...
// GemStone changes END
		...
		return baseCC.replace(baseRowLocation, 
					sparseRowArray, 
					changedColumnBitSet);
