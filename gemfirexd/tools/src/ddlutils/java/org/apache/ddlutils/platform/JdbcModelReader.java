...
public class JdbcModelReader
{
    ...
    protected List initColumnsForColumn()
    {
        ...
        result.add(new MetaDataColumnDescriptor("DATA_TYPE",      Types.INTEGER, Integer.valueOf(java.sql.Types.OTHER)));
...
// GemStone changes END
    ...
    protected Collection readTables(String catalog, String schemaPattern, String[] tableTypes) throws SQLException
    {
        ...
        try
        {
            ...
            {
// GemStone changes BEGIN
              // ignore unknown JDBC typeCode errors and move to next table
              // (see #42571 for errors with Oracle system tables that get
              //  picked up even with tableType as "TABLE")
              Map   values = null;
              Table table  = null;
              try {
                values = readColumns(tableData, getColumnsForTable());
                table = readTable(metaData, values);
              } catch (ModelException e) {
                if (e.getMessage().contains("Unknown JDBC type")) {
                  _log.warn("Skipping table "
                      + (values != null ? values.get("TABLE_NAME") : "(null)")
                      + " due to " + e.getMessage());
                }
                else {
                  throw e;
...
// GemStone changes BEGIN
            final String origSchemaPattern = metaData.getSchemaPattern();
            final String schemaName;
            if ((schemaName = table.getSchema()) != null) {
...
// GemStone changes BEGIN
            if (schemaName != null) {
              metaData.setSchemaPattern(origSchemaPattern);
            }
// GemStone changes END
        }
        return table;
...
// GemStone changes BEGIN
            String pkSchema = (String)values.get("PKTABLE_SCHEM");
            String pkTable = (String)values.get("PKTABLE_NAME");
            if (pkSchema != null && pkSchema.length() > 0) {
              pkTable = pkSchema + '.' + pkTable;
            }
            fk.setForeignTableName(pkTable);
            /* (original code)
