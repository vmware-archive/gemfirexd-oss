...
// GemStone changes BEGIN
  /**
   * Stores the last table being used for inserts so that batch queue can be
   * flushed if the table changes.
   */
  private Table _lastTable = null;

  /**
   * Map of beans to original identities for identity column update lookup
   * for FK tables in batch mode after purging queue of a table.
   */
  private HashMap<DynaBean, Identity> _origIdentityMap = null;
...
// GemStone changes BEGIN
    ...
    private void populateIdentityMap(Table table, DynaBean bean,
...
// GemStone changes BEGIN
                if (_origIdentityMap != null) {
                    for (Object element : _batchQueue) {
                        final DynaBean bean = (DynaBean)element;
                        populateIdentityMap(_model.getDynaClassFor(bean)
                            .getTable(), bean, _origIdentityMap.get(bean));
                    }
                    _origIdentityMap.clear();
                }
// GemStone changes END
...
// GemStone changes END
    ...
    private String getFKName(Table owningTable, ForeignKey fk)
    {
        ...
        else
        {
            ...
            result.append(owningTable.getQualifiedName());
            result.append("[");
