...
public class DynaClassCache
{
    ...
    public DynaBean createNewInstance(Table table) throws SqlDynaException
    {
        ...
        {
            throw new SqlDynaException("Could not create a new dyna bean for table "+table.getQualifiedName(), ex);
        }
    ...
    {
        SqlDynaClass answer = (SqlDynaClass)_dynaClassCache.get(table.getQualifiedName());

        if (answer == null)
        {
            answer = createDynaClass(table);
            _dynaClassCache.put(table.getQualifiedName(), answer);
        }
        return answer;
    }
