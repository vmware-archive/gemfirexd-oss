/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.tools.ij.util
...
/**
	...
	@see com.pivotal.gemfirexd.internal.tools.JDBCDisplayUtil
 */
public final class util implements java.security.PrivilegedAction {
   ...
   java.lang.reflect.Method[] methods = ds.getClass().getMethods();
	...
	// See com.pivotal.gemfirexd.internal.tools.JDBCDisplayUtil for more general displays.
...
AppUI.out.println("SIZE="+l);
	...
	public static void DisplayMulti(LocalizedOutput out, PreparedStatement ps,
		...
		StopWatch timer /* GemStoneAddition */) throws SQLException, ijException {
	...
	static final String getSystemProperty(String propertyName) {
		try
		{
			if (propertyName.startsWith("ij.") || propertyName.startsWith("gemfirexd."))
			{
				util u = new util();
				u.key = propertyName;
...
// GemStone changes BEGIN
      ...
      { "jdbc:snappydata:", "io.snappydata.jdbc.EmbeddedDriver" }
		  ...
		  { "jdbc:derby:",				"com.pivotal.gemfirexd.jdbc.EmbeddedDriver" },
		  */
