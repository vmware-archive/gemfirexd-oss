...
public class ResultSetsFromPreparedStatementTest extends BaseJDBCTestCase
{
    ...
    public static String insertInto(String table, int params) {
        StringBuilder tmp = new StringBuilder("insert into "+table+" values ( ?");
        while (--params > 0) {
            tmp.append(", ?");
        }
        tmp.append(")");
...
//     }
    ...
    public void testNestedLoopResultSet() throws Exception {
      // [sjigyasu] The test is invalid for GFXD because it tries to
      // do a create while a transaction is progress.
      if (isTransactional) {
        return;
      }
            ...
            throws SQLException
    {
        ...
        for (int i = 0; i < mgrs.length; ++i) {
            ...
            JDBC.assertUnorderedResultSet(rs, results[i], false);
            // GemStone changes END
            // Re-execute tst with the same parameters
            rs = tst.executeQuery();
            // GemStone changes BEGIN
    ...
    public void testHashLeftOuterJoinResultSet() throws Exception {
      // [sjigyasu] The test is invalid for GFXD because it tries to
      // do a create/drop while a transaction is progress.
      if (isTransactional) {
        return;
      }
     ...
     */
    //GemFireXD does not support SERIALIZABLE
    //public void testUpdateResultSetWithIsolation() throws SQLException {
    public void _testUpdateResultSetWithIsolation() throws SQLException {
        createTestTable("dept", DS, "dept_data");
        commit();
    ...
    public void testDeleteResultSet() throws Exception {
        ...
        for (int i = 0; i < non_mgrs.length; ++i) {
            ...
            JDBC.assertUnorderedResultSet(rs, emp, false);
            // GemStone changes END
            // Re-execute tst with the same parameters
            tst.execute();
            rs = sel.executeQuery();
   ...
   /**
     ...
     */
    //GemFireXD does not support CASCADE rules for DELETE
    //public void testDeleteCascadeUpdateResultSet() throws Exception {
    public void _testDeleteCascadeUpdateResultSet() throws Exception {
        createTestTable("dept", DS, "dept_data");
        createTestTable("emp",
    ...
    private void testGeneratedIdentity(String dataType, String generatedType)
        ...
        s.execute("CREATE TABLE T(GI "+dataType+" PRIMARY KEY GENERATED "+
                  generatedType+
                  //GemFireXD does not allow START WITH/INCREMENT BY on GENERATED ALWAYS columns
    ...
    //GemFireXD only allows generated columns as INT or BIGINT
    /*
