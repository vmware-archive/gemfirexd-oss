...
public class checkPoint
{
    ...
    public static void main( String args[])
    {
        ...
        try
        {
			...
			stmt.executeUpdate("CALL SYSCS_UTIL.CHECKPOINT_DATABASE()");
			...
			for(int uniqueid =0 ; uniqueid < 3500 ; uniqueid++)
			{
				...
				if(uniqueid > 2500 && !modifiedIntervals)
				{
					...
					//modify the values.
					String value = "150001";
					stmt.executeUpdate("call SYSCS_UTIL.SET_DATABASE_PROPERTY" +
									   "('gemfirexd.storage.logSwitchInterval', " +
									   "'" + value + "'"+ ")");
					stmt.executeUpdate("call SYSCS_UTIL.SET_DATABASE_PROPERTY" +
									   "('gemfirexd.storage.checkpointInterval', " +
									   "'" + value + "'" + ")");
					rs	= 
						stmt.executeQuery("values SYSCS_UTIL.GET_DATABASE_PROPERTY" +
										  "('gemfirexd.storage.checkpointInterval')");
					while(rs.next()){
	...
	{
		derbyHome = System.getProperty("gemfirexd.system.home");
