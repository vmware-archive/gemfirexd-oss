...
//              Begin GemFire Changes
		//Added a SYSDISKSTORE table so the count is 21
		//Also added SYSGATEWAY* tables, so now it is 25
		// Removed old WAN tables, so now it is 23
		// Yogesh added back one WAN table so now it is 24
		// Added SYSHDFSSTORES table so the count is 25
		assertEquals(25, JDBC.assertDrainResults(rs));
