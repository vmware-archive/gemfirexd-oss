...
public class BackupRestoreReEncryptTester extends TesterObject {
	...
	public void doBackup() throws SQLException {
		...
		CallableStatement cs = connex
				.prepareCall("CALL SYSCS_UTIL.BACKUP_DATABASE_AND_ENABLE_LOG_ARCHIVE_MODE_NOWAIT(?, ?)");
	...
	public void doConsistCheck(String dbURL, String dbType) throws SQLException {
		 ...
		 * SYSCS_UTIL.CHECK_TABLE(schemaname, tablename) FROM
		...
		long start = System.currentTimeMillis();
		ResultSet rs = stmt
				.executeQuery("SELECT schemaname, tablename,	SYSCS_UTIL.CHECK_TABLE(schemaname, tablename)	FROM sys.sysschemas s, sys.systables t	WHERE s.schemaid = t.schemaid");
		while (rs.next()) {
