...
public class PrepareExecuteDDL extends BaseJDBCTestCase {
  ...
  {
    "SELECT * FROM APP.PED001",
    "SELECT A, B FROM APP.PED001",
    "GRANT SELECT ON APP.PED001 TO U_PED_001",
    "GRANT SELECT(A,B) ON APP.PED001 TO U_PED_001",
    "REVOKE SELECT(A,B) ON APP.PED001 FROM U_PED_001",
    "REVOKE SELECT ON APP.PED001 FROM U_PED_001",
  };
  ...
  {
    "ALTER TABLE APP.PED001 ADD COLUMN D BIGINT",
    "ALTER TABLE APP.PED001 ADD CONSTRAINT PED001_PK PRIMARY KEY (A)",
    //GemFireXD does not support ALTER ... LOCKSIZE
    //"ALTER TABLE PED001 LOCKSIZE ROW",
    //"ALTER TABLE PED001 LOCKSIZE TABLE",
    "DROP TABLE APP.PED001",
  };
  ...
  {
    setSystemProperty("gemfirexd.sql-authorization", "true");
    connDDL = openDefaultConnection();
    Statement s = connDDL.createStatement();

    s.execute(
    "CREATE TABLE APP.PED001 (A INT NOT NULL, B DECIMAL(6,4), C VARCHAR(20))");
...
// GemStone changes END
    Statement s = connDDL.createStatement();
    try {
      s.execute("DROP TABLE APP.PED001");
    } catch (SQLException e) {
