...
call SYSCS_UTIL.COMPRESS_TABLE('APP', 'T1', 1);
call SYSCS_UTIL.COMPRESS_TABLE('APP', 'T2', 1);
call SYSCS_UTIL.COMPRESS_TABLE('APP', 'T3', 1);
call SYSCS_UTIL.COMPRESS_TABLE('APP', 'T4', 1);
...
create view vz5a (z1, z2, z3, z4) as
  select distinct xx1.c1, xx1.c2, 'bokibob' bb, xx1.c from
    (select c1, c2, c3, c
      from --GEMFIREXD-PROPERTIES joinOrder=FIXED
        t2, tc
      where tc.c = t2.i) xx1
  union select 'i','j','j',i from t2;
...
select t1.i, vz5a.* from
  t1
   left outer join
     vz5a --GEMFIREXD-PROPERTIES joinStrategy=NESTEDLOOP
   on
    t1.i = vz5a.z4;
...
select t1.i, vz5b.* from
  t1
   left outer join
     vz5b --GEMFIREXD-PROPERTIES joinStrategy=NESTEDLOOP
   on
    t1.i = vz5b.z4;
...
select t1.i, vz5c.* from
  t1
   left outer join
     vz5c --GEMFIREXD-PROPERTIES joinStrategy=NESTEDLOOP
   on
    t1.i = vz5c.z4;
...
select t1.i, vz5d.* from
  t1
   left outer join
     vz5d --GEMFIREXD-PROPERTIES joinStrategy=NESTEDLOOP
   on
    t1.i = vz5d.z4;
...
select x1.z4, x2.c2 from
  ...
  ) x2 (c1, c2) --GEMFIREXD-PROPERTIES joinStrategy=NESTEDLOOP  
on x1.z4 = x2.c2;
...
select x1.z4, x2.c2 from
  ...
  (select z1, z4, z3 from vz
    union select '1', i+1, '3' from t1
  ) x1 (z1, z4, z3) --GEMFIREXD-PROPERTIES joinStrategy=NESTEDLOOP 
on x1.z4 = x2.c2;
...
call SYSCS_UTIL.COMPRESS_TABLE('APP', 'T3', 1);
call SYSCS_UTIL.COMPRESS_TABLE('APP', 'T4', 1);
call SYSCS_UTIL.COMPRESS_TABLE('APP', 'T5', 1);
call SYSCS_UTIL.COMPRESS_TABLE('APP', 'T6', 1);
...
select * from t4
  left join topview --GEMFIREXD-PROPERTIES joinStrategy=NESTEDLOOP
  on t4.c4 = topview.col3;
...
select * from t4
  left join topview2 --GEMFIREXD-PROPERTIES joinStrategy=NESTEDLOOP
  on t4.c4 = topview2.col3;
...
select * from topview x1
  left join topview3 --GEMFIREXD-PROPERTIES joinStrategy=NESTEDLOOP
  on topview3.col3 = x1.col3;
...
select * from topview x1
  left join topview4 --GEMFIREXD-PROPERTIES joinStrategy=NESTEDLOOP
  on topview4.col3 = x1.col3;
...
select * from topview x1
  left join topview5 --GEMFIREXD-PROPERTIES joinStrategy=NESTEDLOOP
  on topview5.col3 = x1.col3;
