...
public class CoalesceTest extends BaseJDBCTestCase
{
    ...
    private static String[] TABLES = {
        ...
        // GemStone changes BEGIN  added rnum int (for sorting)
        "create table tF (rnum int, dateCol date, charCol char(10), varcharCol varchar(50))",
        ...
        "create table tE (smallintCol smallint, intCol integer, bigintCol bigint, decimalCol1 decimal(22,2), decimalCol2 decimal(8,6), decimalCol3 decimal(41,28), realCol real, doubleCol double)",
        "create table tD (c1 int, c2 char(254))",
        // GemStone changes BEGIN  added rnum int (for sorting)
        "create table tB (rnum int, c1 char(254), c2 char(40), vc1 varchar(253), vc2 varchar(2000), lvc1 long varchar, lvc2 long varchar, clob1 CLOB(200), clob2 CLOB(33K))",
        "create table tC (cbd1 char(254) for bit data, cbd2 char(40) for bit data, vcbd1 varchar(253) for bit data, vcbd2 varchar(2000) for bit data, lvcbd1 long varchar for bit data, lvcbd2 long varchar for bit data, blob1 BLOB(200), blob2 BLOB(33K), rnum int)",
        // GemStone changes END
    ...
    public static Test suite() {
        ...
        return new CleanDatabaseTestSetup(suite)
        {
            protected void decorateSQL(Statement stmt) throws SQLException
            {
                ...
                // GemStone changes BEGIN   (need a column for result ordering)
                //StringBuilder createSQL = new StringBuilder("create table AllDataTypesTable (");
                StringBuilder createSQL = new StringBuilder("create table AllDataTypesTable ( RNUM int, ");
                // GemStone changes END
                ...
                for (int row = 0; row < SQLData[0].length; row++)
                {
                    ...
                    createSQL = new StringBuilder("insert into AllDataTypesTable values("+row+", ");
                    // GemStone changes END
                    for (int type = 0; type < SQLTypes.length - 1; type++)
                    {
    ...
    public void testCoalesceSyntax() throws Throwable
    {
        ...
        //   Row order unpredictable in GemFireXD
        Integer expected[][]={{null},{1}};
        JDBC.assertUnorderedResultSet(s.executeQuery("select coalesce(coalesce,c12) from coalesce"), expected, false);
        ...
        // GemStone changes BEGIN
        //   Row order unpredictable in GemFireXD
        Integer expected1[][]={{null},{1}};
        JDBC.assertUnorderedResultSet(s.executeQuery("select coalesce(value,c12) from value"), expected, false);
    ...
    public void testCompatibleDatatypesCombinations() throws Throwable
    {
        ...
        for (int firstColumnType = 0; firstColumnType < SQLTypes.length; firstColumnType++) {
            StringBuilder coalesceString = new StringBuilder("SELECT COALESCE(" + ColumnNames[firstColumnType]);
            for (int secondColumnType = 0; secondColumnType < SQLTypes.length; secondColumnType++) {
                try {
                    ...
                    // GemStone changes BEGIN: added rownumber ordering order by RNUM
                    dumpRS(s.executeQuery(coalesceString + ") from AllDataTypesTable order by RNUM"),expectedValues[index]);
                    // GemStone changes END
                } catch (SQLException sqle) {
    ...
    public void testAllDatatypesCombinations() throws  Throwable
    {
        ...
        for (int firstColumnType = 0; firstColumnType < SQLTypes.length; firstColumnType++ ) {
            for (int secondColumnType = 0; secondColumnType < SQLTypes.length; secondColumnType++) {
                try {
                    // GemStone changes BEGIN   : added order by rnum
                    String coalesceString = "SELECT COALESCE(" + ColumnNames[firstColumnType] + "," + ColumnNames[secondColumnType] + ") from AllDataTypesTable order by rnum";
                    // GemStone changes END
                ...
                try {
                    // GemStone changes BEGIN:  added order by rnum
                    String valueString = "SELECT VALUE(" + ColumnNames[firstColumnType] + "," + ColumnNames[secondColumnType] + ") from AllDataTypesTable order by rnum";
                    // GemStone changes END
    ...
    {
        // GemStone changes BEGIN: added row number column
        s.executeUpdate("insert into tF values(1,null, null, null)");
        s.executeUpdate("insert into tF values(2,date('1992-01-02'), '1992-01-03', '1992-01-04')");
        // GemStone changes END
        ...
        dumpRS(s.executeQuery("select coalesce(dateCol,dateCol) from tF order by rnum"), expectedValue);
        dumpRS(s.executeQuery("select value(dateCol,dateCol) from tF order by rnum"), expectedValue);

        dumpRS(s.executeQuery("select coalesce(dateCol,charCol) from tF order by rnum"), expectedValue);
        dumpRS(s.executeQuery("select value(dateCol,charCol) from tF order by rnum"), expectedValue);

        dumpRS(s.executeQuery("select coalesce(charCol,dateCol) from tF order by rnum"), expectedValue1);
        dumpRS(s.executeQuery("select value(charCol,dateCol) from tF order by rnum"), expectedValue1);

        dumpRS(s.executeQuery("select coalesce(dateCol,varcharCol) from tF order by rnum"), expectedValue);
        dumpRS(s.executeQuery("select value(dateCol,varcharCol) from tF order by rnum"), expectedValue);

        dumpRS(s.executeQuery("select coalesce(varcharCol,dateCol) from tF order by rnum"), expectedValue2);
        dumpRS(s.executeQuery("select value(varcharCol,dateCol) from tF order by rnum"), expectedValue2);

        // GemStone changes BEGIN: added row number column
        s.executeUpdate("insert into tF values(3,date('1992-01-01'), 'I am char', 'I am varchar')");
        // GemStone changes END
        try {
            dumpRS(s.executeQuery("select coalesce(charCol,dateCol) from tF order by rnum"), "");
        } catch (SQLException sqle) {
            assertSQLState("22007", sqle);
        }

        try {
            dumpRS(s.executeQuery("select value(charCol,dateCol) from tF order by rnum"), "");
        } catch (SQLException sqle) {
            assertSQLState("22007", sqle);
        }

        try {
            dumpRS(s.executeQuery("select coalesce(varcharCol,dateCol) from tF order by rnum"), "");
        } catch (SQLException sqle) {
            assertSQLState("22007", sqle);
        }

        try {
            dumpRS(s.executeQuery("select value(varcharCol,dateCol) from tF order by rnum"), "");
        } catch (SQLException sqle) {
            assertSQLState("22007", sqle);
        }
    ...
    public void testNumericCoalesce() throws Throwable
    {
        ...
        String[] expectedValues = {
                ...
                "COL1(datatype : DECIMAL, precision : 41, scale : 28) 1.0000000000000000000000000000 null ",
                ...
                "COL1(datatype : DECIMAL, precision : 41, scale : 28) 2.0000000000000000000000000000 null ",
                ...
                "COL1(datatype : DECIMAL, precision : 47, scale : 28) 3.0000000000000000000000000000 null ",
                ...
                "COL1(datatype : DECIMAL, precision : 48, scale : 28) 4.0000000000000000000000000000 null ",
                "COL1(datatype : DECIMAL, precision : 22, scale : 2) 4.00 null ",
        ...
        dumpRS(s.executeQuery("select coalesce(smallintCol,smallintCol) from tE order by smallintCol"), expectedValues[0]);
        dumpRS(s.executeQuery("select coalesce(smallintCol,intCol) from tE order by smallintCol"), expectedValues[1]);
        dumpRS(s.executeQuery("select coalesce(smallintCol,bigintCol) from tE order by smallintCol"), expectedValues[2]);
        dumpRS(s.executeQuery("select coalesce(smallintCol,decimalCol1) from tE order by smallintCol"), expectedValues[3]);
        dumpRS(s.executeQuery("select coalesce(smallintCol,decimalCol2) from tE order by smallintCol"), expectedValues[4]);
        dumpRS(s.executeQuery("select coalesce(smallintCol,decimalCol3) from tE order by smallintCol"), expectedValues[5]);
        dumpRS(s.executeQuery("select coalesce(smallintCol,realCol) from tE order by smallintCol"), expectedValues[6]);
        dumpRS(s.executeQuery("select coalesce(smallintCol,doubleCol) from tE order by smallintCol"), expectedValues[6]);
        dumpRS(s.executeQuery("select coalesce(intCol,intCol) from tE order by intCol"), expectedValues[7]);
        dumpRS(s.executeQuery("select coalesce(intCol,smallintCol) from tE order by intCol"), expectedValues[7]);
        dumpRS(s.executeQuery("select coalesce(intCol,bigintCol) from tE order by intCol"), expectedValues[8]);

        dumpRS(s.executeQuery("select coalesce(intCol,decimalCol1) from tE order by intCol"), expectedValues[9]);
        dumpRS(s.executeQuery("select coalesce(intCol,decimalCol2) from tE order by intCol"), expectedValues[10]);
        dumpRS(s.executeQuery("select coalesce(intCol,decimalCol3) from tE order by intCol"), expectedValues[11]);
        dumpRS(s.executeQuery("select coalesce(intCol,realCol) from tE order by intCol"), expectedValues[12]);
        dumpRS(s.executeQuery("select coalesce(intCol,doubleCol) from tE order by intCol"), expectedValues[12]);
        dumpRS(s.executeQuery("select coalesce(bigintCol,bigintCol) from tE order by bigintCol"), expectedValues[13]);
        dumpRS(s.executeQuery("select coalesce(bigintCol,smallintCol) from tE order by bigintCol"), expectedValues[13]);
        dumpRS(s.executeQuery("select coalesce(bigintCol,intCol) from tE order by bigintCol"), expectedValues[13]);
        dumpRS(s.executeQuery("select coalesce(bigintCol,decimalCol1) from tE order by bigintCol"), expectedValues[14]);

        dumpRS(s.executeQuery("select coalesce(bigintCol,decimalCol2) from tE order by bigintCol"), expectedValues[15]);
        dumpRS(s.executeQuery("select coalesce(bigintCol,decimalCol3) from tE order by bigintCol"), expectedValues[16]);

        dumpRS(s.executeQuery("select coalesce(bigintCol,realCol) from tE order by bigintCol"), expectedValues[17]);
        dumpRS(s.executeQuery("select coalesce(bigintCol,doubleCol) from tE order by bigintCol"), expectedValues[17]);
        dumpRS(s.executeQuery("select coalesce(decimalCol1,decimalCol1) from tE order by decimalCol1"), expectedValues[18]);
        dumpRS(s.executeQuery("select coalesce(decimalCol1,decimalCol2) from tE order by decimalCol1"), expectedValues[19]);
        dumpRS(s.executeQuery("select coalesce(decimalCol1,decimalCol3) from tE order by decimalCol1"), expectedValues[20]);

        dumpRS(s.executeQuery("select coalesce(decimalCol1,smallintCol) from tE order by decimalCol1"), expectedValues[21]);
        dumpRS(s.executeQuery("select coalesce(decimalCol1,intCol) from tE order by decimalCol1"), expectedValues[21]);
        dumpRS(s.executeQuery("select coalesce(decimalCol1,bigintCol) from tE order by decimalCol1"), expectedValues[21]);

        dumpRS(s.executeQuery("select coalesce(decimalCol1,realCol) from tE order by decimalCol1"), expectedValues[22]);
        dumpRS(s.executeQuery("select coalesce(decimalCol1,doubleCol) from tE order by decimalCol1"), expectedValues[22]);

        dumpRS(s.executeQuery("select coalesce(realCol,realCol) from tE order by realCol"), expectedValues[23]);

        dumpRS(s.executeQuery("select coalesce(realCol,smallintCol) from tE order by realCol"), expectedValues[24]);
        dumpRS(s.executeQuery("select coalesce(realCol,intCol) from tE order by realCol"), expectedValues[24]);
        dumpRS(s.executeQuery("select coalesce(realCol,bigintCol) from tE order by realCol"), expectedValues[24]);
        dumpRS(s.executeQuery("select coalesce(realCol,decimalCol1) from tE order by realCol"), expectedValues[24]);
        dumpRS(s.executeQuery("select coalesce(realCol,doubleCol) from tE order by realCol"), expectedValues[24]);

        dumpRS(s.executeQuery("select coalesce(doubleCol,doubleCol) from tE order by doubleCol"), expectedValues[25]);
        dumpRS(s.executeQuery("select coalesce(doubleCol,smallintCol) from tE order by doubleCol"), expectedValues[25]);
        dumpRS(s.executeQuery("select coalesce(doubleCol,intCol) from tE order by doubleCol"), expectedValues[25]);
        dumpRS(s.executeQuery("select coalesce(doubleCol,bigintCol) from tE order by doubleCol"), expectedValues[25]);
        dumpRS(s.executeQuery("select coalesce(doubleCol,decimalCol1) from tE order by doubleCol"), expectedValues[25]);
        dumpRS(s.executeQuery("select coalesce(doubleCol,realCol) from tE order by doubleCol"), expectedValues[25]);
    ...
    public void testMiscellaneousCoalesce() throws SQLException
    {
        ...
        ps = prepareStatement("select coalesce(?,c2) from tD order by c1");
        ps.setNull(1,Types.CHAR);
        dumpRS(ps.executeQuery(), expectedValues[index++]);

        ps = prepareStatement("select value(?,c2) from tD order by c1");
        ps.setNull(1,Types.BIGINT);
        dumpRS(ps.executeQuery(), expectedValues[index++]);

        ps = prepareStatement("select coalesce(c1,?) from tD order by c1");
        try {
            ps.setString(1,"abc");
    ...
    public void testCharCoalesce() throws Throwable
    {
        ...
        // GemStone changes BEGIN  : add row number column to insert
        s.executeUpdate("insert into tB values(1,'c1 not null', 'c2 not null', 'vc1 not null', 'vc2 not null', 'lvc1 not null', 'lvc2 not null', 'clob1 not null', 'clob2 not null')");
        s.executeUpdate("insert into tB values(2,'c1 not null but c2 is', null, 'vc1 is not null but vc2 is', null, null, null,null,null)");
        s.executeUpdate("insert into tB values(3,null,'c2 not null but c1 is', null, 'vc2 is not null but vc1 is', 'lvc1 not null again', 'lvc2 not null again', 'clob1 not null again', 'clob2 not null again')");
        s.executeUpdate("insert into tB values(4,null,null, null, null, null, null, null, null)");
        // GemStone changes END

        int index = 0;
        // GemStone changes BEGIN  : added order by rnum to queries
        dumpRS(s.executeQuery("select coalesce(c1,c2) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(c1,c2) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(c2,c1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(c2,c1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(c1,vc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(c1,vc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(vc1,c1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(vc1,c1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(vc1,vc2) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(vc1,vc2) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(vc2,vc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(vc2,vc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(c1,lvc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(c1,lvc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(lvc1,c1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(lvc1,c1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(vc1,lvc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(vc1,lvc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(lvc1,vc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(lvc1,vc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(lvc1,lvc2) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(lvc1,lvc2) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(clob1,c1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(clob1,c1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(c1,clob2) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(c1,clob2) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(clob1,vc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(clob1,vc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(vc2,clob2) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(vc2,clob2) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(clob1,lvc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(clob1,lvc1) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(lvc2,clob2) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(lvc2,clob2) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(clob1,clob2) from tB order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(clob1,clob2) from tB order by rnum"), expectedValues[index++]);
        // GemStone changes END
    }
    ...
    public void testCharForBitDataCoalesce() throws SQLException, UnsupportedEncodingException
    {
        ...
        // GemStone changes BEGIN  : added 9th column for row number to guarantee sort order
        ...
        int index = 0;
        // GemStone changes BEGIN
        //   Through remainder of method, added order by rnum to all queries
        dumpRS(s.executeQuery("select coalesce(cbd1,cbd2) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(cbd1,cbd2) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(cbd2,cbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(cbd2,cbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(cbd1,vcbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(cbd1,vcbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(vcbd1,cbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(vcbd1,cbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(vcbd1,vcbd2) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(vcbd1,vcbd2) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(vcbd2,vcbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(vcbd2,vcbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(cbd1,lvcbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(cbd1,lvcbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(lvcbd1,cbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(lvcbd1,cbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(vcbd1,lvcbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(vcbd1,lvcbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(lvcbd1,vcbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(lvcbd1,vcbd1) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select coalesce(lvcbd1,lvcbd2) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(lvcbd1,lvcbd2) from tC order by rnum"), expectedValues[index++]);

        try {
            dumpRS(s.executeQuery("select coalesce(blob1,cbd1) from tC order by rnum"), expectedValues[index]);
        } catch (SQLException sqle) {
            assertSQLState("42815", sqle);
        }
        index++;
        ...
        try {
            dumpRS(s.executeQuery("select value(blob1,cbd1) from tC order by rnum"), expectedValues[index]);
        } catch (SQLException sqle) {
            assertSQLState("42815", sqle);
        }
        index++;
        ...
        try {
            dumpRS(s.executeQuery("select coalesce(cbd1,blob2) from tC order by rnum"), expectedValues[index]);
        } catch (SQLException sqle) {
            assertSQLState("42815", sqle);
        }
        index++;
        ...
        try {
            dumpRS(s.executeQuery("select value(cbd1,blob2) from tC order by rnum"), expectedValues[index]);
        } catch (SQLException sqle) {
            assertSQLState("42815", sqle);
        }
        index++;
        ...
        try {
            dumpRS(s.executeQuery("select coalesce(blob1,vcbd1) from tC order by rnum"), expectedValues[index]);
        } catch (SQLException sqle) {
            assertSQLState("42815", sqle);
        }
        index++;
        ...
        try {
            dumpRS(s.executeQuery("select value(blob1,vcbd1) from tC order by rnum"), expectedValues[index]);
        } catch (SQLException sqle) {
            assertSQLState("42815", sqle);
        }
        index++;
        ...
        try {
            dumpRS(s.executeQuery("select coalesce(vcbd2,blob2) from tC order by rnum"), expectedValues[index]);
        } catch (SQLException sqle) {
            assertSQLState("42815", sqle);
        }
        index++;
        ...
        try {
            dumpRS(s.executeQuery("select value(vcbd2,blob2) from tC order by rnum"), expectedValues[index]);
        } catch (SQLException sqle) {
            assertSQLState("42815", sqle);
        }
        index++;
        ...
        try {
            dumpRS(s.executeQuery("select coalesce(blob1,lvcbd1) from tC order by rnum"), expectedValues[index]);
        } catch (SQLException sqle) {
            assertSQLState("42815", sqle);
        }
        index++;
        ...
        try {
            dumpRS(s.executeQuery("select value(blob1,lvcbd1) from tC order by rnum"), expectedValues[index]);
        } catch (SQLException sqle) {
            assertSQLState("42815", sqle);
        }
        index++;
        ...
        try {
            dumpRS(s.executeQuery("select coalesce(lvcbd2,blob2) from tC order by rnum"), expectedValues[index]);
        } catch (SQLException sqle) {
            assertSQLState("42815", sqle);
        }
        index++;
        ...
        try {
            dumpRS(s.executeQuery("select value(lvcbd2,blob2) from tC order by rnum"), expectedValues[index]);
        } catch (SQLException sqle) {
            assertSQLState("42815", sqle);
        }
        index++;

        dumpRS(s.executeQuery("select coalesce(blob1,blob2) from tC order by rnum"), expectedValues[index++]);
        dumpRS(s.executeQuery("select value(blob1,blob2) from tC order by rnum"), expectedValues[index++]);
        // GemStone changes END
    }
    ...
    //GemStone changes BEGIN
    // method for comparing result set without assuming any implicit ordering
    // This method is kinda hideous.  If the original test had been written
    // with expected data expressed as a type+an array of expected values, this
    ...
    private void dumpRS(ResultSet rs, String expectedValue) throws SQLException
    {
        ...
        // Extract the data type information from the passed expectedValue string
        // minor hack assuming the expected data values do not contain a ) character
        StringBuilder unmatchedValues = new StringBuilder();
        if (expectedValue.indexOf(")") > 0) {
          String expectedTypeString = expectedValue.substring(0, expectedValue.lastIndexOf(")")+1);
          assertEquals("Data type mismatch", heading.toString(), expectedTypeString);
          String expectedDataValues = expectedValue.substring(expectedTypeString.length()+1);
          unmatchedValues = new StringBuilder(expectedDataValues);
          // Now confirm the data rows are equal, without ordering.  Because
          // there's no reliable way to break apart the ugly data values passed
          // in "expectedRows", we're stuck with a crude validation methodology
          // of removing string portions.
          while (rs.next())
          {
              ...
              int foundSpot=unmatchedValues.indexOf(row.toString());
              if (foundSpot < 0) {
                fail ("Result data includes unexpected data >"+row+"< expecting >"+expectedDataValues+"<");
              }
              // We found the result set row within the expected results;
              // now remove it
              unmatchedValues.replace(foundSpot, foundSpot+row.length(), "");
          }
        }
        if (unmatchedValues.length() > 0) {
          fail("Did not find "+unmatchedValues+" in result set");
        }
        rs.close();
