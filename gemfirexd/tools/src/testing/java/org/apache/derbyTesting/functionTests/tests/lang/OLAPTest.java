...
public class OLAPTest extends BaseJDBCTestCase {
  ...
  public void testBasicOperations()
    ...
    //  Throughout this method, added order by clauses to get predictable result set ordering
    ...
    //FIXME 
    /*
    //GemFireXD returns rows in unpredictable orders, so row numbers will not necessarily correlate to
      //  the rows as expressed in all the "expectedRows" arrays.  We can't compensate for this with
      //  an order by clause in testcases that do subqueries.
      // At least run the above tests so some coverage exists for row_number() OLAP function
      
      // This query returned no rows at one time
      rs = s.executeQuery("select * from (select row_number() over () as r,x from t2,t3 where x=y) s(r,x) where r < 3 order by x");
      expectedRows = new String[][]{{"1", "4"}, {"2", "5"}};
      JDBC.assertUnorderedResultSet(rs, expectedRows);


      rs = s.executeQuery("select * from (select row_number() over () as r, t1.* from t1 order by a) as tr where r < 3");
      expectedRows = new String[][]{{"1", "10", "100"}, {"2", "20", "200"}};
      JDBC.assertUnorderedResultSet(rs, expectedRows);

      rs = s.executeQuery("select * from (select row_number() over () as r, t1.* from t1 order by t1.a) as tr where r > 3");
      expectedRows = new String[][]{{"4", "40", "400"}, {"5", "50", "500"}};
      JDBC.assertUnorderedResultSet(rs, expectedRows);

      // Two instances of row_number columns 
      rs = s.executeQuery("select row_number() over(), tr.* from (select row_number() over () as r, t1.* from t1) as tr where r > 2 and r < 5 order by a");
      expectedRows = new String[][]{{"1", "3", "30", "300"}, {"2", "4", "40", "400"}};
      JDBC.assertUnorderedResultSet(rs, expectedRows);

      // Two instances of row_number columns, with projection 
      rs = s.executeQuery("select row_number() over(), tr.b from (select row_number() over () as r, t1.* from t1) as tr where r > 2 and r < 5 order by a");
