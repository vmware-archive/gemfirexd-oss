...
public class SysinfoCPCheckTest extends BaseJDBCTestCase {
    ...
    public static Test suite() {
        ...
        // check to see if we have gemfirexd.jar or gemfirexd-client.jar
    ...
    public void testClassPathChecker() throws IOException {
        ...
        final String[][] tstargs = {
                ...
                {"a", "0", "USAGE: java com.pivotal.gemfirexd.internal.tools.sysinfo -cp [" +
                    " [ embedded ][ server ][ client] [ db2driver ] [ tools ]" +
                    " [  anyClass.class ] ]", null},
                {"embedded", "6", Success, "gemfirexd.jar"}, 
                {"server", "10", Success, "gemfirexd.jar"},
                {"tools", "6", Success, "gemfirexd-tools.jar"},
                {"client", "6", Success, "gemfirexd-client.jar"},
        ...
        for (tst=0; tst<tstargs.length ; tst++)
        {
            ...
            if (tstargs[tst][0] == null)
                com.pivotal.gemfirexd.internal.tools.sysinfo.main(new String[] {"-cp"} );
            else
                com.pivotal.gemfirexd.internal.tools.sysinfo.main(
    ...
    public boolean checkClientOrServer(String kind)
    {
        if (kind == null)
            return true;
        // if there is no gemfirexd.jar, the syntax should still
