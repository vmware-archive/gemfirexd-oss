...
// GemStone changes BEGIN
		...
		if ((rowloc=cc.fetch(rowloc, r2.getRowArray(), (FormatableBitSet) null, false))==null)
        {
...
// GemStone changes END
		...
		c3row[0] = c3;
// GemStone changes BEGIN
        if ((rowloc=cc.fetch(rowloc, c3row, singleColumn, false))==null)
        {
			throw T_Fail.testFailMsg("(insertAndFetch) fetch found no row.");
        }
// GemStone changes END
...
// GemStone changes END
		...
		throws StandardException, T_Fail
	{
		...
		scan1.reopenScan(
			...
			null, // qualifier - accept all rows
			null, // stop position - last row in conglomerate
            0, null);   // unused if stop position is null.
...
// GemStone changes BEGIN
                if ((rowloc1=cc.fetch(
                        rowloc1, two_column_row.getRowArray(), (FormatableBitSet) null, false)) == null)
                {
                    throw T_Fail.testFailMsg(
...
// GemStone changes BEGIN
                if ((rowloc1=cc.fetch(
                        rowloc1, third_column_row, fetch_desc, false)) == null)
                {
                    throw T_Fail.testFailMsg(
                        "(alterTable) Allowed fetch of bad row, bad ret val.");
                }
...
// GemStone changes END
        ...
        if ((rowloc2=cc.fetch(
                rowloc2, two_column_row.getRowArray(), (FormatableBitSet) null, false)) == null)
        {
			throw T_Fail.testFailMsg(
                "(alterTable) Row not there.");
        }
	...
	**/
		...
		throws StandardException, T_Fail
	{
		...
		try {
		cc2.fetch(rowloc, r1.getRowArray(), (FormatableBitSet)null, false);
...
// GemStone changes BEGIN
        ...
        if ((deleted_page_rowloc=cc.fetch(deleted_page_rowloc, big_row.getRowArray(), null, false)) == null)
        {
            throw T_Fail.testFailMsg(
...
// GemStone changes BEGIN
        ...
        if ((deleted_page_rowloc=cc.fetch(deleted_page_rowloc, big_row.getRowArray(), null, true, false))==null)
        {
            throw T_Fail.testFailMsg(
                "(readUncommitted) fetch should ret false for reclaimed page.");
        }
...
// GemStone changes BEGIN
        ...
        if ((deleted_page_rowloc=cc.fetch(deleted_row_rowloc, big_row.getRowArray(), null, false))==null)
        {
            throw T_Fail.testFailMsg(
