...
public class BlobClob4BlobTest extends BaseJDBCTestCase {
    ...
    private static int addPatternPrefix(
            StringBuilder sb, char[] pattern, int pl, int fakeCount, int pos) {
    ...
    public void testLockingClob() throws Exception {
        if (isAvoidGFXDBugs()) {
          fail("Fast-fail test since lock timeouts are not supported.");
        }
    ...
    public void testVarbinary() throws Exception{
        ...
        HashSet expectedBlobSizes = new HashSet();
        ...
        for (int i = 0; i < 10; i++) {
            ...
            ps.executeUpdate();
            // GemStone changes BEGIN
            expectedBlobSizes.add(new Integer(i));
            // GemStone changes END
            val = val.trim() + "x";
        }
        ...
        while (rs.next()) {
            ...
            assertEquals("FAIL - mismatch columnSize="+columnSize+" vs. blob length="+blob.length(),columnSize, blob.length());
            assertTrue("FAIL - unexpected blob size="+columnSize, expectedBlobSizes.remove(new Integer(columnSize)));
            // GemStone changes END
            j++;
        }
        ps.close();
        ...
        commit();
        // GemStone changes BEGIN
        assertTrue("FAIL - blobs not found in result set",(expectedBlobSizes.size()==0));
        // GemStone changes END
    }
    ...
    public void testSelfDestructiveClob2() throws Exception {
        if (isAvoidGFXDBugs()) {
          fail("Fast-fail test since rollback of DDL operations is not supported.");
        }
        insertDefaultData();
        Statement stmt = createStatement();
        ResultSet rs = stmt.executeQuery(
