...
public final class SysDiagVTIMappingTest extends BaseJDBCTestCase {
    ...
    public static Test suite()
    {
        ...
        /* Some of the VTIs that are tested in this class require a gemfirexd.log
...
//GemStone changes END
    ...
    public void testSpaceTable() throws Exception
    {
        ...
        // GemFireXD returns all zeroes for sizes in SPACE_TABLE
        st.executeUpdate("set schema APP");
        ...
        st.executeUpdate("set schema APP");
        rs = st.executeQuery(
            "SELECT * FROM NEW com.pivotal.gemfirexd.internal.diag.SpaceTable('T1') as x");
        ...
        rs = st.executeQuery(
            "SELECT * FROM NEW "
            + "com.pivotal.gemfirexd.internal.diag.SpaceTable('APP', 'T1') as x");
     ...
     * mapping for com.pivotal.gemfirexd.internal.diag.StatementDuration() works
    ...
    public void testStatementDuration() throws Exception
    {
        ...
        rs = st.executeQuery(
            "SELECT * FROM NEW "
            + "com.pivotal.gemfirexd.internal.diag.StatementDuration" + vtiArg + " as x");
    ...
    /**
     * Just run a couple of sanity checks to makes sure the table
     * mapping for com.pivotal.gemfirexd.internal.diag.ErrorLogReader() works
     * correctly and fails where it is supposed to.
     */
    public void testErrorLogReader() throws Exception
    {
        ...
        rs = st.executeQuery(
            "SELECT * FROM NEW" +
            " com.pivotal.gemfirexd.internal.diag.ErrorLogReader" + vtiArg + " as x");
     ...
     */
    public void _testCorrelatedReferences() throws SQLException
    {
        Statement   st = createStatement();
        String      [][] expRS = new String [][] {};
        ...
        String vtiMethodName, String args, String colName) throws SQLException
    {
        ...
        CallableStatement cSt = prepareCall(
            "call SYSCS_UTIL.COMPRESS_TABLE(?, ?, 1)");
        ...
        cSt.setString(1, "SYSCS_DIAG");
        cSt.setString(2, vtiTableName.toUpperCase());
        //GemFireXD does not support inplace compress - expect 0A000
        //assertStatementError("42X05", cSt);
        assertStatementError("0A000", cSt);

        //FIXME
        // These statements crash in getRegion() because the region
        // was not initialized during DML
        // Commenting out as it is such an undocumented way to use GemFireXD
        //assertStatementError("42X08", st,
        //    "update new com.pivotal.gemfirexd.internal.diag." + vtiMethodName + args
        //    + " set " + colName + " = NULL");
        
        //assertStatementError("42X08", st,
        //    "delete from new com.pivotal.gemfirexd.internal.diag." + vtiMethodName + args
        //    + " where 1 = 0");
