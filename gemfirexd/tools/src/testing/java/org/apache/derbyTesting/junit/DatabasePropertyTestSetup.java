...
public class DatabasePropertyTestSetup extends BaseJDBCTestSetup {
     ...
     * @param deadlockTime Time in seconds for gemfirexd.locks.deadlockTimeout.
     * @param waitTime Time in seconds for gemfirexd.locks.waitTimeout
     */
    public static Test setLockTimeouts(Test test, int deadlockTime, int waitTime)
    {
        ...
        if (deadlockTime >= 0)
        {
            properties.setProperty("gemfirexd.locks.deadlockTimeout",
                Integer.toString(deadlockTime));
        }
        if (waitTime >= 0) {
            properties.setProperty("gemfirexd.locks.waitTimeout",
     ...
     * since that is the defined behaviour for gemfirexd.user.username.
            ...
            String passwordToken)
    {
        ...
        authProps.setProperty("gemfirexd.authentication.required", "true");
        authProps.setProperty("gemfirexd.authentication.provider", "BUILTIN");
        
        for (int i = 0; i < users.length; i++)
        {
            String user = users[i];
            userProps.setProperty("gemfirexd.user." + user,
