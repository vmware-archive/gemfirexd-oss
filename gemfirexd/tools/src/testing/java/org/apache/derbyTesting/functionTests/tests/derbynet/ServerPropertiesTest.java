...
/** 
 * This test tests the gemfirexd.properties, system properties and command line
...
public class ServerPropertiesTest  extends BaseJDBCTestCase {
    ...
    public static Test suite()
    {
        ...
        String[] badServerProperties = {
                ...
                "gemfirexd.drda.debug="
                };
        // fixture hits error DRDA_MissingNetworkJar (Cannot find gemfirexd.jar) so,
        // only run with jars
   ...
   private void  assertSuccessfulCmd(String expectedString, String[] Cmd) throws InterruptedException, IOException {
    ...
    throws SQLException, InterruptedException, IOException {
        ...
        // set gemfirexd.drda.portNumber to an alternate number in gemfirexd.properties
        int firstAlternatePort = getAlternativePort();
        final Properties derbyProperties = new Properties();
        derbyProperties.put("gemfirexd.drda.portNumber", 
                new Integer(firstAlternatePort).toString());

        final String derbyHome = getSystemProperty("gemfirexd.system.home");
            ...
            public Object run(){
                ...
                try {
                    FileOutputStream propFile = 
                        new FileOutputStream(derbyHome + File.separator + "gemfirexd.properties");
                    derbyProperties.store(propFile,"testing gemfirexd.properties");
                    propFile.close();
        ...
        if (b.booleanValue())
        {
            checkWhetherNeedToShutdown(new int[] {1527}, "failed to write gemfirexd.properties");
        }
        // have to shutdown engine to force read of gemfirexd.properties
        ...
        setSystemProperty("gemfirexd.drda.portNumber", 
        ...
        // we get rid of gemfirexd.properties...
        ...
        String[] commandArray = {"-Dgemfirexd.system.home=" + derbyHome,
            "com.pivotal.gemfirexd.internal.drda.NetworkServerControl", "-p",
        ...
        // shutdown with -p
        commandArray = new String[] {"-Dgemfirexd.system.home=" + derbyHome,
                "com.pivotal.gemfirexd.internal.drda.NetworkServerControl", "-p",
                String.valueOf(fourthAlternatePort).toString(), 
        ...
        removeSystemProperty("gemfirexd.drda.portNumber");

        // shutdown server with port set in gemfirexd.properties
        actionResult = shutdownServer(firstAlternatePort, false);
        checkWhetherNeedToShutdown ( new int[] {1527, firstAlternatePort},
            actionResult);
        // remove gemfirexd.properties
        Boolean ret = (Boolean) AccessController.doPrivileged
        (new java.security.PrivilegedAction() {
            public Object run() {
                return Boolean.valueOf((new File(
                    derbyHome+File.separator + "gemfirexd.properties")).delete());
            }
        }
        );
        if (ret.booleanValue() == false) {
            checkWhetherNeedToShutdown ( new int[] {1527, firstAlternatePort},
                "unable to remove gemfirexd.properties");
        }
        // have to shutdown engine to force re-evaluation of gemfirexd.properties
        TestConfiguration.getCurrent().shutdownEngine();
    ...
    {        
        ...
        String[] traceCmd = new String[] {
            "com.pivotal.gemfirexd.internal.drda.NetworkServerControl", "trace", "on" };
        assertSuccessfulCmd("Trace turned on for all sessions.", traceCmd);
        verifyProperties(expectedTraceOn);     

        traceCmd = new String[] {
                "com.pivotal.gemfirexd.internal.drda.NetworkServerControl", "trace", "off" };
    ...
    throws SQLException, IOException, InterruptedException
    {
        ...
        String[] cmd = new String[] {
            "com.pivotal.gemfirexd.internal.drda.NetworkServerControl", "logconnections", "on" };
        assertSuccessfulCmd("Log Connections changed to on.", cmd);
        verifyProperties(expectedLogConnectionsOn);     

        cmd = new String[] {
                "com.pivotal.gemfirexd.internal.drda.NetworkServerControl", "logconnections", "off" };
