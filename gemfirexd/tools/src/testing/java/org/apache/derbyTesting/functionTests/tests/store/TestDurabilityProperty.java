...
/**
 * This program tests the system when the gemfirexd.system.durability property is
 * set to 'test'. 
 * <BR>
 * When the gemfirexd.system.durability is set to 'test', the system will not do 
 * any sync to the
 ...
 * 1. setting the gemfirexd.system.durability=test is actually not
 *    doing the syncs by timing inserts
 * 2. check if a warning message exists in gemfirexd.log 
 ...
 * database booted with gemfirexd.system.durability=test set, then shutdown
 * and database booted without gemfirexd.system.durability=test
 * 
 * @version 1.0
 */
public class TestDurabilityProperty {
    public static void main(String[] args) {
        try {
            // Test 1: check if gemfirexd.system.durability=test
            // mode is not doing syncs 
            testNoSyncs(args);
            
            String derbyHome = System.getProperty("gemfirexd.system.home");
            // Test 2
            // Check if gemfirexd.log has the warning message
            report("Is warning message about gemfirexd.system.durability=test present in gemfirexd.log ="
            ...
            // gemfirexd.system.durability set to test. In this case the gemfirexd.log 
            ...
            report(
                ...
                "gemfirexd.system.durability=test and test for marker in log.ctrl file");
     ...
     * 
     * @param mode
     *            value for gemfirexd.system.durability property
     * @param create
    ...
    public static long timeTakenToInsert(String mode, boolean create,
        ...
        System.setProperty("gemfirexd.system.durability", mode);
        Connection conn = com.pivotal.gemfirexd.internal.tools.ij.startJBMS();
     ...
     * but in case of the gemfirexd.system.durability=test mode, the syncs dont 
     * happen.
     */
    ...
    /**
     * When gemfirexd.system.durability is set, a warning message is written out to
     * gemfirexd.log indicating that the property is set and that it does not
     * guarantee recoverability This test tests if a message is written out to
     * gemfirexd.log or not
     */
    public static boolean isMessageInDerbyLog(String derbyHome) throws Exception {
        BufferedReader reader = null;
        File derbylog = null;
        try {
            derbylog = new File(derbyHome, "gemfirexd.log");
            reader = new BufferedReader(new FileReader(derbylog));

            String line = null;
            while ((line = reader.readLine()) != null) {
                if (line.indexOf("gemfirexd.system.durability=test") != -1)
                    return true;
    ...
    /**
     * if database is booted with gemfirexd.system.durability=test, 
     ...
     * com.pivotal.gemfirexd.internal.impl.store.log.LogToFile for IS_DURABILITY_TESTMODE_NO_SYNC_FLAG
     */
    ...
    /**
     * Test for case when database is booted without gemfirexd.system.durability=test
     * but previously has been booted with the gemfirexd.system.durability=test. In 
     * this scenario,the log control file should still have the marker to say
     * that this mode was set previously, and gemfirexd.log must also have a warning
     * message
     * @param derbyHome value of gemfirexd.system.home where the database is
     * @throws Exception
     */
    public static void markerNotOverwritten(String derbyHome) throws Exception
    {
        ...
        // unset property
        System.setProperty("gemfirexd.system.durability","");
        conn = com.pivotal.gemfirexd.internal.tools.ij.startJBMS();
        conn.close();
        markerInControlFile(derbyHome);
        report("Is warning message about gemfirexd.system.durability=test present in gemfirexd.log ="
                + isMessageInDerbyLog(derbyHome));
    ...
    /**
     * Test if gemfirexd.system.durability=test property is broken or not. We time
    ...
    public static void testNoSyncs(String[] args) throws Exception {
        ...
        report("1. With gemfirexd.system.durability=test,"
                + "Test to see if syncs are not happening ");
        // use the ij utility to read the property file and
        // make the initial connection.
        com.pivotal.gemfirexd.internal.tools.ij.getPropertyArg(args);
        ...
        // gemfirexd.system.durability=test there are no guarantees on consistency
        // of database so dont want to mess up numbers for the all syncs case

        // gemfirexd.system.durability is not test so it will default to
        ...
        if (isMessageInDerbyLog(derbyHome))
            report("ERROR! System should not have been booted with"
                    + "gemfirexd.system.durability=test mode here");
        create = false;
        // gemfirexd.system.durability=test and autocommit=true
        long timeWithTestModeCommitOn = timeTakenToInsert("test", create, true);
        // gemfirexd.system.durability=test and autocommit=false
        ...
        // inserts in autocommit mode with gemfirexd.system.durability=test mode
        long upperBound = 3000;

        // if it takes a lot of time to do the inserts then do extra checks
        // to determine if gemfirexd.system.durability=test mode is broken or not
        ...
        if (timeWithTestModeCommitOn > upperBound) {
            ...
            // in gemfirexd.system.durability=test autocommit on or off should
            ...
            if (Math.abs(timeWithTestModeCommitOn - timeWithTestModeCommitOff) > proximityRange) {
                ...
                if (timeWithTestModeCommitOn > timeCommitOn
                    ...
                    report("FAIL -- gemfirexd.system.durability=test mode seems to be broken.");
