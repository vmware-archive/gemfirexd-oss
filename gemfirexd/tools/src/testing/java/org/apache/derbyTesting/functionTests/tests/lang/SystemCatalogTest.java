...
public class SystemCatalogTest extends BaseJDBCTestCase {
   ...
   * @throws SQLException
   */
  // GemFireXD has some explain tables, which are type 'T'
  // Comment this out for now
  //public void testSystemCatalogTableTypes() throws SQLException {
  public void _testSystemCatalogTableTypes() throws SQLException {
    Statement s = createStatement();
   ...
   * @throws SQLException
   */
  // He's right. This did break. 
  // GemFireXD has new tables called
  // GATEWAYSENDERS, GATEWAYRECEIVERS and ASYNCEVENTLISTENERS
  // All the columns from these tables need to be added here
  //FIXME
  //public void testSystemCatalogColumns() throws SQLException {
  public void _testSystemCatalogColumns() throws SQLException {
    // GemStone changes BEGIN
    //   Seems like this will break frequently...
    String [][] expected = {
        {"SYSALIASES", "ALIAS", "2", "VARCHAR(128) NOT NULL"},
        {"SYSALIASES", "ALIASID", "1", "CHAR(36) NOT NULL"},
        {"SYSALIASES", "ALIASINFO", "8", "com.pivotal.gemfirexd.internal.catalog.AliasInfo"},
        {"SYSALIASES", "ALIASSCHEMANAME", "10", "VARCHAR(128) NOT NULL"},
        ...
        {"SYSCHECKS", "REFERENCEDCOLUMNS", "3", "com.pivotal.gemfirexd.internal.catalog.ReferencedColumns NOT NULL"},
        {"SYSCOLPERMS", "COLPERMSID", "1", "CHAR(36) NOT NULL"},
        {"SYSCOLPERMS", "COLUMNS", "6", "com.pivotal.gemfirexd.internal.iapi.services.io.FormatableBitSet NOT NULL"},
        ...
        {"SYSCOLUMNS", "COLUMNDATATYPE", "4", "com.pivotal.gemfirexd.internal.catalog.TypeDescriptor NOT NULL"},
        ...
        {"SYSCONGLOMERATES", "CONGLOMSCHEMANAME", "9", "VARCHAR(128)"},
        {"SYSCONGLOMERATES", "DESCRIPTOR", "6", "com.pivotal.gemfirexd.internal.catalog.IndexDescriptor"},
        ...
        {"SYSDEPENDS", "DEPENDENTFINDER", "2", "com.pivotal.gemfirexd.internal.catalog.DependableFinder NOT NULL"},
        {"SYSDEPENDS", "DEPENDENTID", "1", "CHAR(36) NOT NULL"},
        {"SYSDEPENDS", "PROVIDERFINDER", "4", "com.pivotal.gemfirexd.internal.catalog.DependableFinder NOT NULL"},
        {"SYSDEPENDS", "PROVIDERID", "3", "CHAR(36) NOT NULL"},
        // GemStone changes BEGIN
        //{"SYSDISKSTORES", "ALLOWFORCECOMPACTION", "4", "CHAR(254) NOT NULL"},
        //{"SYSDISKSTORES", "AUTOCOMPACT", "3", "CHAR(254) NOT NULL"},
        {"SYSDISKSTORES", "ALLOWFORCECOMPACTION", "4", "CHAR(6) NOT NULL"},
        {"SYSDISKSTORES", "AUTOCOMPACT", "3", "CHAR(6) NOT NULL"},
        // GemStone changes BEGIN
        {"SYSDISKSTORES", "COMPACTIONTHRESHOLD", "5", "INTEGER NOT NULL"},
        {"SYSDISKSTORES", "DIR_PATH_SIZE", "9", "VARCHAR(32672) NOT NULL"},
        {"SYSDISKSTORES", "MAXLOGSIZE", "2", "BIGINT NOT NULL"},
        // GemStone changes BEGIN
        //{"SYSDISKSTORES", "NAME", "1", "CHAR(254) NOT NULL"},
        {"SYSDISKSTORES", "NAME", "1", "VARCHAR(128) NOT NULL"},
        // GemStone changes END
        {"SYSDISKSTORES", "QUEUESIZE", "8", "INTEGER NOT NULL"},
        {"SYSDISKSTORES", "TIMEINTERVAL", "6", "BIGINT NOT NULL"},
        {"SYSDISKSTORES", "WRITEBUFFERSIZE", "7", "INTEGER NOT NULL"},
        ...
        {"SYSSCHEMAS", "DEFAULTSERVERGROUPS", "4", "VARCHAR(128) NOT NULL"},
        ...
        {"SYSSTATISTICS", "STATISTICS", "8", "com.pivotal.gemfirexd.internal.catalog.Statistics NOT NULL"},
        ...
        {"SYSTABLES", "DATAPOLICY", "8", "VARCHAR(24) NOT NULL"},
        {"SYSTABLES", "DISKATTRS", "13", "LONG VARCHAR"},
        {"SYSTABLES", "EVICTIONATTRS", "12", "LONG VARCHAR"},
        {"SYSTABLES", "EXPIRATIONATTRS", "11", "LONG VARCHAR"},
        {"SYSTABLES", "GATEWAYENABLED", "17", "BOOLEAN NOT NULL"},
        {"SYSTABLES", "GATEWAYSNEDERS", "18", "VARCHAR(128)"},
        {"SYSTABLES", "LISTENERS", "16", "LONG VARCHAR"},
        {"SYSTABLES", "LOADER", "14", "VARCHAR(128)"},
        {"SYSTABLES", "LOCKGRANULARITY", "6", "CHAR(1) NOT NULL"},
        {"SYSTABLES", "PARTITIONATTRS", "9", "LONG VARCHAR"},
        {"SYSTABLES", "RESOLVER", "10", "LONG VARCHAR"},
        {"SYSTABLES", "SCHEMAID", "4", "CHAR(36) NOT NULL"},
        {"SYSTABLES", "SERVERGROUPS", "7", "VARCHAR(128) NOT NULL"},
        {"SYSTABLES", "TABLEID", "1", "CHAR(36) NOT NULL"},
        {"SYSTABLES", "TABLENAME", "2", "VARCHAR(128) NOT NULL"},
        {"SYSTABLES", "TABLESCHEMANAME", "5", "VARCHAR(128) NOT NULL"},
        {"SYSTABLES", "TABLETYPE", "3", "CHAR(1) NOT NULL"},
        {"SYSTABLES", "WRITER", "15", "VARCHAR(128)"},
        ...
        {"SYSTRIGGERS", "REFERENCEDCOLUMNS", "12", "com.pivotal.gemfirexd.internal.catalog.ReferencedColumns"},
  ...
  // GemFireXD has new tables called
  // GATEWAYSENDERS, GATEWAYRECEIVERS and ASYNCEVENTLISTENERS
  // All the indexes from these tables need to be added here
  //public void testSystemCatalogIndexes() throws SQLException{
  public void _testSystemCatalogIndexes() throws SQLException{
    String [][] expected =
    {
        ...
        {"SYSDISKSTORES", "SYSDISKSTORES_HEAP", "false"},
        {"SYSDISKSTORES", "SYSDISKSTORES_INDEX1", "true"},
  ...
  public void testOverThirtyCharsInTableName() throws SQLException {
    ...
    s.execute("create table t234567890123456789012345678901234567890 (c2345678901234567890123456789012345678901234567890 int)");
    ...
    rs = s.executeQuery("select COLUMNNAME from sys.syscolumns where {fn length(COLUMNNAME)} > 40 order by columnname");
    JDBC.assertSingleValueResultSet(rs, "C2345678901234567890123456789012345678901234567890");
    rs.close();
  ...
  public void testColumnDatatypesInSystemCatalogs() throws SQLException {
    ...
    assertTrue(rs.next());
    assertTrue(rs.getString(2).startsWith("LOCALSORTEDMAP (1)"));
    assertFalse(rs.next());
    rs.close();
  ...
  /**
   * Run SYSCS_UTIL.CHECK_TABLE on each system table.
   *
   * @throws SQLException
   */
  public void testCheckConsistencyOfSystemCatalogs() throws SQLException {
    Statement s = createStatement();
    ResultSet rs = s.executeQuery("select tablename, SYSCS_UTIL.CHECK_TABLE('SYS', tablename)from sys.systables where tabletype = 'S' and tablename != 'SYSDUMMY1' order by tablename");
