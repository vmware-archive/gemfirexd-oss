...
// GemStone changes END
	 ...
	 * 
	 * @param testName
	 * @param sqlBuffer  - StringBuilder with SQL Text
	 * @param numParams  - Number of parameters
	 * @param paramValue - Parameter value
	 * @return true if the check fails
	 */
	private boolean checkT0Query(String testName, 
				StringBuilder sqlBuffer, int numParams, int paramValue) {
	...
	private boolean logicalOperators(int numOperands) throws SQLException {
		...
		StringBuilder sqlBuffer = new StringBuilder((numOperands * 20) + 512);
	...
	private boolean inClause(int numParams) throws SQLException {
		...
		StringBuilder sqlBuffer = new StringBuilder((numParams * 20) + 512);
	...
	{ 		
		...
		StringBuilder createView = new StringBuilder("create view " + viewName + 
												   " as select * from t0 " );
		for (int i = 1; i < 100; i ++)
		{
    		...
    		int numUnions) throws SQLException
	{
		...
		StringBuilder selectSQLBuffer  =
			new StringBuilder(((numUnions/100) * unionClause.length()) + 512);
