...
public final class GrantRevokeDDLTest extends BaseJDBCTestCase {
    ...
    public void testGrantRevokeDDL() throws Exception
    {
        ...
        //GemFireXD does not need to create a schema after connected as that schema
        ...
        cSt = samConnection.prepareCall(
            " call SYSCS_UTIL.COMPRESS_TABLE('SAM', 'SAMTABLE', 1)");
        assertUpdateCount(cSt, 0);
        cSt.close();
        
        cSt = samConnection.prepareCall(
            " call "
            + "SYSCS_UTIL.INPLACE_COMPRESS_TABLE('SAM', "
            + "'SAMTABLE', 1, 1, 1)");
        assertUpdateCount(cSt, 0);
        cSt.close();
        ...
        cSt = samConnection.prepareCall(
            "call SYSCS_UTIL.COMPRESS_TABLE('SWIPER', 'MYTAB', 1)");
        assertStatementError("38000", cSt);
        cSt.close();
        
        cSt = samConnection.prepareCall(
            " call "
            + "SYSCS_UTIL.INPLACE_COMPRESS_TABLE('SWIPER', "
            + "'MYTAB', 1, 1, 1)");
        assertUpdateCount(cSt, 0);
        cSt.close();
        ...
        cSt = samConnection.prepareCall(
            "call SYSCS_UTIL.EXPORT_TABLE('SAM', "
        ...
        cSt = samConnection.prepareCall(
            " call "
            + "SYSCS_UTIL.SET_DATABASE_PROPERTY('gemfirexd.storag"
            + "e.pageSize', '4096')");
        assertStatementError("42504", cSt);
        cSt.close();
        ...
        assertStatementError("42504", st_samConnection,
            " values "
            + "SYSCS_UTIL.GET_DATABASE_PROPERTY('gemfirexd.storag"
            + "e.pageSize')");

        PreparedStatement psgua = samConnection.prepareStatement(
            "VALUES SYSCS_UTIL.GET_USER_ACCESS(CURRENT_USER)");
        
        assertStatementError("42504", psgua);
        psgua.close();

        cSt = samConnection.prepareCall(
             "CALL SYSCS_UTIL.SET_USER_ACCESS(CURRENT_USER, NULL)");
        assertStatementError("42504", cSt);
        cSt.close();
        ...
        st.executeUpdate(
            ...
            + "SYSCS_UTIL.EXPORT_TABLE to public");
        
        st.executeUpdate(
            " grant execute on procedure "
            + "SYSCS_UTIL.SET_DATABASE_PROPERTY to sam");
        
        st.executeUpdate(
            " grant execute on function "
            + "SYSCS_UTIL.GET_DATABASE_PROPERTY to sam");
        ...
        st.executeUpdate(
                ...
                + "SYSCS_UTIL.EXPORT_TABLE from public restrict");
            
        st.executeUpdate(
                " revoke execute on procedure "
                + "SYSCS_UTIL.SET_DATABASE_PROPERTY from sam restrict");
            
        st.executeUpdate(
                " revoke execute on function "
                + "SYSCS_UTIL.GET_DATABASE_PROPERTY from sam restrict");
    ...
    public void testGrantRevokeDDL2() throws SQLException {
        ...
        cSt3 = user3.prepareCall(
            "CALL SYSCS_UTIL.BACKUP_DATABASE('backup1')");
        assertStatementError("42504", cSt3);
        
        cSt3 = user3.prepareCall(
            " CALL "
            + "SYSCS_UTIL.BACKUP_DATABASE_AND_ENABLE_LOG_ARCH"
            + "IVE_MODE('backup3', 1)");
        assertStatementError("42504", cSt3);
        ...
        cSt3 = user3.prepareCall(
            "CALL SYSCS_UTIL.FREEZE_DATABASE()");
        assertStatementError("42504", cSt3);
        
        cSt3 = user3.prepareCall(
            " CALL SYSCS_UTIL.UNFREEZE_DATABASE()");
        assertStatementError("42504", cSt3);
        ...
        cSt3 = user3.prepareCall(
            " CALL SYSCS_UTIL.CHECKPOINT_DATABASE()");
        assertStatementError("42504", cSt3);
        ...
        cSt2 = user2.prepareCall(
            " CALL SYSCS_UTIL.SET_RUNTIMESTATISTICS(0)");
        assertUpdateCount(cSt2, 0);
        ...
        cSt3 = user3.prepareCall(
            " CALL SYSCS_UTIL.EXPORT_TABLE ('USER3', "
            + "'TABLEEXP1', 'myfile.del', null, null, null)");
        assertStatementError("42504", cSt3);
        
        cSt3 = user3.prepareCall(
            " CALL SYSCS_UTIL.IMPORT_TABLE ('USER3', "
        ...
        cSt3 = user3.prepareCall(
            " CALL SYSCS_UTIL.IMPORT_DATA ('USER3', "
        ...
        cSt3 = user3.prepareCall(
            ...
            + "('gemfirexd.locks.deadlockTimeout', '10')");
        assertStatementError("42504", cSt3);
        
        assertStatementError("42504", st_user3,
            " VALUES "
            + "SYSCS_UTIL.GET_DATABASE_PROPERTY('gemfirexd.locks."
            + "deadlockTimeout')");
        ...
        cSt3 = user3.prepareCall(
            "CALL SYSCS_UTIL.COMPRESS_TABLE('USER3', "
            + "'TABLEEXP1', 1)");
        assertUpdateCount(cSt3, 0);
        
        cSt3 = user3.prepareCall(
            " call "
            + "SYSCS_UTIL.INPLACE_COMPRESS_TABLE('USER3', "
            + "'TABLEEXP1', 1, 1, 1)");
        assertUpdateCount(cSt3, 0);
        ...
        assertStatementError("42504", st_user3,
            "VALUES SYSCS_UTIL.CHECK_TABLE('USER3', 'TABLEEXP1')");
    ...
    public void testRevokeDropsFKWithSharedConglom() throws SQLException
    {
        ...
        JDBC.assertUnorderedResultSet(st_mamta2.executeQuery(
            "select * from fkt2 --GEMFIREXD-PROPERTIES constraint=FKDUP"),
            new String [][] {{"1", "2"}, {"2", "3"}});
