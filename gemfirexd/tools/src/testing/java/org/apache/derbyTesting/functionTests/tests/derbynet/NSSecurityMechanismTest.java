 ...
 * mechanism by setting gemfirexd.drda.securityMechanism, in that case the clients 
...
{
    // values for gemfirexd.drda.securityMechanism property
...
// GemStone changes END
     ...
     * gemfirexd.drda.securityMechanism.  Please check out the following
...
// GemStone changes BEGIN
                // doesn't make sense to stop server from client in GemFireXD
                // also see #41296
                /*
...
// GemStone changes END
    ...
    public void testSecurityMechanismOnEmbedded() throws SQLException {
      ...
      // GemStone changes BEGIN
      // need to add gemfirexd connectionAttributes here as well since
      // it overwrites previous connectionAttibutes set by JDBCDataSource
        /* JDBCDataSource.setBeanProperty(
            ds, "connectionAttributes", "securityMechanism=8");
         */
        JDBCDataSource.setBeanProperty(ds,
                                       "connectionAttributes",
                                       "securityMechanism=8;mcast-port=0;host-data=true");
      // GemStone changes END
...
// GemStone changes BEGIN
            {
                // GemFireXD now has uniform policy of throwing USER ID
                // null exception if a mechanism other than default "4"
                // (user only authentication) is used
                if (dbUrl.matches("^.*;securityMechanism=[^4].*$")
                    && !dbUrl.contains(";user=")) {
                  assertSQLState08001("08001.C.7", sqle);
                }
                else {
...
// GemStone changes END
     ...
     * gemfirexd.drda.securityMechanism and to the value it is set to.  See method
     * (fixture) testNetworkServerSecurityMechanism() to check if server is 
     * using the gemfirexd.drda.securityMechanism to restrict client connections
     ...
     * Note: #2 can happen if server is started with gemfirexd.drda.securityMechanism
     * and thus restricts what security mechanisms the client can connect with.
     * This will happen for the test run when gemfirexd.drda.securityMechanism is
     ...
     * gemfirexd.drda.securityMechanism to restrict client connections based on
    ...
    private void assertUSRSSBPWD_with_BUILTIN(String[] expectedValues)
        ...
        cs.setString(1, "gemfirexd.user.neelima");
        cs.setString(2, "lee");
        cs.execute();

        cs.setString(1, "gemfirexd.user.APP");
        cs.setString(2, "APP");
        cs.execute();

        cs.setString(1, "gemfirexd.authz-full-access-users");
        ...
        // booted - gemfirexd.authentication.required is a
        // static property.
        assertConnectionUsingDriverManager(getJDBCUrl(
            "user=APP;password=APP;shutdown=true;securityMechanism=" +
