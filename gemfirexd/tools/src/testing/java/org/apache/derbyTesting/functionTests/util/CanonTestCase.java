...
abstract class CanonTestCase extends BaseJDBCTestCase {
    ...
    void compareCanon(String canon) throws Throwable {
        ...
        try {
            // Gemstone START
            // For GemFireXD testing, LANG scripts are not yet converted
            // but should be run each build to ensure that basic functionality
            // does not break.
            // Therefore, right now we will check for XJ001 exceptions *only*
            // and ignore the master file
 
            // Rad output line by line
            ...
            for (int lineNumber = 1;; lineNumber++) {
                ...
                {
                  // We found a fatal error from GemFireXD
                  // (NPE, Java Assertion, etc)
                  // TODO : look for CastExceptions and IllegalStateExceptions too
                  assertEquals("Found exception at line " + lineNumber + " with text "+testLine, 
                             "String", "DifferentString");     // force assert
                }
            }
