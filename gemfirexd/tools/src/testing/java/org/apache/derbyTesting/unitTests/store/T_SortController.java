...
public class T_SortController extends T_Generic
{
	...
	private void setSortBufferSize(final String buf_length) {
    	AccessController.doPrivileged(new PrivilegedAction() {
		    public Object run()  {
		    	System.setProperty("gemfirexd.storage.sortBufferMax", buf_length);
		    	return null;
		    }
	    });
	}
	...
	**/
		...
		throws StandardException
	{
		...
		for (int i = 0; i < in.length; i++)
		{
			...
			ValueRow v = new ValueRow(in[i].nColumns());
			v.setRowArray(in[i].getRowArray());
			sort.insert(v);
		}
...
class T_DummySortObserver implements SortObserver
{
	 ...
	 */
	public ExecRow insertNonDuplicateKey(
	    ExecRow insertRow)	
	{
		return insertRow;
	}
		...
		throws StandardException
	{
		...
		if (lastElement > 0)
		{
			...
			vector.removeElementAt(lastElement - 1);
                        ValueRow v = new ValueRow(template.nColumns());
                        v.setRowArray(retval);
			return v;
		}
		ValueRow v = new ValueRow(template.nColumns());
		v.setRowArray(template.getRowArrayClone());
		return v;
	}
...
class T_SumForIntCol implements SortObserver
{
		...
		throws StandardException
	{
		...
		// to SQLInteger.
		SQLInteger increment = (SQLInteger) insertRow.getColumn(columnId);
		SQLInteger sum = (SQLInteger) existingRow.getColumn(columnId);
