...
public class ImportExportTest extends BaseJDBCTestCase {
    ...
    public void testCastingProblem() throws Exception
    {
        ...
        try {
            doImportFromFile( getConnection(), fileName, "T5", null, null, null, 0 );
            fail();
		} catch (SQLException e) {
			assertSQLState("XIE0R", e);
		}
...
// GemStone changes BEGIN
		// GemFireXD doesn't work with replace, see ticket #41456
...
// GemStone changes END
			  ...
			  int replace) throws SQLException 
    {
		...
		// GemStone changes BEGIN
    // In GemFireXD, the rows are not guaranteed to be exported 
    // in same order that they were imported, so instead
    // of comparing files, we should compare the contents of the 
    // (unordered) tables to make sure they are the same.
