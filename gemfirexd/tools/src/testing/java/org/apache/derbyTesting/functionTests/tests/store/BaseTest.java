...
public abstract class BaseTest
{
        ...
        throws Throwable
    {
        ...
        {
			com.pivotal.gemfirexd.internal.tools.JDBCDisplayUtil.ShowSQLException(
                System.out, sqle);
		...
		throws SQLException
    {
        ...
        ResultSet rs = 
            s.executeQuery(
                "select schemaname, tablename, SYSCS_UTIL.CHECK_TABLE(schemaname, tablename) " + 
		...
		throws SQLException
    {
        Statement s = conn.createStatement();
        s.executeUpdate(
            "CREATE FUNCTION D_CONGLOMID_PRINT(DBNAME VARCHAR(128), CONGLOMID INT) RETURNS VARCHAR(32000) RETURNS NULL ON NULL INPUT EXTERNAL NAME 'com.pivotal.gemfirexd.internal.impl.store.raw.data.D_DiagnosticUtil.diag_conglomid' LANGUAGE JAVA PARAMETER STYLE JAVA");
        s.executeUpdate(
            "CREATE FUNCTION DIAG_CONGLOMID(DBNAME VARCHAR(128), CONGLOMID INT) RETURNS VARCHAR(32000) RETURNS NULL ON NULL INPUT EXTERNAL NAME 'com.pivotal.gemfirexd.internal.impl.store.raw.data.D_DiagnosticUtil.diag_conglomid' LANGUAGE JAVA PARAMETER STYLE JAVA");
        s.close();
        conn.commit();
    ...
    {
        String stmt_str = 
            "select conglomeratename, isindex, numallocatedpages, numfreepages, numunfilledpages, pagesize, estimspacesaving from new com.pivotal.gemfirexd.internal.diag.SpaceTable('" +
