...
public class PrimaryKeyTest extends BaseJDBCTestCase
{
  ...
  public void testExplicitNullabilityOfConstraints() throws SQLException {
    ...
    // GemStone changes BEGIN : our error code is 42831 instead of 42X01
    try {
      assertStatementError("42831" , s , "create table neg2(c1 int null constraint asdf primary key)");
      assertStatementError("42831" , s , "create table neg2(c1 int null, c2 int, constraint asdf primary key(c1, c2))");
      //-- test that a unique key can not be explicitly nullable
      //FIXME - GemFireXD allows this
      //assertStatementError("42831" , s , "create table neg1(c1 int null unique)");
      //assertStatementError("42831" , s , "create table neg1(c1 int null, c2 int, constraint asdf unique(c1))");
    } finally {
      // Workaround for GFXD ticket 39408, so subsequent testcases are not affected
      try {
        s.execute("drop table neg2");
        s.execute("drop table neg1");
  ...
  public void testCatalog() throws SQLException {
    Statement s = createStatement();
    assertUpdateCount(s , 0 , "create table pos1(c1 int primary key)");
    ResultSet rs = s.executeQuery("select tablename, SYSCS_UTIL.CHECK_TABLE('SYS', tablename) from sys.systables where CAST(tabletype AS CHAR(1)) = 'S'  and CAST(tablename AS VARCHAR(128)) != 'SYSDUMMY1'");
                String[][] expectedCheckTables = new String[][]
                       {{"SYSCONGLOMERATES","1"},
                        ...
                        {"SYSDISKSTORES","1"},
                        {"SYSHDFSSTORES","1"},
                        {"ASYNCEVENTLISTENERS","1"},
                        {"GATEWAYSENDERS","1"},
                        {"GATEWAYRECEIVERS","1"},
                        // GemStone changes END
            {"SYSROLES", "1"}};
                // GemStone changes BEGIN  : use unordered validation
                JDBC.assertUnorderedResultSet(rs,expectedCheckTables);
                // GemStone changes END
    //-- drop tables
    assertUpdateCount(s , 0 , "drop table pos1");
    //-- verify it again
                rs = s.executeQuery("select tablename, SYSCS_UTIL.CHECK_TABLE('SYS', tablename) from sys.systables where CAST(tabletype AS CHAR(1)) = 'S'  and CAST(tablename AS VARCHAR(128)) != 'SYSDUMMY1'");
                // GemStone changes BEGIN  : use unordered validation
                JDBC.assertUnorderedResultSet(rs, expectedCheckTables);
                // GemStone changes END
  }
