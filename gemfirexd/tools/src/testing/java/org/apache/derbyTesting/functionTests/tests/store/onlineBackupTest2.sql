...
call SYSCS_UTIL.BACKUP_DATABASE_AND_ENABLE_LOG_ARCHIVE_MODE(
                                              'extinout/mybackup', 1);
call SYSCS_UTIL.BACKUP_DATABASE_AND_ENABLE_LOG_ARCHIVE_MODE_NOWAIT(
                                              'extinout/mybackup', 1);
...
async bthread1 'call SYSCS_UTIL.BACKUP_DATABASE(
                                    ''extinout/ulbackup1'')' ;
set connection c2;
...
async bthread1 
  'call SYSCS_UTIL.BACKUP_DATABASE(''extinout/ulbackup2'')';
set connection c2;
-- sleep for a while for the backup thread to 
-- really get into the wait state
call sleep(1000);
