...
public final class XMLTypeAndOpsTest extends BaseJDBCTestCase {
    ...
    {
        TestSuite suite = new TestSuite("XMLTypeAndOpsTest");
     ...
     */
    // FIXME: #48136
    public void DISABLED_testIllegalOps() throws Exception
    {
    ...
    // Disabled due to trigger limitations. Added a test
    // in BugsTest to cover test case here.
    // BugsTest.testTriggerSetXML
    public void _testTriggerSetXML() throws Exception
...
// GemStone changes BEGIN
        // GemFireXD results are not in order
...
// GemStone changes BEGIN
        final boolean isClientConnection = !getConnection().getClass()
            .getName().contains("EmbedConnection");
        // GemFireXD results are not in order
        JDBC.assertUnorderedResultSet(rs, expRS, true, isClientConnection);
        /* (original code)
        JDBC.assertFullResultSet(rs, expRS, true);
        */
// GemStone changes END
...
// GemStone changes BEGIN
        // GemFireXD results are not in order
        JDBC.assertUnorderedResultSet(rs, expRS, true,
            !getConnection().getClass().getName().contains("EmbedConnection"));
        /* (original code)
        JDBC.assertFullResultSet(rs, expRS, true);
        */
// GemStone changes END
...
// GemStone changes END
     ...
     * related op.
     */
    public void DISABLED_43204_testTriggerSPSWithXML() throws Exception
    {
        Statement st = createStatement();
