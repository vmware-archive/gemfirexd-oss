...
public class RunTest
{
    ...
    static File clPropFile; // testname_gemfirexd.properties or default
    ...
    /** The value of gemfirexd.ui.codeset if it has been specified in the
     * properties file. */
...
// Gemstone changes END
        ...
        // Check for properties files, including gemfirexd.properties
...
//System.out.println("basedir delete status: " + status );
    ...
    private static String createPropString()
        throws ClassNotFoundException, FileNotFoundException, IOException
    {
        // Check for existence of app properties and/or gemfirexd.properties files
        // Copy the gemfirexd.properties to the db base directory
		...
		InputStream isCl = null; // For test_gemfirexd.properties
		InputStream isClDef = null; // For default_gemfirexd.properties
        ...
        // into properties to be merged with app and/or gemfirexd props
        ...
        /* If there are more than one gemfirexd.properties, the rule is to
           load either the test_gemfirexd.properties or the default one,
        */
        
        // Check for default_gemfirexd.properties
        isClDef = loadTestResource(clDefProp);
		// Check for test specific props 
        isCl = loadTestResource("tests/" + testDirName + "/" + testBase + "_gemfirexd.properties");
//System.out.println("**************");
//System.out.println("isCl = " + isCl);
//System.out.println(defaultPackageName + testBase + "_gemfirexd.properties");
//System.out.println("**************");
        
        // Now load and merge the properties based on above rules
		if (isCl != null) // In case there exists a test_gemfirexd.properties
		{
		    clp.load(isCl);
clp.list(System.out);
		}
		// Try the default_gemfirexd.properties instead
        ...
        // forcing it lower unless set in _gemfirexd.properties file for a specific test
        if (jvmName.startsWith("j9"))
        {
            if (clp.getProperty("gemfirexd.language.statementCacheSize")==null)
        ...
        else
        {
            ...
            if ( !srvProps.isEmpty() )
            {
                for (Enumeration e = srvProps.propertyNames(); e.hasMoreElements();)
                {
                    ...
                    if (key.equals("gemfirexd.debug.true")) // Add to existing prop
                    {
                        String cval = clp.getProperty("gemfirexd.debug.true");
        ...
        if ( !clp.isEmpty() )
        {
            ...
            // a bug in the EPOC jvm. 
      	    try { clPropFile = new File((new File(baseDir, "gemfirexd.properties")).getCanonicalPath()); }
    	    catch (IOException e) {
...
//System.out.println("**************");
        ...
        if ( !ap.isEmpty() )
        {
			...
			if (ap.getProperty("gemfirexd.ui.codeset") != null) {
				// gemfirexd.ui.codeset is specified explicitly, don't override
			...
			if (verbose)
				System.out.println("console.encoding:" + conEnc + 
							   ...
							   " gemfirexd.ui.codeset: " + ap.getProperty("gemfirexd.ui.codeset"));
...
// GemStone changes END 
        ...
        throws IOException
    {
        ...
        if (keepfiles == false)
        {
            ...
            if (deleteBaseDir)
            {
		        ...
		        else 
		        {
			        ...
			        //no permission in Java to drop gemfirexd.log
			        //File logfile = new File(baseDir, "gemfirexd.log");
			        //System.out.println("delete gemfirexd.log ");
        ...
        // GemStone changes BEGIN
        // delete persistent datadictionary files between tests
        deleteFile(new File("datadictionary"));
        // GemStone changes END
	    ...
	    throws FileNotFoundException, IOException, Exception
	{
        ...
        if ( ij.startsWith("ij") )
        {
            ...
            v.addElement("com.pivotal.gemfirexd.internal.tools." + ij);
            if (ij.equals("ij"))
            {
