    ...
    public static Test suite() {
        ...
        try {
            ...
            embeddedAutoLoad = jdbcDrivers
                    .indexOf("com.pivotal.gemfirexd.jdbc.EmbeddedDriver") != -1;

            clientAutoLoad = jdbcDrivers
                    .indexOf("com.pivotal.gemfirexd.jdbc.ClientDriver") != -1;
...
// GemStone changes BEGIN
            // not valid for GemFireXD that uses FabricService API
            /* (original code)
            if (Derby.hasServer())
...
// GemStone changes BEGIN
      // don't run testUnsuccessfulConnect since the database name is
      // suppressed from the connection URL anyway for GemFireXD
   ...
   * Test we can connect successfully to a database.
    ...
    private void explicitLoad(boolean instanceOnly) throws Exception
    {
        ...
        if (!instanceOnly) {
          ...
          // GemStone changes BEGIN
          // unsuccessfulConnect fails in GemFireXD since the database
          // name is suppressed in the test configuration
     ...
     * <P>
     * gemfirexd.drda.startNetworkServer=false or not set
     * <BR>
     *     network server should not auto boot.
     * <P>
     * gemfirexd.drda.startNetworkServer=true
     * <BR>
     ...
     */
// GemStone changes BEGIN
    // not valid for GemFireXD
    public void DISABLED_testAutoNetworkServerBoot() throws Exception
// GemStone changes END
    {
        boolean nsAutoBoot = "true".equalsIgnoreCase(
                getSystemProperty("gemfirexd.drda.startNetworkServer"));
    ...
    {
        String jdbcDrivers = getSystemProperty("jdbc.drivers");
        return jdbcDrivers.indexOf("com.pivotal.gemfirexd.jdbc.EmbeddedDriver") != -1;
    }
    ...
    private boolean hasDerbyThreadGroup() {
        ...
        for (int i = 0; i < actual; i++)
        {
          ...
          // if (allGroups[i].getName().startsWith("derby."))
            if (allGroups[i].getName().startsWith("gemfirexd."))
          // GemStone changes END
                return true;
        }
        return false;
