...
public class dblook_test {
		...
		throws Exception
	{
		...
		ResultSet rs = null;
		try {
			rs = stmt.executeQuery("VALUES SYSCS_UTIL.GET_DATABASE_PROPERTY('gemfirexd.distributedsystem.collation')");
			rs.next();
		...
		while (rs.next()) {
			...
			for (int i = 1; i <= cols; i++) {
				...
				// metadata for system objects are checked in).
					...
					(value.indexOf("com.pivotal.gemfirexd.internal") != -1) &&
		...
		TreeMap orderedRows)
	{
		...
		StringBuilder newRowId = new StringBuilder();
		...
		if (oldRow != null) {
			...
			StringBuilder oldRowId = new StringBuilder();
		...
		throws Exception
	{
		...
		StringBuilder dependsData = new StringBuilder();
