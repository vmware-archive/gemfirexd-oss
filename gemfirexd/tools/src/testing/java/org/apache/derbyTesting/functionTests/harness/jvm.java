...
import com.pivotal.gemfirexd.internal.impl.tools.sysinfo.ZipInfoProperties;
    ...
    */
	...
	public static String findCodeBase(boolean[] isJar)
	{
		...
		ZipInfoProperties zip[]= 
			com.pivotal.gemfirexd.internal.impl.tools.sysinfo.Main.getAllInfo (classpath);
		for (int i = 0; i < zip.length; i++)
		{
			// it's a url so should just have forward slashes
			String location = zip[i].getLocation().replace('\\','/');
			if (location.indexOf("gemfirexd.jar") != -1)
			{
				isJar[0] = true;
