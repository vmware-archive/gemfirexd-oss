...
// GemStone changes BEGIN
    ...
    protected static void preTearDown() {
      ...
      // locks before invoking cleanDatabase which will require write locks
      try {
        for (ContextManager cm : ContextService.getFactory().getAllContexts()) {
          LanguageConnectionContext lcc = (LanguageConnectionContext)cm
              .getContext(LanguageConnectionContext.CONTEXT_ID);
          if (lcc != null) {
            try {
              lcc.internalCommit(true);
            } catch (StandardException se) {
              // any exceptions here deliberately ignored
            }
          }
        }
      } catch (RuntimeException e) {
        // any runtime exceptions here ignored (e.g. ShutdownException)
      }
    }
...
// GemStone changes END
    ...
    */
    ...
    /**
     * Check the table using SYSCS_UTIL.CHECK_TABLE.
     */
