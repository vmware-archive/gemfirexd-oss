...
public class OnlineBackupTest3 {
    ...
    void installJarTest() throws Exception{
        ...
        try {
           ...
           conn2_stmt.execute(
            "call SYSCS_UTIL.BACKUP_DATABASE_NOWAIT('extinout/mybackup')");
        } catch (SQLException sqle) {
        ...
        conn1_stmt.execute("CALL SYSCS_UTIL.SET_DATABASE_PROPERTY( " + 
        ...
        try {
            // set the database class with both the jars  installed above.
            stmt.execute("CALL SYSCS_UTIL.SET_DATABASE_PROPERTY( " + 
                           "'gemfirexd.distributedsystem.classpath', " + 
                           "'APP.customer_app:APP.math_routines') " ) ;
