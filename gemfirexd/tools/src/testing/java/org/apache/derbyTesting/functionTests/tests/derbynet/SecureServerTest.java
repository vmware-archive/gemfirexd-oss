...
public class SecureServerTest extends BaseJDBCTestCase
{
    ...
    // basic properties file which tests that properties are picked up from gemfirexd.properties
    private static  final   String  BASIC = "functionTests/tests/derbynet/SecureServerTest.gemfirexd.properties";
    ...
    public static Test suite()
    {
        ...
        // Need gemfirexd.jar in the classpath!
        if (!Derby.hasServer())
            return suite;
        ...
        )
    {
        ...
        // if using the custom gemfirexd.properties, copy the custom properties to a visible place
        if ( customDerbyProperties != null )
        {
            testSetup = new SupportFilesSetup
                (
                 testSetup,
                 null,
                 new String[] { "functionTests/tests/derbynet/SecureServerTest.gemfirexd.properties" },
                 null,
                 new String[] { "gemfirexd.properties" }
                 );
        }
    ...
    private static  String[]  getStartupProperties( boolean authenticationRequired, String customDerbyProperties )
    {
        ...
        {
            list.add( "gemfirexd.authentication.required=true" );
            list.add( "gemfirexd.authentication.provider=BUILTIN" );
            list.add( "gemfirexd.user." + SST_USER_NAME + "=" + SST_PASSWORD );
        }

        if ( customDerbyProperties != null )
        {
            list.add( "gemfirexd.system.home=extinout" );
        }
        ...
        throws Exception
    {
        ...
        buffer.append( " com.pivotal.gemfirexd.internal.drda.NetworkServerControl -p " + portNumber + " " + commandSpecifics );
