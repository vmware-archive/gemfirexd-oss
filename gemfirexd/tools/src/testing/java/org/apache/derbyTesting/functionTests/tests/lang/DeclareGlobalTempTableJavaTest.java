...
public class DeclareGlobalTempTableJavaTest extends BaseJDBCTestCase {
     ...
     */
    //GemFireXD has a know problem creating objects in SESSION schema
    //public void testDerby1706() throws SQLException {
    public void _testDerby1706() throws SQLException {
     ...
     * @throws SQLException
     */
    // GemFireXD has a problem with creating physical tables in SESSION schema
    //public void testFeaturesAllowedOnPhysicalTablesOfSESSIONSchema()
    public void _testFeaturesAllowedOnPhysicalTablesOfSESSIONSchema()
            throws SQLException {
        Statement s = createStatement();
        try {
     ...
     * @throws SQLException
     */
    //GemFireXD has a problem creating physical tables in SESSION
    //public void testConstraintsAllowedOnSESSIONPhysicalTables()
    public void _testConstraintsAllowedOnSESSIONPhysicalTables()
            throws SQLException {
        Statement s = createStatement();
        try {
            s.executeUpdate("CREATE SCHEMA SESSION");
	...
	JDBC.assertSingleValueResultSet(rs1 , "56");
        ...
        // [sjigyasu]
        // If the test is transactional, i.e. flag TEST_SWITCH_IL_AC is set, then the connection created would
        // have isolation level set as READ_COMMITTED.  The autocommit is set to false in the setup() of
        // this test. So t2 should already have been dropped because t2 was created within the transaction
        // and an insert failed above ("23505").  So verify that we get an error dropping the table.
        // Our global temp table documentation does not mention what the behavior should be in non-tx case.
        // So keep the test as is for that.
        if (isTransactional) {
          assertStatementError("42Y55", s, "DROP TABLE SESSION.t2");
          assertStatementError("42Y55", s, "DROP TABLE SESSION.t3");
        } else {
          assertUpdateCount(s , 0 , "DROP TABLE SESSION.t2");
          assertUpdateCount(s , 0 , "DROP TABLE SESSION.t3");
        }
    ...
    public void testCreateView() throws SQLException {
        ...
        // Commit the changes first since GFXD does not allow DDL statements in the middle of
        // a transaction if there are pending changes to be committed/rolled back.
     ...
     *                Thrown if some unexpected error happens
     */
    //GemFireXD has a problem creating physical tables in SESSION schema
    //public void testStatementCaching() throws SQLException {
    public void _testStatementCaching() throws SQLException {
        Statement s = createStatement();
     ...
     *                Thrown if some unexpected error happens
     */
    //GemFireXD has a problem if DGTT used in statement is dropped
    //public void testSameGTTNameForTwoConnections() throws SQLException {
    public void _testSameGTTNameForTwoConnections() throws SQLException {
        Statement s1 = createStatement();
     ...
     * @throws SQLException
     */
    //GemFireXD has a problem with DGTT in prepared statement being dropped
    //public void testPreparedStatement1() throws SQLException {
    public void _testPreparedStatement1() throws SQLException {
        Statement s = createStatement();
    ...
    public void _testEntryForSESSIONTablesToSysCatalog() throws SQLException {
        Statement s = createStatement();
        assertUpdateCount(s , 0 , "DECLARE GLOBAL TEMPORARY TABLE SESSION.t2(c21 int) on commit delete rows not logged");
     ...
     * @throws SQLException
     */
    public void testEntryForSESSIONtoSysCatalog() throws SQLException {
        Statement s = createStatement();
        try {
            s.executeUpdate("drop schema SESSION restrict");
        } catch (SQLException e) {
    ...
    public void _testDatabaseMetadata() throws SQLException {
        Statement s = createStatement();
        int count = 0;
     ...
     * @throws SQLException
     */
    //GemFireXD has a problem with WHERE CURRENT OF on DGTT
    //public void testDeleteWhereCurrentOfOnGTT() throws SQLException {
    public void _testDeleteWhereCurrentOfOnGTT() throws SQLException {
        Statement s = createStatement();
        assertUpdateCount(s , 0 , "DECLARE GLOBAL TEMPORARY TABLE SESSION.t2(c21 int, c22 int) on commit delete rows not logged");
