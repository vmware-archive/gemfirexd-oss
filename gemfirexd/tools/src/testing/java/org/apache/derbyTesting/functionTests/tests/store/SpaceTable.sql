...
select conglomeratename, isindex, numallocatedpages, numfreepages, pagesize, estimspacesaving
	from new com.pivotal.gemfirexd.internal.diag.SpaceTable('IDELETEU') t
...
select conglomeratename, isindex, numallocatedpages, numfreepages, pagesize, estimspacesaving
	from new com.pivotal.gemfirexd.internal.diag.SpaceTable('PLATYPUS') t
	order by conglomeratename; 
...
select conglomeratename, isindex, numallocatedpages, numfreepages, numunfilledpages, pagesize, estimspacesaving
	from new com.pivotal.gemfirexd.internal.diag.SpaceTable('NONEXISTING') t
	order by conglomeratename; 
...
select conglomeratename, isindex, numallocatedpages, numfreepages, pagesize, estimspacesaving
	from new com.pivotal.gemfirexd.internal.diag.SpaceTable('platypus2') t
	order by conglomeratename; 

select conglomeratename, isindex, numallocatedpages, numfreepages, pagesize, estimspacesaving
    from SYS.SYSSCHEMAS s,
         SYS.SYSTABLES t,
         new com.pivotal.gemfirexd.internal.diag.SpaceTable(SCHEMANAME,TABLENAME) v
...
drop table foo_int;
call SYSCS_UTIL.SET_DATABASE_PROPERTY('gemfirexd.storage.pageReservedSpace','65');
create table foo_int (a int);
drop table foo_char;
create table foo_char (a char(100));
drop table foo_varchar;
...
--  8K pagesize 
call SYSCS_UTIL.SET_DATABASE_PROPERTY('gemfirexd.storage.pageSize', '8192');
drop table foo_int;
create table foo_int (a int);
drop table foo_char;
create table foo_char (a char(100));
