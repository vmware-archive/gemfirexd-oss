...
public class LangScripts_SchemaDUnit extends DistributedSQLTestBase {

	public LangScripts_SchemaDUnit(String name) {
		super(name);
	  ...
	  public void testLangScript_SchemasTestNoPartitioning() throws Exception
	  {
		...
		{ "create schema app", "X0Y68" },
	     ...
	     // DUnit uses random schema names, so comparing won't work
		...
		{ "create schema x", null },
		 ...
		 // FIXME
		 // This gives an odd 'no projection columns' assertion and in non-debug builds a 'TEST.S not visible' error
		 // But only for partitioned tables with data
	     //{ "select * from test.s where exists (select test.s.* from test.t)", new String[][] {
		 //	{null,null,null,null},{"0","0","0","0"},{"1","1","1","1"} } },
		 // FIXME
		 // Dropping the GTT here throws a rollback exception, let's not even create one
	  ...
	  public void tearDown2() throws Exception {
	    // need to cleanup "create schema app" else following tests
	    // keep trying to create it on restart which is already present
	    Connection conn = TestUtil.getConnection();
            Statement stmt = conn.createStatement();
            JDBC.dropSchema(conn.getMetaData(), "APP");
            stmt.execute("drop schema app RESTRICT");
            super.tearDown2();
	  }
}
