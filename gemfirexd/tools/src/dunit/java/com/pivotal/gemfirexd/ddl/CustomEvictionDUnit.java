...
public class CustomEvictionDUnit extends DistributedSQLTestBase {
  ...
  public void testHDFSEvictionGlobalIndex() throws Exception {
    ...
    try {
      checkDirExistence("./myhdfs");
      stmt.execute("create hdfsstore myhdfs namenode 'localhost' homedir './myhdfs' ");
      stmt.execute("create table trade.portfolio (cid int not null, sid int not null, qty int not null, availQty int not null, subTotal decimal(30,20), tid int, constraint portf_pk primary key (cid, sid), constraint qty_ck check (qty>=0), constraint avail_ch check (availQty>=0 and availQty<=qty)) persistent hdfsstore (myhdfs) "
  ...
  public void testHDFSEvictionHA() throws Exception {
    ...
    try {
      checkDirExistence("./myhdfs");
      stmt.execute("create hdfsstore myhdfs namenode 'localhost' homedir './myhdfs' ");
      stmt.execute("create table trade.portfolio (cid int primary key, sid int not null) BUCKETS 1 redundancy 2 persistent hdfsstore (myhdfs) "
          + "EVICTION BY CRITERIA ( sid > 500  ) EVICT INCOMING ");
