...
/**
 * Copied from InsertUpdateForeignKey
 * @author sbawaska
 *
 */
public class InsertUpdateHDFSDUnit extends DistributedSQLTestBase {

  private static final long serialVersionUID = -8734462070204243295L;

  public InsertUpdateHDFSDUnit(String name) {
    super(name);
  }
  ...
  public void testInsertOnHDFS() throws Exception {
    try {
      startVMs(1, 1);

      clientSQLExecute(1, "drop hdfsstore if exists "+getName());
      clientSQLExecute(1, "create schema hdfs");
      clientSQLExecute(1, "create hdfsstore "+getName()+" namenode 'localhost' homedir './"+getName()+"' batchtimeinterval 500 milliseconds");
      clientSQLExecute(1, "create table hdfs.t1(id int not null, col1 int,  primary key(id)) "+persistent()+" hdfsstore ("+getName() + ')');
      for (int i=0; i<200; i++) {
        clientSQLExecute(1, "insert into hdfs.t1 values("+i+", "+i+")");
      }
      ...
      checkKeyViolation(1, "insert into hdfs.t1 values(1, 990)");
      //checkKeyViolation(1, "update t1 set id = 100 where id=2");

      removeExpectedException(new int[] { 1 }, new int[] { 1 }, new Object[] {
          FunctionExecutionException.class,
      "java.sql.SQLIntegrityConstraintViolationException" });

      clientSQLExecute(1, "drop table hdfs.t1");
      clientSQLExecute(1, "drop hdfsstore "+getName());
    } finally {
      TestUtil.shutDown();
    }
  }

  public void testUpdateOnHDFS() throws Exception {
    try {
      startVMs(1, 1);
      ...
      Statement s = TestUtil.getStatement();
      int numUpdates = s.executeUpdate("update hdfs.t1 set col1 = 100 where id=3");
      assertEquals(1, numUpdates);

      ResultSet rs = s.executeQuery("select col1 from hdfs.t1 -- GEMFIREXD-PROPERTIES queryHDFS=true \n where id=3");
      assertTrue(rs.next());
  ...
  public void testDeleteOnHDFS() throws Exception {
    try {
      startVMs(1, 1);
      ...
      Statement s = TestUtil.getStatement();
      int numUpdates = s.executeUpdate("delete from hdfs.t1 where id=4");
      assertEquals(1, numUpdates);
   ...
   */
  public void testForeignKeyInsert() throws Exception {
    try {
      startVMs(1, 1);
      ...
      clientSQLExecute(1, "create schema hdfs");
      clientSQLExecute(1, "create hdfsstore "+getName()+" namenode 'localhost' homedir './"+getName()+"' batchtimeinterval 500 milliseconds");
      clientSQLExecute(1, "create table hdfs.t1(id int not null, primary key(id)) "+persistent()+" hdfsstore ("+getName() + ')');
      clientSQLExecute(1,
          "create table hdfs.t2(id int not null, fkId int not null, "
              + "primary key(id), foreign key (fkId) references hdfs.t1(id))");
      // enable this after fixing 48641
      //      clientSQLExecute(1,
      //          "create table hdfs.t2(id int not null, fkId int not null, "
      //              + "primary key(id), foreign key (fkId) references hdfs.t1(id)) "+persistent()+" hdfsstore("+getName()+")");
      clientSQLExecute(1, "insert into hdfs.t1 values(1)");
      clientSQLExecute(1, "insert into hdfs.t1 values(2)");
      clientSQLExecute(1, "insert into hdfs.t2 values(1, 1)");

      shutDown();
      startVMs(1, 1);

      clientSQLExecute(1, "insert into hdfs.t2 values(2,2)");
      ...
      for (int i = 20; i < 30; i++) {
        ...
        checkFKViolation(i % 2 == 0 ? 1 : -1, "insert into hdfs.t2 values(2, " + i
            + " )");
      }
      ...
      clientSQLExecute(1, "drop table hdfs.t2");
      clientSQLExecute(1, "drop table hdfs.t1");
      clientSQLExecute(1, "drop hdfsstore if exists "+getName());
    } finally {
      TestUtil.shutDown();
  ...
  private void shutDown() throws Exception {
    pause(5000); // wait for data to be flushed to HDFS
    shutDownAll();
  }
   ...
   */
  public void testForeignKeyUpdate() throws Exception {
    try {
      ...
      clientSQLExecute(1, "update hdfs.t2 set hdfs.t2.fkId=2 where hdfs.t2.id=1");
      ...
      for (int i = 20; i < 30; i++) {
        checkFKViolation(i % 2 == 0 ? 1 : -1, "update hdfs.t2 set hdfs.t2.fkId=" + i
            + " where hdfs.t2.id=1");
      }
   ...
   */
  public void _testInsertForeignTablePartitionedByColumnNotPk()
      throws Exception {
    startVMs(1, 1);
   ...
   */
  public void _testServerGroupsReplicatedAccessorFK() throws Exception {
   ...
   */
  public void _test40171() throws Exception {
    startVMs(1, 1);
   ...
   * @throws Exception
   */
  public void _testIndexScans () throws Exception {
    startVMs(2, 2);
    Connection conn = TestUtil.jdbcConn;
    Statement s = conn.createStatement();
   ...
   * @throws Exception
   */
  public void _testBug41168() throws Exception {
  ...
  public void _testBug43104() throws Exception {
    startVMs(1, 1);
    Connection conn = TestUtil.jdbcConn;
    Statement s = conn.createStatement();
