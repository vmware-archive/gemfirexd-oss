...
public class LangScripts_DerivedDUnit extends DistributedSQLTestBase {
	
	public LangScripts_DerivedDUnit(String name) {
	    super(name);
	}
	
	  public void testLangScript_DerivedTest() throws Exception
	  {
		...
		{ "select * from s ss (f, e, d, c, b, a) where f = 0", new String [][] {
			{"0","1","2","3","4","5"} } },
		// FIXME
		// Throws colocation exception even though this is a self-join
		...
		{ "create table tab2(tab2_c1 int, tab2_c2 int) partition by column(tab2_c1) colocate with (tab1)", null },
		{ "insert into tab1 values (1, 1), (2, 2)", null },
		{ "insert into tab2 values (1, 1), (2, 2)", null },
		// FIXME
		// This also throws colocation exception, even though the two tables are explicitly colocated!
