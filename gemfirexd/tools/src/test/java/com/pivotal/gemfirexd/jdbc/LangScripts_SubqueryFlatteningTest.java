...
public class LangScripts_SubqueryFlatteningTest extends JdbcTestBase {

  public static void main(String[] args) {
    TestRunner.run(new TestSuite(LangScripts_SubqueryFlatteningTest.class));
  }
  
  public LangScripts_SubqueryFlatteningTest(String name) {
    super(name); 
  }

  // This test is the as-is LangScript conversion, without any partitioning clauses
  public void testLangScript_SubqueryFlatteningTestNoPartitioning() throws Exception
  {
	...
	{ "create table outer1 (c1 int, c2 int, c3 int)" + getOffHeapSuffix(), null },
	{ "create table outer2 (c1 int, c2 int, c3 int)"+ getOffHeapSuffix(), null },
	{ "create table noidx (c1 int)"+ getOffHeapSuffix(), null },
	{ "create table idx1 (c1 int primary key)"+ getOffHeapSuffix(), null },
	{ "create unique index idx1_1 on idx1(c1)", null },
     { "create table idx2 (c1 int, c2 int primary key)"+ getOffHeapSuffix(), null },
     { "create unique index idx2_1 on idx2(c1, c2)", null },
     { "create table nonunique_idx1 (c1 int)"+ getOffHeapSuffix(), null },
	...
	{ "create table business(businesskey int, name varchar(50), changedate int)"+ getOffHeapSuffix(), null },
     { "create table nameelement(parentkey int, parentelt varchar(50), seqnum int)"+ getOffHeapSuffix(), null },
     { "create table categorybag(cbparentkey int, cbparentelt varchar(50), krtModelKey varchar(50), keyvalue varchar(50))"+ getOffHeapSuffix(), null },
	...
	{ "drop table outer1", null },
     ...
     { "CREATE TABLE APP.COLLS (ID VARCHAR(128) NOT NULL, COLLID SMALLINT NOT NULL)"+ getOffHeapSuffix(), null },
     { "CREATE INDEX APP.NEW_INDEX3 ON APP.COLLS (COLLID)", null },
     { "CREATE INDEX APP.NEW_INDEX2 ON APP.COLLS (ID)", null },
     { "ALTER TABLE APP.COLLS ADD CONSTRAINT NEW_KEY2 UNIQUE (ID, COLLID)", null },
     { "CREATE TABLE APP.DOCS (ID VARCHAR(128) NOT NULL)"+ getOffHeapSuffix(), null },
     { "CREATE INDEX APP.NEW_INDEX1 ON APP.DOCS (ID)", null },
     { "ALTER TABLE APP.DOCS ADD CONSTRAINT NEW_KEY1 PRIMARY KEY (ID)", null },
	...
	{ "CREATE TABLE APP.COLLS (ID VARCHAR(128), COLLID SMALLINT NOT NULL)"+ getOffHeapSuffix(), null },
     { "CREATE INDEX APP.NEW_INDEX3 ON APP.COLLS (COLLID)", null },
     { "CREATE INDEX APP.NEW_INDEX2 ON APP.COLLS (ID)", null },
     { "insert into colls values ('123', 2)", null },
     { "insert into colls values ('124', -5)", null },
	...
	{ "drop table docs", null },
     { "CREATE TABLE APP.DOCS (ID VARCHAR(128))"+ getOffHeapSuffix(), null },
     { "CREATE INDEX APP.NEW_INDEX1 ON APP.DOCS (ID)", null },
     { "insert into docs values '24', '25', '36', '27', '124', '567'", null },
	...
	{ "SELECT count(ID) FROM DOCS WHERE ID > ANY (SELECT ID FROM COLLS WHERE COLLID IN (-2,1) )", new String[][] { {"4"} } },
	{ "SELECT count(ID) FROM DOCS WHERE ID <> ALL (SELECT ID FROM COLLS WHERE COLLID IN (-2,1) )", new String[][] { {"0"} } },
     { "create table t1 (c1 int not null)"+ getOffHeapSuffix(), null },
     { "create table t2 (c1 int not null)"+ getOffHeapSuffix(), null },
     { "create table t3 (c1 int not null)"+ getOffHeapSuffix(), null },
     { "create table t4 (c1 int)"+ getOffHeapSuffix(), null },
     ...
     { "drop table t3", null },
     { "drop table t4", null },
	{ "create table digits (d int)"+ getOffHeapSuffix(), null },
     { "insert into digits values 1, 2, 3, 4, 5, 6, 7, 8, 9, 0", null },
     { "create table odd (o int)"+ getOffHeapSuffix(), null },
	...
	{ "drop table digits", null },
     { "drop table odd", null },
	{ "CREATE TABLE tab_a (PId BIGINT NOT NULL)"+ getOffHeapSuffix(), null },
     { "CREATE TABLE tab_c (Id BIGINT NOT NULL PRIMARY KEY,  PAId BIGINT NOT NULL, PBId BIGINT NOT NULL)"+ getOffHeapSuffix(), null },
     ...
     { "CREATE TABLE tab_v (OId BIGINT NOT NULL, UGId BIGINT NOT NULL, val CHAR(1) NOT NULL)"+ getOffHeapSuffix(), null },
     ...
     { "CREATE TABLE tab_b (Id BIGINT NOT NULL PRIMARY KEY, OId BIGINT NOT NULL)"+ getOffHeapSuffix(), null },
     ...
     { "CREATE TABLE tab_d (Id BIGINT NOT NULL PRIMARY KEY, PAId BIGINT NOT NULL, PBId BIGINT NOT NULL)"+ getOffHeapSuffix(), null },
  ...
  public void testLangScript_SubqueryFlatteningWithPartitioning() throws Exception
  {
    ...
    Object[][] Script_SubqueryFlatteningUTPartitioning = {
    ...
    // This will fail on the first one that succeeds where it shouldn't
    // or throws unknown exception
    JDBC.SQLUnitTestHelper(stmt,Script_SubqueryFlatteningUTPartitioning);
