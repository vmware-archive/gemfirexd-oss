...
public class LangScripts_OuterJoinTest extends JdbcTestBase {

  public static void main(String[] args) {
    TestRunner.run(new TestSuite(LangScripts_OuterJoinTest.class));
  }
  
  public LangScripts_OuterJoinTest(String name) {
    super(name); 
  }
  ...
  public void testLangScript_OuterJoinTestNoPartitioning() throws Exception
  {
	...
	// The original test checked the runtime plans, we don't do that for GemFireXD, just the result sets
	...
	{ "create table tt1(c1 int, c2 int, c3 int)"+ getOffHeapSuffix(), null },
	{ "create table tt2(c1 int, c2 int, c3 int)"+ getOffHeapSuffix(), null },
	{ "create table tt3(c1 int, c2 int, c3 int)"+ getOffHeapSuffix(), null },
	{ "create table empty_table(c1 int)"+ getOffHeapSuffix(), null },
	{ "create table insert_test(c1 int, c2 int, c3 int)"+ getOffHeapSuffix(), null },
	{ "create table oj(oj int)"+ getOffHeapSuffix(), null },
	...
	{ "select count(*) from insert_test", new String[][] { {"26"} } },
	{ "delete from insert_test", null },
	{ "create table a (c1 int)"+ getOffHeapSuffix(), null },
	{ "create table b (c2 float)"+ getOffHeapSuffix(), null },
	{ "create table c (c3 char(30))"+ getOffHeapSuffix(), null },
	...
	{ "select tt1.c2, tt1.c1, tt1.c3, tt2.c1, tt2.c3 from t1 left outer join tt1 on t1.c1 = tt1.c1 left outer join tt2 on tt1.c2 = tt2.c2", new String[][] {
		...
		{"3","3","3","3","3"},{"3","3","3","3","3"},{"4","4","4",null,null} } },
	{ "create table x (c1 int, c2 int, c3 int)"+ getOffHeapSuffix(), null },
	{ "create table y (c3 int, c4 int, c5 int)"+ getOffHeapSuffix(), null },
	...
	{ "CREATE TABLE inventory(itemno INT NOT NULL PRIMARY KEY, capacity INT)"+ getOffHeapSuffix(), null },
	{ "INSERT INTO inventory VALUES (1, 4)", null },
	{ "INSERT INTO inventory VALUES (2, 2)", null },
	{ "INSERT INTO inventory VALUES (3, 2)", null },
	{ "CREATE TABLE timeslots (slotno INT NOT NULL PRIMARY KEY)"+ getOffHeapSuffix(), null },
	{ "INSERT INTO timeslots VALUES(1)", null },
	{ "INSERT INTO timeslots VALUES(2)", null },
	{ "create table reservations(slotno INT CONSTRAINT timeslots_fk REFERENCES timeslots, itemno INT CONSTRAINT inventory_fk REFERENCES inventory, name VARCHAR(100), resdate DATE)"+ getOffHeapSuffix(),null },
	...
	{ "create table reservations(slotno INT CONSTRAINT timeslots_fk REFERENCES timeslots, itemno INT CONSTRAINT inventory_fk REFERENCES inventory,name VARCHAR(100))"+ getOffHeapSuffix(), null },
	...
	{ "CREATE TABLE properties (name VARCHAR(50),value VARCHAR(200))"+ getOffHeapSuffix(), null },
	...
	{ "CREATE TABLE timeslots (slot TIME NOT NULL PRIMARY KEY)"+ getOffHeapSuffix(), null },
	...
	{ "CREATE TABLE reservations (itemno INT CONSTRAINT inventory_fk REFERENCES inventory,slot TIME CONSTRAINT timeslots_fk REFERENCES timeslots,resdate DATE NOT NULL,name VARCHAR(100) NOT NULL,quantity INT,CONSTRAINT reservations_u UNIQUE(name, resdate))"+ getOffHeapSuffix(), null },
	...
	{ "create table xxx (a int not null)"+ getOffHeapSuffix(), null },
	{ "create table yyy (a int not null)"+ getOffHeapSuffix(), null },
	...
	{ "drop table xxx", null},
	{ "drop table yyy", null },
	{ "create table ttab1 (a int, b int)"+ getOffHeapSuffix(), null },
	{ "insert into ttab1 values (1,1),(2,2)", null },
	{ "create table ttab2 (c int, d int)"+ getOffHeapSuffix(), null },
	...
	{ "CREATE TABLE APP.GOVT_AGCY (GVA_ID NUMERIC(20,0) NOT NULL, GVA_ORL_ID NUMERIC(20,0) NOT NULL, GVA_GAC_ID NUMERIC(20,0))"+ getOffHeapSuffix(), null },
	{ "CREATE TABLE APP.GEO_STRC_ELMT (GSE_ID NUMERIC(20,0) NOT NULL, GSE_GSET_ID NUMERIC(20,0) NOT NULL, GSE_GA_ID_PRNT NUMERIC(20,0) NOT NULL, GSE_GA_ID_CHLD NUMERIC(20,0) NOT NULL)"+ getOffHeapSuffix(), null },
	{ "CREATE TABLE APP.GEO_AREA (GA_ID NUMERIC(20,0) NOT NULL, GA_GAT_ID NUMERIC(20,0) NOT NULL, GA_NM VARCHAR(30) NOT NULL, GA_ABRV_NM VARCHAR(5))"+ getOffHeapSuffix(), null},
	{ "CREATE TABLE APP.REG (REG_ID NUMERIC(20,0) NOT NULL, REG_NM VARCHAR(60) NOT NULL, REG_DESC VARCHAR(240), REG_ABRV_NM VARCHAR(15), REG_CD NUMERIC(8,0) NOT NULL, REG_STRT_DT TIMESTAMP NOT NULL, REG_END_DT TIMESTAMP NOT NULL, REG_EMPR_LIAB_IND CHAR(1) NOT NULL DEFAULT 'N',REG_PAYR_TAX_SURG_CRTF_IND CHAR(1) NOT NULL DEFAULT 'N', REG_PYT_ID NUMERIC(20,0), REG_GA_ID NUMERIC(20,0) NOT NULL, REG_GVA_ID NUMERIC(20,0) NOT NULL, REG_REGT_ID NUMERIC(20,0) NOT NULL, REG_PRNT_ID NUMERIC(20,0))"+ getOffHeapSuffix(), null },
  ...
  public void testLangScript_OuterJoinWithPartitioning() throws Exception
  {
    // This form of the OuterJoin.sql test has partitioning activated
    // TODO : Create specialized partitioning for these tables for OuterJoin
    //  Currently, default partitioning is active
    Object[][] Script_OuterJoinUTPartitioning = {
	// test outer joins (NO NATURAL JOIN)
	// The original test checked the runtime plans, we don't do that for GemFireXD, just the result sets
	{ "create table t1(c1 int)"+ getOffHeapSuffix(), null },
	{ "create table t2(c1 int)"+ getOffHeapSuffix(), null },
    ...
    // This will fail on the first one that succeeds where it shouldn't
    // or throws unknown exception
    JDBC.SQLUnitTestHelper(stmt,Script_OuterJoinUTPartitioning);
