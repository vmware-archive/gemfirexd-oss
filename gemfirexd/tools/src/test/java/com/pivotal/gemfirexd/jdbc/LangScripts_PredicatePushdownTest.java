...
public class LangScripts_PredicatePushdownTest extends JdbcTestBase {

  public static void main(String[] args) {
    TestRunner.run(new TestSuite(LangScripts_PredicatePushdownTest.class));
  }
  ...
  public LangScripts_PredicatePushdownTest(String name) {
    super(name); 
  }

  // This test is the as-is LangScript conversion, without any partitioning clauses
  public void testLangScript_PredicatePushdownTestNoPartitioning() throws Exception
  {
	...
	{ "CREATE TABLE APP.T1 (I INTEGER, J INTEGER)", null },
	{ "insert into t1 values (1, 2), (2, 4), (3, 6), (4, 8), (5, 10)", null },
	{ "CREATE TABLE APP.T2 (I INTEGER, J INTEGER)", null },
	{ "insert into t2 values (1, 2), (2, -4), (3, 6), (4, -8), (5, 10)", null },
	{ "CREATE TABLE APP.T3 (A INTEGER, B INTEGER)", null },
	...
	{ "CREATE TABLE APP.T4 (A INTEGER, B INTEGER)", null },
	...
	{ "select * from (select * from t1 union all select * from t2) x1, (select * from t3 union all select * from t4) x2 where x1.i = x2.a", new String[][] {
		...
		{"1","2","1","1"},{"2","-4","2","2"},{"3","6","3","3"},
		{"3","6","3","12"},{"4","-8","4","4"},{"4","-8","4","16"} } },
	{ "create table tc (c1 char, c2 char, c3 char, c int)", null },
	...
	{ "CREATE INDEX APP.T3_IX1 ON APP.T3 (A)", null },
     { "CREATE INDEX APP.T3_IX2 ON APP.T3 (B)", null },
     { "CREATE INDEX APP.T4_IX1 ON APP.T4 (A)", null },
     { "CREATE INDEX APP.T4_IX2 ON APP.T4 (B)", null },
	{ "CREATE TABLE APP.T5 (I INTEGER, J INTEGER)", null },
     { "insert into t5 values (5, 10)", null },
     { "CREATE TABLE APP.T6 (P INTEGER, Q INTEGER)", null },
     { "insert into t5 values (2, 4), (4, 8)", null },
     { "CREATE TABLE APP.XX1 (II INTEGER NOT NULL, JJ CHAR(10), MM INTEGER, OO DOUBLE, KK BIGINT)", null },
     { "CREATE TABLE APP.YY1 (II INTEGER NOT NULL, JJ CHAR(10), AA INTEGER, OO DOUBLE, KK BIGINT)", null },
     { "ALTER TABLE APP.YY1 ADD CONSTRAINT PK_YY1 PRIMARY KEY (II)", null },
     { "ALTER TABLE APP.XX1 ADD CONSTRAINT PK_XX1 PRIMARY KEY (II)", null },
	...
	{ "drop view v1", null },
     ...
     { "drop table xx1", null },
     { "drop table yy1", null },
	{ "CREATE TABLE APP.T1 (I INTEGER, D DOUBLE, C CHAR(10))", null },
     { "CREATE TABLE APP.T2 (I2 INTEGER, D2 DOUBLE, C2 CHAR(10))", null },
     { "CREATE TABLE APP.T3 (I3 INTEGER, D3 DOUBLE, C3 CHAR(10))", null },
     ...
     { "CREATE TABLE APP.T4 (C4 CHAR(10))", null },
     { "insert into t4 values '1', '2', '3', '4', '5', '6', '7', '8', '9'", null },
     { "insert into t4 select rtrim(c4) || rtrim(c4) from t4", null },
     { "CREATE TABLE APP.T5 (I5 INTEGER, D5 DOUBLE, C5 CHAR(10))", null },
     { "CREATE TABLE APP.T6 (I6 INTEGER, D6 DOUBLE, C6 CHAR(10))", null },
  ...
  public void testLangScript_PredicatePushdownWithPartitioning() throws Exception
  {
    // This form of the PredicatePushdown.sql test has partitioning activated
    // TODO : Create specialized partitioning for these tables for PredicatePushdown
    //  Currently, default partitioning is active
    Object[][] Script_PredicatePushdownUTPartitioning = {
	// The original test checked against runtime plans. GemFireXD will not, just check results.
	{ "CREATE TABLE APP.T1 (I INTEGER, J INTEGER)", null },
	{ "insert into t1 values (1, 2), (2, 4), (3, 6), (4, 8), (5, 10)", null },
	{ "CREATE TABLE APP.T2 (I INTEGER, J INTEGER)", null },
    ...
    // This will fail on the first one that succeeds where it shouldn't
    // or throws unknown exception
    JDBC.SQLUnitTestHelper(stmt,Script_PredicatePushdownUTPartitioning);
