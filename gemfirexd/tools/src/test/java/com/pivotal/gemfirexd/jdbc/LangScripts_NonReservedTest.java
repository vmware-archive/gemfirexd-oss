...
public class LangScripts_NonReservedTest extends JdbcTestBase {

  public static void main(String[] args) {
    TestRunner.run(new TestSuite(LangScripts_NonReservedTest.class));
  }
  
  public LangScripts_NonReservedTest(String name) {
    super(name); 
  }
  ...
  public void testLangScript_NonReservedWithPartitioning() throws Exception
  {
    // This form of the NonReserved.sql test has partitioning activated
    // TODO : Create specialized partitioning for these tables for NonReserved
    //  Currently, default partitioning is active
    Object[][] Script_NonReservedUTPartitioning = {
	// This tests that SQL92 formally reserved words are now unreserved
	// INTERVAL
	{ "create table interval(interval int)", null },
	{ "create index interval on interval(interval)", null },
    ...
    // This will fail on the first one that succeeds where it shouldn't
    // or throws unknown exception
    JDBC.SQLUnitTestHelper(stmt,Script_NonReservedUTPartitioning);
