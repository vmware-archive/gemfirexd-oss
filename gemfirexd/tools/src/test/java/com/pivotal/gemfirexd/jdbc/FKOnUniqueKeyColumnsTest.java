...
public class FKOnUniqueKeyColumnsTest extends JdbcTestBase {

  public FKOnUniqueKeyColumnsTest(String name) {
    super(name);
  }

  private static String REPLICATE = "replicate";
  private static String PARTITION = "";
  
  public static String getBaseType(int i) {
    switch (i) {
      case 0:
        return REPLICATE;
      case 1:
  ...
  public static String getReferBaseType(int i) {
    switch (i) {
      case 0:
        return REPLICATE;
      case 1:
  ...
  public void testFKOnUniqueColumnsWhichIsInGlobalIndex() throws Exception {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      String baseTableType = getBaseType(i);
      String referbaseTableType = getReferBaseType(i);
  ...
  /**
   * This is the case where PK columns are a superset of partitioning columns.
   */
  public void testFKOnPKColumnsForSupersetPartitioning() throws Exception {
    Connection conn = getConnection();
    Statement s = conn.createStatement();
    for (int i = 0; i < 2; i++) {
      System.out.println("testFKOnPKColumnsForSupersetPartitioning " + i);
      String referbaseTableType = getReferBaseType(i);
      s.execute("create table base"
          + "(c1 int not null, c11 varchar(10), c12 int, "
          + "c2 int not null, c3 varchar(20) not null, primary key (c2, c3)) "
          + "partition by column(c3)");
      s.execute("insert into base values(1, '1', 1, 2, 'name1'), "
          + "(11, '11', 1, 12, 'name2'), (21, '21', 21, 22, 'name3'), "
          + "(31, '31', 31, 32, 'name4')");
