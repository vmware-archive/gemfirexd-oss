...
public class RuntimeStatisticsParser {
    ...
    public boolean rowsQualifiedEquals(int qualRows)
    {
      ...
      boolean retVal = (statistics.indexOf("Number of rows qualified=" + qualRows
          + "\n") != -1);
      if (retVal == false)
        BaseTestCase
            .println("RuntimeStatisticsParser#rowsQualifiedEquals expects "
                + qualRows + " rows qualified in " + statistics);
      return retVal;
      /*(originally) return (statistics.indexOf("Number of rows qualified=" +
           qualRows + "\n") != -1); */
