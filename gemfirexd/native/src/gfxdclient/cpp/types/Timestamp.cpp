...
namespace _gfxd_impl
{
  ...
  public:
    ...
    typedef boost::date_time::nano_res::fractional_seconds_type fractional_seconds_type;
...
Timestamp Timestamp::parseString(const std::string& str, const bool utc,
  ...
  try {
    ...
    _gfxd_impl::nano_time_duration td =
        boost::date_time::parse_delimited_time_duration<
            _gfxd_impl::nano_time_duration>(timeStr);
...
std::ostream& operator <<(std::ostream& stream, Timestamp ts) {
  if (boost::chrono::get_timezone(stream)
      == boost::chrono::timezone::utc) {
