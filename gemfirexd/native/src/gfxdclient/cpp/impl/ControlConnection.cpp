...
ControlConnection::ControlConnection(const ClientService& service) :
    m_gfxdServerType(service.m_reqdServerType), m_gfxdServerTypeSet(
      ...
      try {
        ...
        if (forFailover) {
          ...
          preferredServer = controlLocator.getPreferredServer(
              this.gfxdServerTypeSet, this.serverGroups, failedServers);
        ...
        if (preferredServer.getPort() <= 0) {
          ...
          SearchRandomServer search = new SearchRandomServer(this.gfxdServerType,
    ...
    const std::exception* failure)
{
    ...
    boost::shared_ptr<protocol::TProtocol> protocol;
      while (true) {
        try {
          ...
          if (m_gfxdServerType.isThriftSSL()) {
            transport = new GfxdTSSLSocket(controlAddr, this.sslParams);
          }
          else {
            transport = new GfxdTSocket(controlAddr);
          }
          if (this.gfxdServerType.isThriftBinaryProtocol()) {
  ...
  private GFXDException unexpectedError(Throwable t, HostAddress host) {
    ...
    return ClientExceptionUtil.newGFXDException(SQLState.JAVA_EXCEPTION, t,
        t.getClass(), t.getMessage() + " (Server=" + host + ')');
  }

  private GFXDException failoverExhausted(Throwable cause) {
    return ClientExceptionUtil.newGFXDException(
