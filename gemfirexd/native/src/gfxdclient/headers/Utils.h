...
#define AUTOPTR_UNWRAP(T) \
    typename com::pivotal::gemfirexd::client::UnwrapAutoPtr<T>::type
    ...
    namespace gemfirexd
    {
      ...
      namespace functor
      {
        struct WriteStream
        {
          ...
          void operator()(const char* buf, const size_t bufLen)
          {
      ...
      public:
        DestroyArray(const ARR_TYPE* arr) :
            m_arr(arr)
        {
        }
        ~DestroyArray()
      ...
      namespace client
      {
        ...
        public:
          ...
          inline static float int32ToFloat(int32_t i) {
            float2int_ u;
            u.m_i32 = i;
            return u.m_f;
          }

          inline static int32_t float2Int32(float f) {
            float2int_ u;
            u.m_f = f;
            return u.m_i32;
          }
          ...
          {
            return (serverType == thrift::ServerType::THRIFT_GFXD_CP)
          ...
          static void printFullMessage(const std::exception& stde,
              std::string& str);

          static std::ostream& printFullMessage(const std::exception& stde,
              std::ostream& out);
...
template<typename TPROC>
bool com::pivotal::gemfirexd::client::Utils::convertUTF8ToUTF16(
...
template<typename TWCHAR, typename TPROC>
void com::pivotal::gemfirexd::client::Utils::convertUTF16ToUTF8(
...
std::ostream& operator <<(std::ostream& out,
  const com::pivotal::gemfirexd::thrift::ServerType::type serverType);

std::ostream& operator<<(std::ostream& out,
  const com::pivotal::gemfirexd::client::_SqleHex hexstr);

std::ostream& operator <<(std::ostream& out,
  const com::pivotal::gemfirexd::thrift::HostAddress& hostAddr);
