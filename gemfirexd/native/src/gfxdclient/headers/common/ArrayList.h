  ...
  namespace pivotal {
    namespace gemfirexd {
      ...
      public:
        ...
        ArrayList() :
            m_list(NULL), m_capacity(0), m_size(0) {
        }
        ...
        // C++11 move constructor, move assignment operator and for push_back
#if __cplusplus >= 201103L
        ArrayList(ArrayList&& other) :
            m_list(other.m_list), m_capacity(other.m_capacity),
            m_size(other.m_size) {
          other.m_list = NULL;
          other.m_capacity = 0;
          other.m_size = 0;
...
template<typename T>
void com::pivotal::gemfirexd::ArrayList<T>::init(const size_t capacity) {
  if (capacity != 0) {
...
template<typename T>
void com::pivotal::gemfirexd::ArrayList<T>::changeCapacity(
...
template<typename T>
void com::pivotal::gemfirexd::ArrayList<T>::shrinkCapacity() {
...
template<typename T>
com::pivotal::gemfirexd::ArrayList<T>::~ArrayList() {
  if (m_list != NULL) {
    if (m_size > 0) {
      _clear();
    }
...
template<typename T>
template<typename TIter>
void com::pivotal::gemfirexd::ArrayList<T>::push_back(TIter start,
...
template<typename T>
size_t com::pivotal::gemfirexd::ArrayList<T>::capacity(const size_t newSize) {
  const size_t capacity = m_capacity;
  if (newSize != capacity) {
...
template<typename T>
void com::pivotal::gemfirexd::ArrayList<T>::resize(const size_t newSize) {
...
template<typename T>
void com::pivotal::gemfirexd::ArrayList<T>::reserve(const size_t newSize) {
  if (newSize > m_capacity) {
    changeCapacity(newSize);
  }
}
...
template<typename T>
void com::pivotal::gemfirexd::ArrayList<T>::_clear() {
  T* tp = m_list;
...
template<typename T>
bool com::pivotal::gemfirexd::ArrayList<T>::operator==(
...
namespace std {
  template<typename T>
  inline void swap(com::pivotal::gemfirexd::ArrayList<T> list1,
      com::pivotal::gemfirexd::ArrayList<T>& list2) {
    list1.swap(list2);
  }
}
...
std::ostream& operator<<(std::ostream& out,
    const com::pivotal::gemfirexd::ArrayList<T>& list) {
  out << '(';
  if (list.size() > 0) {
    typename com::pivotal::gemfirexd::ArrayList<T>::const_iterator tp =
        list.cbegin();
    typename com::pivotal::gemfirexd::ArrayList<T>::const_iterator end =
        list.cend();
    out << *tp;
    while (++tp != end) {
      out << ',';
