 ...
 * com.pivotal.gemfirexd.internal.modules.properties
 *
 * <P>
...
public class propertyconfig {
	...
	/**
	 * For each module with a particular tag in gemfirexd.module.<tag>, see if
					...
					throws IOException
	{
		...
		// copy this code from
		// com.pivotal.gemfirexd.internal.impl.services.monitor.BaseMonitor 
		//
		for (Enumeration e = moduleList.propertyNames(); e.hasMoreElements(); ) 
		{
			...
			{
				String tag = key.substring("gemfirexd.module.".length());
				...
				if (match)
				{
					...
					// gemfirexd.env.classes.<tag>
					String envKey = "gemfirexd.env.classes.".concat(tag);
					if (moduleList.getProperty(envKey) != null)
						outputProp.put(envKey, moduleList.getProperty(envKey));

					// gemfirexd.env.jdk.<tag>
					// 
					...
					// gemfirexd.env.jdk.<name>J1 or
					// gemfirexd.env.jdk.<name>J2
					...
					//
					//gemfirexd.env.jdk.<tag>
					envKey = "gemfirexd.env.jdk.".concat(tag);
					...
					if (moduleList.getProperty(envKey) != null)
					{
						...
						if (tag.endsWith("J1") || tag.endsWith("J2"))
						{
							...
							String alternateImplKey = 
								"gemfirexd.module."+ alternateTag;
							String alternateJDKEnv = 
								"gemfirexd.env.jdk."+ alternateTag;
								...
								(moduleList.getProperty(alternateImplConfigKey) != null))
							{
								...
								// gemfirexd.module.<tag>J2 and
								// gemfirexd.module.<tag>J1 and
								// gemfirexd.env.jdk.<tag>J2 and
								// gemfirexd.env.jdk.<tag>J1 and
	...
	private static void printUsageAndExit()
	{
		...
		buf.append("Usage propertyConfig <masterFile> <config> <outputFile>\n")
			...
			.append("com.pivotal.gemfirexd.internal.modules.properties.\n")
