...
public class MemberLevelStatsJUnitTest extends MBeanStatsTestCase {
  ...
  public MemberLevelStatsJUnitTest(String name) {
    super(name);
  }
  ...
  public void init() {
    ...
    for(int i = 0; i<4 ; i++){
      PartitionedRegionStats stats = new PartitionedRegionStats(system, getName()+i);
  ...
  private static long toSeconds(long duration) {
    return nanoSeconds.toSeconds(duration);
  }
  
  private static long toMillis(long duration) {
  ...
  public void testPlainCounters() throws InterruptedException {
    ...
    cachePerfStats.incGetInitialImageKeysReceived(1);
