...
public class HfileSortedOplogJUnitTest extends BaseHoplogTestCase {
  ...
  public void testReaderDetechAndUseRecycledFs() throws Exception {
    ...
    HFileSortedOplog hop = new HFileSortedOplog(store1, new Path(getName()+"-"+time+"-1.hop"), blockCache, stats, storeStats);
  ...
  public void testNewScannerDetechAndUseRecycledFs() throws Exception {
    ...
    hop = new HFileSortedOplog(store1, new Path(getName()+"-"+time+"-1.hop"), blockCache, stats, storeStats);
    hop.getReader();
  ...
  /*
   * Verifies that FS failure causes scanner to fail and generate io error
   */
  public void testRecycleFsWhileScanning() throws Exception {
    ...
    try {
      ...
      try {
        ...
        //expected. active scanner will fail if fs is lost
      }
    } finally {
      cache.getLogger().info("<ExpectedException action=remove>java.io.IOException</ExpectedException>");
    }
