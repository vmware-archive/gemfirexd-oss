...
public class DiskRegionJUnitTest extends DiskRegionTestingBase
{
  ...
  public DiskRegionJUnitTest(String name) {
    super(name);
  }
  ...
  public void testFaultingInRemovalFromAsyncBuffer()
  {
    ...
    long startTime = System.currentTimeMillis();
    DistributedTestBase.join(thread1, 20 * 1000, null);
    DistributedTestBase.join(thread2, 20 * 1000, null);
    DistributedTestBase.join(thread3, 20 * 1000, null);
    DistributedTestBase.join(thread4, 20 * 1000, null);
    DistributedTestBase.join(thread5, 20 * 1000, null);
...
//          }
  ...
  public void testSingleDirectoryNotHanging()
  {
    ...
    long startTime = System.currentTimeMillis();
    DistributedTestBase.join(thread1, 40 * 1000, null);
    long interval = System.currentTimeMillis() - startTime;
    if (interval > 40000) {
  ...
  public void testOperationGreaterThanMaxOplogSize()
  {
    ...
    long startTime = System.currentTimeMillis();
    DistributedTestBase.join(thread1, 9 * 1000, null);
    long interval = System.currentTimeMillis() - startTime;
    if (interval > 9000) {
  ...
  public void testOperationGreaterThanMaxDirSize()
  {
    ...
    long startTime = System.currentTimeMillis();
    DistributedTestBase.join(thread1, 33 * 1000, null);
    long interval = System.currentTimeMillis() - startTime;
    if (interval > 33000) {
  ...
  public void testDiskFullExcep()
  {
    ...
    assertEquals(true, region.isDestroyed());

    //Asif Recreate region so that it gets destroyed in teardown
    region = DiskRegionHelperFactory
      .getSyncPersistOnlyRegion(cache, diskProps, Scope.LOCAL);
  ...
  public void testNoDiskFullExcep()
  {
    ...
    assertEquals(false, region.isDestroyed());
  }
  /**
   * DiskRegDiskAccessExceptionTest : Disk region test for DiskAccessException.
   */
  ...
  private void dskAccessExpHelperMethod(final Region region, final boolean synchMode)
  {
      ...
      public void run()
      {
        ...
        CacheObserver old = CacheObserverHolder
            .setInstance(new CacheObserverAdapter() {
              ...
              {
                try {
                  DistributedTestBase.join(t1, 60 * 1000, null);
                }
    ...
    testThread.start();
   // region.clear();
    DistributedTestBase.join(testThread, 40 * 1000, null);
    assertFalse(failureCause, testFailed);
  ...
  public void testClearInteractionWithCreateOperation_Bug37606()
  {
    ...
    CacheObserver old = CacheObserverHolder
        .setInstance(new CacheObserverAdapter() {
          ...
          {
            th.start();
            DistributedTestBase.staticPause(7 * 1000);
    ...
    CacheObserver old = CacheObserverHolder
        .setInstance(new CacheObserverAdapter() {
          public void beforeDiskClear()
          {
            ...
            DistributedTestBase.staticPause(10 * 1000);
//            try {	
...
//       System.getProperties().setProperty(DiskStoreImpl.COMPLETE_COMPACTION_BEFORE_TERMINATION_PROPERTY_NAME, "");
  ...
  // this is no longer a valid test since the new overflow only regions
  // no longer copy forward.
  // public void testBug41140() throws Exception {
//     // to reproduce the deadlock:
//     // 1. use an overflow only region with sync writes and a small max-oplog-size and rolling enabled.
//     // 2. have one thread that keeps creating entries
//     // 3. have another thread doing entry destroys
//     // the goal is to a destroy and entry just as the roller is trying to copy it forward.
//     DiskRegionProperties props = new DiskRegionProperties();
//     props.setRegionName("testBug41140");
//     props.setRolling(true);
//     props.setDiskDirs(dirs);
//     props.setMaxOplogSize(300);
//     props.setOverFlowCapacity(1);
//     region = DiskRegionHelperFactory.getSyncOverFlowOnlyRegion(cache, props);

//     final AtomicBoolean terminate = new AtomicBoolean();
//     final AtomicLong creatorProgress = new AtomicLong();
    
//     final Thread creator = new Thread(new Runnable() {
//         public void run() {
//           final byte[] payload = new byte[100];
//           while (!terminate.get()) {
//             region.put(new Long(creatorProgress.incrementAndGet()), payload);
//           }
//         }
//       });

//     final AtomicLong destroyerProgress = new AtomicLong();
//     final Thread destroyer = new Thread(new Runnable() {
//         public void run() {
//           while (!terminate.get()) {
//             Long key = new Long(destroyerProgress.incrementAndGet());
//             while (!terminate.get() && !region.containsKey(key)) {
//               // spin
//             }
//             if (!terminate.get()) {
//               region.destroy(key);
...
//     // monitor progress to detect deadlock
//     boolean ok;
//     int count = 4;
//     do {
//       long destroyerStart = destroyerProgress.get();
//       Thread.sleep(999);
//       long destroyerEnd = destroyerProgress.get();
//       ok = destroyerEnd != destroyerStart; 
//     } while (ok && count-- > 0);
//     terminate.set(true);
//     // give the thread we started a chance to stop. However if we are deadlocked
//     // then they will never terminate.
//     creator.join(500);
//     destroyer.join(500);
