...
public class OplogJUnitTest extends DiskRegionTestingBase
{
  ...
  public OplogJUnitTest(String name) {
    super(name);
  }
...
//     DistributedTestBase.join(writer.getThread(), 10 * 1000, null);
//     closeDown();
//   }
...
//       Oplog oplog = ((LocalRegion)region).getDiskRegion().getChild(j);
// //       if (oplog != null) {
// //         DistributedTestBase.join(oplog.getAsynchWriter().getThread(), 10 * 1000, null);
// //       }
//     }
//     assertFalse(failureCause, testFailed);
//     closeDown();
  ...
  public void testBug35012()
  {
    ...
    try {
      ...
      try {
            ...
            Thread.yield();
            DiskRegion dr = ((LocalRegion)region).getDiskRegion();
            dr.testHook_getChild().forceRolling(dr, false);
      }
      finally {
...
//   }
  ...
  public void disabled_bug48922_testAsyncStats() throws InterruptedException
  {
    ...
    put100Int();
    DistributedTestBase.waitForCriterion(evFull, 2 * 1000, 200, true);
    assertEquals(0, dss.getFlushes());
    region.writeToDisk();
    DistributedTestBase.waitForCriterion(ev, 2 * 1000, 200, true);
    DistributedTestBase.waitForCriterion(ev2, 1000, 200, true);
    put100Int();
    DistributedTestBase.waitForCriterion(evFull, 2 * 1000, 200, true);
    region.writeToDisk();
    DistributedTestBase.waitForCriterion(ev, 2 * 1000, 200, true);
    DistributedTestBase.waitForCriterion(ev3, 1000, 200, true);
    closeDown();
  }
...
//   }
  ...
  public void testPutClearPut()
  {
    try {
      ...
      try {
        DistributedTestBase.join(clearOp, 30 * 1000, null);
      }
      catch (Exception e) {
        testFailed = true;
        failureCause = "Encountered Exception=" + e;
    ...
    try {
      ...
      CacheObserverHolder.setInstance(new CacheObserverAdapter(){
        ...
        public void afterSettingDiskRef() {
          ...
          clearTh.start();
          try {
            DistributedTestBase.join(clearTh, 120 * 1000, null);
  ...
  public void testOverFlowOnlySyncDestroyTx()
  {
    ...
    // also check that for non-local regions a tombstone entry should be created
    region.destroyRegion();
    region = DiskRegionHelperFactory.getSyncOverFlowOnlyRegion(cache,
        diskProps, Scope.DISTRIBUTED_ACK);
    assertNotNull(region);
    try {
      region.put("key", "createValue");
      region.put("key1", "createValue1");
      ...
      region.destroy("key");
      cache.getCacheTransactionManager().commit();
      RegionEntry entry = ((LocalRegion)region).entries.getEntry("key");
