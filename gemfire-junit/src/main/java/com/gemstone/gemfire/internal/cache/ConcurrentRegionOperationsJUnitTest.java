...
public class ConcurrentRegionOperationsJUnitTest extends DiskRegionTestingBase
{
  ...
  public ConcurrentRegionOperationsJUnitTest(String name) {
    super(name);
  }
  ...
  // hanging! restore this when fixing #47728
  ...
  private Region concurrencyTest(Region r1)
  {
    ...
    for (int i = 0; i < numberOfPutsThreads; i++) {
      DistributedTestBase.join(putThreads[i], 40*1000, null);
    }
    for (int i = 0; i < numberOfGetsThreads; i++) {
      DistributedTestBase.join(getThreads[i], 40*1000, null);
    }
    for (int i = 0; i < numberOfDestroysThreads; i++) {
      DistributedTestBase.join(destroyThreads[i], 40*1000, null);
    }
    for (int i = 0; i < numberOfClearThreads; i++) {
      DistributedTestBase.join(clearThreads[i], 40*1000, null);
    }
    for (int i = 0; i < numberOfForceRollThreads; i++) {
      DistributedTestBase.join(forceRollThreads[i], 40*1000, null);
    }
  ...
  public void testConcurrentClearAndRegionDestroyBug()
  {
    ...
    region.clear();
    DistributedTestBase.join(th, 20 * 1000, null);
