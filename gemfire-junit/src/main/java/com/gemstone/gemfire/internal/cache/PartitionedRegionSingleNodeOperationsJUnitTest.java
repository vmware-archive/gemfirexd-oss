...
public class PartitionedRegionSingleNodeOperationsJUnitTest extends TestCase
{
  ...
  /**
   * Constructor for PartitionedRegionSingleNodeOperations.
   * 
   * @param arg
   */
  public PartitionedRegionSingleNodeOperationsJUnitTest(String arg)
  ...
  public void test000Put() throws Exception
  {
    ...
    final LogWriter prLogger = pr.getLogWriterI18n().convertToLogWriter();
    prLogger.info( "<ExpectedException action=add>"
        + expectedExceptions
        + "</ExpectedException>");
    try {
    ...
    prLogger.info( "<ExpectedException action=remove>"
        + expectedExceptions
        + "</ExpectedException>");
    ...
    for (int num = 0; num < maxEntries; num++) {
      if (entryIsLocal(pr.getEntry(new Integer(num)))) {
  ...
  public void test001Destroy() throws Exception
  {
    ...
    pr.getLogWriterI18n().convertToLogWriter().info(addExpected);
    ...
    pr.getLogWriterI18n().convertToLogWriter().info(removeExpected);
  ...
  public void test003DestroyRegion() throws Exception
  {
    ...
    logger.info("JDEBUG 1");
    pr.destroyRegion();
    logger.info("JDEBUG 2");
...
//        PartitionedRegionTestHelper.createCache()).get(regionName));
  ...
  {
    logger.fine("Getting inside testClose method");
    PartitionedRegion pr = (PartitionedRegion)PartitionedRegionTestHelper
...
//      }
  ...
  {
    logger.fine("Getting inside testCloseDestroy");
    PartitionedRegion pr = (PartitionedRegion)PartitionedRegionTestHelper
