 ...
 * @since 3.0
 */
public class DataSerializableTest extends TestCase
  implements Serializable {

  /** A <code>ByteArrayOutputStream</code> that data is serialized to */
  private transient ByteArrayOutputStream baos;

  public DataSerializableTest(String name) {
    super(name);
  }
...
//   }
   ...
   * DataSerializableTest.SerializableImpl not specially cased}.
   */
  ...
  /**
   * Tests data serializing {@link TimeUnit}s
   */
  public void testTimeUnit()
    throws IOException, ClassNotFoundException {

    DataOutputStream out = getDataOutput();
    InternalDataSerializer.writeTimeUnit(TimeUnit.NANOSECONDS, out);
    InternalDataSerializer.writeTimeUnit(TimeUnit.MILLISECONDS, out);
    InternalDataSerializer.writeTimeUnit(TimeUnit.MICROSECONDS, out);
    InternalDataSerializer.writeTimeUnit(TimeUnit.SECONDS, out);
    out.flush();

    DataInput in = getDataInput();
    assertEquals(TimeUnit.NANOSECONDS,
                 InternalDataSerializer.readTimeUnit(in));
  ...
  public void testTimeUnitObject()
    ...
    DataOutputStream out = getDataOutput();
    DataSerializer.writeObject(TimeUnit.NANOSECONDS, out);
    DataSerializer.writeObject(TimeUnit.MICROSECONDS, out);
    DataSerializer.writeObject(TimeUnit.MILLISECONDS, out);
    DataSerializer.writeObject(TimeUnit.SECONDS, out);
    out.flush();
  ...
  public void testLateDeSerializer() throws Exception {
    ...
    try {
      ...
      DistributedTestBase.join(thread, 30 * 1000, null);
      InternalDataSerializer.unregister(id);
    }
  }
