...
public class CollaborationJUnitTest extends TestCase {
  ...
  protected LogWriter log = new LocalLogWriter(LogWriterImpl.INFO_LEVEL);
  protected Collaboration collaboration;
  
  public CollaborationJUnitTest(String name) {
    super(name);
  }
  ...
  public void testBlocksUntilRelease() throws Exception {
    ...
    DistributedTestBase.waitForCriterion(ev, 5 * 1000, 200, true);
    ...
    Thread threadB = new Thread(group, new Runnable() {
      public void run() {
        ...
        try {
          ...
          DistributedTestBase.waitForCriterion(ev2, 20 * 1000, 200, true);
        }
        finally {
          collaboration.release();
        }
    ...
    DistributedTestBase.join(threadA, 30 * 1000, null);
    ...
    this.flagTestBlocksUntilRelease = false;
    DistributedTestBase.join(threadB, 30 * 1000, null);
  ...
  public void testLateComerJoinsIn() throws Exception {
    ...
    Thread threadA = new Thread(group, new Runnable() {
      public void run() {
        ...
        try {
          ...
          DistributedTestBase.waitForCriterion(ev, 60 * 1000, 200, true);
        }
        finally {
          collaboration.release();
        }
    ...
    Thread threadB = new Thread(group, new Runnable() {
      public void run() {
        ...
        try {
          ...
          DistributedTestBase.waitForCriterion(ev2, 60 * 1000, 200, true);
        }
        finally {
          collaboration.release();
        }
    ...
    DistributedTestBase.join(threadD, 30 * 1000, null);
    ev = new WaitCriterion() {
      public boolean done() {
        return threadCFlag_TestLateComerJoinsIn;
      }
    ...
    DistributedTestBase.join(threadC, 30 * 1000, null);
    assertFalse(this.collaboration.hasCurrentTopic(threadC));
    assertFalse(this.collaboration.isCurrentTopic(topicA));
    assertFalse(this.collaboration.isCurrentTopic(topicB));
  }
  ...
  public void testFairnessStressfully() throws Exception {
    ...
    try {
      ...
      DistributedTestBase.waitForCriterion(ev, 5 * 60 * 1000, 200, true);
    }
    finally {
    ...
    for (int t = 0; t < threads.length; t++) {
      DistributedTestBase.join(threads[t], 30 * 1000, null);
    }
