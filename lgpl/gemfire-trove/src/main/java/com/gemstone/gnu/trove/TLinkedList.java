 ...
 * @see com.gemstone.gnu.trove.TLinkable
 */

public class TLinkedList extends AbstractSequentialList
    implements Serializable {

    private static final long serialVersionUID = 1812496028869442036L;
    ...
    public boolean remove(Object o) {
        if (o instanceof TLinkable) {
            ...
            if (n == null && p == null) { // emptying the list
                // GemStoneAddition: check if really head and tail
                if (link != _head || link != _tail) return false;
                _head = _tail = null;
            } else if (n == null) { // this is the tail
                // GemStoneAddition: check if really tail
                if (link != _tail) return false;
                // make previous the new tail
                link.setPrevious(null);
                p.setNext(null);
                _tail = p;
            } else if (p == null) { // this is the head
                // GemStoneAddition: check if really head
                if (link != _head) return false;
