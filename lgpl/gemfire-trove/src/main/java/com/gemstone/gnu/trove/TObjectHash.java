 ...
 * @version $Id: TObjectHash.java,v 1.16 2004/02/25 14:05:29 ericdf Exp $
 */
abstract public class TObjectHash extends THash implements Serializable, TObjectHashingStrategy {
  
    /**
     * optional statistics object to track number of hash collisions
     * and time spent probing based on hash collisions
     */
    protected transient HashingStats stats;
    ...
    public TObjectHash(int initialCapacity, float loadFactor, TObjectHashingStrategy strategy) {
        super(initialCapacity, loadFactor);
        this._hashingStrategy = strategy != null ? strategy : this;
    }

    // GemStoneAddition
    /**
     * Set the statistics object for tracking hash code collisions.
     * Should be called before the first element is added.
     */
    public final void setHashingStats(HashingStats stats) {
      this.stats = stats;
    }

    // GemStoneAddition
    public final void setHashingStrategy(TObjectHashingStrategy strategy) {
      this._hashingStrategy = strategy;
    }

    // GemStoneAddition
    public final boolean isEqualHashingStrategy(Class<?> classId) {
      return classId.isInstance(this._hashingStrategy);
    }

    // GemStoneAddition
    public final String getHashingStrategyName() {
      return this._hashingStrategy.getClass().getSimpleName();
    }
...
// GemStone changes END
    ...
    protected int index(Object obj) {
        ...
        set = _set;
        length = set.length;
        hashingStrategy = _hashingStrategy;
    ...
    protected int insertionIndex(Object obj) {
        ...
        } else if (cur != REMOVED && hashingStrategy.equals(cur, obj)) {
