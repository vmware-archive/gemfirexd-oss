...
implements javax.management.NotificationListener, 
  ...
  private void _setRefreshInterval(int refreshInterval) {
    LogWriter logWriter = this.member.getDistributedSystem().getLogWriter();
    ...
    try {
      ...
      // catch test for fatal JVM error (see above). However, there is
  ...
  public void handleNotification(Notification notification, Object hb) {
    ...
    if (typeStatResourceStats.equals(notification.getType()) && 
      ...
      try {
        ...
        logWriter.fine(e); // dead in water, print, and then ignore
  ...
  throws com.gemstone.gemfire.admin.AdminException {
    ...
    for (int i = 0; i < this.statistics.length; i++) {
      ...
      String unit = this.statistics[i].getUnits();
      attrInfo.setDescription(this.statistics[i].getDescription() + (unit != null && !unit.isEmpty() ? " [" + unit + "] " : ""));
