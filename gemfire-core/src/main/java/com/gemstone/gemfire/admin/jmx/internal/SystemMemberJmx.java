...
//        Assert.assertTrue(cache != null); (cannot be null)
    ...
    public static void handleNotification(SystemMemberJmx member,
                                          ...
                                          Object hb) {
      LogWriter logWriter = member.getDistributedSystem().getLogWriter();
      if (RefreshNotificationType.SYSTEM_MEMBER_CONFIG.
        ...
        try {
          ...
          if (logWriter.fineEnabled()) {
            logWriter.fine(e); // print stack trace and then ignore
          } else {
     ...
     */
    /*default*/static void sendNotification(LogWriterI18n logWriterI18n, 
                                ManagedResource resource, Notification notif) {
      try {
        if (MBeanUtil.isRegistered(resource.getObjectName())) {
          ...
          if (logWriterI18n.fineEnabled()) {
            logWriterI18n.fine("Sent '"+notif.getType()+"' notification");
          }
        }
