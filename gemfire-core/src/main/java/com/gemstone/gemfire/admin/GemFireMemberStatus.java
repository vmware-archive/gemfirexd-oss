...
public class GemFireMemberStatus implements Serializable {
  ...
  public Set getConnectedOutgoingGateways() {
    //return this._connectedOutgoingGateways;
	Collection vals = _outgoingGateways.values();
	Set ids = new HashSet();
	GatewayStatus gate;
	for (Iterator iter=vals.iterator(); iter.hasNext() ;)
	{
		gate = (GatewayStatus)iter.next();
  ...
  public Set getUnconnectedOutgoingGateways() {
    //return this._unconnectedOutgoingGateways;
	Collection vals = _outgoingGateways.values();
	Set ids = new HashSet();
	GatewayStatus gate;
	for (Iterator iter=vals.iterator(); iter.hasNext() ;)
  ...
  public Map getGatewayQueueSizes() {
	  ...
	  for (Iterator iter=vals.iterator(); iter.hasNext() ;)
	  {
		  GatewayStatus val = (GatewayStatus)iter.next();
		  queues.put( val.getId(), Integer.valueOf(val.getQueueSize()) );
	  }
	  return queues;	  
	  //return this._gatewayQueueSizes;
  }

  public int getGatewayQueueSize(String gatewayId) {
    //Integer gatewayQueueSize = (Integer) getGatewayQueueSizes().get(gatewayId);
    //return gatewayQueueSize == null ? 0 : gatewayQueueSize.intValue();
	GatewayStatus gate = (GatewayStatus)_outgoingGateways.get(gatewayId);
    return gate==null ? 0 : gate.getQueueSize();
  }
  ...
  public String toString() {
    ...
    buffer
      ...
      .append("; connectedOutgoingGateways=")
      .append(this.getConnectedOutgoingGateways())
      .append("; unconnectedOutgoingGateways=")
      .append(this.getUnconnectedOutgoingGateways())
      .append("; connectedIncomingGateways=")
      .append(this._connectedIncomingGateways)
...
//      Assert.assertTrue(cache != null); (cannot be null)
  ...
  protected void initializeGatewayHub() {
    GatewayHub hub = cache.getGatewayHub();
    if (hub == null) {
      setIsGatewayHub(false);
    } else {
      setIsGatewayHub(true);
      setGatewayHubStatus(new GatewayHubStatus(hub));
      setIsPrimaryGatewayHub(hub.isPrimary());

      // Initialize outgoing gateways
      Iterator allOutgoingGateways = hub.getGateways().iterator();
      _outgoingGateways = new HashMap();
      while (allOutgoingGateways.hasNext()) {
        Gateway gateway = (Gateway) allOutgoingGateways.next();
        String gatewayId = gateway.getId();
        _outgoingGateways.put(gatewayId, new GatewayStatus(gateway));
      }

      // Initialize incoming gateways
      Map connectedIncomingGateways = InternalBridgeMembership.getConnectedIncomingGateways();
      setConnectedIncomingGateways(connectedIncomingGateways);
    }
  }
