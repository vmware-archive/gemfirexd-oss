...
public class SequenceFileHoplog extends AbstractHoplog{
  ...
  public boolean fixFileSize() throws IOException {
   ...
   // times
    logger.fine("Fixing size of hoplog " + path);
    Exception e = null;
    ...
    for (int i =0; i < 3; i++) {
      try {
        ...
        e = ie;
        logger.fine("Retry run " + (i + 1) + ": Hoplog " + path + " is still a temporary " +
    ...
    logger.info (LocalizedStrings.DEBUG, "Hoplog " + path + " is still a temporary " +
        "hoplog because the node managing it wasn't shutdown properly last time. Failed to " +
        "fix the hoplog because an exception was thrown " + e);
  ...
  private class SequenceFileHoplogWriter implements HoplogWriter {
    ...
    public void close() throws IOException {
      writer.close();
      logger.fine("Completed creating hoplog " + path);
    }
    
    @Override
    public void hsync() throws IOException {
      writer.hsyncWithSizeUpdate();
      logger.fine("hsync'ed a batch of data to hoplog " + path);
    }
    ...
    @Override
    public void append(byte[] key, int keyOffest, int keyLength, byte[] value,
