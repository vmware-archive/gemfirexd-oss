...
public final class Operation implements java.io.Serializable {
  ...
  private static final byte OP_TYPE_DISK_RECOVERY= OpType.DISK_RECOVERY;
  ...
  private static final byte OP_DETAILS_NONE = 0;
  private static final byte OP_DETAILS_SEARCH = 1;
  private static final byte OP_DETAILS_LOCAL_LOAD = 2;
  private static final byte OP_DETAILS_NET_LOAD = 4;
  private static final byte OP_DETAILS_EXPIRE = 8;
  private static final byte OP_DETAILS_EVICT = 16;
  private static final byte OP_DETAILS_PUTALL = 32;
  private static final byte OP_DETAILS_GUARANTEES_OLD_VALUE = 64;
  ...
  public static final Operation REGION_LOAD_SNAPSHOT
    = new Operation("REGION_LOAD_SNAPSHOT",
                    false, // isLocal
                    true, // isRegion
                    OP_TYPE_DESTROY, // @todo darrel: should it be a destroy?
                    OP_DETAILS_NONE
                    );
  ...
  /**
   * This op type is used during invocation of internal
   * <code>IndexUpdater.onEvent</code> when recovering from disk.
   */
  public static final Operation DISK_RECOVERY
    = new Operation("DISK_RECOVERY",
                    false, // isLocal
                    false, // isRegion
                    OP_TYPE_DISK_RECOVERY,
                    OP_DETAILS_NONE
                    );

  /**
   * Bulk DML operation . Used by GemFireXD in DBSynchronizer and Wan Gateways
   */
  public static final Operation BULK_DML_OP = new Operation("BULK_DML_OP",
      false, // isLocal
      false, // isRegion
   ...
   *    OP_DETAILS_PUTALL
   */
  private final transient byte opDetails;
  ...
  private Operation(String name, boolean isLocal, boolean isRegion, byte opType, byte opDetails) {
    this.name = name;
  ...
  /**
   * Returns true if it is a GemFireXD bulk dml operation.
   */
  public boolean isBulkDMLOp() {
    return this.opType == OP_TYPE_UPDATE && this.name.equals("BULK_DML_OP");
  }
  ...
  /**
   * Returns true if the operation being done is during disk recovery for
   * GemFireXD index updates.
   * 
   * @since 7.0
   */
  public boolean isDiskRecovery() {
    return this.opType == OP_TYPE_DISK_RECOVERY;
  }
