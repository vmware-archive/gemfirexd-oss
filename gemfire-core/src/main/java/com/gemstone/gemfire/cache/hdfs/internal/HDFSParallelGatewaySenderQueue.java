...
public class HDFSParallelGatewaySenderQueue extends ParallelGatewaySenderQueue {
  ...
  public void remove(int batchSize) throws CacheException {
    ...
    while (event != null && destroyed < batchSize) {
      ...
      while (bucketId == currentBucketId) {
        ...
        if (!this.sender.isRunning()){
          if (logger.fineEnabled()) {
            logger.fine("ParallelGatewaySenderQueue#remove: Cache is closing down. Ignoring remove request.");
          }
          return;
        }
      }
      try {
        ...
        if (logger.fineEnabled()) {
          logger.fine("ParallelGatewaySenderQueue#remove: " + "Got ForceReattemptException for " + this
          + " for bucket = " + bucketId);
        }
      }
      catch(EntryNotFoundException e) {
        if (logger.fineEnabled()) {
          logger.fine("ParallelGatewaySenderQueue#remove: " + "Got EntryNotFoundException for " + this
            + " for bucket = " + bucketId );
        }
      }
    }
  ...
  */ 
  ...
  protected ArrayList peekAhead(int bucketId, HDFSBucketRegionQueue hrq) throws CacheException {
    ...
    if (logger.fineEnabled()) {
      logger.fine(this + ": Peekahead for the bucket " + bucketId);
    }
    ArrayList  list = hrq.peekABatch();
    if (logger.fineEnabled() && list != null ) {
      logger.fine(this + ": Peeked" + list.size() + "objects from bucket " + bucketId);
    }
 ...
 * @param region 
  ...
  public HDFSGatewayEventImpl get(PartitionedRegion region, byte[] regionKey, int bucketId) throws ForceReattemptException  {
    try {
      ...
      if (logger.fineEnabled()) {
        logger.fine("HDFSParallelGatewaySenderQueue#get: " + "Got EntryNotFoundException for " + this
            + " for bucket = " + bucketId);
      }
    }
    return null;
    ...
    public void run2() {
      ...
      for (PartitionedRegion prQ : prQs) {
        ...
        for (Integer bId : buckets) {
          ...
          if (logger.finerEnabled()) {
            logger.finer("Rolling over the list for bucket id: " + bId);
          }
