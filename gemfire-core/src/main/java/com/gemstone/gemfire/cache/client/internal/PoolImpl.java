...
//    }
    ...
    this.securityLogger = ds.getSecurityLogWriter().convertToLogWriterI18n();
    ...
    connectionFactory = new ConnectionFactoryImpl(source, endpointManager, ds,
        socketBufferSize, HANDSHAKE_TIMEOUT, readTimeout, logger, proxyId,
        this.cancelCriterion,usedByGateway, gatewaySender, pingInterval, multiuserSecureModeEnabled);
    if(subscriptionEnabled) {
      queueManager = new QueueManagerImpl(this, endpointManager, source,
          connectionFactory, subscriptionRedundancyLevel, pingInterval, logger,
          securityLogger, proxyId);
    }
    ...
    executor = new OpExecutorImpl(manager, queueManager, endpointManager,
        ...
        logger, new PoolOrCacheStopper(), this);
  ...
  private void start() {
    ...
    if(this.getLoggerI18n().fineEnabled()) {
      List locators = getLocators();
      if(!locators.isEmpty()) {
        getLoggerI18n().fine("PoolImpl - starting pool with locators: " + locators);
      } else {
        getLoggerI18n().fine("PoolImpl -starting pool with servers: " + getServers());
      }
    }
    ...
    if(queueManager != null) {
      getLoggerI18n().fine("starting queueManager");
      queueManager.start(backgroundProcessor);
    }
    getLoggerI18n().fine("scheduling pings every " + pingInterval + " milliseconds");
    ...
    if (getLoggerI18n().configEnabled()) {
      getLoggerI18n()
          .config(
  ...
  public synchronized void basicDestroy(boolean keepAlive) {
    if (!isDestroyed()) {
      this.destroyed = true;
      if (getLoggerI18n().configEnabled()) {
        getLoggerI18n().config(LocalizedStrings.PoolImpl_DESTROYING_CONNECTION_POOL_0, name);
      }
      ...
      try {
        if (backgroundProcessor != null) {
          ...
          if(!backgroundProcessor.awaitTermination(SHUTDOWN_TIMEOUT, TimeUnit.MILLISECONDS)) {
            logger.warning(LocalizedStrings.PoolImpl_TIMEOUT_WAITING_FOR_BACKGROUND_TASKS_TO_COMPLETE);
          }
        }
      } catch(RuntimeException e) {
        logger.error(LocalizedStrings.PoolImpl_ERROR_ENCOUNTERED_WHILE_STOPPING_BACKGROUNDPROCESSOR, e);
      } catch(InterruptedException e) {
        logger.error(LocalizedStrings.PoolImpl_INTERRUPTED_WHILE_STOPPING_BACKGROUNDPROCESSOR, e);
      }
      ...
      try {
        ...
        logger.error(LocalizedStrings.PoolImpl_ERROR_ENCOUNTERED_WHILE_STOPPING_CONNECTION_SOURCE, e);
      } 
      ...
      try {
        ...
        logger.error(LocalizedStrings.PoolImpl_ERROR_ENCOUNTERED_WHILE_STOPPING_CONNECTION_MANAGER, e);
      }
      ...
      try {
        ...
        logger.error(LocalizedStrings.PoolImpl_ERROR_ENCOUNTERED_WHILE_STOPPING_SUBSCRIPTION_MANAGER, e);
      }
      
      try {
        endpointManager.close();
      } catch(RuntimeException e) {
        logger.error(LocalizedStrings.PoolImpl_ERROR_ENCOUNTERED_WHILE_STOPPING_ENDPOINT_MANAGER, e);
      }
      ...
      try {
        ...
        logger.error(LocalizedStrings.PoolImpl_ERROR_WHILE_CLOSING_STATISTICS, e);
      }
    }
  }
  ...
  public void endpointsNetDownForDUnitTest() {
    logger.fine("PoolImpl - endpointsNetDownForDUnitTest");
    ...
    for(Iterator itr = endpoints.values().iterator(); itr.hasNext();) {
      ...
      logger.fine("PoolImpl Simulating crash of endpoint " + endpoint);
...
//        throw e;
        if (getLogger().fineEnabled()) {
          getLogger().fine("Pool task <" + this + "> cancelled", 
              getLogger().finerEnabled() ? e : null);
        }
      } catch(Throwable t) {
        Error err;
        ...
        getLogger().error(LocalizedStrings.PoolImpl_UNEXPECTED_ERROR_IN_POOL_TASK_0, this, t);
      }
    ...
    public abstract LogWriterI18n getLogger();
  }
