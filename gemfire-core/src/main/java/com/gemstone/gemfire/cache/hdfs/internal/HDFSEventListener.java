 ...
 */
public class HDFSEventListener implements AsyncEventListener<Object, Object> {
  ...
  public boolean processEvents(List<AsyncEvent<Object, Object>> events) {
    ...
    try {
      ...
      for (AsyncEvent asyncEvent : events) {
        ...
        LocalRegion region = hdfsEvent.getRegion();
        ...
        if (prevBucketId != hdfsEvent.getBucketId() || region != previousRegion){
          ...
          if (bucketOrganizer == null)
            throw new BucketMovedException("Bucket moved.", hdfsEvent.getBucketId(), region.getName());
        ...
        try {
          ...
          logger.warning(new StringIdImpl(0,
              "Error while converting HDFSGatewayEvent to PersistedEventImpl."), e);
          return false;
        }
