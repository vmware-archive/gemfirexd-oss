...
//    }
  ...
  private SingletonValue<HFileReader> getReaderContainer() {
    return new SingletonValue<HFileReader>(new SingletonBuilder<HFileReader>() {
      ...
      public HFileReader create() throws IOException {
        logger.fine("Creating hoplog reader");
  ...
  private void compareAndClose(HFileReader hfileReader, boolean clearCache) throws IOException {
    ...
    if (hfileReader == null) {
      ...
      if (! result) {
        logger.fine("skipping close, provided hfileReader mismatched");
        return;
      } 
      rdr = hfileReader;
    }
  ...
  private class HFileSortedOplogWriter implements HoplogWriter {
    ...
    @Override
    public final void append(byte[] key, byte[] value) throws IOException {
      writer.append(key, value);
      bfw.add(key, 0, key.length);
    }

    @Override
    public void append(byte[] key, int keyOffset, int keyLength, byte[] value,
      ...
      if (keyOffset != 0 || keyLength != key.length) {
        byte[] newKey = new byte[keyLength];
        System.arraycopy(key, keyOffset, newKey, 0, keyLength);
        key = newKey;
      }
      if (valueOffset != 0 || valueLength != value.length) {
        byte[] newValue = new byte[valueLength];
        System.arraycopy(value, valueOffset, newValue, 0, valueLength);
        value = newValue;
      }
      writer.append(key, value);
      bfw.add(key, 0, key.length);
    ...
    public void close(EnumMap<Meta, byte[]> metadata) throws IOException {
      if (closed.get()) {
        logger.finer("Writer already closed");
        return;
      }
...
//      }
      ...
      writer.close();
      logger.fine("Completed closing writer");
      closed.set(true);
...
//    }
  ...
  private void handleReadIOError(HFileReader hfileReader, IOException e, boolean skipFailIfSafe) {
    ...
    if (hfileReader != null && hfileReader.previousFS != currentFs) {
      if (logger.fineEnabled()) {
        logger.fine("Detected new FS client, closing old reader");
        if (currentFs != null) {
          logger.fine("CurrentFs:" + currentFs.getUri() + "-"
              + currentFs.hashCode());
        }
        if (hfileReader.previousFS != null) {
          logger.fine("OldFs:" + hfileReader.previousFS.getUri() + "-"
      ...
      try {
        ...
        logger.fine("Failed to close reader", ex);
      }
      if (skipFailIfSafe) {
        logger.fine("Not faling after io error since FS client changed");
        return;
      }
    }
  ...
  class HFileReader implements HoplogReader, Closeable {
    ...
    public byte[] read(byte[] key) throws IOException {
      ...
      logger.fine("Throwing err from read delegate ", err);
      throw err;
    }
    ...
    public HoplogIterator<byte[], byte[]> scan(byte[] from, boolean fromInclusive, byte[] to,
      ...
      logger.fine("Throwing err from scan delegate ", err);
      throw err;
    }
    ...
    private void validate() throws IOException {
      ...
      if (logger.fineEnabled()) {
        logger.fine("Hoplog version is " + Hex.toHex(ver));
      }
