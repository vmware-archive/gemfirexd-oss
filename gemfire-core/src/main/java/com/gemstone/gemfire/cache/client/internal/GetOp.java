...
public class GetOp {
  ...
  public static Object execute(ExecutablePool pool, LocalRegion region,
    ...
    AbstractOp op = new GetOpImpl(pool.getLoggerI18n(), region, key,
        callbackArg, prSingleHopEnabled, clientEvent);

    LogWriterI18n log = region.getLogWriterI18n();
    if (log.fineEnabled()) {
      log.fine("GetOp invoked for key " + key);
    }
  ...
  static class GetOpImpl extends AbstractOp {
     ...
     */
    public GetOpImpl(LogWriterI18n lw,
                     LocalRegion region,
                     Object key,
                     Object callbackArg, 
                     boolean prSingleHopEnabled,
                     EntryEventImpl clientEvent) {
      super(lw, MessageType.REQUEST, callbackArg != null ? 3 : 2);
      if (region.getLogWriterI18n().fineEnabled()) {
        region.getLogWriterI18n().fine("constructing a GetOp for key " + key/*, new Exception("stack trace")*/);
      }
      this.region = region ;
