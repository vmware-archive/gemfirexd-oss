...
public class ExecuteFunctionNoAckOp {
  ...
  public static void execute(PoolImpl pool, Function function,
    ...
    AbstractOp op = new ExecuteFunctionNoAckOpImpl(logger, function, args,
        memberMappedArg, hasResult, isFnSerializationReqd, groups, allServers);
    try {
      ...
      if (allServers && groups.length == 0) {
        if (logger.fineEnabled()) {
          logger
              .fine("ExecuteFunctionNoAckOp#execute : Sending Function Execution Message:"
      ...
      if (logger.fineEnabled()) {
        logger
            .fine("ExecuteFunctionNoAckOp#execute : Exception occured while Sending Function Execution Message:"
  ...
  public static void execute(PoolImpl pool, String functionId,
    ...
    List servers = null;
    LogWriterI18n logger = pool.getLoggerI18n();
    AbstractOp op = new ExecuteFunctionNoAckOpImpl(logger, functionId, args,
        memberMappedArg, hasResult, isFnSerializationReqd, isHA, optimizeForWrite, groups, allServers);
    try {
      // In case of allServers getCurrentServers and call
      // executeOn(ServerLocation server, Op op)
      if (allServers && groups.length == 0) {
  ...
  private static class ExecuteFunctionNoAckOpImpl extends AbstractOp {
     ...
     */
    public ExecuteFunctionNoAckOpImpl(LogWriterI18n lw, Function function,
        Object args, MemberMappedArgument memberMappedArg,
        byte hasResult, boolean isFnSerializationReqd, String[] groups, boolean allMembers) {
