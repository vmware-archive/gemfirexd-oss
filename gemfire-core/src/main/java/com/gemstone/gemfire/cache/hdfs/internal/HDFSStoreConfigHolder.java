 ...
 * @author ashvina
 */
public class HDFSStoreConfigHolder implements HDFSStore, HDFSStoreFactory {  
  ...
  public HDFSStoreFactory setMaxFileSize(int maxFileSize) {
    assertIsPositive(CacheXml.HDFS_MAX_FILE_SIZE, maxFileSize);
  ...
  public static abstract class AbstractHDFSCompactionConfigHolder implements
      HDFSCompactionConfig, HDFSCompactionConfigFactory {
    ...
    public AbstractHDFSCompactionConfigHolder() {
      logger = ComponentLogWriter.getHoplogLogWriter("", LogService.logger());
    }
    ...
    void copyFrom(HDFSCompactionConfig config) {
      ...
      setAutoCompaction(config.getAutoCompaction());
    ...
    void copyFrom(HDFSCompactionConfigMutator mutator) {
      ...
      if (mutator.getAutoCompaction() != null) {
        logAttrMutation("AutoCompaction", mutator.getAutoCompaction());
        setAutoCompaction(mutator.getAutoCompaction());
      }
    ...
    @Override
    public HDFSCompactionConfigFactory setAutoCompaction(boolean auto) {
    ...
    public HDFSCompactionConfigFactory setMaxThreads(int count) {
      assertIsPositive(CacheXml.HDFS_COMPACTION_MAX_CONCURRENCY, count);
    ...
    public HDFSCompactionConfigFactory setOldFilesCleanupIntervalMins(int interval) {
      assertIsPositive(CacheXml.HDFS_COMPACTION_OLD_FILES_CLEANUP_INTERVAL_MINS, interval);
