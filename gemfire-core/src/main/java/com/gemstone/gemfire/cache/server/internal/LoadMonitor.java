...
public class LoadMonitor implements ConnectionListener {
  ...
  public LoadMonitor(ServerLoadProbe probe, int maxConnections,
      long pollInterval, int forceUpdateFrequency, LogWriterI18n logger,
      BridgeServerAdvisor advisor) {
    this.probe = probe;
  ...
  public void stop() {
    ...
    try {
      ...
      logger.warning(LocalizedStrings.LoadMonitor_INTERRUPTED_WAITING_FOR_POLLING_THREAD_TO_FINISH);
        ...
        try {
          ...
          if (!previousLoad.equals(load)
            ...
            if (logger.fineEnabled()) {
              logger.fine("Bridge Server Load Monitor Transmitting load "
                  + load + " to locators " + locators);
            }
            ...
            if (logger.fineEnabled()) {
              logger.fine("Bridge Server Load Monitor Load " + load
                  + " hasn't changed, not transmitting. skippedLoadUpdates="
                  + skippedLoadUpdates);
            }
          }
        }
          ...
          SystemFailure.checkFailure();
          logger.warning(LocalizedStrings.LoadMonitor_CACHESERVER_LOAD_MONITOR_ERROR_IN_POLLING_THREAD, t);
        }
      } // while
    }
  }
