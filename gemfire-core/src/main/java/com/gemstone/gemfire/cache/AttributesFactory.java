 ...
 * 
 * <dt>EnableGateway [<em>default:</em> <code>false</code>]</dt>
 *     <dd>Whether or not updates are sent to any defined <code>Gateway</code>s
 *     If <code>true</code>, updates will be sent to all defined
 *     <code>Gateway</code>s after they are applied to the local cache.
 *     Note that this attribute is only meaningful on a cache that
 *     defines <code>Gateway</code>s.<br>
 *     {@link #setEnableGateway} {@link RegionAttributes#getEnableGateway}</dd>
 *
...
public class AttributesFactory<K,V> {
  ...
  /**
   * This is a fix to allow notification replicated regions to have eviction action as LocalDestroy. This ensures cleanup of emitted notifications. 
   */
  private static final String NOTIFICATION_REGION_CACHE_LISTENER = "com.gemstone.gemfire.management.internal.NotificationCacheListener";
  ...
  public AttributesFactory(RegionAttributes<K,V> regionAttributes) {
    ...
    this.regionAttributes.enableGateway = regionAttributes.getEnableGateway();
    this.regionAttributes.gatewayHubId = regionAttributes.getGatewayHubId();
...
//      }
    ...
    this.regionAttributes.enableOffHeapMemory = regionAttributes.getEnableOffHeapMemory();
  ...
  public void setDataPolicy(DataPolicy dataPolicy) {
    ...
    // If current data policy for this region is PARTITIONED
    // and we are trying to overwrite it as REPLICATE, or vice versa, throw
    // sqlstate
    if ((this.regionAttributes.getDataPolicy().withReplication() && dataPolicy
        .withPartitioning())
        || (this.regionAttributes.getDataPolicy().withPartitioning() && dataPolicy
            .withReplication())) {
      // This thrown exception will be caught by GemFireXD grammar if the user
      // specified an illegal combination of replicate/partitioned clauses.
      // Unfortunately, there is no easy way to throw a GemFireXD SQL error from
      // this class
      throw new IllegalArgumentException(LocalizedStrings
          .AttributesFactory_DATA_POLICY_0_IS_NOT_ALLOWED_FOR_A_PARTITIONED_REGION_DATAPOLICIES_OTHER_THAN_1_ARE_NOT_ALLOWED
              .toLocalizedString(new Object[] {
                  this.regionAttributes.dataPolicy,
  ...
  public void setPersistBackup(boolean persistBackup) {
    if (persistBackup) {
      ...
      if (this.regionAttributes.getDataPolicy().isPersistentReplicate()) {
  ...
  /**
   * Sets whether or not updates are sent to defined <code>Gateway</code>s.
   *
   * @since 5.0
   */
  public void setEnableGateway(boolean enableGateway) {
    this.regionAttributes.enableGateway = enableGateway;
    this.regionAttributes.setHasEnableGateway(true);
  }

  /**
   * Sets the id of the <code>GatewayHub</code> to which events are
   * distributed.
   * @param gatewayHubId the id of the <code>GatewayHub</code>
   *
   * @since 5.1
   */
  public void setGatewayHubId(String gatewayHubId) {
    this.regionAttributes.gatewayHubId = gatewayHubId;
  }

  /**
   * Sets whether or not updates are sent to defined <code>Gateway</code>s.
   *
   * @deprecated as of GemFire 5.0, use {@link #setEnableGateway}
   *
   * @since 4.2
   */
  @Deprecated
  public void setEnableWAN(boolean enableGateway) {
    setEnableGateway(enableGateway);
  }
...
//    this.regionAttributes.setHasPublisher(true);
  ...
  */
  ...
  public void setPartitionAttributes(PartitionAttributes partition) {
    if (partition != null) {
      ...
      if (this.regionAttributes.hasPartitionAttributes() 
        ...
        ((PartitionAttributesImpl)this.regionAttributes.partitionAttributes).merge((PartitionAttributesImpl) partition);
      } else {
        this.regionAttributes.partitionAttributes = partition;
        this.regionAttributes.setHasPartitionAttributes(true);
      }
      
      ((PartitionAttributesImpl) this.regionAttributes.partitionAttributes).setEnableOffHeapMemory(this.regionAttributes.enableOffHeapMemory);
    }
    else {
  ...
  public void setEnableOffHeapMemory(boolean enableOffHeapMemory) {
    this.regionAttributes.enableOffHeapMemory = enableOffHeapMemory;
    this.regionAttributes.setHasEnableOffHeapMemory(true);
    
    if (this.regionAttributes.partitionAttributes != null) {
      ((PartitionAttributesImpl) this.regionAttributes.partitionAttributes).setEnableOffHeapMemory(enableOffHeapMemory);
    }
  }
  ...
  public RegionAttributes<K,V> create() {
    ...
    // As of 6.5 we automatically enable stats if expiration is used.
    {
      ...
      // GemFireXD does not handle PRELOADED, so do not change the policy
  ...
  public static void validateAttributes(RegionAttributes<?, ?> attrs) {
    ...
    if (attrs.getDataPolicy().withReplication()
      ...
      if (!isForBucketRegion) {
        ...
        // Allow local destroy for internal notification regions. The following check ensures that for Notification regions used for management , factory will allow local destroy for replicated regions.
        // Not to import management package into core gemfire , using string names for class rather than instanceof 
        boolean notificationRegion = false;
        if (attrs.getCacheListeners().length != 0
            && attrs.getCacheListeners()[0].getClass().getName().equals(NOTIFICATION_REGION_CACHE_LISTENER)) {
          notificationRegion = true;
        }
        if (!notificationRegion && attrs.getEvictionAttributes().getAction().isLocalDestroy()) {
...
//        }
    ...
    if (pa != null) {
      // GemFireXD will do its own validation and explicitly invoke
      // validateAttributes when required
      if (!GemFireCacheImpl.gfxdSystem()) {
        ((PartitionAttributesImpl)pa).validateWhenAllAttributesAreSet(
            attrs instanceof RegionAttributesCreation);
      }
...
//      }
      if (pa.getLocalMaxMemory() < 0 && pa.getLocalMaxMemory() != PartitionAttributesImpl.USE_CALCULATED_VALUE) {
      ...
      if (pa.getLocalMaxMemory() == 0 && attrs.getDataPolicy().withPersistence()) {
  ...
  extends UserSpecifiedRegionAttributes<K,V> implements Cloneable, Serializable {
    ...
    // for GemFireXD the default for concurrency-checks-enabled is determined
    // from GemFireXD layer
    boolean concurrencyChecksEnabled = defaultConcurrencyChecksEnabled();
    boolean earlyAck = false;
    boolean enableGateway = false;
    String gatewayHubId = ""; 
    ...
    boolean enableOffHeapMemory = false;
    ...
    private static boolean defaultConcurrencyChecksEnabled() {
      final StaticSystemCallbacks sysCb = GemFireCacheImpl
          .getInternalProductCallbacks();
      return sysCb == null || sysCb.defaultRegionConcurrencyChecksEnabled();
    }
    ...
    public String toString() {
      ...
      buf
        ...
        .append("; enableGateway=").append(enableGateway)
      ...
      buf.append("; enableOffHeapMemory=").append(enableOffHeapMemory);
      return buf.toString();
    }
    ...
    public boolean getPersistBackup() {
      return getDataPolicy() == DataPolicy.PERSISTENT_REPLICATE;
    }
