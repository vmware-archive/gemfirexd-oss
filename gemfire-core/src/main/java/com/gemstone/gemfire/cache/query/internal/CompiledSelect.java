...
public class CompiledSelect extends AbstractCompiledValue {
        ...
        if (!this.whereClause.isDependentOnCurrentScope(context)) { // independent
          ...
          try {
            ...
            if (evalAsFilters) {
              ...
              // Asif: Hack for customer. Ignore order by attribs for a while
            ...
            releaseReadLockOnUsedPartitionedIndex(planInfo);
          }
        }
      }
  ...
  /**
   * A PartitionedIndex is locked during query to prevent it from being
   ...
   * @param planInfo
   */
  private void releaseReadLockOnUsedPartitionedIndex(PlanInfo planInfo) {
    List inds = planInfo.indexes;
   /* LogWriterI18n logger = GemFireCacheImpl.getInstance().getLoggerI18n();
    if (logger.fineEnabled()) {
      logger.fine("Releasing read locks on " + inds.size() + " indexes ");
    }*/
  ...
  private int doNestedIterations(int level, SelectResults results, ExecutionContext context,
    ...
    if (level == iterList.size()) {
      ...
      if (sr == null) {
        throw new CompiledSelect.NullIteratorException(); // result of query is UNDEFINED
      }
...
class OrderByComparator implements Comparator 
{
  ...
  public OrderByComparator(ObjectType objType) {
    ...
    if(objType.isStructType()) {
      orderByMap = new THashMap(new StructBag.ObjectArrayTHashingStrategy());
    }else {
