      ...
      if (oldListeners != null) {
        ...
        for (Iterator iter = oldListeners.iterator(); iter.hasNext();) {
          try {
            ...
            logger.warning( LocalizedStrings.CqAttributesFactory_EXCEPTION_OCCURED_WHILE_CLOSING_CQ_LISTENER_ERROR_0, ex.getLocalizedMessage());
            ...
            logger.warning(LocalizedStrings.CqAttributesFactory_RUNTIME_EXCEPTION_OCCURED_WHILE_CLOSING_CQ_LISTENER_ERROR_0, t.getLocalizedMessage());
        ...
        if (oldListeners != null) {
          if (oldListeners.remove(cql)) {
            ...
            try {
              ...
              logger.warning(LocalizedStrings.CqAttributesFactory_EXCEPTION_CLOSING_CQ_LISTENER_ERROR_0, ex.getLocalizedMessage());
              logger.fine(ex.getMessage(), ex);
            }
            catch (Throwable t) {
              Error err;
              ...
              // is still usable:
              SystemFailure.checkFailure();
              logger.warning(LocalizedStrings.CqAttributesFactory_RUNTIME_EXCEPTION_OCCURED_CLOSING_CQ_LISTENER_ERROR_0, t.getLocalizedMessage());
              logger.fine(t.getMessage(), t);
            }        
          }
        }
