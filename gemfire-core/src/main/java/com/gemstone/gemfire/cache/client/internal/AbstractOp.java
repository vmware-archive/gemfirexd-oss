...
public abstract class AbstractOp implements Op {
  ...
  private void setMsgTransactionId() {
    if (participateInTransaction()
        && getMessage().getTransactionId() == TXManagerImpl.NO_TX) {
      final TXId txId = TXManagerImpl.getCurrentTXId();
      if (txId != null) {
        // TODO: merge: client TX should be updated to deal with full TXIds
        getMessage().setTransactionId(txId.getUniqId());
      }
    }
  }
