...
public abstract class HDFSSplitIterator {
  ...
  protected LogWriterI18n logger = ComponentLogWriter.getHoplogLogWriter("HDFSSplitIterator",
      LogService.logger());
  ...
  public final boolean hasNext() throws IOException {
    while (currentHopIndex < split.getNumPaths()) {
      ...
      if (iterator == null) {
        ...
        currentHopIndex++;
        while (currentHopIndex < split.getNumPaths() && !fs.exists(split.getPath(currentHopIndex))){
          logger.warning(LocalizedStrings.HOPLOG_CLEANED_UP_BY_JANITOR, split.getPath(currentHopIndex).toString());
          currentHopIndex++;
        }
        if (currentHopIndex >= split.getNumPaths()) {
          return false;
