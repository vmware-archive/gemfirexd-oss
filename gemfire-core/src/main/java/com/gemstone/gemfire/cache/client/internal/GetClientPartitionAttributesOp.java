...
public class GetClientPartitionAttributesOp {
  ...
  public static ClientPartitionAdvisor execute(ExecutablePool pool, String regionFullPath) {
    ...
    AbstractOp op = new GetClientPartitionAttributesOpImpl(logger, regionFullPath);
    if (logger.fineEnabled()) {
      logger
          .fine("GetClientPartitionAttributesOp#execute : Sending GetClientPartitionAttributesOp Message:"
              + op.getMessage()
              + " for region: "
              + regionFullPath
              + " to server using pool: " + pool);
    }
  ...
  static class GetClientPartitionAttributesOpImpl extends AbstractOp {
    ...
    public GetClientPartitionAttributesOpImpl(LogWriterI18n lw, String regionFullPath) {
      super(lw, MessageType.GET_CLIENT_PARTITION_ATTRIBUTES, 1);
      logger = lw;
    ...
    protected Object processResponse(Message msg) throws Exception {
      switch (msg.getMessageType()) {
        ...
        case MessageType.RESPONSE_CLIENT_PARTITION_ATTRIBUTES:
          if (logger.fineEnabled()) {
            logger
                .fine("GetClientPartitionAttributesOpImpl#processResponse: received message of type : " + MessageType.getString(msg.getMessageType()));
          }
          int bucketCount;
          ...
          if (logger.fineEnabled()) {
            logger
                .fine("GetClientPartitionAttributesOpImpl#processResponse: received all the results from server successfully.");
          }
        ...
        case MessageType.EXCEPTION:
          if (logger.fineEnabled()) {
            logger
                .fine("GetClientPartitionAttributesOpImpl#processResponse: received message of type EXCEPTION");
          }
