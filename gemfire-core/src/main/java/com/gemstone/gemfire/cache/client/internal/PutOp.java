...
public class PutOp {
  ...
  {
    AbstractOp op = new PutOpImpl(pool.getLoggerI18n(), region, key, value, deltaBytes,
        event, operation,
        requireOldValue, expectedOldValue,
        callbackArg, false/*donot send full obj; send delta*/, prSingleHopEnabled);
  ...
  public static Object execute(ExecutablePool pool, String regionName,
    ...
    AbstractOp op = new PutOpImpl(pool.getLoggerI18n(), regionName, key, value, deltaBytes,
        event, operation,
        requireOldValue, expectedOldValue,
        callbackArg,  false/*donot send full obj; send delta*/, prSingleHopEnabled);
  ...
  public static void execute(Connection con,
                             ...
                             boolean prSingleHopEnabled)
  {
    AbstractOp op = new PutOpImpl(pool.getLoggerI18n(), regionName, key, value,
        null, event, Operation.CREATE,
        false, null, callbackArg, false /*donot send full Obj; send delta*/, prSingleHopEnabled);
  ...
  private static class PutOpImpl extends AbstractOp {
    ...
    public PutOpImpl(LogWriterI18n lw , String regionName, Object key, Object value, 
        byte[] deltaBytes,
        EntryEventImpl event, Operation op,
        boolean requireOldValue, Object expectedOldValue,
        Object callbackArg, boolean sendFullObj, boolean prSingleHopEnabled) {
        super(lw, MessageType.PUT,
            7 + (callbackArg != null? 1:0) + (expectedOldValue != null? 1:0));
        if (lw.fineEnabled()) {
          lw.fine("PutOpImpl constructing(1) message for " + event.getEventId() + "; operation=" + op
              /*+ "; requireOldValue=" + requireOldValue + "; expectedOldValue="
              + expectedOldValue*/);
        }
        this.key = key;
        ...
        if (!sendFullObj && deltaBytes != null && op == Operation.UPDATE) {
          ...
          this.deltaSent = true;
          if (lw.fineEnabled()) {
            lw.fine("PutOp: Sending delta for key " + this.key);
          }
        }
    ...
    public PutOpImpl(LogWriterI18n lw, Region region, Object key, Object value,
        byte[] deltaBytes, 
        EntryEventImpl event, Operation op, 
        boolean requireOldValue, Object expectedOldValue,
        Object callbackArg, boolean sendFullObj, boolean prSingleHopEnabled) {
      ...
      this.prSingleHopEnabled = prSingleHopEnabled;
      if (lw.fineEnabled()) {
        lw.fine("PutOpImpl constructing message with operation=" + op
            /*+ "; requireOldValue=" + requireOldValue + "; expectedOldValue="
            + expectedOldValue + "; event=" + event*/);
      }
        ...
        throws Exception
    {
      ...
      if (msgType == MessageType.REPLY) {
        ...
        if (msgType == MessageType.PUT_DELTA_ERROR) {
          if (lw.fineEnabled()) {
            lw.fine("PutOp: Sending full value as delta failed on server...");
          }
          AbstractOp op = new PutOpImpl(this.lw, this.regionName, this.key,
                this.value, null, this.event, Operation.CREATE,
                this.requireOldValue, this.expectedOldValue,
                this.callbackArg, true /* send full obj */, this.prSingleHopEnabled);
