...
public class DestroyOp {
  ...
  public static Object execute(ExecutablePool pool, LocalRegion region,
    ...
    if (log != null && log.fineEnabled()) {
      log.fine("Preparing DestroyOp for " + key + " operation=" + operation /*+ " value=" + expectedOldValue*/);
    }
    AbstractOp op = new DestroyOpImpl(pool.getLoggerI18n(), region, key,
        expectedOldValue, operation, event, callbackArg, prSingleHopEnabled);
  ...
  private static class DestroyOpImpl extends AbstractOp {
     ...
     */
    public DestroyOpImpl(LogWriterI18n lw,
                         LocalRegion region,
                         Object key,
                         Object expectedOldValue,
                         Operation operation,
                         ...
                         Object callbackArg,
                         boolean prSingleHopEnabled) {
      super(lw, MessageType.DESTROY, callbackArg != null ? 6 : 5);
      this.key = key;
      this.region = region ;
    ...
    public DestroyOpImpl(LogWriterI18n lw, String region, Object key,
        Object expectedOldValue, Operation operation, 
        EntryEventImpl event, Object callbackArg) {
      super(lw, MessageType.DESTROY, callbackArg != null ? 6 : 5);
      this.key = key;
    ...
    protected Object processResponse(Message msg, Connection con) throws Exception {
      ...
      if (isReply) {
        ...
        if ((flags & HAS_VERSION_TAG) != 0) {
          ...
          if (log.fineEnabled()) {
            log.fine("received Destroy response with " + tag);
            }
        } else if (log.fineEnabled()) {
          log.fine("received Destroy response with no version tag");
        }
      }
      if (prSingleHopEnabled) {
        byte version = 0 ;
...
//        }
      ...
      if (msg.getMessageType() == MessageType.REPLY && (flags & HAS_ENTRY_NOT_FOUND_PART) != 0) {
        ...
        if (log.fineEnabled() && (flags & HAS_ENTRY_NOT_FOUND_PART) != 0) {
          log.fine("destroy response has entryNotFound="+entryNotFound);
        }
        if (entryNotFound) {
      ...
      if (this.operation == Operation.REMOVE && entryNotFound) {
        if (log.fineEnabled()) {
          log.fine("received REMOVE response from server with entryNotFound=" + entryNotFound);
        }
