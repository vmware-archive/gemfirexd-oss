...
public class ServerRegionProxy extends ServerProxy implements ServerRegionDataAccess {
  ...
  /* (non-Javadoc)
   * @see com.gemstone.gemfire.cache.client.internal.ServerRegionDataAccess#putAll(java.util.Map, com.gemstone.gemfire.internal.cache.EventID)
   */
  public VersionedObjectList putAll(Map map, EventID eventId, boolean skipCallbacks) {
    ...
    if (this.pool.getPRSingleHopEnabled() && (txId == null)) {
      return PutAllOp.execute(this.pool, this.region, map, eventId, skipCallbacks, this.pool.getRetryAttempts());
    }
    else {
  ...
  /* (non-Javadoc)
   * @see com.gemstone.gemfire.cache.client.internal.ServerRegionDataAccess#getAll(java.util.List)
   */
  public VersionedObjectList getAll(List keys) {
    //recordTXOperation(ServerRegionOperation.GET_ALL, null, keys);
    final TXId txId = TXManagerImpl.getCurrentTXId();
    VersionedObjectList result;
    if (this.pool.getPRSingleHopEnabled() && (txId == null)) {
      result = GetAllOp.execute(this.pool, this.region, keys,this.pool.getRetryAttempts());
    }
    else {
      result = GetAllOp.execute(this.pool, this.regionName, keys);
    }
    if (result != null) {
    ...
    if (this.pool.getPRSingleHopEnabled()) {
      ...
      if (serverRegionExecutor.getFilter().isEmpty()) {
        ...
        if (serverToFilterMap == null || serverToFilterMap.isEmpty()) {
          ...
          ExecuteRegionFunctionSingleHopOp.execute(this.pool, this.region,
              function, serverRegionExecutor, resultCollector, hasResult,
              serverToFilterMap, retryAttempts, false);
        }
      }
    }
    else {
  ...
  /* (non-Javadoc)
   * @see com.gemstone.gemfire.cache.client.internal.ServerRegionDataAccess#getAllOnPrimary(com.gemstone.gemfire.cache.client.internal.Connection, java.util.List)
   */
  public VersionedObjectList getAllOnPrimaryForRegisterInterest(Connection con, List keys) {
    if (con != null) {
      return GetAllOp.executeOn(con, this.pool, this.regionName, keys, true);
    }
    else {
      return GetAllOp.executeOnPrimary(this.pool, this.regionName, keys, true);
    }
  }
   ...
   */
  public TXRemoteCommitMessage afterCompletion(int status, int txId) {
  ...
  /** test hook */
  // TODO: merge: update client TX to deal with new TX model
  /*
