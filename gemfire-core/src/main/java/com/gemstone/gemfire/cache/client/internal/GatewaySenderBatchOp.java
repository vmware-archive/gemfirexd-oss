...
public class GatewaySenderBatchOp {
   ...
   */
  public static void executeOn(Connection con, ExecutablePool pool, List events, int batchId, boolean removeFromQueueOnException)
  {
    AbstractOp op = null;
    //System.out.println("Version: "+con.getWanSiteVersion());
    if (con.getWanSiteVersion() <= Version.GFE_651.ordinal()) {
      op = new GatewaySenderGFEBatchOpImpl(pool.getLoggerI18n(), events, batchId, removeFromQueueOnException, con.getDistributedSystemId());
    } else {
  ...
  public static Object executeOn(Connection con, ExecutablePool pool)
  {
    ...
    if (con.getWanSiteVersion() <= Version.GFE_651.ordinal()) {
      op = new GatewaySenderGFEBatchOpImpl(pool.getLoggerI18n());
    } else {
      // Default should create a batch of server version (ACCEPTOR.VERSION)
      op = new GatewaySenderGFEBatchOpImpl(pool.getLoggerI18n());
    }
  ...
  static class GatewaySenderGFEBatchOpImpl extends AbstractOp {
     ...
     */
    public GatewaySenderGFEBatchOpImpl(LogWriterI18n lw, List events, int batchId, boolean removeFromQueueOnException, int dsId)  {
      super(lw, MessageType.GATEWAY_RECEIVER_COMMAND, calcPartCount(events));
      ...
      for (Iterator i = events.iterator(); i.hasNext();) {
        ...
        if ((action >= 0 && action <= 2) || (action == 4)) {
          ...
          // part
          getMessage().addBytesPart(event.getSerializedValue());
          Object callbackArg = event.getSenderCallbackArgument();
          getMessage().addObjPart(callbackArg);
        }
      }
    ...
    public GatewaySenderGFEBatchOpImpl(LogWriterI18n lw) {
      super(lw, MessageType.GATEWAY_RECEIVER_COMMAND, 0);
    }
        ...
        case MessageType.EXCEPTION:
          ...
          if (obj instanceof List) {
            ...
            if (getLogger().fineEnabled()) {
              getLogger().fine(
                  " We got an exception from the GatewayReceiver. "
                      + msg.getMessageType() + " obj :" + obj);
            }
