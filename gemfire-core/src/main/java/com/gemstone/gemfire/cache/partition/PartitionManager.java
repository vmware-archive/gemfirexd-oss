...
public final class PartitionManager {
    ...
    try {
      ...
      if (primary == null) {
        ...
        if(logger.fineEnabled()) {
          logger.fine("createPrimaryBucket: " + pr + ", bucket " + bucketId + " no existing primary, creating primary");
        }
        createdBucket = createBucket(self, pr, bucketId, destroyExistingRemote);
      }
      else if (self.equals(primary)) {
        if (destroyExistingLocal) {
          logger.fine("createPrimaryBucket: " + pr + ", bucket " + bucketId + " already primary, destroying local primary");
          if (dumpBucket(self, region, bucketId)) {
            createdBucket = createBucket(self, region, bucketId, destroyExistingRemote);
          }
        } else {
          if(logger.fineEnabled()) {
            logger.fine("createPrimaryBucket: " + pr + ", bucket " + bucketId + " already primary, no action needed");
          }
        }
      }
      else {
        if (destroyExistingRemote) {
          if(logger.fineEnabled()) {
            logger.fine("createPrimaryBucket: " + pr + ", bucket " + bucketId + ", " + primary + " is primary, destroying it");
          }
  ...
  private static boolean createBucket(final DistributedMember target,
    ...
    if(currentBucketOwners.size() > pr.getRedundantCopies() && includesTarget) {
      ...
      if(destroyExistingRemote) {
        if(logger.fineEnabled()) {
          logger.fine("createPrimaryBucket: "
                  + pr
                  + ", bucket "
                  + bucketId
                  + " Redundancy is exceeded due to concurrent bucket create, destroying remote bucket: " + remoteBucket);
        }
        ...
        if(logger.fineEnabled()) {
          logger.fine("createPrimaryBucket: "
                  ...
                  + ", bucket "
                  + bucketId
                  + " Redundancy is exceeded due to concurrent bucket create, destroying my bucket");
        }
