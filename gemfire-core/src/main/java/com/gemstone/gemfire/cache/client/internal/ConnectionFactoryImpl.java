...
//    long startTime = System.currentTimeMillis();
    ...
    do {
      ...
      if(server == null) {
        ...
        logger.fine("Source was unable to locate any servers");
      ...
      try {
        ...
        fatalException = srce;
        logger.fine("ServerRefusedConnectionException attempting to connect to " + server , srce);
      } catch (Exception e) {
        logger.fine("Error attempting to connect to " + server , e);
      }
...
//    }
  ...
  public ClientUpdater createServerToClientConnection(Endpoint endpoint,
    ...
    if (logger != null && logger.fineEnabled()) {
      logger.fine("Establishing: " + clientUpdateName);
    }
    //ds.getLogWriter().warning("CREATIN QUEUE CONNECTION:"+endpoint.getLocation()+" MEMBERID:"+endpoint.getMemberId(),new Exception());
//  Launch the thread
    CacheClientUpdater updater = new CacheClientUpdater(clientUpdateName,
        endpoint.getLocation(), isPrimary, ds, handshake, qManager,
