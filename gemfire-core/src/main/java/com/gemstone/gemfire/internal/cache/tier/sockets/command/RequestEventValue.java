  ...
  public void cmdExecute(Message msg, ServerConnection servConn, long start)
    ...
    if (eventIDPart == null) {
      ...
      if (logger.finerEnabled()) {
        logger.finer(servConn.getName()
            + ": Received get event value request (" + msg.getPayloadLength()
      ...
      if (haContainer == null) {
        ...
        try {
          ...
          if (data == null) {
            ...
            if (logger.fineEnabled()) {
              logger.fine("Value retrieved for event " + event);
            }
        ...
        if (logger.fineEnabled()) {
          logger.fine(servConn.getName()
              + ": Wrote get event value response back to "
              + servConn.getSocketString() + " for ha container "
              + haContainer.getName());
        }
      }
    }
