...
public class GatewaySenderAdvisor extends DistributionAdvisor {
  ...
  private final ThreadGroup threadGroup = 
    LogWriterImpl.createThreadGroup("GatewaySenderAdvisor Threads", (LogWriterI18n)null);
  
  private final AbstractGatewaySender sender;
  
  private final LogWriterI18n logger;
  
  private GatewaySenderAdvisor(DistributionAdvisee sender, LogWriterI18n logger) {
    super(sender);
  ...
  private void checkCompatibility(GatewaySenderProfile sp) {
    ...
    //it should be fine as it is jvm specific
  ...
  protected void profileRemoved(Profile profile) {
    if (profile instanceof GatewaySenderProfile) {
      ...
      if (!sp.isParallel) {// SerialGatewaySender
        ...
        if (!((SerialGatewaySenderImpl)this.sender).isPrimary()) {//IF this sender is not primary
          if (!adviseEldestGatewaySender()) {//AND this is not the eldest sender 
        	logger.fine("Sender " + this.sender + " is not the eldest in the system. Giving preference to eldest sender to become primary...");
        	return;
          }
          launchLockObtainingVolunteerThread();
        }
  ...
  public void initDLockService() {
    ...
    if (logger.fineEnabled()) {
      logger.fine(this + ": Obtained DistributedLockService: "
          + this.lockService);
    }
  }
  ...
  public boolean volunteerForPrimary() {
    if (logger.fineEnabled()) {
      logger.fine("Sender : " + this.sender.getId() + " is volunteering for Primary ");
    }
    ...
    if (advisePrimaryGatewaySender() == null) {
      ...
      if (logger.fineEnabled()) {
        logger.fine("Sender : " + this.sender
            + " no Primary available. So going to acquire distributed lock");
      }
  ...
  public void makePrimary() {
    logger.info(LocalizedStrings.SerialGatewaySenderImpl_0__STARTING_AS_PRIMARY, this.sender);
    ...
    synchronized (this.primaryLock) {
      this.isPrimary = true;
      logger.info(LocalizedStrings.SerialGatewaySenderImpl_0__BECOMING_PRIMARY_GATEWAYSENDER,
          this.sender);
  ...
  public void makeSecondary() {
    if (logger.fineEnabled()) {
      logger.fine(this.sender + ": Did not obtain the lock on "
          + this.lockToken + ". Starting as secondary gateway sender.");
    }

    // Set primary flag to false
    logger.info(LocalizedStrings.SerialGatewaySenderImpl_0__STARTING_AS_SECONDARY_BECAUSE_PRIMARY_GATEWAY_SENDER_IS_AVAIALABLE_ON_MEMBER_2,
  ...
  public void launchLockObtainingVolunteerThread() {
    this.lockObtainingThread = new Thread(threadGroup, new Runnable() {
      ...
      public void run() {
          final ReadLock rlock = GatewaySenderAdvisor.this.sender.lifeCycleLock
              .readLock();
          rlock.lock();
          try {
            ...
            if (logger.fineEnabled()) {
              logger.fine(this + ": Obtaining the lock on "
                  + GatewaySenderAdvisor.this.lockToken);
            }

            if (volunteerForPrimary()) {
              logger.fine(this + ": Obtained the lock on "
                  + GatewaySenderAdvisor.this.lockToken);
              logger
                  .info(
              ...
              // check for running status again
              if (!GatewaySenderAdvisor.this.sender.isRunning()) {
                return;
              }
            ...
            if (GatewaySenderAdvisor.this.sender.getStopper()
  ...
  public void waitToBecomePrimary() throws InterruptedException
  {
    ...
    synchronized (this.primaryLock) {
      while (!isPrimary()) {
        logger.info(LocalizedStrings.GatewayImpl_0__WAITING_TO_BECOME_PRIMARY_GATEWAY, this.sender.getId());
  ...
  public static final class GatewaySenderProfile extends DistributionAdvisor.Profile {
    ...
    public void processIncoming(DistributionManager dm, String adviseePath,
      ...
      if (cache != null) {
        AbstractGatewaySender sender = (AbstractGatewaySender)cache
