...
//    callbackArgPart = null; (redundant assignment)
    ...
    if (logger.fineEnabled()) {
      logger.fine(servConn.getName() + ": Received invalidate request ("
...
//            }
          ...
          if (e instanceof GemFireSecurityException) {
            ...
            logger.warning(LocalizedStrings.BaseCommand_0_UNEXPECTED_EXCEPTION, servConn.getName(), e); 
          }
          return;
        }
        ...
        if (logger.fineEnabled()) {
          logger.fine(servConn.getName()
              + ": Sent invalidate response for region " + regionName + " key "
              + key);
        }
