...
public class ObjectUtil
{
	...
	public static Object getPrimitive(Gfsh gfsh, String value, boolean quoted) throws ParseException
	{
		...
		if (value.endsWith("b")) {
			// byte
			obj = Byte.valueOf(value.substring(0, value.length() - 1));
		} else if (value.endsWith("c")) {
			// char
			obj = value.charAt(0);
		} else if (value.endsWith("s")) {
			// short
			obj = Short.valueOf(value.substring(0, value.length() - 1));
		} else if (value.endsWith("i")) {
			// int
			obj = Integer.valueOf(value.substring(0, value.length() - 1));
		} else if (value.endsWith("l")) {
			// long
			obj = Long.valueOf(value.substring(0, value.length() - 1));
		} else if (value.endsWith("f")) {
			// float
			obj = Float.valueOf(value.substring(0, value.length() - 1));
		} else if (value.endsWith("d")) {
			// double
			obj = Double.valueOf(value.substring(0, value.length() - 1));
