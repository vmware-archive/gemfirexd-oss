...
public class ControllerAdvisor extends GridAdvisor {
  ...
  public final void handshake() {
    super.handshake();
    // also inform local GemFireXD advisor if any
    final GemFireCacheImpl cache = GemFireCacheImpl.getInstance();
    final DistributionAdvisee gfxdAdvisee;
    final DistributionAdvisor gfxdAdvisor;
    GridProfile profile = (GridProfile) createProfile();
    if (cache != null && (gfxdAdvisee = cache.getGfxdAdvisee()) != null
        && (gfxdAdvisor = gfxdAdvisee.getDistributionAdvisor()) != null) {
      gfxdAdvisor.putProfile(profile);
    }
  }
