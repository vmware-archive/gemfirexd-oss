...
public final class PrimaryRequestMessage extends PartitionMessage
{
  ...
  private PrimaryRequestMessage(Set recipients, int regionId,
      ReplyProcessor21 processor, int bId) {
      ...
      PartitionedRegion pr, long startTime) throws CacheException, ForceReattemptException
  {
    ...
    if (DistributionManager.VERBOSE) {
      l.fine("PrimaryRequestMessage operateOnRegion: " + pr.getFullPath());
    }
      ...
      try {
        if (msg instanceof PrimaryRequestReplyMessage) {
          ...
          if (reply.isPrimary) {
            this.msg = reply;
            if (DistributionManager.VERBOSE) {
              getDistributionManager().getLoggerI18n().fine("PrimaryRequestResponse primary is " + this.msg.getSender()); 
            }
          } else {
            if (DistributionManager.VERBOSE) {
              getDistributionManager().getLoggerI18n().fine("PrimaryRequestResponse " + this.msg.getSender() + " is not primary"); 
            }
          }
        } else {
