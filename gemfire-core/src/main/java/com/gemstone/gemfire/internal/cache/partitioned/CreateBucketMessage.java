...
public final class CreateBucketMessage extends PartitionMessage {
  ...
  protected boolean operateOnPartitionedRegion(DistributionManager dm, 
    ...
    LogWriter l = r.getCache().getLogger();
    if (DistributionManager.VERBOSE) {
      l.fine("CreateBucketMessage operateOnRegion: " + r.getFullPath());
    }
  ...
  public static final class CreateBucketReplyMessage extends ReplyMessage {
    ...
    public void process(final DM dm, final ReplyProcessor21 processor) {
      ...
      if (DistributionManager.VERBOSE) {
        l.fine("CreateBucketReplyMessage process invoking reply processor with processorId:" + this.processorId);
      }

      if (processor == null) {
        if (DistributionManager.VERBOSE) {
          l.fine("CreateBucketReplyMessage processor not found");
        }
        return;
      }
    ...
    public InternalDistributedMember waitForResponse()  throws ForceReattemptException
    {
      try {
        ...
        if (t instanceof CancelException) {
          String msg = "NodeResponse got remote cancellation, throwing PartitionedRegionCommunication Exception";
          getDistributionManager().getLoggerI18n().fine(msg, t);
          return null;
        }
        if (t instanceof PRLocallyDestroyedException) {
          String msg = "NodeResponse got local destroy on the PartitionRegion , throwing ForceReattemptException";
          getDistributionManager().getLoggerI18n().fine(msg, t);
          return null;
        }
