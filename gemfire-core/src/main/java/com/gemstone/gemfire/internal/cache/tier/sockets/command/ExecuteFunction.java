    ...
    try {
      ...
      if (logger.warningEnabled()) {
        logger.warning(LocalizedStrings.
          ExecuteFunction_EXCEPTION_ON_SERVER_WHILE_EXECUTIONG_FUNCTION_0, 
          function, exception);
      }
      if (hasResult == 1) {
    ...
    if (function == null) {
      ...
      try {
        ...
        try {
          ...
          ServerToClientFunctionResultSender.flushTXPendingOps();

          long startExecution = stats.startTime();
          stats.startFunctionExecution(functionObject.hasResult());
          if(logger.fineEnabled()){
            logger.fine("Executing Function on Server: " + servConn.toString() + "with context :" + context.toString());
          }
