...
public final class FetchKeysMessage extends PartitionMessage
  ...
  public static FetchKeysResponse sendInterestQuery(
    ...
    FetchKeysResponse p = (FetchKeysResponse)tmp.createReplyProcessor(r,
        Collections.singleton(recipient), tx);
    FetchKeysMessage m = new FetchKeysMessage(recipient, r.getPRId(), p, tx,
        bucketId, itype, arg, allowTombstones);
    Set failures =r.getDistributionManager().putOutgoing(m); 
    if (failures != null && failures.size() > 0 ) {
      throw new ForceReattemptException(LocalizedStrings.FetchKeysMessage_FAILED_SENDING_0.toLocalizedString(m));
    }
  ...
  PartitionResponse createReplyProcessor(PartitionedRegion r, Set recipients,
    ...
    return new FetchKeysResponse(r.getSystem(), r, recipients, tx);
  }
  ...
  protected boolean operateOnPartitionedRegion(DistributionManager dm,
    ...
    if (l.fineEnabled()) {
      l.fine("FetchKeysMessage operateOnRegion: " + r.getFullPath()
          + " bucketId: " + this.bucketId
          + " type: " + InterestType.getString(interestType)
    ...
    if (ds != null) {
      try {
        ...
        if (DistributionManager.VERBOSE) {
          l.fine("FetchKeysMessage sending " + keys.size()
              + " keys back using processorId: " + getProcessorId() + ": " + keys);
        }
        ...
        if (l.fineEnabled()) {
          l.fine("FetchKeysMessage Encountered PRLocallyDestroyedException");
        }
      ...
      l.warning(LocalizedStrings.FetchKeysMessage_FETCHKEYSMESSAGE_DATA_STORE_NOT_CONFIGURED_FOR_THIS_MEMBER);
    }
  ...
  public void fromData(DataInput in)
    ...
    try {
      ...
      // added in 7.5 release, 2013.  Older versions of the product do not send allowTombstones
    }
  }
...
//                throw new InternalGemFireError(LocalizedStrings.FetchKeysMessage_ALREADY_PROCESSED_LAST_CHUNK.toLocalizedString());
  ...
  public static class FetchKeysResponse extends PartitionResponse  {
    ...
    public FetchKeysResponse(InternalDistributedSystem ds,
        PartitionedRegion pr, Set recipients, final TXStateInterface tx) {
    ...
    public Set waitForKeys() throws ForceReattemptException
    {
      try {
        ...
        if (t instanceof CancelException) {
          getDistributionManager().getLoggerI18n().fine("FetchKeysResponse got remote CacheClosedException; forcing reattempt.", t);
        ...
        if (t instanceof ForceReattemptException) {
          getDistributionManager().getLoggerI18n().fine("FetchKeysResponse got remote ForceReattemptException; rethrowing.", e);
