 ...
 * For the new transaction model this has been modified to be sent to all the
 * nodes which has either the primary or the redundant copy of the key.
 *
 * @author Mitch Thomas
 * @author bruce
 * @since 5.0
...
public final class PutMessage extends PartitionMessageWithDirectReply implements NewValueImporter {
   ...
   * 
   * Now also sent across for DESERIALIZATION_POLICY_EAGER.
   */
 ...
 /** whether new value is formed by applying delta **/
  ...
  PutMessage(PutMessage original,EntryEventImpl event, Set<?> members ) {
    ...
    if (original.valBytes != null) {
      ...
      if(original.valObj instanceof CachedDeserializable) {
        if (original.valObj instanceof StoredObject && (CachedDeserializableFactory
            .preferObject() || !((StoredObject)original.valObj).isSerialized())) {
  ...
  @Override
  public PartitionMessage getMessageForRelayToListeners(EntryEventImpl ev,
      Set members) {
  ...
  private PutMessage(InternalDistributedMember recipient,
                     boolean notifyOnly,
                     int regionId,
    ...
    initialize(notifyOnly, event, lastModified, ifNew, ifOld, expectedOldValue,
  ...
  private final void initialize(boolean notifyOnly, EntryEventImpl event,
      final long lastModified, boolean ifNew, boolean ifOld,
   ...
   */
  public static PutResponse send(final InternalDistributedMember recipient,
                                 PartitionedRegion r,
                                 EntryEventImpl event,
                                 final long lastModified,
                                 boolean ifNew,
  ...
  throws ForceReattemptException {
    ...
    final PutMessage m = prepareSend(r.getSystem(), recipient, r, event,
    ...
    return (PutResponse)m.processor;
  }

  /**
   * Prepares a {@link PutMessage} for sending to the given recipient but does
   * not actually send using {@link DistributionManager}.
   * 
   * @param sys
   ...
   * @param r
   *          the PartitionedRegion for which the put was performed
   * @param event
   ...
   * 
   * @return the PutMessage that was prepared for sending to the recipient
   */
  public static PutMessage prepareSend(final InternalDistributedSystem sys,
      final InternalDistributedMember recipient, PartitionedRegion r,
      EntryEventImpl event, final long lastModified, boolean ifNew,
      boolean ifOld, Object expectedOldValue, boolean requireOldValue,
      final boolean cacheWrite) {
    //recipient can be null for event notifications
    //Assert.assertTrue(recipient != null, "PutMessage NULL recipient");

    final TXStateInterface tx = event.getTXState(r);
    final PutResponse processor = new PutResponse(sys, recipient,
        event.getKey(), tx);
    final PutMessage m = new PutMessage(recipient, false, r.getPRId(),
        processor, event, tx, lastModified, ifNew, ifOld, expectedOldValue,
        requireOldValue, cacheWrite);
    m.setInternalDs(sys);
    m.setSendDelta(true);
   ...
   * @throws ForceReattemptException if the peer is no longer available
   */
  public static PartitionResponse send(final Set<DistributedMember> recipients,
                                       PartitionedRegion r,
                                       EntryEventImpl event,
                                       final long lastModified,
                                       boolean ifNew,
  ...
  throws ForceReattemptException {
    final TXStateInterface tx = event.getTXState(r);
    PutResponse processor = new PutResponse(r.getSystem(), recipients,
        event.getKey(), tx);
...
//  }
  ...
  EntryEventImpl createListenerEvent(EntryEventImpl sourceEvent,
      PartitionedRegion r, InternalDistributedMember member) {
  ...
  public final void toData(DataOutput out) throws IOException {
    ...
    if (this.valObj != null && this.sendDeltaWithFullValue
    ...
    if (this.hasDelta) {
      ...
      if ((this.valObj != null || getValBytes() != null) && this.event.getDeltaBytes() != null
...
//      l.fine("PutMessage operateOnRegion: "
//          + r.getFullPath());
//    }
    ...
    if (r.keyRequiresRegionContext()) {
      ...
      else if (this.valObj != null && !(this.valObj instanceof
          com.gemstone.gemfire.internal.cache.delta.Delta)) {
    ...
    ev.setLockingPolicy(getLockingPolicy());
    ...
    if (this.hasDelta) {
      ...
      switch (this.deserializationPolicy) {
        ...
        case DistributedCacheOperation.DESERIALIZATION_POLICY_EAGER:
          // new value is a Delta; now also for other objects
...
//        this.op = r.doCacheWriteBeforePut(event, ifNew);  // TODO fix this for bug 37072
      ...
      if (DistributionManager.VERBOSE) {
        l.info(
            ...
            "PutMessage " + (result? "updated bucket" : "did not update bucket")
            + /*PartitionedRegionHelper.getHashKey(r, event.getOperation(), event.getKey(),
                event.getCallbackArgument()) +*/ " with key: "
            + getKey() + " val: " +
//            getValBytes()
  ...
  protected final void appendFields(StringBuilder buff) {
    ...
    buff.append("; ifOld=").append(this.ifOld).append("; ifNew=")
        .append(this.ifNew).append("; op=").append(this.getOperation());
    if (this.versionTag != null) {
    ...
    if (this.sendDeltaWithFullValue) {
      buff.append("; sendDeltaWithFullValue=").append(
          this.sendDeltaWithFullValue);
    }
    ...
    if (this.event != null && this.event.getDeltaBytes() != null) {
      buff.append("; deltaBytes=").append(this.event.getDeltaBytes().length);
    }
    else if (this.deltaBytes != null) {
      buff.append("; deltaBytes=").append(this.deltaBytes.length);
    }
    if (this.filterInfo != null) {
      buff.append("; ");
  ...
  public static final class PutReplyMessage extends ReplyMessage implements OldValueImporter {
                            ...
                            EntryEventImpl ev)
    {
      ...
      PutReplyMessage m = new PutReplyMessage(processorId, result, op, ex,
          null, sourceMessage, ev.getVersionTag());
    ...
    public void process(final DM dm, final ReplyProcessor21 rp) {
      ...
      if (DistributionManager.VERBOSE) {
        l.fine("PutReplyMessage process invoking reply processor with processorId:" + this.processorId);
      }
      //dm.getLogger().warning("PutResponse processor is " + ReplyProcessor21.getProcessor(this.processorId));
      if (rp == null) {
        if (DistributionManager.VERBOSE) {
      ...
      if (DistributionManager.VERBOSE) {
        ...
        logger.info(LocalizedStrings.PutMessage_0__PROCESSED__1, new Object[] {rp, this});
      }
...
//      return this.oldValue;
    ...
    public void toData(DataOutput out) throws IOException {
      ...
      out.writeByte(this.op.ordinal);
//      InternalDistributedSystem.getLoggerI18n().info(LocalizedStrings.DEBUG, "DEBUG: PutResponse serializing this value: " + this.oldValue);
    ...
    public String toString() {
      ...
      sb.append("PutReplyMessage ").append("processorid=")
          .append(this.processorId).append(" returning ").append(this.result)
          .append(" op=").append(op).append(" exception=")
   ...
   * @since 5.1
   */
  public static final class PutResponse extends PartitionResponse  {
    ...
    public PutResponse(final InternalDistributedSystem ds,
    ...
    public PutResponse(InternalDistributedSystem ds, Set<?> recipients,
    ...
    public final synchronized void setResponse(PutReplyMessage response) {
      // for multiple results in case of transaction, fail if it failed anywhere
      if (this.op == null || !response.result) {
        ...
        if (this.putMessage.bridgeContext == null
         ...
         Runnable sendFullObject = new Runnable() {
           public void run() {
             ...
             if (log.fineEnabled()) {
               log.fine("Sending full object(" + putMsg + ") to "
