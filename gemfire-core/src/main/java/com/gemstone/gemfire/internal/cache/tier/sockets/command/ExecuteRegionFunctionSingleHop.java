...
public class ExecuteRegionFunctionSingleHop extends BaseCommand {
  ...
  public void cmdExecute(Message msg, ServerConnection servConn, long start)
    ...
    if (function == null || regionName == null) {
      ...
      if (logger.warningEnabled()) {
        logger.warning(LocalizedStrings.ONE_ARG, servConn.getName() + ": " + message);
      }
      ...
      try { 
        ...
        if (fe.getCause() instanceof FunctionInvocationTargetException) {
          if (logger.fineEnabled() && functionObject.isHA()) {
            logger.fine("Exception on server while executing function: "
                + function + " :" + message);
          }
