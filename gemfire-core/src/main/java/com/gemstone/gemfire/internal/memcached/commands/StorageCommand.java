...
public abstract class StorageCommand extends AbstractCommand {
   ...
   */
  private static ScheduledExecutorService expiryExecutor = new ScheduledThreadPoolExecutor(1);
  ...
  private ByteBuffer processBinaryComand(RequestReader request, Cache cache) {
    ...
    ByteBuffer retVal = processBinaryStorageCommand(key, value, cas, flags, cache, request.getResponse());
    if (expTime > 0) {
      scheduleExpiration(key, expTime, cache);
    }
    if (getLogger().fineEnabled()) {
  ...
  public abstract ByteBuffer processBinaryStorageCommand(Object key, byte[] value, long cas, int flags, Cache cache, ByteBuffer response);
