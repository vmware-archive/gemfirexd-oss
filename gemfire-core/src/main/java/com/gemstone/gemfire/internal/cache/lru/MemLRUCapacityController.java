  ...
  protected EnableLRU createLRUHelper() {
    return new AbstractEnableLRU() {
      ...
      public int entrySize( Object key, Object value )
        ...
        // For off-heap we do not want to count the size of DataAsAddress objects since they are not stored off-heap.
        // But their implementation of Sizable always returns 0 so we don't need to special case them here.
...
//    Shouldn't we defer to the user's object sizer for these things?
    if ( o instanceof byte[]  || o instanceof String || o instanceof byte[][]) {
