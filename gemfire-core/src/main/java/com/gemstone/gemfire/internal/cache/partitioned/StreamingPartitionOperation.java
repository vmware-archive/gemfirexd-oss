  ...
  protected class StreamingPartitionResponse extends ReplyProcessor21 {
    ...
    class Status {
      ...
      protected synchronized boolean trackMessage(StreamingReplyMessage m) {
        ...
        LogWriterI18n log = StreamingPartitionOperation.this.sys.getLogWriter().convertToLogWriterI18n();
        if (log.fineEnabled()) {
          log.fine("Streaming Message Tracking Status: Processor id " + getProcessorId() +
                   ";Sender " + m.getSender() +
                   ";Messages Processed: " + this.msgsProcessed +
                   ";NumMsgs:" + this.numMsgs);
        }
    ...
    protected synchronized void processException(DistributionMessage msg,
      ...
      if (t instanceof ForceReattemptException ||
        ...
        if (getDistributionManager().getLoggerI18n().fineEnabled()) {
          getDistributionManager().getLoggerI18n().fine("StreamingPartitionResponse received exception " +
              t + " for member " + msg.getSender() + " query retry required.");
        }
...
//          boolean firstObject = true;
          ...
          if (!this.replyLastMsg) {
            ...
            if (logger.finerEnabled()) {
              logger.finer("Writing this object to StreamingPartitionMessage outStream: '"
                          + nextObject + "'");
            } 
