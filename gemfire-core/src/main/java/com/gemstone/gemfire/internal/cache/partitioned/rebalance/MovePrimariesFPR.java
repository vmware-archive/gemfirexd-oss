...
public class MovePrimariesFPR extends RebalanceDirectorAdapter {
  private static final LogWriterI18n logger = LogService.logger();
          ...
          if (fpa.hasBucket(bucket.getId()) && fpa.isPrimary()) {
            ...
            if (source != target) {
              ...
              if (logger.fineEnabled()) {
                logger.fine("PRLM#movePrimariesForFPR: For Bucket#" + bucket.getId() + ", moving primary from source " + bucket.getPrimary() + " to target " +
                    target);
              }
