...
public final class DumpB2NRegion extends PartitionMessage
  ...
  private DumpB2NRegion(Set recipients, int regionId,
      ReplyProcessor21 processor, int bId, boolean justPrimaryInfo,
  ...
  public static DumpB2NResponse send(Set recipients, PartitionedRegion r,
    ...
    DumpB2NResponse p = new DumpB2NResponse(r.getSystem(), recipients, tx);
...
//      pr = null; (redundant assignment)
      ...
      super.basicProcess(dm);
    }
    catch (CancelException e) {
      sendReply(this.sender,this.processorId , dm, new ReplyException(e), pr, 0);
    }
    catch (PRLocallyDestroyedException e) {
//        dm.getLogger().warning(LocalizedStrings.DumpB2NRegion_PARTITIONEDREGION_WITH_ID_0_IS_LOCALLY_DESTROYED, this.regionId, e);
      sendReply(this.sender,this.processorId , dm, new ReplyException(e), pr, 0);
      return;
    }
    catch (RegionDestroyedException rde) {
//        dm.getLogger().warning( LocalizedStrings.DumpB2NRegion_PARTITIONEDREGION_WITH_ID_0_IS_DESTROYED, this.regionId, rde);
...
//    final LogWriterI18n l = pr.getCache().getLogger();
//    if (DistributionManager.VERBOSE) {
//      l.fine("DumpB2N operateOnRegion: " + pr.getFullPath());
//    }
...
//    if (DistributionManager.VERBOSE) {
//      l.fine(getClass().getName() + " dumped b2n region");
//    }
    return false;
  }
  ...
  public static final class DumpB2NReplyMessage extends ReplyMessage {
    ...
    public void process(final DM dm, final ReplyProcessor21 processor)
    {
      ...
      if (DistributionManager.VERBOSE) {
        l.fine("DumpB2NReplyMessage process invoking reply processor with processorId:"
            + this.processorId);
      }
      
      if (processor == null) {
        if (DistributionManager.VERBOSE) {
          l.fine("DumpB2NReplyMessage processor not found");
        }
        return;
      }
      ...
      if (DistributionManager.VERBOSE) {
        ...
        logger.info(LocalizedStrings.DumpB2NRegion_0__PROCESSED__1, new Object[] {processor, this});
      }
  ...
  public static class DumpB2NResponse extends PartitionResponse  {
      ...
      if (msg instanceof DumpB2NReplyMessage) {
        ...
        if (DistributionManager.VERBOSE) {
          getDistributionManager().getLoggerI18n().fine(
              "DumpB2NResponse got a primaryInfo "
              + reply.getPrimaryInfo() + " from " + reply.getSender());
        }
      }
      super.process(msg);
    }
    ...
    public List waitForPrimaryInfos() 
      ...
      try {
        ...
        final String msg = "B2NResponse got ForceReattemptException; rethrowing";
        getDistributionManager().getLoggerI18n().fine(msg, e);
        throw e;
      }
      catch (CacheException e) {
        getDistributionManager().getLoggerI18n().fine("B2NResponse got remote CacheException, throwing ForceReattemptException.", e);
