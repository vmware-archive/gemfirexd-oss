...
public final class PRFunctionStreamingResultCollector extends
    FunctionStreamingResultCollector implements ResultCollector {
  ...
  protected synchronized void processException(DistributionMessage msg,
    ...
    if (getDistributionManager().getLoggerI18n().fineEnabled()) {
      getDistributionManager().getLoggerI18n().fine(
          "StreamingPartitionResponseWithResultCollector received exception "
              + ex.getCause() + " for member " + msg.getSender());
    }
    ...
    Throwable t = unwrapNodeFailureException(ex.getCause());
    if (isNodeFailureException(t)) {
      ((PartitionedRegionFunctionExecutor)this.execution).addFailedNode(msg
          .getSender().getId());
      this.exception = ex;
    }
    else if (t instanceof BucketMovedException) {
      this.exception = ex;
    }
