...
public class BecomePrimaryBucketMessage extends PartitionMessage {
  ...
  extends ReplyMessage {
    ...
    public void process(final DM dm, final ReplyProcessor21 processor) {
      ...
      if (DistributionManager.VERBOSE) {
        logger.fine("BecomePrimaryBucketReplyMessage process invoking reply processor with processorId:"
                + this.processorId);
      }

      if (processor == null) {
        if (DistributionManager.VERBOSE) {
          logger.fine("BecomePrimaryBucketReplyMessage processor not found");
        }
        return;
      }
  ...
  public static class BecomePrimaryBucketResponse extends PartitionResponse {
          ...
          if (reply.isSuccess()) {
            if (DistributionManager.VERBOSE) {
              getDistributionManager().getLoggerI18n().fine(
                  "BecomePrimaryBucketResponse return OK");
            }
          }
          else if (DistributionManager.VERBOSE) {
            getDistributionManager().getLoggerI18n().fine(
                "BecomePrimaryBucketResponse return NOT_PRIMARY");
          }
        }
      }
      finally {
    ...
    protected void processException(ReplyException ex) {
      getDistributionManager().getLoggerI18n().fine(
          "BecomePrimaryBucketMessage ignoring exception", ex);
    }
