...
public final class ManageBucketMessage extends PartitionMessage {
  ...
  protected boolean operateOnPartitionedRegion(DistributionManager dm, 
    ...
    if (DistributionManager.VERBOSE) {
      l.fine("ManageBucketMessage operateOnRegion: " + r.getFullPath());
    }
  ...
  public static final class ManageBucketReplyMessage extends ReplyMessage {
    ...
    public void process(final DM dm, final ReplyProcessor21 processor) {
      ...
      if (DistributionManager.VERBOSE) {
        l.fine("ManageBucketReplyMessage process invoking reply processor with processorId:" + this.processorId);
      }

      if (processor == null) {
        if (DistributionManager.VERBOSE) {
          l.fine("ManageBucketReplyMessage processor not found");
        }
        return;
      }
      ...
      if (DistributionManager.VERBOSE) {
        ...
        logger.info(LocalizedStrings.ManageBucketMessage_0__PROCESSED__1, new Object[] {processor, this});
      }
        ...
        if (msg instanceof ManageBucketReplyMessage) {
          ...
          if (DistributionManager.VERBOSE) {
            getDistributionManager().getLoggerI18n().fine("NodeResponse return value is " + 
                reply.acceptedBucket + " isInitializing=" + reply.notYetInitialized);
          }
        } else {
    ...
    public boolean waitForAcceptance()  throws ForceReattemptException
    {
      try {
        ...
        if (t instanceof CancelException) {
          getDistributionManager().getLoggerI18n().fine("NodeResponse got remote cancellation, throwing PartitionedRegionCommunication Exception.", t);
        ...
        if (t instanceof PRLocallyDestroyedException) {
          getDistributionManager().getLoggerI18n().fine("NodeResponse got local destroy on the PartitionRegion , throwing ForceReattemptException.", t);
