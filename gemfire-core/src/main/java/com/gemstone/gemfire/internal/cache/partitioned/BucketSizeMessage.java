...
public final class BucketSizeMessage extends PartitionMessage
  ...
  private BucketSizeMessage(InternalDistributedMember recipient, int regionId,
    ...
    super(recipient, regionId, processor, null /* no TX */);
  ...
  public static final class BucketSizeReplyMessage extends HighPriorityDistributionMessage {
    ...
    protected void process(final DistributionManager dm) {
      ...
      if (DistributionManager.VERBOSE) {
        l.fine("PRDistributedBucketSizeReplyMessage process invoking reply processor with processorId:" + this.processorId);
      }
      ...
      if (processor == null) {
        if (DistributionManager.VERBOSE) {
          l.fine("PRDistributedBucketSizeReplyMessage processor not found");
        }
        return;
      }
      ...
      if (DistributionManager.VERBOSE) {
        ...
        logger.info(LocalizedStrings.BucketSizeMessage_0__PROCESSED__1, new Object[] {processor, this});
      }
    ...
    public void process(DistributionMessage msg)
    {
      try {
        if (msg instanceof BucketSizeReplyMessage) {
          ...
          if (DistributionManager.VERBOSE) {
            getDistributionManager().getLoggerI18n().fine("BucketSizeResponse return value is " + 
                this.returnValue);
          }
        }
      } finally {
    ...
    public long waitForSize() throws ForceReattemptException {
      try {
        ...
        if (t instanceof com.gemstone.gemfire.CancelException) {
          String msg = "BucketSizeResponse got remote cancellation; forcing reattempt.";
        ...
        if (t instanceof ForceReattemptException) {
          getDistributionManager().getLoggerI18n().fine("BucketSizeResponse got remote Region destroyed; forcing reattempt.", t);
