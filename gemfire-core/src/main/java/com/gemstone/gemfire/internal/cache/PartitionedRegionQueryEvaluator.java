...
public class PartitionedRegionQueryEvaluator extends StreamingPartitionOperation
{
  ...
  protected boolean processData(List objects, InternalDistributedMember sender,
    ...
    if (this.sys.getLogWriter().fineEnabled()) {
      this.sys.getLogWriter().fine("Results per member, for " + sender + " size: " + objects.size());
    }
    ...
    synchronized (results) {
      if (!QueryMonitor.isLowMemory()) {
        ...
        if (this.sys.getLogWriter().fineEnabled()) {
          this.sys.getLogWriter().fine(
              "query canceled while gathering results, aborting");
        }
  ...
  throws InterruptedException, QueryException {
    ...
    if (this.query.isQueryWithFunctionContext() && !n2b.isEmpty()) {
      if (this.sys.getLogWriter().fineEnabled()) {
        this.sys.getLogWriter().fine(
            "Remote buckets found for query executed in a Function.");
      }
    ...
    if (this.sys.getLogWriter().fineEnabled()) {
      this.sys.getLogWriter().fine("Sending query execution request to " + 
          n2b.size() + " remote members for the query:" + 
      ...
      if (this.sys.getLogWriter().fineEnabled()) {
        ...
        for (Iterator itr = n2b.entrySet().iterator(); itr.hasNext();) {
          ...
          if (notReceivedMembers != null && !notReceivedMembers.isEmpty()) {
            ...
            if (this.sys.getLogWriter().fineEnabled()) {
              this.sys.getLogWriter().fine("Failed sending to members " + notReceivedMembers + " retry required");
            }
          }
        }
        if (th != null) {
    ...
    if (this.node2bucketIds.containsKey(this.pr.getMyId())) {
      if (this.sys.getLogWriter().fineEnabled()) {
        this.sys.getLogWriter().fine("Started query execution on local " +
            " data for query:" + this.query.getQueryString());
      }
    ...
    if (processor != null) {
      try {
        ...
        if (this.sys.getLogWriter().fineEnabled()) {
          this.sys.getLogWriter().fine("Following remote members failed " + failedMembers +
              " and retry flag is set to: " +requiresRetry);
        }
  ...
  public void queryBuckets(final TestHook th) throws QueryException, InterruptedException {
    ...
    while (needsRetry && retry < MAX_PR_QUERY_RETRIES) {
      ...
      if (needsRetry) {
        ...
        if (query.isQueryWithFunctionContext()) {
          if (this.sys.getLogWriter().fineEnabled()) {
            this.sys.getLogWriter().fine("No of retry attempts are: " + retry);
          }
          break;
        }
        ...
        if (this.sys.getLogWriter().fineEnabled()) {
          this.sys.getLogWriter().fine("PR Query retry: " + retry
              + " total: " + this.pr.getCachePerfStats().getPRQueryRetries());
        }
    ...
    if (this.bucketsToQuery.isEmpty()) {
      this.sys.getLogWriter().fine(
          "PartitionedRegionQueryHelper#queryBuckets: Query executed successfully.");
    }
    else {
    */
    if (needsRetry) {
      ...
      if (this.sys.getLogWriter().fineEnabled()) {
        this.sys.getLogWriter().fine(msg + " Unable to query some of the buckets from the set :" + 
            this.caclulateRetryBuckets());
      }
      ...
      if (anyOfTheseBucketsHasStorage(this.bucketsToQuery)) {
        if (retry >= MAX_PR_QUERY_RETRIES) {
          ...
          this.sys.getLogWriter().severe(msg);
          throw new QueryException(msg);
        } else {
          failMissingBuckets();
        }
  ...
  private Set<Integer> caclulateRetryBuckets() {
    ...
    if (this.sys.getLogWriter().fineEnabled()) {
      ...
      logStr.append("]");
      this.sys.getLogWriter().fine(logStr.toString());
    }
  ...
  private void addResultsToResultSet() throws QueryException {
    ...
    for (Map.Entry<InternalDistributedMember, Collection<Collection>> e : this.resultsPerMember.entrySet()) {
      ...
      for (Collection res : e.getValue()) {
        ...
        if (res != null) {
          if (this.sys.getLogWriter().fineEnabled()) {
            this.sys.getLogWriter().fine("Query Result from member :" + e.getKey() + 
                ": " + res.size());
          }
  ...
  private void buildSortedResult(CompiledSelect cs, int limit) throws QueryException {
    ...
    //If order by clause is present , then compute dependency locally
        ...
        for (int i = 0; i < orderByAttrs.size(); i++) {
          ...
          if (o instanceof CompiledSortCriterion) {
            ...
            if (orderBy.equals(projStr)) {
              ...
              tempQueryBuffer.append(' ');
              tempQueryBuffer.append(temp.substring(projStr.length() + 1));
            }
            else {
              throw new QueryException("Order by clause " + orderBy
      ...
      if (this.sys.getLogWriter().fineEnabled()) {
        this.sys.getLogWriter().fine(
            "The temp query generated to evaluate order-by on PR commulative results: "
                + tempQueryBuffer.toString());
      }
        ...
        for (Collection res : memberResults) {
          if (res != null) {
            context = new QueryExecutionContext((new Object[] { res }), this.pr
                .getCache(), this.cumulativeResults);
  ...
  private void buildSortedResultBackup(CompiledSelect cs, int limit) throws QueryException {
    ...
    try {
      ...
      if (this.sys.getLogWriter().fineEnabled()) {
        this.sys.getLogWriter().fine("The temp query generated to evaluate order-by on PR commulative results: " + 
            tmpSortQuery.toString());
      }
      ...
      for (Iterator i=this.resultsPerMember.values().iterator(); i.hasNext(); ) {
        ...
        if (res!=null && res.isConsumable()) {
          context = new QueryExecutionContext((new Object[] {res}), this.pr.getCache(), this.cumulativeResults);
          q.executeUsingContext(context);
          res.clear();
        }
      }   
  ...
  throws QueryException {
    ...
    Set<InternalDistributedMember> taintedMembers = new HashSet<InternalDistributedMember>();
    ...
    Put the tainted members on the end of the list.
    allNodes.removeAll(taintedMembers);
    Collections.shuffle(allNodes, PartitionedRegion.rand);
    allNodes.addAll(taintedMembers);
    */
    ...
    for (Iterator dsItr = allNodes.iterator(); dsItr.hasNext() && (bucketIds.size() < totalBucketsToQuery); ) {
      ...
      if(taintedMembers.contains(nd)) {
        ...
        if (this.sys.getLogWriter().fineEnabled()) {
          this.sys.getLogWriter().fine("Retrying query to tainted member : " + nd);
        }
      }
      */
    ...
    if (this.sys.getLogWriter().fineEnabled()) {
      this.sys.getLogWriter().fine("Node to bucketId map: " + ret);
    }
    return ret;
  }
  ...
  private boolean executeQueryOnLocalNode() throws QueryException, InterruptedException
  {
    ...
    if (this.pr.getDataStore() != null) {
      ...
      try {
        ...
        if (pr.getCache().isCopyOnRead() && qp.isIndexUsed() && pr.getCompressor() == null) {
        ...
        if (this.sys.getLogWriter().fineEnabled()) {
          int size = 0;
          for (Collection<Collection> res : resultCollector) {
            size+=res.size();
          }
          this.sys.getLogWriter().fine("Results per member, for " + me + " size: " + size);
        }
        */
      } catch (ForceReattemptException retryRequired) {
        if (this.sys.getLogWriter().fineEnabled()) {
          this.sys.getLogWriter().fine("Caught exception during local portion of query " +
        ...
        try {
          ...
          if (this.sys.getLogWriter().fineEnabled()) {
            this.sys.getLogWriter().fine("Local bucket not found.", bme);
          }
          return true;
        }
  ...
  */
  ...
  protected void memberStreamCorrupted(InternalDistributedMember sender) {
    ...
    /*
    if (this.sys.getLogWriter().infoEnabled()) {
      this.sys.getLogWriter().fine("Tainting member " + sender); 
    }
    ...
    if (bucketIds != null) {
      ...
      if (this.sys.getLogWriter().fineEnabled()) {
        this.sys.getLogWriter().fine("Problem on member " + sender
            + " results in throwing away these bucketIds: " + removedBucketIds);
      }
    ...
    */
    ...
    /* alternative strategy: re-query
      PartitionedRegion.getLogWriter().info(
                                            "PartitionedRegionQueryHelper#queryBuckets: Query = "
                                            + this.query);
  ...
  public static class PRQueryResultCollector {
    ...
    public PRQueryResultCollector() {
      ...
      this.usedIndexInfoMap = new THashMap(); //{indexName, IndexInfo} Map
    }
