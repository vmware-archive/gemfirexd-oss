          ...
          if (this.handshake.isOK()) {
            try {
              ...
              if (!crHelper.isShutdown()) {
                if (logger.warningEnabled()) {
                  logger.warning( LocalizedStrings.ServerConnection_0_UNEXPECTED_CANCELLATION, getName(), e);
                }
              }
              cleanup();
              return false;
            ...
            if (logger.warningEnabled()) {
              logger.warning( LocalizedStrings.
                ServerConnection_0_RECEIVED_UNKNOWN_HANDSHAKE_REPLY_CODE_1, 
                new Object[] { this.name, Byte.valueOf(this.handshake.getCode()) } );
            }
...
//        + "   diff=" + (oldIdArray.length - memberIdByteArray.length));
 ...
 protected void setPrincipal(Principal principal) {
  ...
  private boolean processHandShake() {
    ...
    synchronized (getCleanupTable()) {
      ...
      if (this.proxyId.isDurable()) {
        if (logger.fineEnabled()) {
            logger
                .fine("looking if the Proxy existed for this durable client or not :"
                    + this.proxyId);
        }
        ...
        if (proxy != null) {
          if (this.logger.fineEnabled()) {
            logger.fine("Proxy existed for this durable client :" + this.proxyId + " and proxy : " + proxy);
          }
          if(proxy.isPrimary()){
            epType = (byte)2 ; 
      ...
      if (numRefs != null) {
        ...
        // Check the member limit. If over the limit, reject the client.
        if (acceptHandShake(epType, qSize)) {
          clientJoined = true;
          numRefs = new Counter();
      ...
      if (logger.fineEnabled()) {
        logger.fine( (registerClient? "" : "not ") + "registering client " + proxyId);
      }
  ...
  {
    try {
      this.handshake.accept(this.theSocket.getOutputStream(), this.theSocket
          .getInputStream(), epType, qSize, this.communicationMode,
          this.principal);
    }
    ...
    if (logger.fineEnabled()) {
      logger.fine(this.name + ": Accepted handshake");
    }
...
//  }
  ...
  public void setCq(String cqName, boolean isDurable) throws Exception
  {
    if (this.requestMsg.isSecureMode()) {
      if (this.logger.fineEnabled()) {
        this.logger.fine("setCq() security header found registering CQname = "
            + cqName);
      }
      try {
        ...
        if (this.logger.fineEnabled()) {
          this.logger.fine("While setting cq got exception ", ex);
        }
        throw ex;
      }
    } else if (this.logger.fineEnabled()) {
      this.logger.fine("setCq() security header is not found ");
    }
  }
  ...
  public void removeCq(String cqName, boolean isDurable) {
    if (this.requestMsg.isSecureMode()) {
      if (this.logger.fineEnabled()) {
        this.logger
            .fine("removeCq() security header found registering CQname = "
                + cqName);
      }
      try {
      ...
      this.logger.fine("removeCq() security header is not found ");
    }
  }
      ...
      if (msg != null) {
        ...
        if (!this.processMessages || (crHelper.isShutdown())) {
          if (this.logger != null && this.logger.fineEnabled()) {
            this.logger.fine(getName() + " ignoring message of type "
                + MessageType.getString(msg.getMessageType()) + " from client "
                + this.proxyId + " due to shutdown.");
          }
          return;
        }
        ...
        if (this.logger != null && this.logger.finerEnabled()) {
          this.logger.finer(getName() + " received "
              + MessageType.getString(msg.getMessageType())
              + " with txid " + msg.getTransactionId());
//              + " message from client " + this.proxyId); no need to log this - it's in getName()
  ...
  public byte[] setCredentials(Message msg)
    ...
    try
    {
      ...
      Principal principal = HandShake.verifyCredentials(methodName, credentials,
          system.getSecurityProperties(), system.getLogWriter(), system
  ...
  private void setSecurityPart() {
    try {
      ...
      if (logger.warningEnabled()) {
        logger.warning(
            LocalizedStrings.ServerConnection_SERVER_FAILED_TO_ENCRYPT_DATA_0,
            ex, null);
      }
...
//        + this.requestMsg.msgType);
    if (AcceptorImpl.isAuthenticationRequired()
        ...
        && (!(this.requestMsg.msgType == MessageType.CLIENT_READY
            ...
            || this.requestMsg.msgType == MessageType.GET_ENTRY
      ...
      if (this.logger.fineEnabled() && AcceptorImpl.isAuthenticationRequired()) {
        this.logger.fine("ServerConnection.updateAndGetSecurityPart() "
            + "not adding security part for msg type "
  ...
  public void run() {
    ...
    if (getAcceptor().isSelector()) {
      ...
      try {
        ...
        logger.warning(LocalizedStrings.ServerConnection_0__UNEXPECTED_EXCEPTION, ex, null);
      }
      finally {
...
//       }
    ...
    if (logger.fineEnabled()) {
      logger.fine (this.name + ": Closed connection");
    }
    return true;
  }
  ...
  public AuthorizeRequest getAuthzRequest() 
    ...
    if (AcceptorImpl.isAuthenticationRequired()) {
      ...
      if (this.logger.fineEnabled()) {
        this.logger.fine("getAuthzRequest() authrequest: "
            + ((authReq == null) ? "NULL (only authentication is required)"
                : "not null"));
      }
  ...
  public boolean isGemFireXDSystem() {
    return this.acceptor.isGemFireXDSystem();
  }
}
