    ...
    try { // always set this.alive before stopping
      while (isAlive()) {
        try { // handle handle exceptions
          ...
          ControllableProcess.getOrCreateLogWriter().error("Unable to control process with " + this.file + ". Please add tools.jar from JDK to classpath for improved process control.");
          // allow to loop around and check isAlive()
        }
      }
    } finally {
  ...
  private void work() throws IOException {
    try { // always delete file after invoking handler
      ...
      try {
        ...
        ControllableProcess.getOrCreateLogWriter().warning("Unable to delete " + this.file, e);
      }
    }
  }
