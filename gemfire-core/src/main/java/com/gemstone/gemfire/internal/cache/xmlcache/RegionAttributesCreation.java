...
public class RegionAttributesCreation extends UserSpecifiedRegionAttributes implements Serializable {
  ...
  * */
  ...
  /**
   * Whether to enable a gateway for this region
   *
   * @since 4.2
   */
  ...
  public RegionAttributesCreation(CacheCreation cc, RegionAttributes attrs, boolean defaults) {
    ...
    this.gatewayHubId = attrs.getGatewayHubId();
...
//     }
    ...
    if (this.enableGateway != other.getEnableGateway()) {
      throw new RuntimeException(LocalizedStrings.RegionAttributesCreation_ENABLE_GATEWAY_IS_NOT_THE_SAME.toLocalizedString());
    }
    if (! equal(this.gatewayHubId, other.getGatewayHubId())) {
      throw new RuntimeException(LocalizedStrings.RegionAttributesCreation_GATEWAY_HUB_ID_IS_NOT_THE_SAME.toLocalizedString());
    }
    ...
    if (this.enableOffHeapMemory != other.getEnableOffHeapMemory()) {
  ...
  public boolean getPersistBackup() {
    return getDataPolicy().isPersistentReplicate();
  }
  ...
  public void setPersistBackup(boolean persistBackup) {
    if (persistBackup) {
      ...
      if (getDataPolicy().isPersistentReplicate()) {
...
//    setHasPublisher(true);
 ...
 private void verifyNonNegativeDirSize(int[] sizes){
  ...
  void inheritAttributes(Cache cache, boolean setDefaultPool) {
    ...
    if (!hasPersistBackup()) {
      if (parentIsUserSpecified) {
        if (parentWithHas.hasPersistBackup()) {
          setPersistBackup(parent.getPersistBackup());
        }
      } else {
        setPersistBackup(parent.getPersistBackup());
      }
    ...
    if (!hasEnableGateway()) {
      if (parentIsUserSpecified) {
        if (parentWithHas.hasEnableGateway()) {
          setEnableGateway(parent.getEnableGateway());
        }
      } else {
        setEnableGateway(parent.getEnableGateway());
      }
    ...
    if (!this.hasGatewayHubId()) {
      if (parentIsUserSpecified) {
        if (parentWithHas.hasGatewayHubId()) {
          setGatewayHubId(parent.getGatewayHubId());
        }
      } else {
        setGatewayHubId(parent.getGatewayHubId());
      }
  ...
  public void setEnableOffHeapMemory(boolean enableOffHeapMemory) {
    ...
    setHasEnableOffHeapMemory(true);
  }
