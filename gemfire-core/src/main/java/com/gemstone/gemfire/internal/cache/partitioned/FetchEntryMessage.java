...
public final class FetchEntryMessage extends PartitionMessage
  ...
  private FetchEntryMessage(InternalDistributedMember recipient, int regionId,
      ReplyProcessor21 processor, final TXStateInterface tx, final Object key,
  ...
  public static FetchEntryResponse send(InternalDistributedMember recipient,
      ...
      final boolean access) throws ForceReattemptException {
    ...
    FetchEntryResponse p = new FetchEntryResponse(r.getSystem(), Collections
        .singleton(recipient), r, key, tx);
    FetchEntryMessage m = new FetchEntryMessage(recipient, r.getPRId(), p, tx,
        key, access);
...
//    }
    ...
    if (ds != null) {
      try {
        ...
        final KeyInfo keyInfo = r.getKeyInfo(this.key);
        final TXStateInterface tx = getTXState(r);
        val = r.getDataView(tx).getEntryOnRemote(keyInfo, r, true);
        r.getPrStats().endPartitionMessagesProcessing(startTime); 
        FetchEntryReplyMessage.send(getSender(), getProcessorId(), val, dm,
            null, this);
      }
      catch (TransactionException tex) {
        FetchEntryReplyMessage.send(getSender(), getProcessorId(), null, dm, 
            new ReplyException(tex), null);
      }
      catch (PRLocallyDestroyedException pde) {
        FetchEntryReplyMessage.send(getSender(), getProcessorId(), null, dm,
            new ReplyException(new ForceReattemptException(LocalizedStrings
                .FetchEntryMessage_ENCOUNTERED_PRLOCALLYDESTROYED
                    .toLocalizedString(), pde)), null);
      }
      catch (EntryNotFoundException enfe) {
        FetchEntryReplyMessage.send(getSender(), getProcessorId(), null, dm, 
            new ReplyException(LocalizedStrings
                .FetchEntryMessage_ENTRY_NOT_FOUND.toLocalizedString(),
                enfe), null);
      }
      catch (PrimaryBucketException pbe) {
        FetchEntryReplyMessage.send(getSender(), getProcessorId(), null, dm, 
            new ReplyException(pbe), null);
      }
  ...
  public static final class FetchEntryReplyMessage extends ReplyMessage
   {
    ...
    public void process(final DM dm, final ReplyProcessor21 processor)
    {
      ...
      if (DistributionManager.VERBOSE) {
        ...
        logger.info(LocalizedStrings.FetchEntryMessage_0__PROCESSED__1, new Object[] {processor, this});
      }
  ...
  public static class FetchEntryResponse extends PartitionResponse
   {
    ...
    public FetchEntryResponse(InternalDistributedSystem ds, Set recipients,
        PartitionedRegion theRegion, Object key, final TXStateInterface tx) {
