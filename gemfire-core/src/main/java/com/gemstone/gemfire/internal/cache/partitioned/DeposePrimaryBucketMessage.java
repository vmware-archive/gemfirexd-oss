...
public class DeposePrimaryBucketMessage extends PartitionMessage {
  ...
  extends ReplyMessage {
    ...
    public void process(final DM dm, final ReplyProcessor21 processor) {
      ...
      if (DistributionManager.VERBOSE) {
        logger.fine("DeposePrimaryBucketReplyMessage process invoking reply processor with processorId:"
                + this.processorId);
      }

      if (processor == null) {
        if (DistributionManager.VERBOSE) {
          logger.fine("DeposePrimaryBucketReplyMessage processor not found");
        }
        return;
      }
  ...
  public static class DeposePrimaryBucketResponse extends
          ...
          if (reply.isSuccess()) {
            if (DistributionManager.VERBOSE) {
              getDistributionManager().getLoggerI18n().fine(
                  "DeposePrimaryBucketResponse return OK");
            }
          }
          else if (DistributionManager.VERBOSE) {
            getDistributionManager().getLoggerI18n().fine(
                "DeposePrimaryBucketResponse return NOT_PRIMARY");
          }
        }
      }
      finally {
    ...
    protected void processException(ReplyException ex) {
      getDistributionManager().getLoggerI18n().fine(
          "DeposePrimaryBucketMessage ignoring exception", ex);
    }
  }
