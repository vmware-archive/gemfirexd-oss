...
public class RemoveBucketMessage extends PartitionMessage {
  ...
  extends ReplyMessage {
    ...
    public void process(final DM dm, final ReplyProcessor21 processor) {
      ...
      if (DistributionManager.VERBOSE) {
        logger.fine("RemoveBucketReplyMessage process invoking reply processor with processorId:"
                + this.processorId);
      }

      if (processor == null) {
        if (DistributionManager.VERBOSE) {
          logger.fine("RemoveBucketReplyMessage processor not found");
        }
        return;
      }
  ...
  public static class RemoveBucketResponse extends PartitionResponse {
        ...
        if (msg instanceof RemoveBucketReplyMessage) {
          ...
          if (DistributionManager.VERBOSE) {
            getDistributionManager().getLoggerI18n().fine(
                "RemoveBucketResponse is " + removed);
          }
        }
      } finally {
        super.process(msg);
    ...
    protected void processException(ReplyException ex) {
      getDistributionManager().getLoggerI18n().fine(
          "RemoveBucketMessage ignoring exception", ex);
    }
