...
public class CommitCommand extends BaseCommand {
  ...
  public void cmdExecute(Message msg, ServerConnection servConn, long start)
    ...
    if (txMgr.isHostedTxRecentlyCompleted(txId)) {
      ...
      if (logger.fineEnabled()) {
        logger.fine("TX: returning a recently committed txMessage for tx:"+txId);
      }
    ...
    if (logger.fineEnabled()) {
      logger.fine("TX: committing client tx:"+txId);
    }
    try {
  ...
  protected static void writeCommitResponse(TXRemoteCommitMessage response,
    ...
    if (logger.fineEnabled()) {
      logger.fine("TX: sending a "
          + (response == null ? "null" : "nonNull")
          + " for transaction:" + origMsg.getTransactionId());
          /* TODO: merge: update for client TX
          + new TXId((InternalDistributedMember) servConn.getProxyID()
              .getDistributedMember(), origMsg.getTransactionId()));
          */
    }
