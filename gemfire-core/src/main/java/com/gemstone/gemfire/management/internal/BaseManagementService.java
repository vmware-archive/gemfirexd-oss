...
public abstract class BaseManagementService extends ManagementService {
  ...
  /** Log writer */
  protected static LogWriterI18n logger;
      ...
      for (InternalDistributedSystem sys : existingSystems) {
        try {
          ...
          if (logger.fineEnabled()) {
            logger.fine("DistributedSystemDisconnectedException"
                + e.getMessage());
          }
        }
      }
      ...
      if (systems.isEmpty()) {
        for (Object key : instances.keySet()) {
          ...
          try {
            ...
            if (logger.fineEnabled()) {
              logger
                  .fine("ManagementException while removing InternalDistributedSystem"
                      + e.getMessage());
            }
          }
        }
