...
public final class DeployCommands extends AbstractCommandsSupport implements CommandMarker {
   ...
   * 
   * @param group
   *          Group to deploy the JAR to or null for all members
   * @param jar
   *          JAR file to deploy
   * @param dir
  ...
  @CliMetaData(interceptor = "com.gemstone.gemfire.management.internal.cli.commands.DeployCommands$Interceptor", relatedTopic={CliStrings.TOPIC_GEMFIRE_CONFIG})
   ...
   * 
   * @param group
   *          Group to undeploy the JAR from or null for all members
   * @param jar
   *          JAR or JARs to undeploy (separated by comma)
  ...
  public final Result undeploy(
      ...
      @CliOption(key = { CliStrings.UNDEPLOY__JAR },
                 help = CliStrings.UNDEPLOY__JAR__HELP, unspecifiedDefaultValue = "")
      @CliMetaData (valueSeparator = ",") String jar) {
    ...
    try {
      ...
      ResultCollector<?, ?> rc = CliUtil.executeFunction(this.undeployFunction, new Object[] { jar }, targetMembers);
