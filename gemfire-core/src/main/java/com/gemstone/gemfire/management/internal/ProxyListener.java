...
public interface ProxyListener extends MembershipListener {
  ...
  /**
   * This is a very special method, Only to be invoked from MonitoringRegionCacheListener.
   * The need for this interface arises as some time a create op is not meant to create proxies or aggregate.
   * But only to feed data to existing proxies. 
   * 
   * e.g. When manager starts and not a single ManagementTask cycle has been run. This will result in lost updates.
   * To capture all updates we can feed the newly created data to existing aggregate.
   *  
   * @param objectName
   *          name of the proxy object
   * @param interfaceClass
