...
public class RegionMBeanCompositeDataFactory {
  ...
  public static RegionAttributesData getRegionAttributesData(RegionAttributes regAttrs) {
    ...
    boolean enableOffHeapMemory = regAttrs.getEnableOffHeapMemory();

    boolean diskSynchronus = regAttrs.isDiskSynchronous();
    
    boolean hdfsWriteOnly = regAttrs.getHDFSWriteOnly();
    String hdfsStoreName = regAttrs.getHDFSStoreName();

    RegionAttributesData regionAttributesData = new RegionAttributesData(cacheLoaderClassName, cacheWriteClassName,
        keyConstraintClassName, valueContstraintClassName, regionTimeToLive, regionIdleTimeout, entryTimeToLive,
        entryIdleTimeout, customEntryTimeToLive, customEntryIdleTimeout, ignoreJTA, dataPolicy, scope, initialCapacity,
        loadFactor, lockGrantor, multicastEnabled, concurrencyLevel, indexMaintenanceSynchronous, statisticsEnabled,
        gatewayEnabled, gatewayHubId, subsciptionConflationEnabled, asyncConflationEnabled, poolName, isCloningEnabled,
        diskStoreName, interestPolicy, diskSynchronus, cacheListeners, compressorClassName, enableOffHeapMemory,
