...
public class RegionCommandsController extends AbstractCommandsController {
  ...
  public String describeRegion(@PathVariable("name") final String regionNamePath) {
    ...
    command.addOption(CliStrings.DESCRIBE_REGION__NAME, regionNamePath);
    return processCommand(command.toString());
  }

  @RequestMapping(value = "/regions/{name}", method = RequestMethod.PUT)
  @ResponseBody
  public String alterRegion(@PathVariable("name") final String regionNamePath,
                            ...
                            @RequestParam(value = CliStrings.ALTER_REGION__EVICTIONMAX, required = false) final Integer evictionMax)
  {
    ...
    command.addOption(CliStrings.ALTER_REGION__REGION, regionNamePath);

    if (hasValue(entryIdleTimeExpiration)) {
      command.addOption(CliStrings.ALTER_REGION__ENTRYEXPIRATIONIDLETIME, String.valueOf(entryIdleTimeExpiration));
    }

    if (hasValue(entryIdleTimeExpirationAction)) {
      command.addOption(CliStrings.ALTER_REGION__ENTRYEXPIRATIONIDLETIMEACTION, entryIdleTimeExpirationAction);
    }

    if (hasValue(entryTimeToLiveExpiration)) {
      command.addOption(CliStrings.ALTER_REGION__ENTRYEXPIRATIONTIMETOLIVE, String.valueOf(entryTimeToLiveExpiration));
    }

    if (hasValue(entryTimeToLiveExpirationAction)) {
      command.addOption(CliStrings.ALTER_REGION__ENTRYEXPIRATIONTTLACTION, entryTimeToLiveExpirationAction);
    }

    if (hasValue(regionIdleTimeExpiration)) {
      command.addOption(CliStrings.ALTER_REGION__REGIONEXPIRATIONIDLETIME, String.valueOf(regionIdleTimeExpiration));
    }

    if (hasValue(regionIdleTimeExpirationAction)) {
      command.addOption(CliStrings.ALTER_REGION__REGIONEXPIRATIONIDLETIMEACTION, regionIdleTimeExpirationAction);
    }

    if (hasValue(regionTimeToLiveExpiration)) {
      command.addOption(CliStrings.ALTER_REGION__REGIONEXPIRATIONTTL, String.valueOf(regionTimeToLiveExpiration));
    }

    if (hasValue(regionTimeToLiveExpirationAction)) {
      command.addOption(CliStrings.ALTER_REGION__REGIONEXPIRATIONTTLACTION, regionTimeToLiveExpirationAction);
    }

    if (hasValue(cacheListeners)) {
      command.addOption(CliStrings.ALTER_REGION__CACHELISTENER, StringUtils.concat(cacheListeners,
        StringUtils.COMMA_DELIMITER));
    }

    if (hasValue(cacheLoader)) {
      command.addOption(CliStrings.ALTER_REGION__CACHELOADER, cacheLoader);
    }

    if (hasValue(cacheWriter)) {
      command.addOption(CliStrings.ALTER_REGION__CACHEWRITER, cacheWriter);
    }

    if (hasValue(asyncEventQueueIds)) {
      command.addOption(CliStrings.ALTER_REGION__ASYNCEVENTQUEUEID, StringUtils.concat(asyncEventQueueIds,
        StringUtils.COMMA_DELIMITER));
    }

    if (hasValue(gatewaySenderIds)) {
      command.addOption(CliStrings.ALTER_REGION__GATEWAYSENDERID, StringUtils.concat(gatewaySenderIds,
        StringUtils.COMMA_DELIMITER));
    }
  ...
  @RequestMapping(value = "/regions/{name}", method = RequestMethod.DELETE)
  @ResponseBody
  public String destroyRegion(@PathVariable("name") final String regionNamePath) {
    ...
    command.addOption(CliStrings.DESTROY_REGION__REGION, regionNamePath);
    return processCommand(command.toString());
  }
