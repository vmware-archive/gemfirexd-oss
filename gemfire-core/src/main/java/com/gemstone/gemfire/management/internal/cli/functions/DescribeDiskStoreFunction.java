...
public class DescribeDiskStoreFunction extends FunctionAdapter implements InternalEntity {
  ...
  public void execute(final FunctionContext context) {
    ...
    try {
      if (cache instanceof InternalCache) {
        ...
        final String diskStoreName = (String) context.getArguments();
        final String memberId = member.getId();
        final String memberName = member.getName();

        final DiskStore diskStore = gemfireCache.findDiskStore(diskStoreName);

        if (diskStore != null) {
          final DiskStoreDetails diskStoreDetails = new DiskStoreDetails(diskStore.getDiskStoreUUID(),
      ...
      logError(cache, "Error occurred while executing 'describe disk-store'!", e);
      context.getResultSender().sendException(e);
    }
  }

  private void setDiskDirDetails(final DiskStore diskStore, final DiskStoreDetails diskStoreDetails) {
    final File[] diskDirs = diskStore.getDiskDirs();
    final Integer[] diskDirSizes = ArrayUtils.toIntegerArray(diskStore.getDiskDirSizes());

    // TODO decide whether to use an assertion or guard against ArrayIndexOutOfBoundsExceptions with
    // ArrayUtils.getElementAtIndex, demonstrated below...
  ...
  protected boolean isPersistent(final Region region) {
    return PERSISTENT_DATA_POLICIES.contains(region.getAttributes().getDataPolicy());
  }
  ...
  protected void setCacheServerDetails(final InternalCache cache, final DiskStore diskStore, final DiskStoreDetails diskStoreDetails) {
    for (CacheServer cacheServer : cache.getCacheServers()) {
      if (isUsingDiskStore(cacheServer, diskStore)) {
        final DiskStoreDetails.CacheServerDetails cacheServerDetails = new DiskStoreDetails.CacheServerDetails(
  ...
  protected String getDiskStoreName(final Gateway gateway) {
    return (gateway.getQueueAttributes() == null ? null : StringUtils.defaultIfBlank(
      gateway.getQueueAttributes().getDiskStoreName(), DiskStoreDetails.DEFAULT_DISK_STORE_NAME));
  }
  ...
  protected boolean isPersistent(final Gateway gateway) {
    return (gateway.getQueueAttributes() != null && gateway.getQueueAttributes().getEnablePersistence());
  }
  ...
  protected boolean isUsingDiskStore(final Gateway gateway, final DiskStore diskStore) {
  ...
  protected void setGatewayDetails(final InternalCache cache, final DiskStore diskStore, final DiskStoreDetails diskStoreDetails) {
    for (GatewaySender gatewaySender : cache.getGatewaySenders()) {
      if (isUsingDiskStore(gatewaySender, diskStore)) {
        final DiskStoreDetails.GatewayDetails gatewayDetails = new DiskStoreDetails.GatewayDetails(gatewaySender.getId());
        gatewayDetails.setPersistent(isPersistent(gatewaySender));
        diskStoreDetails.add(gatewayDetails);
      }
    }

    for (GatewayHub gatewayHub : cache.getGatewayHubs()) {
      for (Gateway gateway : gatewayHub.getGateways()) {
        if (isUsingDiskStore(gateway, diskStore)) {
          final DiskStoreDetails.GatewayDetails gatewayDetails = new DiskStoreDetails.GatewayDetails(gateway.getId());
          gatewayDetails.setPersistent(isPersistent(gateway));
          diskStoreDetails.add(gatewayDetails);
        }
      }
    }
  ...
  protected void setPdxSerializationDetails(final InternalCache cache, final DiskStore diskStore, final DiskStoreDetails diskStoreDetails) {
    if (cache.getPdxPersistent()) {
      final String diskStoreName = StringUtils.defaultIfBlank(cache.getPdxDiskStore(),
        DiskStoreDetails.DEFAULT_DISK_STORE_NAME);
