...
public class QuorumCheckerImpl implements QuorumChecker {
    ...
    try {
      ...
      for (int i=0; i<members.size(); i++) {
        ...
        if (!receivedAcks.contains(addr)) {
          ...
          if (debugLogger != null && debugLogger.fineEnabled()) {
            debugLogger.fine("quorum check: sending request to " + addr);
          }
          try {
      ...
      for ( ;; ) {
        ...
        if (remaining <= 0) {
          if (debugLogger != null && debugLogger.fineEnabled()) {
            debugLogger.fine("quorum check: timeout waiting for responses.  " + receivedAcks.size() + " responses received");
          }
          break;
        }
        if (debugLogger != null && debugLogger.fineEnabled()) {
          debugLogger.fine("quorum check: waiting up to " + remaining + "ms to receive a quorum of responses");
        }
        Thread.sleep(500);
        if (receivedAcks.size() == members.size()) {
          // we've heard from everyone now so we've got a quorum
          if (debugLogger != null && debugLogger.fineEnabled()) {
            debugLogger.fine("quorum check: received responses from all members that were in the old distributed system");
          }
      ...
      if (debugLogger != null && debugLogger.fineEnabled()) {
        debugLogger.fine("quorum check: contacted " + receivedAcks.size() + " processes with " + ackedWeight + " member weight units.  Threshold for a quorum is " + lossThreshold);
      }
  ...
  private void startPingResponder() {
    ...
    this.pingResponder = new Thread("GemFire Auto-reconnect responder") {
      public void run() {
        ...
        LogWriter log = new ManagerLogWriter(LogWriterImpl.INFO_LEVEL, System.out);
        ...
        while (!stopper.get()) {
          ...
          try {
            ...
            log.info("received " + packet.getLength() + " bytes from " + senderSockAddr);
            if (packet.getLength() == 4) {
              if (buffer[0] == 'p' && buffer[1] == 'i' && buffer[2] == 'n' && buffer[3] == 'g') {
                log.info("received ping-pong request from " + senderSockAddr + " - sending response");
                ...
                pongReceived(senderSockAddr, log);
              }
            }
          } catch (IOException e) {
            try {
  ...
  public void pongReceived(SocketAddress senderSockAddr, GFLogWriter log) {
    log.info(LocalizedStrings.ONE_ARG,
        "received ping-pong response from " + senderSockAddr);
    IpAddress memberAddr = addressConversionMap.get(senderSockAddr);
    if (memberAddr != null) {
      log.info(LocalizedStrings.ONE_ARG,
          "quorum check: mapped address to member ID " + memberAddr);
