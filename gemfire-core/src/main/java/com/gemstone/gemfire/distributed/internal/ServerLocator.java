...
public class ServerLocator implements TcpHandler, DistributionAdvisee {
  ...
  public Object processRequest(Object request) throws IOException {
    ...
    if(getLogWriterI18n().fineEnabled()) {
      getLogWriterI18n().fine("ServerLocator: Received request " + request);
    }
    ...
    if(getLogWriterI18n().fineEnabled()) {
      getLogWriterI18n().fine("ServerLocator: Sending response " + response);
    }
  ...
  public void profileCreated(Profile profile) {
    if(profile instanceof BridgeServerProfile) {
      ...
      if(getLogWriterI18n().fineEnabled()) {
        getLogWriterI18n().fine("ServerLocator: Received load from a new server " + location + ", " + bp.getInitialLoad());
      }
  ...
  public void profileRemoved(Profile profile) {
    if(profile instanceof BridgeServerProfile) {
      ...
      if(getLogWriterI18n().fineEnabled()) {
        getLogWriterI18n().fine("ServerLocator: server departed " + location);
      }
      synchronized(ownerMap) {
  ...
  private void logServers() {
    if (productUseLog != null) {
      ...
      int connectionLoad = 0;
      for (ServerLoad l: loadMap.values()) {
        connectionLoad += l.getConnectionLoad();
      }
      sb.append("server summary: ")
        .append(loadMap.size())
        .append(" cache servers with ")
        .append(connectionLoad)
        .append(" client connection load\n")
