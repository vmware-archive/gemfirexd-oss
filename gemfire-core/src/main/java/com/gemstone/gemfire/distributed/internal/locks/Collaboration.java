...
public class Collaboration {
  ...
  /** LogWriterI18n for debugging. Enabled only for fine logging. */
  ...
  public void enableDebug(LogWriterI18n logForDebugging) {
    this.log = logForDebugging;
  }
    ...
    synchronized (this.topicsQueue) {
      ...
      if (this.currentTopic == null) {
        if (this.log.fineEnabled()) {
          this.log.fine("Collaboration.acquireUninterruptibly: " +
              this.toString() + 
              ": no current topic, setting topic to " + topicObject);
        }
        ...
        if (this.log.fineEnabled()) {
          this.log.fine("Collaboration.acquireUninterruptibly: " 
              + this.toString() + 
              ": already current topic: " + topicObject);
        }
        this.currentTopic.addThread(Thread.currentThread());
        return;
      }
      ...
      // if other topic then add to pending topics and then wait
        ...
        if (this.log.fineEnabled()) {
          this.log.fine("Collaboration.acquireUninterruptibly: " 
              + this.toString() + ": adding pendingTopic " + pendingTopic 
            + "; current topic is " + this.currentTopic);
        }
      }
    } // synchronized
    ...
    synchronized (this.topicsQueue) {
      ...
      if (topic != null) {
        synchronized (topic) {
          ...
          if (this.log.fineEnabled()) {
            this.log.fine("Collaboration.setCurrentTopic: " + this.getIdentity()
                + ": new topic is " +  topic);
          }
        ...
        if (this.log.fineEnabled()) {
          this.log.fine("Collaboration.setCurrentTopic: " + this.toString() 
              + " setting current topic to null");
        }
    ...
    synchronized (topic) {
      while (!topic.isCurrentTopic()) {
        ...
        try {
          ...
          if (this.log.fineEnabled()) {
            this.log.fine("Collaboration.awaitTopic: " + getIdentity() + 
              " waiting for topic " + topic 
              + "; current topic probably " + sniff.toString() 
              + ", which may have a thread count of " + sniff.threadCount());
          }
          topic.wait();
        }
  ...
  public void release() {
    synchronized (this.topicsQueue) {
      ...
      if (this.log.fineEnabled()) {
        this.log.fine("Collaboration.release: " + this.toString() 
            + " releasing topic");
      }
      if (topic.isEmptyAfterRemovingThread(Thread.currentThread())) {
        if (this.log.fineEnabled()) {
          this.log.fine("Collaboration.release: " + this.toString() 
              + " released old topic " + topic);
        }
