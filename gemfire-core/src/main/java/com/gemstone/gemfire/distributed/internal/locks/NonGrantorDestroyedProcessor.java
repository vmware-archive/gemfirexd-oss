  ...
  extends PooledDistributionMessage implements MessageWithReply {
    ...
    protected static void send(String serviceName,
      ...
      if (DLockLogWriter.fineEnabled(dm)) {
        String s = "NonGrantorDestroyedMessage sending " + msg + " to " + grantor;
        DLockLogWriter.fine(dm, s);
      }
    ...
    private void basicProcess(final DM dm) {
      ...
      try {
        ...
        if (svc != null && svc.isCurrentlyOrIsMakingLockGrantor()) {
          ...
          if (grantor != null) {
            grantor.handleDepartureOf(getSender(), false);
        ...
        if (DLockLogWriter.fineEnabled(dm)) {
          DLockLogWriter.fine(dm, "Processing of NonGrantorDestroyedMessage " +
              "resulted in InterruptedException", e);
        }
      }
      catch (LockServiceDestroyedException e) {
        if (DLockLogWriter.fineEnabled(dm)) {
          DLockLogWriter.fine(dm, "Processing of NonGrantorDestroyedMessage " +
              "resulted in LockServiceDestroyedException", e);
        }
      }
      catch (LockGrantorDestroyedException e) {
        if (DLockLogWriter.fineEnabled(dm)) {
          DLockLogWriter.fine(dm, "Processing of NonGrantorDestroyedMessage " +
              "resulted in LockGrantorDestroyedException", e);
        }
      }
      finally {
        if (!replied) {
