...
public class PartitionByPrimaryKeyPartitionClause extends
    AbstractPartitionClause {
  ...
  public boolean generatePartitionClause(TableInfo table) {
    ...
    if (pk != null){
      List<ColumnInfo> primaryList = pk.getColumns();
      if(primaryList.size() > 0){
        partColumns.addAll(primaryList);
        success = true;
      }  
    } 
    return success;
  ...
  public String getParitionClause() {
    return availablePartitionClause[1];
  }
}
