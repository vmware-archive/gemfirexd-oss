...
public class DeleteReservation extends Transactions {
  ...
  public boolean doTxn(Connection conn, long f_id, Long c_id, String c_id_str, String ff_c_id_str, Long ff_al_id, Long r_seatnum) throws SQLException {
    ...
    if (c_id == null) {
      ...
      if (results.next()) {
        ...
        results.close();
        Log.getLogWriter().info(String.format("No Customer record was found [c_id_str=%s, ff_c_id_str=%s, ff_al_id=%s]",
    ...
    stmt = this.getPreparedStatement(conn, GetCustomerReservation, c_id, f_id, r_seatnum);
    //stmt.setLong(1, c_id);
    //stmt.setLong(2, f_id);
    //stmt.setLong(3, r_seatnum);
    ...
    if (updated != 1) {
      throw new TestException("Delete reservation deletes " + updated + " row for " + DeleteReservation.getSQL());
    }
    ...
    updated = stmt.executeUpdate();
    if (updated != 1) {
      throw new TestException("Update customer updates " + updated + " row for " + DeleteReservation.getSQL());
    }
    ...
    if (ff_al_id != null) {
      ...
      updated = stmt.executeUpdate();
      if (updated != 1) {
        Log.getLogWriter().info(String.format("Updated " + updated + " row. " +
        		"Failed to update FrequentFlyer info [c_id=%d, ff_al_id=%d]", c_id, ff_al_id));
      }
    }
    
    if (logDML) Log.getLogWriter().info(String.format("Deleted reservation on flight %d for customer %d [seatsLeft=%d]", f_id, c_id, seats_left+1));        
    return true;  
  }
}
