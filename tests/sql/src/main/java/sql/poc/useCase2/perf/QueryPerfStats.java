...
public class QueryPerfStats extends PerformanceStatistics {
  ...
  public static StatisticDescriptor[] getStatisticDescriptors() {
    ...
    return new StatisticDescriptor[] {              
      ...
      factory().createIntCounter
         ( 
          SELECTQUERIES,
          "Number of select queries completed.",
          "operations",
           largerIsBetter
          ),
       factory().createLongCounter
         ( 
          SELECT_QUERY_TIME,
          "Total time spent performing select queries.",
          "nanoseconds",
          !largerIsBetter
          ),
    };
  ...
  /////////////////// Accessing stats ////////////////////////
   ...
   public int getSelectQueries() {
     return statistics().getInt(SELECTQUERIES);
   }
   public long getSelectQUERYTime() {
     return statistics().getLong(SELECT_QUERY_TIME);
   }
  ...
  public void incSelectQueries(int amount) {
    statistics().incInt(SELECTQUERIES, amount);
  }
  /**
   * increase the time on the queries by the supplied amount
   */
  public void incSelectQueryTime(long amount) {
    statistics().incLong(SELECT_QUERY_TIME, amount);
  }
  /**
   * @return the timestamp that marks the start of the query
   */
  ...
  public void endSelectQuery(long start) {
    long ts = NanoTimer.getTime();
    statistics().incInt(SELECTQUERIES, 1);
    long elapsed = ts-start;
    statistics().incLong(SELECT_QUERY_TIME, elapsed);
  }
}
