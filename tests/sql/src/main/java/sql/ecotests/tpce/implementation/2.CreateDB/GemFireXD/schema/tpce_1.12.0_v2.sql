-- This version of schema implements the partition/colocation strategy in addition to v1.
...
CREATE TABLE settlement (
   ...
   PRIMARY KEY (se_t_id)
) PARTITION BY COLUMN (se_t_id) COLOCATE WITH (trade);
...
) PARTITION BY COLUMN (th_t_id) COLOCATE WITH (trade);
...
) PARTITION BY COLUMN (hh_t_id) COLOCATE WITH (trade);
...
CREATE TABLE cash_transaction (
   ...
   ct_name VARCHAR(100),
   PRIMARY KEY (ct_t_id)
) PARTITION BY COLUMN (ct_t_id) COLOCATE WITH (trade);
...
CREATE TABLE trade_request (
   ...
   PRIMARY KEY (tr_t_id)
) PARTITION BY COLUMN (tr_t_id) COLOCATE WITH (trade);
