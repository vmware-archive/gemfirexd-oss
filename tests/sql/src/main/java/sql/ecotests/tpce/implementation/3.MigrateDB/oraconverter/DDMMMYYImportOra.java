...
public class DDMMMYYImportOra extends com.pivotal.gemfirexd.load.Import {
	public static Map<String, String> monthsMap = new HashMap<String, String>();

	public DDMMMYYImportOra(String inputFileName, String columnDelimiter,
	...
	public String getString(int columnIndex) throws SQLException {
		...
		if (val != null && val.length() > 0) {
			...
			// Oracle default format: 11-NOV-64, the year is two digits, so append 19
			// GemFireXD format is YYYY-MM-DD HH:MI:SS
			case java.sql.Types.DATE:
				return "19" + val.substring(7, 9) + "-"
						+ monthsMap.get(val.substring(3, 6).toLowerCase())
						+ "-" + val.substring(0, 2);
			case java.sql.Types.TIMESTAMP:
				return "19" + val.substring(7, 9) + "-"
						+ monthsMap.get(val.substring(3, 6).toLowerCase())
						+ "-" + val.substring(0, 2) + " 00:00:00";
			default:
				return val;
			}
		} else {
