...
public class TradeSecuritiesDMLDistTxStmtJson extends TradeSecuritiesDMLStmtJson
  ...
  protected static String[] updateJSON = {
    "update trade.securities set price = ?  where sec_id = ? ", 
    "update trade.securities set tid =?,json_details = ? where sec_id = ? ",
    "update trade.securities set symbol = ?, exchange =? , json_details = ? where sec_id = ?",    
    "update trade.securities set price = ? where symbol Between ? AND ? and sec_id < ?" //commit early
    };
  
  protected static String[] deleteJSON = {
    "delete from trade.securities where cast (json_evalpath(json_details,'sec_id') as Integer)  =?",
    
    "delete from trade.securities where (cast(json_evalpath(json_details,'symbol') as varchar(20)) like ? and cast(json_evalpath(json_details,'exchange') as varchar(20)) = ? ) and tid=?", //commit early
     };
  ...
  private void updateGfxdTable(Connection gConn, int[]sec_id, String[]symbol, 
    ...
    for (int i=0 ; i<size ; i++) {
      ...
      else stmt = getStmt(gConn, updateJSON[whichUpdate]); //use only this after bug#39913 is fixed
