...
public class TradeNetworthDMLDistTxStmtJson extends TradeNetworthDMLStmtJson implements
    DMLDistTxStmtIF {
  /*
  ...
  */
  ...
  protected static String[] deleteJSON = {
    //uniq key
    "delete from trade.networth where cast (json_evalpath(json_details,'$..networth.cid') as Integer) = ?", 
    "delete from trade.networth where cast (json_evalpath(json_details,'$..networth.cid') as Integer) in (?,?)"
  };
  ...
  protected void insertToGfxdOnlyTable (Connection conn, int cid, BigDecimal[] cash, 
    ...
    PreparedStatement stmt = getStmt(conn, insertJSON);
    int tid = getMyTid();
  ...
  protected void updateGfxdTable(Connection conn, int[] cid, BigDecimal[] availLoanDelta, 
    ...
    for (int i=0 ; i<size ; i++) {
      ...
      */
      ...
      else stmt = getStmt(conn, updateJSON[whichUpdate[i]]); //use only this after bug#39913 is fixed
  ...
  */
  ...
  protected void deleteFromGfxdTable(Connection gConn, int cid, int cid1, 
    ...
    PreparedStatement stmt = null;
    if (SQLTest.isEdge && !isTicket48176Fixed && isHATest) stmt = getStmtThrowException(gConn, deleteJSON[whichDelete]);
    else stmt = gConn.prepareStatement(deleteJSON[whichDelete]); 
  ...
  public boolean queryGfxd(Connection gConn, boolean withDerby){
    ...
    int whichQuery = getWhichOneQuery(numOfNonUniq, select.length); //randomly select one query sql based on test uniq or not
    int cash = 100000;
    int sec = 100000;
    int tid = getMyTid();
  ...
  protected int getWhichOneQuery(int numOfNonUniq, int total) {
    int whichOne;
    if (testUniqueKeys) {
