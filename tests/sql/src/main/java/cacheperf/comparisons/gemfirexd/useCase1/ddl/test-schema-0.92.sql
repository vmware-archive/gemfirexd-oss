-- Changes for v0.92
--  1. Refact SECL_CHN_DATA_STATUS_HIST to only contain dynamic fields
--  2. Add index SECL_CHN_DATA_STATUS_TXN_ID_MATCH_TIME_IDX on SECL_CHN_DATA_STATUS_HIST (CHANNEL_TXN_ID)
--  3. Add index SECL_MATCHED_DATA_TXN_IDX on SECL_MATCHED_DATA (BO_TXN_ID)
--  4. Add index SECL_OFAC_MESSAGE_FS_TXN_ID_IDX ON SECL_OFAC_MESSAGE(FS_MESSAGE_ID,BO_TXN_ID)
--  5. Add index SECL_OFAC_CHUNKED_MESSAGE_FS_ID_IDX ON SECL_OFAC_CHUNKED_MESSAGE (FS_MESSAGE_ID,CHUNK_ID)
--  7. Add index SECL_BO_RAW_DATA_TXN_IDX ON SECL_BO_RAW_DATA (BO_TXN_ID)
--  8. Add table SECT_JOB_CONTROL
--  9. Add primary key for SECL_BO_DATA_STATUS_HIST
--  10. Add primary key for SECL_BO_RAW_DATA
--  11. Add primary key for SECL_CHN_DATA_STATUS_HIST
--  12. Add column TXN_HOUSEKEEPING_ORPHAN for SECM_TXN_MANAGEMENT
--  13. Change the column CLIENT_FILE_ID from VARCHAR(100) to VARCHAR(255)
...
CREATE INDEX SECT_CHANNEL_DATA_CLIENT_ACCOUNT_MATCHING_IDX
ON SECT_CHANNEL_DATA
    (
     CLIENT_ACCOUNT
    )
