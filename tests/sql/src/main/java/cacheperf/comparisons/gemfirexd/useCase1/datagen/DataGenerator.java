...
public class DataGenerator {
  ...
  static class ColumnRange {
    ...
    private ColumnRange(String qColName, Object start, Object increment, int minNumber,
        int maxNumber, String foreachColumn, boolean isDecimal,
        String[] valueList, String valuePattern) {
    ...
    private boolean getNext() {
      ...
      if (this.valueList != null && this.minNumber == -1 && this.maxNumber == -1) {
        ...
        if (this.currentValue == null || (this.currentLimit > 0 &&
          ...
          if (this.valuePattern != null) {
            if (this.isDecimal) {
              this.currentNValue = ((BigDecimal)currentNValue).add(
                  (BigDecimal)increment);
            }
            else {
              this.currentNValue = (Integer)currentNValue + (Integer)increment;
            }
            this.currentValue = String.format(this.valuePattern,
                this.currentNValue);
          }
          else if(this.foreignKeyColumn != null && this.valueList != null) {
    ...
    public String toString() {
      ...
      if (repeatRetVal != null) sb.append(";").append(this.repeatRetVal);
      return sb.toString();
    }
    ...
    static enum specialTokens {
        ...
        public ColumnRange parse(String col, String line, Connection conn) {
          ...
          if (valueSpec.indexOf("-") <= 0) {
            ...
            int begin = Integer.parseInt(valueLengths[0]),
                end = Integer.parseInt(valueLengths[1]);
            System.out.println("Registering dynamic value map for column "
      ...
      format {
        ...
        public ColumnRange parse(String col, String line, Connection conn)
          ...
          if (optBegin != -1) {
            ...
            int optionalportion = (optEnd - optBegin) - 1; // minus 1 cause [ & ] is inclusive.
            ...
            if(dot == -1) {
              ...
              beginLength = fSpec.length() - 1; //exclude '.' from total length.
              prec = fSpec.length() - dot;
            }
          }
          
          System.out.println("Registering dynamic value map for column " + col
              + ": " + line + " from " + beginLength + (endLength != -1 ? " to " + endLength
              + " length ": "") + " with " + prec + " precision");
      ...
      skipvalue {
        ...
        public ColumnRange parse(String col, String line, Connection conn)
            throws SQLException {
          System.out.println("NOT Registering, rather skipping value for column " + col
              + ": " + line);
          return new ColumnRandom(col, this);
        }
        ...
        if (line.startsWith("::")) {
          ...
          if (specialTokens.valuelist.lineStartsWith(line)) {
            ...
            if (start.indexOf('.') >= 0 || increment.indexOf('.') >= 0) {
              return new ColumnRange(col, new BigDecimal(start), new BigDecimal(
                  increment), minNumber, maxNumber, foreachColumn, true, null,
                  valuePattern);
            }
            else {
              return new ColumnRange(col, Integer.parseInt(start),
                  Integer.parseInt(increment), minNumber, maxNumber,
                  foreachColumn, false, null, valuePattern);
            }
          }
        }
    ...
    public String getNext(DataGenerator generator, int type, int cLen, int cPrec, HashSet<String> primaryKey, int tableIndex, String colName, int i) {
      ...
      if (fixedToken != null) {
        ...
        if (endLen == -1) {
          ...
          this.currentValue = generator.generateValues(type,
              rand.nextInt(endLen - beginLen) + beginLen, precision, primaryKey,
              tableIndex, colName, i, true,
              qColName.substring(qColName.lastIndexOf(".") + 1) + "  ");
        }
      }
            ...
            if ((mappedColumn = ColumnRange.parse(col, mappedColumnS, conn)) == null) {
              ...
              if (parentCol != null) {
                ...
                if (parentColRange == null) {
                  parentColRange = new ColumnRange(col, null, null,
                        -1, -1, null, false, null, null);
        ...
        while (rs.next()) {
          ...
          for (int i = 0; i < rowCount[tableIndex]; ++i) {
            ...
            for (Object[] columnMeta : columnsMeta) {
              ...
              if (value == null) {
                value = generateValues((Integer)columnMeta[1], (Integer)columnMeta[2],
                    (Integer)columnMeta[3], primaryKey, tableIndex, colName, i, false, null);
              }
            ...
            if (i == 0) {
              ...
              columnList = null; // render it unusable so that accidentally we don't write again.
            }
  ...
  protected String generateValues(int type, int cLen, int prec, HashSet<String> primaryKey, int tableIndex, String colName, int i, boolean absoluteLength, String baseValue) {
    ...
    switch (type) {
      ...
      case java.sql.Types.CLOB: {
        ...
        if (absoluteLength && baseValue != null && value.length() > baseValue.length()) {
  ...
  protected String generateString(int len, boolean absoluteLength) {
    ...
    if(!absoluteLength) len = (int)(Math.random() * 9 * len / 10) + 1 + (len / 10);
  ...
  protected String generateNumeric(int len, int prec, boolean absoluteLength) {
    ...
    if (major > 1) {
      ...
      if(!absoluteLength) major = (int)(Math.random() * major) + 1;
    }
