...
statspec updatesPerSecond *client* CoreWorkloadStats * updates
filter=perSecond combine=combineAcrossArchives ops=min,max,mean?,stddev trimspec=workload
;
statspec updateTotal *client* CoreWorkloadStats * updates
filter=none combine=combineAcrossArchives ops=max-min! trimspec=workload
;
statspec updateTotalTime *client* CoreWorkloadStats * updateTime
filter=none combine=combineAcrossArchives ops=max-min! trimspec=workload
;
expr updateResponseTime = updateTotalTime / updateTotal ops=max-min?
;
