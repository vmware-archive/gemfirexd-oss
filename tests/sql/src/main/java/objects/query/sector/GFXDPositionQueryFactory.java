...
public class GFXDPositionQueryFactory extends SQLPositionQueryFactory {

  public GFXDPositionQueryFactory() {
    riskQueryFactory = new GFXDRiskQueryFactory();
  }
    ...
    if (dataPolicy == QueryPrms.REPLICATE) {
      ...
      switch (partitionType) {
        ...
        case QueryPrms.PARTITION_BY_COLUMN:
          stmt +=
              " PARTITION BY COLUMN ("
                  + SectorPrms.getPositionPartitionColumn() + ")";
          break;
      ...
      boolean offheap = SectorPrms.getPositionOffHeap();
      if (offheap) {
        stmt += " OFFHEAP";
      }
      stmts.add(stmt);
