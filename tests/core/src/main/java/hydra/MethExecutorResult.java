...
public class MethExecutorResult implements Serializable {
   ...
   * exception being thrown.  The "result" of is set to {@link
   * #EXCEPTION_OCCURRED}.  If the exception could not be serialized,
   * {@link #getException()} will return {@link
  ...
  public MethExecutorResult(Throwable thr) {
    ...
    if (thr instanceof SchedulingOrder) {
      ...
      try {
        ...
        // [sumedh] NONSERIALIZABLE_EXCEPTION gives no information about the
        // real cause, so using the original exception stack in exception
        //this.exception = NONSERIALIZABLE_EXCEPTION;
        sw = new StringWriter();
