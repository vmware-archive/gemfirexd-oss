...
public class VM implements java.io.Serializable {
  ...
  /** The system to which this VM is (or will be) connected */
  private GemFireSystem system;

  /** The hydra client for this VM */
  private RemoteTestModuleIF client;
   ...
   * given process id.
   */
  VM(Host host, int pid, GemFireSystem system, RemoteTestModuleIF client) {
    this.host = host;
    this.pid = pid;
    this.system = system;
    this.client = client;
   ...
   * connected.  Note thatm at the moment, this doesn't do anything in
   * the VM.
   *
   * @throws IllegalArgumentException
   *         <code>system</code> does not reside on this VM's host
   */
  public void setSystem(GemFireSystem system) {
    if (!system.getHost().equals(this.getHost())) {
      String s = this + " cannot connect to " + system;
   ...
   * connected. 
   */
  public GemFireSystem getSystem() {
    return this.system;
  }
  ...
  public synchronized void bounce() {
    ...
    try {
      ClientVmRecord vm = RemoteTestModule.Master.bounceUnitTestVm(this.pid);
      this.pid = vm.getPid();
      this.client = vm.getRepresentativeClient().getTestModule();
      this.available = true;
    } catch (RemoteException e) {
      RMIException rmie = new RMIException(this, this.getClass().getName(),
        "bounceVM", e, util.TestHelper.getStackTrace(e));
      Log.getLogWriter().severe("ERROR bouncing VM ", rmie);
      throw rmie;
    }
  }
  ...
  public String toString() {
    return "VM " + this.getPid() + " running on " + this.getHost() 
      + " associated with system " + system;
  }

  public static int getCurrentVMNum() {
    return RemoteTestModule.getMyVmid();
  }
