...
public abstract class Host implements java.io.Serializable {
   ...
   */
  protected void addVM(int pid, String systemName, RemoteTestModuleIF client) {
    VM vm = new VM(this, pid, systemNamed(systemName), client);
    this.vms.add(vm);
  }
  ...
  protected void addLocator(int pid, String systemName, RemoteTestModuleIF client) {
    setLocator(new VM(this, pid, systemNamed(systemName), client));
  }
   ...
   *
   * @param n
   *        A zero-based identifier of the GemFire system
   *
   * @throws IllegalArgumentException
   *         <code>n</code> is more than the number of GemFire systems
   */
  public GemFireSystem getSystem(int n) {
    int size = systems.size();
    if (n >= size) {
      String s = "Cannot request GemFire system " + n + 
        ".  There are only " + size + " systems running on " + this;
      throw new IllegalArgumentException(s);

    } else {
      return (GemFireSystem) systems.get(n);
    }
  }
   ...
   * host. 
   */
  protected void addSystem(String name, String systemDirectory) {
    GemFireSystem system =
      new GemFireSystem(name, this, systemDirectory);
    this.systems.add(system);
    systemNames.put(name, system);
  }
  ...
  public GemFireSystem systemNamed(String systemName) {
    return (GemFireSystem)systemNames.get(systemName);
  }
