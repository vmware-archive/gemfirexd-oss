...
<h1>Query Results: ${query}</h1>
<form action="swarm" method="get">
<input type="text" size="10" name="query" value="${query}"/><input type="submit" value="go"/>
</form>
</div>
...
<div id="failcontent">
<c:choose>
<c:when test="${empty failures}">
No query results found. 
</c:when>
<c:otherwise>
<c:forEach items="${failures}" var="dutmd">
