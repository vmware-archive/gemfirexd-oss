...
do
  echo "Preparing compression $mode report..."
  java -cp $GEMFIRE/lib/gemfire.jar:$JTESTS -Xmx1024m -DomitFailedTests=true \
        -DcompReportFile=compressionperf.$mode.txt -Dmode=$mode -DcompareByKey=true \
        -DJTESTS=$JTESTS -Dgemfire.home=$GEMFIRE -DaddTestKey=true \
        perffmwk.PerfComparer $@
  echo "...done"
done

chmod uog+rw compressionperf.ratio.txt compressionperf.raw.txt
