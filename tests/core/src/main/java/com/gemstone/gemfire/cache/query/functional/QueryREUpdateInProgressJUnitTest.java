  ...
  public void testQueriesOnREWhenUpdateInProgress() {
    ...
    for (int i=0; i < queries.length; i++) {
      try {
        ...
        fail("Query executio failed for query: "+ queries[i]+" "+ TestHelper.getStackTrace(e));
      }
    }
    ...
    try {
      ...
      fail("Index creation failed!"+ TestHelper.getStackTrace(e));
    }
    ...
    for (int j = 0; j < len; j++) {
      if ((r[j][0] != null) && (r[j][1] != null)) {
        ...
        if ((type1.getClass().getName()).equals(type2.getClass().getName())) {
          ...
          DistributedTestCase.getLogWriter().error(
