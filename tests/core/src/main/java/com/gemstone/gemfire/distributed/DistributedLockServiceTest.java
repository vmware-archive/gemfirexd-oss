...
public class DistributedLockServiceTest extends DistributedTestCase {
  ...
  private static DistributedLockBlackboard blackboard = DistributedLockBlackboard.getInstance();
  ...
  public void setUp() throws Exception {
    ...
    for (int h = 0; h < Host.getHostCount(); h++) {
      ...
      for (int v = 0; v < host.getVMCount(); v++) {
        host.getVM(v).invoke(
          DistributedLockServiceTest.class, "connectDistributedSystem", null);
      }
    }
  }
  ...
  public void tearDown2() throws Exception {
    ...
    invokeInEveryVM(DistributedLockServiceTest.class,
...
//     disconnectAllFromDS();
  ...
  public static void remoteDumpAllDLockServices() {
    DLockService.dumpAllServices(getLogWriter().convertToLogWriterI18n());
  }
  ...
  public void testCreateDestroy() throws Exception {
    ...
    DistributedTestCase.join(remoteWaitingThread, 10 * 1000, getLogWriter());
  ...
  public void testFairness() throws Exception {
    ...
    for (int i = 0; i < vmThreads.length; i++) {
      final int vm = i;
      getLogWriter().info("[testFairness] lining up " + vmThreads[vm] + 
                          " threads in vm " + vm); 
    ...
    for (int i = 0; i < vmThreads.length; i++) {
      ...
      for (int j = 0; j < vmThreads[vm]; j++) {
        ...
        Integer count = (Integer)Host.getHost(0).getVM(vm).invoke(
            DistributedLockServiceTest.class, 
            "get_count_testFairness", 
            new Object[] { new Integer(thread) });
    ...
    getLogWriter().info("[testFairness] totalLocks=" + totalLocks + 
                        " minLocks=" + minLocks +
                        " maxLocks=" + maxLocks);
    ...
    getLogWriter().info("[testFairness] deviation=" + deviation +
...
//    try {
      ...
      host.getVM(originalGrantor).invoke(
          DistributedLockServiceTest.class, "identifyLockGrantor", args);
      ...
      for (int vm = 0; vm < numVMs; vm++) {
        ...
        Boolean isGrantor = (Boolean)host.getVM(finalvm).invoke(
            DistributedLockServiceTest.class, "isLockGrantor", args);
...
//    }
  ...
  public void testLockFailover() {
    ...
    getLogWriter().fine("[testLockFailover] create services");
    
    Host.getHost(0).getVM(originalGrantorVM).invoke(
        DistributedLockServiceTest.class, "remoteCreateService",
        new Object[] { serviceName });
    
    Host.getHost(0).getVM(oneVM).invoke(
        DistributedLockServiceTest.class, "remoteCreateService",
        new Object[] { serviceName });
    
    Host.getHost(0).getVM(twoVM).invoke(
        DistributedLockServiceTest.class, "remoteCreateService",
        new Object[] { serviceName });
    
    Host.getHost(0).getVM(originalGrantorVM).invoke(
        DistributedLockServiceTest.class, "identifyLockGrantor",
        new Object[] { serviceName });
        
    Boolean isGrantor = (Boolean) Host.getHost(0).getVM(originalGrantorVM).invoke(
        DistributedLockServiceTest.class, "isLockGrantor", 
        new Object[] { serviceName });
    assertEquals("First member calling getLockGrantor failed to become grantor", 
                 Boolean.TRUE, isGrantor);

    // get locks...
    getLogWriter().fine("[testLockFailover] get lock");
    
    Boolean locked = (Boolean) Host.getHost(0).getVM(originalGrantorVM).invoke(
        DistributedLockServiceTest.class, "lock", 
        new Object[] { serviceName, "KEY-"+originalGrantorVM });
    assertEquals("Failed to get lock in testLockFailover", 
                 Boolean.TRUE, locked);
                 
    locked = (Boolean) Host.getHost(0).getVM(twoVM).invoke(
        DistributedLockServiceTest.class, "lock", 
        new Object[] { serviceName, "KEY-"+twoVM });
    assertEquals("Failed to get lock in testLockFailover", 
                 Boolean.TRUE, locked);
                 
    locked = (Boolean) Host.getHost(0).getVM(oneVM).invoke(
        DistributedLockServiceTest.class, "lock", 
        new Object[] { serviceName, "KEY-"+oneVM });
    assertEquals("Failed to get lock in testLockFailover", 
                 Boolean.TRUE, locked);
                 
    // disconnect originalGrantorVM...
    getLogWriter().fine("[testLockFailover] disconnect originalGrantorVM");
    ...
    getLogWriter().fine("[testLockFailover] release locks");
                 
    Boolean unlocked = (Boolean) Host.getHost(0).getVM(twoVM).invoke(
        DistributedLockServiceTest.class, "unlock", 
        new Object[] { serviceName, "KEY-"+twoVM });
    assertEquals("Failed to release lock in testLockFailover", 
                 Boolean.TRUE, unlocked);

    unlocked = (Boolean) Host.getHost(0).getVM(oneVM).invoke(
        DistributedLockServiceTest.class, "unlock", 
        new Object[] { serviceName, "KEY-"+oneVM });
    assertEquals("Failed to release lock in testLockFailover", 
                 Boolean.TRUE, unlocked);
    ...
    unlocked = (Boolean) Host.getHost(0).getVM(twoVM).invoke(
        DistributedLockServiceTest.class, "unlock", 
        new Object[] { serviceName, "KEY-"+oneVM });
    assertEquals("Failed to release lock in testLockFailover", 
                 Boolean.TRUE, unlocked);
    ...
    getLogWriter().fine("[testLockFailover] verify grantor identity");
    
    InternalDistributedMember oneID = (InternalDistributedMember)Host.getHost(0).getVM(oneVM).invoke(
        DistributedLockServiceTest.class, 
        "identifyLockGrantor", 
        new Object[] {serviceName});
    InternalDistributedMember twoID = (InternalDistributedMember)Host.getHost(0).getVM(twoVM).invoke(
        DistributedLockServiceTest.class, 
        "identifyLockGrantor", 
        new Object[] {serviceName});
  ...
  public void testLockThenBecomeLockGrantor() {
    ...
    // create lock services...
    getLogWriter().fine("[testLockThenBecomeLockGrantor] create services");
    ...
    Host.getHost(0).getVM(becomeGrantorVM).invoke(
        DistributedLockServiceTest.class, "remoteCreateService",
        new Object[] { serviceName });
    
    Host.getHost(0).getVM(thirdPartyVM).invoke(
        DistributedLockServiceTest.class, "remoteCreateService",
        new Object[] { serviceName });
    ...
    // control...
    getLogWriter().fine("[testLockThenBecomeLockGrantor] check control");
    Boolean check = (Boolean) Host.getHost(0).getVM(becomeGrantorVM).invoke(
        DistributedLockServiceTest.class, "unlock", 
        new Object[] { serviceName, "KEY-"+becomeGrantorVM });
    assertEquals("Check of control failed... unlock succeeded but nothing locked", 
                 Boolean.FALSE, check);
                 
    // get locks...
    getLogWriter().fine("[testLockThenBecomeLockGrantor] get lock");
    ...
    locked = (Boolean) Host.getHost(0).getVM(thirdPartyVM).invoke(
        DistributedLockServiceTest.class, "lock", 
        new Object[] { serviceName, "KEY-"+thirdPartyVM });
    assertEquals("Failed to get lock in testLockThenBecomeLockGrantor", 
                 Boolean.TRUE, locked);
                 
    locked = (Boolean) Host.getHost(0).getVM(becomeGrantorVM).invoke(
        DistributedLockServiceTest.class, "lock", 
        new Object[] { serviceName, "KEY-"+becomeGrantorVM });
    assertEquals("Failed to get lock in testLockThenBecomeLockGrantor", 
                 Boolean.TRUE, locked);
    ...
    // become lock grantor...
    getLogWriter().fine("[testLockThenBecomeLockGrantor] become lock grantor");
    
    Host.getHost(0).getVM(becomeGrantorVM).invoke(
        DistributedLockServiceTest.class, "becomeLockGrantor",
        new Object[] { serviceName });
    ...
    isGrantor = (Boolean) Host.getHost(0).getVM(becomeGrantorVM).invoke(
        DistributedLockServiceTest.class, "isLockGrantor", 
        new Object[] { serviceName });
    ...
    // verify locks by unlocking...
    getLogWriter().fine("[testLockThenBecomeLockGrantor] release locks");
    ...
    unlocked = (Boolean) Host.getHost(0).getVM(thirdPartyVM).invoke(
        DistributedLockServiceTest.class, "unlock", 
        new Object[] { serviceName, "KEY-"+thirdPartyVM });
    assertEquals("Failed to release lock in testLockThenBecomeLockGrantor", 
                 Boolean.TRUE, unlocked);
  ...
  public void testBecomeLockGrantor() {
    ...
    for (int vm = 0; vm < numVMs; vm++) {
      final int finalvm = vm;
      Boolean locked = (Boolean) Host.getHost(0).getVM(finalvm).invoke(
          DistributedLockServiceTest.class, "lock", 
          new Object[] { serviceName, "obj-"+vm });
    ...
    getLogWriter().fine("[testBecomeLockGrantor] original grantor is " + oldGrantor);
    ...
    for (int vm = 0; vm < numVMs; vm++) {
      if (vm != originalVM) {
        ...
        Host.getHost(0).getVM(finalvm).invoke(
            DistributedLockServiceTest.class, "becomeLockGrantor", args);
        Boolean isGrantor = (Boolean)Host.getHost(0).getVM(finalvm).invoke(
            DistributedLockServiceTest.class, "isLockGrantor", args);
    ...
    getLogWriter().fine("[testBecomeLockGrantor] one vm has called becomeLockGrantor...");
    ...
    getLogWriter().fine("[testBecomeLockGrantor] new Grantor is " + newGrantor);
    ...
    for (int vm = 0; vm < numVMs; vm++) {
      final int finalvm = vm;
      Boolean unlocked = (Boolean) Host.getHost(0).getVM(finalvm).invoke(
          DistributedLockServiceTest.class, "unlock", 
          new Object[] { serviceName, "obj-"+vm });
    ...
    getLogWriter().fine("[testBecomeLockGrantor] finished");
  ...
  public void testTryLock() {
    ...
    // create lock services...
    getLogWriter().fine("[testTryLock] create lock services");
    ...
    getLogWriter().fine("[testTryLock] attempt to get tryLock");
    int lockCount = 0;
    for (int vm = 0; vm < 4; vm++) {
      final int finalvm = vm;
      Boolean locked = (Boolean) Host.getHost(0).getVM(finalvm).invoke(
          DistributedLockServiceTest.class, "tryLock", 
          new Object[] { serviceName, "KEY", waitMillis });
    ...
    getLogWriter().fine("[testTryLock] unlock tryLock");
    int unlockCount = 0;
    for (int vm = 0; vm < 4; vm++) {
      final int finalvm = vm;
      Boolean unlocked = (Boolean) Host.getHost(0).getVM(finalvm).invoke(
            DistributedLockServiceTest.class, "unlock", 
            new Object[] { serviceName, "KEY" });
...
//     doOneGetsThenOtherGets(3, 3);
  ...
  public void testLockDifferentNames()
  {
    ...
    assertEquals(Boolean.TRUE, vm.invoke(
      ...
      "getLockAndIncrement", 
      new Object[] {
        serviceName, "otherVMobj", new Long(-1), new Long(0)
      }));
  ...
  public void testRemoteGetLockAndIncrement() {
    ...
    assertEquals(Boolean.TRUE, vm.invoke(
      ...
      "getLockAndIncrement", 
      new Object[] {
        serviceName, "remoteVMobj", new Long(-1), new Long(0)
      }));
  }
  ...
  public void testLockSameNameDifferentService() 
  {
    ...
    assertEquals(Boolean.TRUE, vm.invoke(
      ...
      "getLockAndIncrement", 
      new Object[] {
        serviceName2, objName, new Long(-1), new Long(0)
      }));
    service1.unlock(objName);
  }
  ...
  throws InterruptedException
  {
    ...
    assertEquals(Boolean.FALSE, vm.invoke(
      this.getClass(), 
      "getLockAndIncrement", 
      new Object[] {
  ...
  throws InterruptedException
  {
    getLogWriter().fine("[testLeaseExpires] prepping");
    long leaseMs = 100;
    ...
    getLogWriter().fine("[testLeaseExpires] acquire first lock");
    ...
    getLogWriter().fine("[testLeaseExpires] acquire lock that expired");
    ...
    getLogWriter().fine("[testLeaseExpires] unlock should throw LeaseExpiredException");
    ...
    getLogWriter().fine("[testLeaseExpires] create service in other vm");
    // Different VM
    VM vm = Host.getHost(0).getVM(0);
    vm.invoke(this.getClass(), "remoteCreateService", new Object[] {serviceName});
    
    getLogWriter().fine("[testLeaseExpires] acquire lock again and expire");
    // lock objName in this VM with a short lease
    assertTrue(service.lock(objName, -1, leaseMs));
    sleep(waitBeforeLockingMs);
    
    getLogWriter().fine("[testLeaseExpires] succeed lock in other vm");
    ...
    getLogWriter().fine("[testLeaseExpires] unlock should throw LeaseExpiredException again");
  ...
  public void testSuspendLockingAfterExpiration() throws Exception {
    getLogWriter().fine("[leaseExpiresThenSuspendTest]");
    ...
    getLogWriter().fine("[leaseExpiresThenSuspendTest] unlock should throw LeaseExpiredException");
    // this thread's unlock should throw LeaseExpiredException
    try {
      dls.unlock(key);
      fail("unlock should have thrown LeaseExpiredException");
    ...
    getLogWriter().fine("[leaseExpiresThenSuspendTest] call to suspend locking");
    Host.getHost(0).getVM(0).invoke(new SerializableRunnable() {
      public void run() {
  ...
  public void testLockInterruptiblyIsInterruptible() {
    ...
    getLogWriter().info("[testLockInterruptiblyIsInterruptible] get and hold the lock");
    final String serviceName = getUniqueName();
    ...
    getLogWriter().info("[testLockInterruptiblyIsInterruptible] call lockInterruptibly");
    ...
    getLogWriter().info("[testLockInterruptiblyIsInterruptible] interrupt calling thread");
    while (!started) Thread.yield();
    thread2.interrupt();
    DistributedTestCase.join(thread2, 20 * 1000, getLogWriter());

    // Expect it got InterruptedException and didn't lock the service
    getLogWriter().info("[testLockInterruptiblyIsInterruptible] verify failed to get lock");
    assertFalse(gotLock);
    if (throwable != null) {
    ...
    getLogWriter().info("[testLockInterruptiblyIsInterruptible] unlock the lock");
    service.unlock("obj");

    // Make sure it didn't get locked by second thread
    getLogWriter().info("[testLockInterruptiblyIsInterruptible] try to get lock with timeout should not fail");
  ...
  public void testLockIsNotInterruptible() {
    // Lock entire service in first thread
    getLogWriter().fine("[testLockIsNotInterruptible] lock in first thread");
    started = false;
    gotLock = false;
    exception = null;
    wasFlagSet = false;
    ...
    // Start second thread that tries to lock in second thread
    getLogWriter().fine("[testLockIsNotInterruptible] attempt lock in second thread");
    Thread thread2 = new Thread(new Runnable() {
      public void run() {
        try {
          started = true;
          gotLock = service.lock("obj", -1, -1);
          getLogWriter().fine("[testLockIsNotInterruptible] thread2 finished lock() - got " + gotLock);
        } 
        catch (VirtualMachineError e) {
          SystemFailure.initiateFailure(e);
          throw e;
          ...
          getLogWriter().warning("[testLockIsNotInterruptible] Caught...", ex);
          exception = ex;
        }
    ...
    // Interrupt second thread
    getLogWriter().fine("[testLockIsNotInterruptible] interrupt second thread");
    while (!started) Thread.yield();
    sleep(500);
    thread2.interrupt();
    ...
    getLogWriter().fine("[testLockIsNotInterruptible] unlock in first thread");
    service.unlock("obj");
    sleep(500);
    ...
    getLogWriter().fine("[testLockIsNotInterruptible] verify second thread got lock");
    assertNull(exception);
    assertTrue(gotLock);
  ...
  public void testSuspendLockingProhibitsLocking() 
  {
    ...
    assertTrue(vm1.invokeBoolean(DistributedLockServiceTest.class, "tryToLock", new Object[] { name }));
      ...
      invokeInEveryVM(new SerializableRunnable() {
        public void run() {
          try {
            ...
            getLogWriter().error("Error in testSuspendLockingBehaves finally", t);
          }
          try {
  ...
  private void doTestSuspendLockingBehaves() throws Exception {
    ...
    // create grantor
    getLogWriter().info("[testSuspendLockingBehaves] Create grantor "+dlsName);
...
//    getLogWriter().info("[testSuspendLockingBehaves] Create DLS in vmThree");
    ...
    // get a lock
    getLogWriter().info("[testSuspendLockingBehaves] line up vms for lock");
...
//    getLogWriter().info("[testSuspendLockingBehaves] start vmTwoLocking");
    ...
    pause(2000); // make sure vmTwo is first in line
...
//    getLogWriter().info("[testSuspendLockingBehaves] assert vmThreeLocking still alive");
    ...
    // let vmTwo get key
    getLogWriter().info("[testSuspendLockingBehaves] unlock so vmTwo can get key");
//    getLogWriter().info("[testSuspendLockingBehaves] vmOne unlock");
    vmOne.invoke(unlockKey);
    DistributedTestCase.join(vmTwoLocking, 10 * 1000, getLogWriter());

    // start suspending in vmOne and vmTwo
    getLogWriter().info("[testSuspendLockingBehaves] start suspending requests");
//    getLogWriter().info("[testSuspendLockingBehaves] start vmOneSuspending");
    ...
    pause(2000); // make sure vmOne is first in line
//    getLogWriter().info("[testSuspendLockingBehaves] start vmTwoSuspending");
    ...
    getLogWriter().info("[testSuspendLockingBehaves] unlock so vmThree can get key");
//    getLogWriter().info("[testSuspendLockingBehaves] vmTwo unlock");
    vmTwo.invoke(unlockKey);
    DistributedTestCase.join(vmThreeLocking, 10 * 1000, getLogWriter());
    
    // have vmOne get back in line for locking key
    getLogWriter().info("[testSuspendLockingBehaves] start another lock request");
//    getLogWriter().info("[testSuspendLockingBehaves] start vmOneLockingAgain");
    ...
    getLogWriter().info("[testSuspendLockingBehaves] let vmOne suspend locking");
...
//    getLogWriter().info("[testSuspendLockingBehaves] vmThree unlock");
    ...
    DistributedTestCase.join(vmOneSuspending, 10 * 1000, getLogWriter());
    
    // start suspending in vmThree
    getLogWriter().info("[testSuspendLockingBehaves] line up vmThree for suspending");
//    getLogWriter().info("[testSuspendLockingBehaves] start vmThreeSuspending");
    ...
    getLogWriter().info("[testSuspendLockingBehaves] let vmTwo suspend locking");
...
//    getLogWriter().info("[testSuspendLockingBehaves] vmOne resumes locking");
    ...
    DistributedTestCase.join(vmTwoSuspending, 10 * 1000, getLogWriter());
    
    // let vmOne get that lock
    getLogWriter().info("[testSuspendLockingBehaves] let vmOne get that lock");
...
//    getLogWriter().info("[testSuspendLockingBehaves] vmTwo resumes locking");
    ...
    DistributedTestCase.join(vmOneLockingAgain, 10 * 1000, getLogWriter());
    
    // let vmThree suspend locking
    getLogWriter().info("[testSuspendLockingBehaves] let vmThree suspend locking");
...
//    getLogWriter().info("[testSuspendLockingBehaves] vmOne unlocks again");
    vmOne.invoke(unlockKey);
    DistributedTestCase.join(vmThreeSuspending, 10 * 1000, getLogWriter());
...
//    getLogWriter().info("[testSuspendLockingBehaves] vmThree resumes locking");
  ...
  public void testSuspendLockingIsNotInterruptible() {
    ...
    // Now thread2 should have gotten the lock, not the exception, but the
    // thread's flag should be set
    getLogWriter().info("[testSuspendLockingIsNotInterruptible]" +
      " gotLock=" + gotLock +
  ...
  public void testBug32461() throws Exception {
    getLogWriter().fine("[testBug32461] prepping");
    ...
    getLogWriter().fine("[testBug32461] VM-A locks/unlocks '32461'");
    ...
    getLogWriter().fine("[testBug32461] VM_B leases '32461' and disconnects");
    ...
    getLogWriter().fine("[testBug32461] VM_C attempts to lock '32461'");
  ...
  public void testReleaseOrphanedGrant_Local() {
    ...
    try {
      ...
      getLogWriter().info("[testReleaseOrphanedGrant_Local] create lock service");
      final String serviceName = getUniqueName();
      final DistributedLockService service = 
        DistributedLockService.create(serviceName, dlstSystem);
      ...
      final Thread thread1 = new Thread(new Runnable() {
        public void run() {
          getLogWriter().info("[testReleaseOrphanedGrant_Local] get the lock");
          ...
          getLogWriter().info("[testReleaseOrphanedGrant_Local] unlock the lock");
          service.unlock("obj");
        }
      });
      thread1.start();
      ...
      final Thread thread2 = new Thread(new Runnable() {
        public void run() {
          try {
            getLogWriter().info("[testReleaseOrphanedGrant_Local] call lockInterruptibly");
      ...
      getLogWriter().info("[testReleaseOrphanedGrant_Local] release 1st thread");
      sleep(500);
      ...
      getLogWriter().info("[testReleaseOrphanedGrant_Local] interrupt 2nd thread");
      thread2.interrupt();
      DistributedTestCase.join(thread2, 20 * 1000, getLogWriter());
  
      // release waitToProcessDLockResponse
      getLogWriter().info("[testReleaseOrphanedGrant_Local] process lock response");
      sleep(500);
      DLockRequestProcessor.setWaitToProcessDLockResponse(false);
      
      // relock obj to make sure zombie release worked
      getLogWriter().info("[testReleaseOrphanedGrant_Local] verify lock not held");
      assertTrue(service.lock("obj", 1000, -1));
    }
    finally {
  ...
  public void doTestReleaseOrphanedGrant_Remote(final boolean destroyLockService) {
    ...
    try {
      getLogWriter().info("[testReleaseOrphanedGrant_Remote] create lock service");
      final String serviceName = getUniqueName();
      final DistributedLockService service = 
        DistributedLockService.create(serviceName, dlstSystem);
      ...
      vm1.invokeAsync(new SerializableRunnable() {
        public void run() {
          getLogWriter().info("[testReleaseOrphanedGrant_Remote] get the lock");
          ...
          getLogWriter().info("[testReleaseOrphanedGrant_Remote] unlock the lock");
      ...
      vm2.invokeAsync(new SerializableRunnable() {
        public void run() {
          getLogWriter().info("[testReleaseOrphanedGrant_Remote] call lockInterruptibly");
      ...
      vm1.invoke(new SerializableRunnable() {
        public void run() {
          getLogWriter().info("[testReleaseOrphanedGrant_Remote] release 1st thread");
          synchronized(threadVM1_testReleaseOrphanedGrant_Remote) {
      ...
      vm2.invoke(new SerializableRunnable() {
        public void run() {
          getLogWriter().info("[testReleaseOrphanedGrant_Remote] interrupt 2nd thread");
          threadVM2_testReleaseOrphanedGrant_Remote.interrupt();
          DistributedTestCase.join(threadVM2_testReleaseOrphanedGrant_Remote, 
              5 * 60 * 1000, getLogWriter());
          if (destroyLockService) {
            getLogWriter().info("[testReleaseOrphanedGrant_Remote] destroy lock service");
            DistributedLockService.destroy(serviceName);
            assertNull(DistributedLockService.getServiceNamed(serviceName));
          }
        }
      ...
      vm2.invoke(new SerializableRunnable() {
        public void run() {
          getLogWriter().info("[testReleaseOrphanedGrant_Remote] process lock response");
          DLockRequestProcessor.setWaitToProcessDLockResponse(false);
        }
      });
      ...
      // relock obj to make sure zombie release worked
      getLogWriter().info("[testReleaseOrphanedGrant_Remote] verify lock not held");
      assertTrue(service.lock("obj", 1000, -1));
    }
    finally {
      vm2.invoke(new SerializableRunnable() {
        public void run() {
          getLogWriter().info("[testReleaseOrphanedGrant_Remote] clean up DebugReleaseOrphanedGrant");
          DLockRequestProcessor.setDebugReleaseOrphanedGrant(false);
          DLockRequestProcessor.setWaitToProcessDLockResponse(false);
        }
      });
      ...
      for (int thread=0; thread<numThreadsPerVM; thread++) {
        ...
        (new Thread(new Runnable() {
          public void run() {
            ...
            Boolean result = (Boolean)host.getVM(finalvm).invoke(DistributedLockServiceTest.class, 
...
//-------------------------------------------------------      
  ...
  public void testTokenCleanup() throws Exception {
    ...
    vmGrantor.invoke(new SerializableRunnable() {
      public void run() {
        getLogWriter().info("[testTokenCleanup] vmGrantor creates grantor");
        connectDistributedSystem();
    ...
    vm1.invoke(new SerializableRunnable() {
      public void run() {
        getLogWriter().info("[testTokenCleanup] vm1 locks key1");
        connectDistributedSystem();
        DLockService dls = (DLockService)
          DistributedLockService.create(dlsName, getSystem());
    ...
    vm1.invoke(new SerializableRunnable() {
      public void run() {
        getLogWriter().info("[testTokenCleanup] vm1 tests recursion");
        connectDistributedSystem();
        DLockService dls = (DLockService)
  ...
  public void testGrantTokenCleanup() throws Exception {
    ...
    vmGrantor.invoke(new SerializableRunnable() {
      public void run() {
        getLogWriter().info("[testGrantTokenCleanup] vmGrantor creates grantor");
        connectDistributedSystem();
        DistributedLockService dls =
          DistributedLockService.create(dlsName, getSystem());
        assertTrue(dls.lock(key1, -1, -1));
        ...
        getLogWriter().info("[testGrantTokenCleanup] vmGrantor unlocks key1");
        dls.unlock(key1);
    ...
    vm1.invoke(new SerializableRunnable() {
      public void run() {
        ...
        getLogWriter().info("[testTokenCleanup] vm1 frees key1");
        dls.unlock(key1);
    ...
    vmGrantor.invoke(new SerializableRunnable() {
      public void run() {
        getLogWriter().info("[testTokenCleanup] vmGrantor frees key1");
        DLockService dls = (DLockService)
          DistributedLockService.getServiceNamed(dlsName);
  ...
  public void testLockQuery() throws Exception {
    ...
    vmGrantor.invoke(new SerializableRunnable() {
      public void run() {
        getLogWriter().info("[testLockQuery] vmGrantor creates grantor");
        connectDistributedSystem();
        DLockService dls = (DLockService)
          DistributedLockService.create(dlsName, getSystem());
    ...
    try {
      ...
      whileVM1Locks = vm1.invokeAsync(new SerializableRunnable() {
        public void run() {
          getLogWriter().info("[testLockQuery] vm1 locks key1");
          connectDistributedSystem();
          DLockService dls = (DLockService)
            DistributedLockService.create(dlsName, getSystem());
          ...
          getLogWriter().info("[testLockQuery] vm1 unlocks key1");
          dls.unlock(key1);
          dls.freeResources(key1);
        }
      });
      ...
      final DistributedMember vm1Member = (DistributedMember)
        ...
        public Object call() throws Exception {
          getLogWriter().info("[testLockQuery] vm1 waits for locking thread");
          synchronized (testLockQuery_whileVM1Locks) {
            long maxWait = 10000;
            StopWatch timer = new StopWatch(true);
      ...
      vmGrantor.invoke(new SerializableRunnable() {
        public void run() {
          getLogWriter().info("[testLockQuery] vmGrantor tests local query");
          DLockService dls = (DLockService)
            DistributedLockService.getServiceNamed(dlsName);
      ...
      vm2.invoke(new SerializableRunnable() {
        public void run() {
          getLogWriter().info("[testLockQuery] vm2 tests remote query");
          connectDistributedSystem();
          DLockService dls = (DLockService)
            DistributedLockService.create(dlsName, getSystem());
      ...
      vm1.invoke(new SerializableRunnable() {
        public void run() {
          getLogWriter().info("[testLockQuery] vm1 notifies/releases key1");
          synchronized (testLockQuery_whileVM1Locks) {
      ...
      DistributedTestCase.join(whileVM1Locks, 10 * 1000, getLogWriter());
      if (whileVM1Locks.exceptionOccurred()) {
        fail("Test failed", whileVM1Locks.getException());
      }
    }
    ...
    vmGrantor.invoke(new SerializableRunnable() {
      public void run() {
        getLogWriter().info("[testLockQuery] vmGrantor tests negative query");
        DLockService dls = (DLockService)
          DistributedLockService.getServiceNamed(dlsName);
    ...
    vm2.invoke(new SerializableRunnable() {
      public void run() {
        getLogWriter().info("[testLockQuery] vm2 tests negative query");
        DLockService dls = (DLockService)
          DistributedLockService.getServiceNamed(dlsName);
  ...
  private static Object getLockAndIncrement(
    String serviceName, Object objectName, long timeout, long holdTime)
  {
  ...
  public static class BasicLockClient implements Runnable {
    ...
    public void run() {
      getLogWriter().info("BasicLockClient running");
      while (this.stayinAlive) {
        synchronized(this.sync) {
          ...
          try {
            ...
            getLogWriter().info("BasicLockClient interrupted");
            this.stayinAlive = false;
          }
        } // sync
      } // while
