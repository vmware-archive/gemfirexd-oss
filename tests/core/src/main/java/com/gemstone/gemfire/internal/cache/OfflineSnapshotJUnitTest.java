...
public class OfflineSnapshotJUnitTest extends TestCase {
  ...
  private void checkSnapshotEntries(Map<Integer, MyObject> expected, String diskStoreName, String regionName) 
      throws Exception {
    final Map<Integer, MyObject> testData = new HashMap<Integer, MyObject>(expected);
    String snapshot = "snapshot-" + diskStoreName + "-" + regionName;
    SnapshotIterator<Integer, MyObject> iter = SnapshotReader.read(new File(snapshot));
    try {
      while (iter.hasNext()) {
