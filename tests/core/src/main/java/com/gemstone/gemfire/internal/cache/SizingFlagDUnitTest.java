...
public class SizingFlagDUnitTest extends CacheTestCase {
  ...
  private void doPRNoLRUDeltaTest(boolean shouldSizeChange) {
    ...
    shouldSizeChange = setDeltaRecalculatesSize(vm0, shouldSizeChange);
    shouldSizeChange = setDeltaRecalculatesSize(vm1, shouldSizeChange);
  ...
  private boolean setDeltaRecalculatesSize(VM vm, final boolean shouldSizeChange) {
    return (Boolean)vm.invoke(new SerializableCallable("setDeltaRecalculatesSize") {
      public Object call() {
        ...
        return Boolean.valueOf(shouldSizeChange);
      }
    });
  }
