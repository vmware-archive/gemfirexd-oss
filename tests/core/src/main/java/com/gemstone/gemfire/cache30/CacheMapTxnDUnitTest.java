...
public class CacheMapTxnDUnitTest extends DistributedTestCase{
    ...
    public static void commitTxn(){
        try{
            ...
            boolean succeeded = region.remove("" + i[5],
    ...
    public static void miscMethodsOwner(){
        try{
            ...
            int j = region.size();
            if(j != 6){
                fail("region.size inside transaction returns incorrect results: " + j);
            }
            ...
            int k = set.size();
            if(j != 6){
                fail("region.entrySet inside transaction returns incorrect results: " + j);
            }
            ...
            k = set.size();
            if(j != 6){
                fail("region.keySet inside transaction returns incorrect results: " + j);
            }
