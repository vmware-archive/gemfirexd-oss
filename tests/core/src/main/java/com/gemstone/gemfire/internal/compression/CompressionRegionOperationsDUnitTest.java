...
public class CompressionRegionOperationsDUnitTest extends CacheTestCase {
  ...
  private Region createRegion(String name, Compressor compressor, boolean offHeap) {
    return getCache().<String,String>createRegionFactory().setDataPolicy(DataPolicy.REPLICATE).setCompressor(compressor).setEnableOffHeapMemory(offHeap).create(name);
  }
}
